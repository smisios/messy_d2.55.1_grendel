...
/*-<a                             href="qh-c.htm#qhull"
  ...
  notes:
    also change date in Changes.txt, Announce.txt, README.txt, qhull.man
                        qhull-news.html
    change version or year: README.txt, qhull.html, Copying.txt file_id.diz
    check download size
...
*/  
char qh_version[]= "version 2.6 1998/8/12"; 
char qh_prompta[]= "\n\
...
void qhull_dll_ (float *feld,int *o_feld) {
  int curlong, totlong; /* used !qh_NOmem */
  int exitcode,numpoints,dim; 
  coordT *points;
  boolT ismalloc;
  char *strings3d[]={"qhull\0","Ft\0","QJ\0","Pp\0"};
  char *strings4d[]={"qhull\0","Ft\0","QJ\0","d\0","Pp\0"}; 
  int i,j;
  double paraboloid;
  ...
  numpoints=(int) feld[0];

  dim=(int) feld[1];

  if (dim == 3)
    qh_init_A (stdin, stdout, stderr, 4, strings3d);  /* sets qh qhull_command */
  else
    qh_init_A (stdin, stdout, stderr, 5, strings4d);  /* sets qh qhull_command */
  ...
  if (dim ==3) 
      ...
      for (i=0;i<numpoints;i++)
	{
	  ...
	  for (j=0;j<3;j++)
	    {
	      points[4*i+j]=(double) feld[3*i+j+2];
	      paraboloid+=(double) feld[3*i+j+2]*feld[3*i+j+2];
	    }
	  points[4*i+3]=paraboloid;
	}
      qh PROJECTdelaunay= False;
      qh normal_size= dim * sizeof(coordT);
    }
