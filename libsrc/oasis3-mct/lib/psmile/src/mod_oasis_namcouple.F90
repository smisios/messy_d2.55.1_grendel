...
!  INTEGER (kind=ip_intwp_p) :: ig_final_nfield    ! number of final fields
  INTEGER (kind=ip_intwp_p), PUBLIC :: ig_final_nfield  ! number of final fields
! ub_ak_20180328-
...
#ifdef FLD3D
  INTEGER (kind=ip_intwp_p) :: ix, numlev
  CHARACTER(LEN=3)          :: sint
#endif
  !* ---------------------------- Poema verses --------------------------
...
#else
  jf = 1
  DO 240 while (jf <= ig_total_nfield)
#endif
  ! ub_ak_20180522-
    ...
    READ (clvari,FMT = 2003) numlev
    IF (numlev > 1)  THEN
       if (OASIS_debug >= 20) THEN
          write(nulprt1,*) ' 3D field found: ',numlev
          CALL oasis_flush(nulprt1)
       end if
       IF (jf + numlev > ig_total_nfield) GOTO 740
    END IF
    ! ub_ak_20180522-
#endif
    ...
    ! ub_ak_20180522+
    DO ix = 2, numlev
       CALL INT2STR(sint, ix)
       cg_output_field(jf+ix-1) = TRIM(cg_output_field(jf))//'_'//sint
       ig_total_ntrans(jf+ix-1) = ig_total_ntrans(jf)
    END DO
    ! replace name of level 1, if multilevel field
    IF (numlev > 1) THEN
       CALL INT2STR(sint, 1)
       cg_output_field(jf) = TRIM(cg_output_field(jf))//'_'//sint
    END IF
    ! ub_ak_20180522-
#endif
    ...
    IF (clvari(1:6) .EQ. 'OUTPUT') THEN
       ...
       IF (numlev >1) THEN
          WRITE (nulprt1,FMT= *) &
               ' error = For OUTPUT fields nlevel statement not allowed'
          CALL oasis_abort()
       END IF
       ! ub_ak_20180522-
#endif
    ...
    ELSE
        ...
        IF (clvari(1:8).eq.'EXPORTED') THEN
            ...
            DO ix = 2, numlev
               ig_nfield = ig_nfield + 1
               lg_state(jf+ix-1)        = .true.
               ig_number_field(jf+ix-1) = ig_nfield
               ig_total_state(jf+ix-1)  = ip_exported
               cg_restart_file(jf+ix-1) = cg_restart_file(jf)
            END DO
#endif
            !* Get restart file name
        ...
        ELSEIF (clvari(1:6) .eq. 'EXPOUT') THEN
            ...
            DO ix = 2, numlev
               ...
               lg_state(jf+ix-1) = .true.
               ig_number_field(jf+ix-1) = ig_nfield
               ig_total_state(jf+ix-1)  = ip_expout
               cg_restart_file(jf+ix-1) = cg_restart_file(jf)
            END DO
#endif
...
#else
    ig_final_nfield = ig_final_nfield + numlev
    jf = jf + numlev
#endif
...
#else
     jf = 1
     DO 250 WHILE (jf <= ig_final_nfield)
#endif
      ...
      IF (ig_total_state(jf) .NE. ip_input) THEN
...
#ifdef FLD3D
              DO ix = 2, numlev
                 IF (lg_state(jf+ix-1)) THEN
                    cficbf(ig_number_field(jf+ix-1)) = cficbf(ig_number_field(jf))
                    cficaf(ig_number_field(jf+ix-1)) = cficaf(ig_number_field(jf)) 
                 END IF
                 cga_locatorbf(jf+ix-1) = cga_locatorbf(jf)
                 cga_locatoraf(jf+ix-1) = cga_locatoraf(jf)
              END DO
#endif
              lncdfgrd = .true.
          ELSE 
              ...
              ELSE
                  ...
                  DO ix = 2, numlev
                     ...
                     cga_locatorbf(jf+ix-1) = cga_locatorbf(jf) 
                     cga_locatoraf(jf+ix-1) = cga_locatoraf(jf)
                     nlonbf(ig_number_field(jf+ix-1)) = nlonbf_notnc
                     nlatbf(ig_number_field(jf+ix-1)) = nlatbf_notnc
                     nlonaf(ig_number_field(jf+ix-1)) = nlonaf_notnc
                     nlataf(ig_number_field(jf+ix-1)) = nlataf_notnc
                  END DO
#endif
          ...
          IF (ig_total_ntrans(jf) .GT. 0) THEN 
              ...
              DO 260 ja = 1, ig_total_ntrans(jf)
                ...
                DO ix = 2, numlev
                   IF (lg_state(jf+ix-1))  &
                        canal(ja,ig_number_field(jf+ix-1)) = clvari
                END DO
#endif
260           CONTINUE
...
#ifdef FLD3D
                        DO ix = 2, numlev
                            nbofld (ig_number_field(jf+ix-1)) = &
                                 nbofld (ig_number_field(jf))
                        END DO
#endif
...
#ifdef FLD3D
                        DO ix = 2, numlev
                           nbnfld (ig_number_field(jf+ix-1)) = &
                                nbnfld (ig_number_field(jf))
                        END DO
#endif
...
#else
      jf = 1
      DO 240 WHILE (jf <= ig_final_nfield)
#endif         
...
!* Get field label number
         ...
         DO ix = 2, numlev
            CALL INT2STR(sint, ix)
            cg_input_field(jf+ix-1) = TRIM(cg_input_field(jf))//'_'//sint
            ig_numlab(jf+ix-1)      = numlev
            IF (lg_state(jf+ix-1)) THEN
               cnaminp(ig_number_field(jf+ix-1)) = cnaminp(ig_number_field(jf))//'_'//sint
               cnamout(ig_number_field(jf+ix-1)) = cnamout(ig_number_field(jf))//'_'//sint
               numlab(ig_number_field(jf+ix-1))  = ig_numlab(jf+ix-1)
            END IF
         END DO
         ! replace name if level 1, if multilevels exist
         IF (numlev > 1) THEN
            CALL INT2STR(sint, 1)
            cg_input_field(jf) = TRIM(cg_input_field(jf))//'_'//sint
         END IF
#endif
...
!* time of the simulation
            ...
            ig_freq(jf) = ntime
#else
            ig_freq(jf:jf+numlev-1) = ntime
#endif
         ELSE
         READ (clvari,FMT = 2004) ig_freq(jf)
#ifdef FLD3D
         DO ix = 2,numlev
            ig_freq(jf+ix-1) = ig_freq(jf)
         END DO
#endif
...
!* Fill up restart file number and restart file name arrays
         ...
         DO ix = 2, numlev
            IF (lg_state(jf+ix-1)) &
                 nfexch(ig_number_field(jf+ix-1)) = ig_freq(jf+ix-1)
            IF (cg_restart_file(jf+ix-1).ne.' ') THEN
               IF (jf+ix-1.eq.1) THEN
                  il_aux = il_aux + 1
                  ig_no_rstfile(jf+ix-1) = il_aux
                  cg_name_rstfile (ig_no_rstfile(jf+ix-1)) =  &
                       cg_restart_file(jf+ix-1)
               ELSEIF (jf+ix-1.gt.1) THEN
                  IF (ALL(cg_name_rstfile.ne.cg_restart_file(jf+ix-1))) THEN
                     il_aux = il_aux + 1
                     ig_no_rstfile(jf+ix-1) = il_aux
                     cg_name_rstfile (ig_no_rstfile(jf+ix-1))=  &
                          cg_restart_file(jf+ix-1)
                  ELSE 
                     DO ib = 1, jf+ix-1 - 1 
                        IF(cg_name_rstfile(ig_no_rstfile(ib)).eq. &
                             cg_restart_file(jf+ix-1)) THEN
                           ig_no_rstfile(jf+ix-1) = ig_no_rstfile(ib)
                        ENDIF
                     ENDDO
                  ENDIF
               ENDIF
...
#ifdef FLD3D
                 DO ix = 2, numlev
                    cstate(ig_number_field(jf+ix-1)) = clvari
                 END DO
#endif
             ELSEIF (clvari(1:6) .eq. 'EXPOUT') THEN
...
#ifdef FLD3D
                 DO ix = 2, numlev
                    cstate(ig_number_field(jf+ix-1)) =  'EXPORTED'
                 END DO
#endif
             ENDIF
!*
...
!     *          prefixes and the grids file must be in NetCDF format       
              ...
              DO ix = 2, numlev
                 ig_lag(jf+ix-1)=0
                 ig_total_nseqn(jf+ix-1)=1
                 IF (lg_state(jf+ix-1)) then
                    nseqn(ig_number_field(jf+ix-1)) = 1
                    nlagn(ig_number_field(jf+ix-1)) = 0
                 ENDIF
              END DO
              llseq=.FALSE.
              lllag=.FALSE.
              IF (mpi_rank_global == 0) THEN
                  WRITE (UNIT=nulprt1,FMT=3043) jf+ix-1
              ENDIF
#endif
           ELSE 
              ...
              DO 245 ilind=ilind1, ilind2
                 ...
                 ELSE
...
#ifdef FLD3D
                        DO ix = 2, numlev
                           ig_total_nseqn(jf+ix-1)=iind
                           IF (lg_state(jf+ix-1)) &
                                nseqn(ig_number_field(jf+ix-1)) = iind
                        END DO
#endif
                    ELSE IF (clind .eq. 'LAG') THEN
                       ...
                       ig_lag(jf+ix-1)=iind
                          IF (lg_state(jf+ix-1)) &
                               nlagn(ig_number_field(jf+ix-1)) = iind
                          IF (mpi_rank_global == 0) THEN
                             WRITE (UNIT = nulprt1,FMT = 3044)jf,ig_lag(jf+ix-1)
                          ENDIF
                       END DO
#endif
                    ENDIF              
...
#ifdef FLD3D
           DO ix = 2, numlev
              csper(ig_number_field(jf+ix-1))  =  csper(ig_number_field(jf))
              nosper(ig_number_field(jf+ix-1)) = nosper(ig_number_field(jf))
              ctper(ig_number_field(jf+ix-1))  =  ctper(ig_number_field(jf)) 
              notper(ig_number_field(jf+ix-1)) = notper(ig_number_field(jf))
           END DO
#endif
       ENDIF
...
#ifdef FLD3D
                    DO ix = 2, numlev
                       ig_local_trans(jf+ix-1) = ig_local_trans(jf)
                    END DO
#endif
                 ENDIF
              ENDDO
...
#ifdef FLD3D
                 DO ix = 2, numlev
                    cmap_file(ig_number_field(jf+ix-1)) = &
                                                 cmap_file(ig_number_field(jf))
                    cmaptyp(ig_number_field(jf+ix-1))   = &
                                                 cmaptyp(ig_number_field(jf))
                    cmapopt(ig_number_field(jf+ix-1))   = &
                                                 cmapopt(ig_number_field(jf))
                 END DO
#endif
...
#ifdef FLD3D
                 DO ix = 2, numlev
                    cmap_method(ig_number_field(jf+ix-1)) = cmap_method(ig_number_field(jf))
                    cgrdtyp(ig_number_field(jf+ix-1))     = cgrdtyp(ig_number_field(jf))
                    cfldtype(ig_number_field(jf+ix-1))    = cfldtype(ig_number_field(jf))
                    crsttype(ig_number_field(jf+ix-1))    = crsttype(ig_number_field(jf))
                    nbins(ig_number_field(jf+ix-1))       = nbins(ig_number_field(jf))
                    cnorm_opt(ig_number_field(jf+ix-1))   = cnorm_opt(ig_number_field(jf))
                    corder(ig_number_field(jf+ix-1))      = corder(ig_number_field(jf))
                    nscripvoi(ig_number_field(jf+ix-1))   = nscripvoi(ig_number_field(jf))
                    varmul(ig_number_field(jf+ix-1))      = varmul(ig_number_field(jf))
                 END DO
#endif
...
#ifdef FLD3D
                 DO ix = 2, numlev
                    cfilfic(ig_number_field(jf+ix-1)) = cfilfic(ig_number_field(jf))
                    nlufil(ig_number_field(jf+ix-1))  = nlufil(ig_number_field(jf))
                    cfilmet(ig_number_field(jf+ix-1)) = cfilmet(ig_number_field(jf))
                 END DO
#endif
...
#ifdef FLD3D
                 DO ix = 2, numlev
                    cconmet(ig_number_field(jf+ix-1)) = cconmet(ig_number_field(jf))
                    cconopt(ig_number_field(jf+ix-1)) = cconopt(ig_number_field(jf))
                 END DO
#endif
              ELSE IF (canal(ja,ig_number_field(jf)) .EQ. 'BLASOLD')THEN
...
#ifdef FLD3D
                    DO ix = 2, numlev
                       afldcobo(ig_number_field(jf+ix-1))    = afldcobo(ig_number_field(jf))
                       cbofld(jc,ig_number_field(jf+ix-1))   = cbofld(jc,ig_number_field(jf))
                       abocoef (jc,ig_number_field(jf+ix-1)) = abocoef (jc,ig_number_field(jf))
                    END DO
#endif
 290             CONTINUE
              ELSE IF (canal(ja,ig_number_field(jf)) .EQ. 'BLASNEW')THEN
...
#ifdef FLD3D
                 DO ix = 2, numlev
                    afldcobn(ig_number_field(jf))         = afldcobn(ig_number_field(jf))
                    cbnfld(jc,ig_number_field(jf+ix-1))   = cbnfld(jc,ig_number_field(jf))
                    abncoef (jc,ig_number_field(jf+ix-1)) = abncoef (jc,ig_number_field(jf))
                 END DO
#endif
 291             CONTINUE
              ELSE 
