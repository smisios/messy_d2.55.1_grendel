...
use vars,   only: pres, rho, dtn, w, tke, t
  ...
  if(nrestart.eq.0) then
     ...
     do k = 1,nzm
        ...
        if(qc0(k).gt.0.) then
           ...
           ! um_hr_201321+ change default CDNC profile to given l.-s. profile -comment out the line below
           ! set at the beginning of crm.F90 - micro_field = crm_buffer
...
end subroutine micro_init_m2005
...
use grid,   only: z, zi, nstep, nstatis, icycle, dtfactor
use vars,   only: docloud, t, gamaz, precsfc, precflux, qpfall, tlat, prec_xy, &
                  s_ar, total_water_prec
