...
subroutine forcing
        ...
        do k=1,nzm
            ...
            do j=1,ny
             do i=1,nx
              t(i,j,k)=t(i,j,k) + ttend(k) * dtn   !!!! update crm temperature with large-scale tendencies
            ...
            if(nneg.gt.0.and.qpoz+qneg.gt.0.) then
             ...
             do j=1,ny
              do i=1,nx
               micro_field(i,j,k,index_water_vapor) = max(0._dp,micro_field(i,j,k,index_water_vapor)*factor)
              end do
             end do
            end if
