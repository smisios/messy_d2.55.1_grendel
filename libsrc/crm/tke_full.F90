...
!bloss         (qpl(i,j,kc)+qpi(i,j,kc)-qpl(i,j,kb)-qpi(i,j,kb)) )
   ...
   if(buoy_sgs.le.0.) then
     ...
     smix=min(REAL(grd,dp),max(0.1_dp*grd, sqrt(0.76*tk(i,j,k)/Ck/sqrt(buoy_sgs+1.e-10))))
   end if
...
!   Pr=1. +2.*ratio
   ...
   if(dosmagor) then
     ...
     tke(i,j,k)=max(0._dp,tke(i,j,k))
     a_prod_sh=(tk(i,j,k)+0.001)*def2(i,j,k)
     a_prod_bu=-(tk(i,j,k)+0.001)*Pr*buoy_sgs
     a_diss=Cee/smix*tke(i,j,k)**1.5       
     tke(i,j,k)=max(0._dp,tke(i,j,k)+dtn*(max(0.,a_prod_sh+a_prod_bu)-a_diss))
