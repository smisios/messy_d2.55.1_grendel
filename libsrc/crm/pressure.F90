...
real(dp) a(nzm),b,c(nzm),e,fff(nzm)      
real(dp) xi,xj,xnx,xny,ddx2,ddy2,pii,factx,facty,eign
real(dp) alfa(nzm-1),beta(nzm-1)
...
do j=1,nyp22-jwall
   ...
   do i=1,nxp1-iwall
      ...
      xi=id
      eign=(2._dp*cos(factx*xnx*xi)-2._dp)*ddx2+ & 
            (2._dp*cos(facty*xny*xj)-2._dp)*ddy2
      if(id+jd.eq.0) then               
         b=1._dp/(eign*rho(1)-a(1)-c(1))
         alfa(1)=-c(1)*b
         beta(1)=fff(1)*b
      else
         b=1._dp/(eign*rho(1)-c(1))
         alfa(1)=-c(1)*b
         beta(1)=fff(1)*b
      end if
      do k=2,nzm-1
        e=1._dp/(eign*rho(k)-a(k)-c(k)+a(k)*alfa(k-1))
        alfa(k)=-c(k)*e
