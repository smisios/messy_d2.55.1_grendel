...
real qp(dimx1_s:dimx2_s, dimy1_s:dimy2_s, nzm) 
...
!  Test if condensation is possible:
    ...
    if(q(i,j,k).gt.qsatt) then
      ...
      qn(i,j,k) = max(0._dp,q(i,j,k)-qsatt)
    ...
    tabs(i,j,k) = tabs1
    qp(i,j,k) = max(0._dp,qp(i,j,k)) ! just in case
