...
! MICROPHYSICAL PROCESSES
     ...
     REAL, DIMENSION(KMS:KME) ::  PRAI      ! CHANGE Q AUTOCONVERSION CLOUD ICE
     REAL, DIMENSION(KMS:KME) ::  PRCI      ! CHANGE Q ACCRETION CLOUD ICE BY SNOW
...
! REPLACE GOFF-GRATCH WITH FASTER FORMULATION FROM MARAT KHAIROUTDINOV
...
!      after Ray (1972)
      ...
      m_complex_water_ray = SQRT(CMPLX(epsr,-epsi,Kind=Kind(0d0)))
...
!      f = frequency in GHz, range 0.01 to 3000
      ...
      m_complex_ice_maetzler = dble(m_complex_ice_maetzler)                   &
                             + CMPLX(0.0d0, (alfa/f + beta*f),Kind=Kind(0d0))
...
!+---+-----------------------------------------------------------------+
      ...
      cumulerror = 0
      get_m_mix_nested = CMPLX(1.0d0,0.0d0,Kind=Kind(0d0))
      ...
      elseif (host .eq. 'water') then
       ...
       if (matrix .eq. 'water') then
        ...
        if (hostmatrix .eq. 'water') then
         get_m_mix_nested = get_m_mix (2.0*m_a, mtmp, m_w,            &
...
!+---+-----------------------------------------------------------------+
      ...
      error = 0
      get_m_mix = CMPLX(1.0d0,0.0d0,Kind=Kind(0d0))
...
!+---+-----------------------------------------------------------------+
      ...
      if (DABS(vol1+vol2+vol3-1.0d0) .gt. 1d-6) then
       ...
       !bloss CALL wrf_debug(150, mp_debug)
       m_complex_maxwellgarnett=CMPLX(-999.99d0,-999.99d0,Kind=Kind(0d0))
       error = 1
       return
      endif
