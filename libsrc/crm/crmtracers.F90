...
REAL(dp), POINTER, DIMENSION(:,:,:,:)  :: tracer    => NULL() 
REAL(dp), POINTER, DIMENSION(:,:,:)    :: fluxbtr   => NULL() ! surface flux of tracers
REAL(dp), POINTER, DIMENSION(:,:,:)    :: fluxttr   => NULL() ! top boundary flux of tracers
REAL(dp), POINTER, DIMENSION(:,:)      :: trwle     => NULL() ! resolved vertical flux
REAL(dp), POINTER, DIMENSION(:,:)      :: trwsb     => NULL() ! SGS vertical flux
REAL(dp), POINTER, DIMENSION(:,:)      :: tradv     => NULL() ! tendency due to vertical advection
REAL(dp), POINTER, DIMENSION(:,:)      :: trdiff    => NULL() ! tendency due to vertical diffusion
REAL(dp), POINTER, DIMENSION(:,:)      :: trphys    => NULL() ! tendency due to physics

CHARACTER(LEN=4),  POINTER, DIMENSION(:) :: tracername  => NULL()
CHARACTER(LEN=10), POINTER, DIMENSION(:) :: tracerunits => NULL()


CONTAINS

 subroutine tracers_init(field_crm, field_global)

use grid

! input 
   REAL(dp) :: field_global
   REAL(dp) :: field_crm(dimx1_s:dimx2_s, dimy1_s:dimy2_s)
...
! ====================================================================
  ...
  SUBROUTINE set_crm_tracers(crmtracers_nml)
  
    use grid

    IMPLICIT NONE

    INTEGER  :: crmtracers_nml

    ntracers = crmtracers_nml

  END SUBROUTINE set_crm_tracers

! ====================================================================

  SUBROUTINE crm_allocate_tracers

    use crmtracers

    IMPLICIT NONE

    ALLOCATE(tracer(dimx1_s:dimx2_s,dimy1_s:dimy2_s,nzm, 0:ntracers));tracer = 0.
    ALLOCATE(fluxbtr(nx,ny,0:ntracers));fluxbtr = 0.
    ALLOCATE(fluxttr(nx,ny,0:ntracers));fluxttr = 0.

    ALLOCATE(trwle(nz,0:ntracers));trwle = 0.
    ALLOCATE(trwsb(nz,0:ntracers));trwsb = 0.
    ALLOCATE(tradv(nz,0:ntracers));tradv = 0.
    ALLOCATE(trdiff(nz,0:ntracers));trdiff = 0.
    ALLOCATE(trphys(nz,0:ntracers));trphys = 0.

    ALLOCATE(tracername(0:ntracers))
    ALLOCATE(tracerunits(0:ntracers))

  END SUBROUTINE crm_allocate_tracers

! ====================================================================

  SUBROUTINE crm_deallocate_tracers
  ...
  END SUBROUTINE crm_deallocate_tracers
