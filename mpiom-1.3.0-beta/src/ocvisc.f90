...
!
      implicit none
      integer i,j,k,l
      REAL AULUX,AULUY,AULVX,AULVY,DLXPSI,DLYPSI,SPEEDU,SPEEDV
      DIMENSION AULUX(IE,JE),AULUY(IE,JE),AULVX(IE,JE),AULVY(IE,JE)     &
     &        ,DLXPSI(IE,JE),DLYPSI(IE,JE),SPEEDU(IE,JE),SPEEDV(IE,JE)
      real TRIDSY(ie,ke,3)
...
!$OMP SINGLE
      ...
      CALL bounds_exch('s',DLYPSI,'ocvisc 6')
!#ifdef bounds_exch_save
      CALL bounds_exch('s',DLXPSI,'ocvisc 5')
