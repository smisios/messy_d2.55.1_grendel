...
#endif
#ifdef __cpl_co2
  USE mo_carbch,          ONLY: co2conc, co2flux
#endif
...
#else
  INTEGER, PARAMETER :: nfldo2a = 7  ! no. of fields passed from ocean to atmosphere
#endif
#endif /*__cpl_co2*/
...
!WRITE(0,*) ' couple_init 2'
    ...
    CALL gather_arr(aofltxwo, gl_txwo, p_io)
    CALL gather_arr(u1e,      gl_u1e,  p_io)
    CALL gather_arr(v1e,      gl_v1e,  p_io)
    CALL gather_arr(aofltywe, gl_tywe, p_io)
    CALL gather_arr(aofltxio, gl_txio, p_io)
    CALL gather_arr(b1e,      gl_b1e,  p_io)
    CALL gather_arr(z1e,      gl_z1e,  p_io)
    CALL gather_arr(aofltyie, gl_tyie, p_io)
    CALL gather_arr(aoflfrio, gl_frio, p_io)
    CALL gather_arr(aoflfrwo, gl_frwo, p_io)
    CALL gather_arr(aoflrhio, gl_rhio, p_io)
    CALL gather_arr(aoflchio, gl_chio, p_io)
    CALL gather_arr(aoflnhwo, gl_nhwo, p_io)
    CALL gather_arr(aoflshwo, gl_shwo, p_io)
    CALL gather_arr(aoflwsvo, gl_wsvo, p_io)
#ifdef __cpl_dms
    CALL gather_arr(dmsflux,  gl_dmsflux, p_io)
    CALL gather_arr(dms,      gl_dms,     p_io)
#endif
#ifdef __cpl_co2
    CALL gather_arr(co2flux,  gl_co2flux, p_io)
#endif
...
!lk      CALL MPI_Comm_Rank(commlocal, rank, ierror)
    ...
    IF (p_parallel_io) THEN
       ...
       WRITE (io_stdout,*) 'Accumulation by mpi-om is broken.'
       WRITE (io_stdout,*) 'Please recompile with -D__accu_by_psmile.'
       CALL flush (io_stdout)
       CALL p_abort
#endif
    ENDIF  ! p_parallel_io
...
! !USES:
  ...
  USE MO_COMMO1,            ONLY: gila_g, giph_g, weto_g, dlxp    & 
                                 ,dlyp, dlxu, dlxv, dlyu, dlyv
...
! Local array for global fields
!  REAL,POINTER  :: dlxp_g(:,:), dlyp_g(:,:)
!  REAL,POINTER  :: dlxu_g(:,:), dlyu_g(:,:)
!  REAL,POINTER  :: dlxv_g(:,:), dlyv_g(:,:)
  REAL,ALLOCATABLE  :: dlxp_g(:,:), dlyp_g(:,:)
  REAL,ALLOCATABLE  :: dlxu_g(:,:), dlyu_g(:,:)
  REAL,ALLOCATABLE  :: dlxv_g(:,:), dlyv_g(:,:)
...
! Deallocate arrays
  ...
  DEALLOCATE(dlxp_g, dlyp_g, dlxu_g, dlxv_g, dlyu_g, dlyv_g)
...
! Gather fields for accumulation
  call gather_arr(tho(:,:,1),gl_sst,p_io)
  call gather_arr(sictho,gl_sictho,p_io)
  call gather_arr(sicomo,gl_sicomo,p_io)
  call gather_arr(sicsno,gl_sicsno,p_io)
  call gather_arr(uko(:,:,1),gl_socu,p_io)
  call gather_arr(vke(:,:,1),gl_socv,p_io)
  call gather_arr(sicuo(:,:),gl_sicu,p_io)
  call gather_arr(sicve(:,:),gl_sicv,p_io)
  if (p_parallel_io) then
     DO jy = 1,jpdim_ocej
        DO jx = 1,jpdim_ocei
...
! ATTN: dmsflux is only needed for diagnostics; for performace reasons it should be removed from the coupling interface
      ...
      IF ( clfield == clstr8o2a(index) ) THEN
         ...
         IF (p_parallel_io) THEN
            DO jx=1,jpdimi
               DO jy=1,jpdimj
                  pfield (jx,jy) = gl_co2flux(jx+1,jy)             ![kg CO2/m**2/sec]  
               ENDDO
            ENDDO
#ifdef __synout
            WRITE(kout,*)' get_o2a: ',clstr8o2a(index)         &
      ...
      CALL couple_abort (modnam,'get_o2a','pb Invalid locator string',io_stderr)
...
!              -----------------------------
      ...
      IF ( CLFIELD == 'TXWOCEAU' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_txwo(JX,JY) = pfield(JX-1,JY)*AMSUO_G_L1(JX,JY)
            ENDDO
         ENDDO
         DO JY = 1, JE_G
            gl_txwo( 1,JY) = gl_txwo(IE_G-1,JY)
            gl_txwo(IE_G,JY) = gl_txwo(  2,JY)
         ENDDO

       ENDIF
       call scatter_arr(gl_txwo, aofltxwo, p_io)
         GOTO 1000
      ENDIF
...
!              -----------------------------
      ...
      IF ( CLFIELD == 'TXWOCEAV' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_u1e(JX,JY) = pfield(JX-1,JY)*AMSUO_G_L1(JX,JY)
            ENDDO
         ENDDO

         DO JY = 1, JE_G
            gl_u1e( 1,JY) = gl_u1e(IE_G-1,JY)
            gl_u1e(IE_G,JY) = gl_u1e(2,JY)
         ENDDO
       ENDIF
      
       call scatter_arr(gl_u1e, u1e, p_io)
         GOTO 1000
      ENDIF
!
...
!              -----------------------------
      ...
      IF ( CLFIELD == 'TYWOCEAU' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_v1e(JX,JY) = pfield(JX-1,JY)*AMSUE_G_L1(JX,JY) 
            ENDDO
         ENDDO
         DO JY = 1, JE_G
            gl_v1e( 1,JY) = gl_v1e(IE_G-1,JY)
            gl_v1e(IE_G,JY) = gl_v1e(2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_v1e, v1e, p_io)
         GOTO 1000
      ENDIF
...
!              -----------------------------
      ...
      IF ( CLFIELD == 'TYWOCEAV' ) THEN
       ...
       IF(p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_tywe(JX,JY) = pfield(JX-1,JY)*AMSUE_G_L1(JX,JY) 
            ENDDO
         ENDDO
         DO JY = 1, JE_G
            gl_tywe( 1,JY) = gl_tywe(IE_G-1,JY)
            gl_tywe(IE_G,JY) = gl_tywe(  2,JY)
         ENDDO
       ENDIF
 
       call scatter_arr(gl_tywe, AOFLTYWE, p_io)
         GOTO 1000
      ENDIF
...
!              ----------------------
      ...
      IF ( CLFIELD == 'TXIOCEAU' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_txio(JX,JY) = pfield(JX-1,JY)*AMSUO_G_L1(JX,JY)
            ENDDO
         ENDDO

         DO JY = 1, JE_G
            gl_txio( 1,JY) = gl_txio(IE_G-1,JY)
            gl_txio(IE_G,JY) = gl_txio(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_txio, AOFLTXIO, p_io)
         GOTO 1000
      ENDIF
...
!              ----------------------
      ...
      IF ( CLFIELD == 'TXIOCEAV' ) THEN
       ...
       IF (p_parallel_io) then
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_b1e(JX,JY) = pfield(JX-1,JY)*AMSUO_G_L1(JX,JY)
            ENDDO
         ENDDO

         DO JY = 1, JE_G
            gl_b1e( 1,JY) = gl_b1e(IE_G-1,JY)
            gl_b1e(IE_G,JY) = gl_b1e(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_b1e, b1e, p_io)
...
!              ---------------------------
      ...
      IF ( CLFIELD == 'TYIOCEAU' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_z1e(JX,JY) = pfield(JX-1,JY)*AMSUE_G_L1(JX,JY)
            ENDDO
         ENDDO
         DO JY = 1, JE_G
            gl_z1e( 1,JY) = gl_z1e(IE_G-1,JY)
            gl_z1e(IE_G,JY) = gl_z1e(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_z1e, z1e, p_io)
...
!              ---------------------------
      ...
      IF ( CLFIELD == 'TYIOCEAV' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_tyie(JX,JY) = pfield(JX-1,JY)*AMSUE_G_L1(JX,JY)
            ENDDO
         ENDDO
         DO JY = 1, JE_G
            gl_tyie( 1,JY) = gl_tyie(IE_G-1,JY)
            gl_tyie(IE_G,JY) = gl_tyie(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_tyie, AOFLTYIE, p_io)
         GOTO 1000
...
!              -------------------------
      ...
      IF ( CLFIELD == 'FRIOCEAN' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_frio(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
            ENDDO
         ENDDO
         DO JY = 1, JE_G
            gl_frio( 1,JY) = gl_frio(IE_G-1,JY)
            gl_frio(IE_G,JY) = gl_frio(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_frio, aoflfrio, p_io)
         GOTO 1000
      ENDIF
...
!              --------------------------
      ...
      IF ( CLFIELD == 'FRWOCEAN' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_frwo(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
            ENDDO
         ENDDO
         DO JY = 1, JE_G
            gl_frwo( 1,JY) = gl_frwo(IE_G-1,JY)
            gl_frwo(IE_G,JY) = gl_frwo(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_frwo, aoflfrwo, p_io)
         GOTO 1000
      ENDIF
...
!              ----------------------------
      ...
      IF ( CLFIELD == 'RHIOCEAN' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1, JE_G
            DO JX = 2,IE_G-1
               gl_rhio(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
            ENDDO
         ENDDO

         DO JY = 1,JE_G   
            gl_rhio( 1,JY) = gl_rhio(IE_G-1,JY)
            gl_rhio(IE_G,JY) = gl_rhio(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_rhio, aoflrhio, p_io)
...
!              ---------------------------------
      ...
      IF ( CLFIELD == 'CHIOCEAN' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1,JE_G
            DO JX = 2,IE_G-1
               gl_chio(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
            ENDDO
         ENDDO
         DO JY = 1,JE_G
            gl_chio( 1,JY) = gl_chio(IE_G-1,JY)
            gl_chio(IE_G,JY) = gl_chio(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_chio, aoflchio, p_io)
         GOTO 1000
      ENDIF
...
!              -------------------------
      ...
      IF ( CLFIELD == 'NHWOCEAN' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1,JE_G
            DO  JX = 2,IE_G-1
               gl_nhwo(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
            ENDDO   
         ENDDO   
         DO JY = 1,JE_G
            gl_nhwo( 1,JY) = gl_nhwo(IE_G-1,JY)
            gl_nhwo(IE_G,JY) = gl_nhwo(  2,JY)
         ENDDO 
       ENDIF
       
        call scatter_arr(gl_nhwo, AOFLNHWO, p_io)
         GOTO 1000
      ENDIF
...
!               ----------------
      ...
      IF ( CLFIELD == 'SHWOCEAN' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1,JE_G
            DO JX = 2,IE_G-1
               gl_shwo(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
            ENDDO
         ENDDO
         DO JY = 1,JE_G
            gl_shwo( 1,JY) = gl_shwo(IE_G-1,JY)
            gl_shwo(IE_G,JY) = gl_shwo(  2,JY)
         ENDDO 
       ENDIF
       
       call scatter_arr(gl_shwo, AOFLSHWO, p_io)
         GOTO 1000
      ENDIF
...
!               --------------------
      ...
      IF ( CLFIELD == 'WSVOCEAN' ) THEN
       ...
       IF (p_parallel_io) THEN
         DO JY = 1,JE_G
            DO JX = 2,IE_G-1
               gl_wsvo(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
            ENDDO
         ENDDO
         DO JY = 1,JE_G
            gl_wsvo( 1,JY) = gl_wsvo(IE_G-1,JY)
            gl_wsvo(IE_G,JY) = gl_wsvo(  2,JY)
         ENDDO
       ENDIF

       call scatter_arr(gl_wsvo, AOFLWSVO, p_io)
         GOTO 1000
      ENDIF
...
!               --------------------
      ...
      IF ( CLFIELD == 'DEPOCEAN' ) THEN
         ...
         IF (p_parallel_io) THEN
            ...
            DO JY = 1,JE_G
               DO JX = 2,IE_G-1
                  gl_dustdep(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
               ENDDO
            ENDDO

            DO JY = 1,JE_G
               gl_dustdep( 1,JY) = gl_dustdep(IE_G-1,JY)
               gl_dustdep(IE_G,JY) = gl_dustdep(  2,JY)
            ENDDO

         ENDIF

         call scatter_arr(gl_dustdep, dustdep, p_io)
...
!               --------------------
      ...
      IF ( CLFIELD == 'CO2OCEAN' ) THEN

         IF (p_parallel_io) THEN

            DO JY = 1,JE_G
               DO JX = 2,IE_G-1
                  gl_co2conc(JX,JY) = pfield(JX-1,JY)*WETO_G(JX,JY,1)
               ENDDO
            ENDDO

            DO JY = 1,JE_G
               gl_co2conc( 1,JY) = gl_co2conc(IE_G-1,JY)
               gl_co2conc(IE_G,JY) = gl_co2conc(  2,JY)
            ENDDO

         ENDIF

         call scatter_arr(gl_co2conc, co2conc, p_io)
      ...
      CALL couple_abort (modnam,'put_a2o','pb Invalid locator string',io_stderr)
