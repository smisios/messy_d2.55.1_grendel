...
#ifdef TIMECHECK
      ttt = p_time()
      WRITE(io_stdout,123)'Time OCADPO/OCADFS',ttt-tttr
#endif
...
! for temperature and salinity in SBR OCTHER.
         ...
         DO L=1,NOCETRA
            ...
            ENDDO
       ENDDO
!       call maschk(ie,je,ke,45)
!         DO L=1,NOCETRA
          DO L=1,NTRAAD
            CALL OCADPO(OCETRA(1,1,1,L))
          ENDDO
...
!                        OCETRA(I,J,K,L)=MAX(0.,OCETRA(I,J,K,L)) 
         ...
         ENDDO
      ELSE
         STOP 'Global physics must go with ADPO!'
      ENDIF
#ifdef TIMECHECK
      ttt = p_time()
      WRITE(io_stdout,123)'Time BGC tracer advection',ttt-tttr
#endif
#endif /*PBGC*/
!--------------------------------------------------------------------
...
#ifdef TIMECHECK
       ttt = p_time()
       WRITE(io_stdout,123)'Time OCJITR',ttt-tttr
#endif
       IF ( ICONTRO .ne. 0 ) THEN
          CALL CONTRO(2713)
       endif
...
#ifdef TIMECHECK
       ttt = p_time()
       WRITE(io_stdout,123)'Time OCTDIFF',ttt-tttr
#endif
...
!
!     Biogeochemistry:  diffusion
#ifdef TIMECHECK
          tttr = p_time()
#endif

!      call maschk(ie,je,ke,49)
...
!      call maschk(ie,je,ke,50)

#ifdef TIMECHECK
          ttt = p_time()
          WRITE(io_stdout,123)'Time OCTDIFF_BGC',ttt-tttr
#endif
