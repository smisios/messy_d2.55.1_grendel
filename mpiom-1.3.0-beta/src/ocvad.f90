...
!$OMP DO
  DO K=1,KE
    DO J=1,JE
      DO I=1,IE
        TRVOL(I,J,K)=DLXP(I,J)*DLYP(I,J)*DDUE(I,J,K)
        TRP(I,J,K)=0.
        TRM(I,J,K)=0.
        TR1(I,J,K)=TRF(I,J,K)
        WTP(I,J,K)=0.
...
!EMR    UWE=0.5*(UKO(I-1,J,K)*DDUO(I,J,K)+UKO(I,J+1,K)*DDUO(I,J+1,K))
        ...
        UM=0.5*DT*(ABS(UWE)-UWE)*DLYU(I-1,J)
        SCHA=MAX(0.,(ABL-ZWABL)/(ABL+1.E-20))
        !
        SCH=MIN(1.,SCHA*TRVOL(I,J,K)/(UP+UM+1.E-20))
        SCHI=1.-SCH
...
!#ifdef bounds_exch_save
!$OMP SINGLE
  CALL bounds_exch('s-',TRP,'ocvad 4')
  CALL bounds_exch('s-',TRM,'ocvad 5')
  CALL bounds_exch('p',WTP,'ocvad 6')
  CALL bounds_exch('p',WTM,'ocvad 7')
!$OMP END SINGLE
!#endif
  !
!$OMP DO
...
!#ifdef bounds_exch_save
  ...
  CALL bounds_exch('p-',TRP,'ocvad 11')
  CALL bounds_exch('p-',TRM,'ocvad 12')
  CALL bounds_exch('p',WTP,'ocvad 13')
  CALL bounds_exch('p',WTM,'ocvad 14')
#endif
!#endif
