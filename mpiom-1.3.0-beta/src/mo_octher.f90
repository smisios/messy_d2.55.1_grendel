...
!hh   river runoff data from ieee extra file: observations 12mon.clim.
      ...
      IF(p_pe==p_io) THEN
#if defined(MESSY) && !defined(_SX6) && !defined(_PATHSCALE)
#ifndef NOENDIANCONVERT
         OPEN(io_in_rval,file='runoff_obs',form='unformatted',convert='big_endian')
         OPEN(io_in_rpos,file='runoff_pos',form='unformatted',convert='big_endian')
#else
