...
!**********************************************************************
      ...
      REAL :: ppm2con, co2atm,ozkohl

#ifdef __c_isotopes
      REAL :: c13flux,c14flux
      REAL :: zatm13c,zatm14c
      REAL :: total13c,total14c,zpror13,zpror14,zprca13,zprca14
#endif
      return
      i=25
      j=14
      k=5
      l=3
      write(0,*)'mas',imas,ocetra(i,j,k+1,l),ocetra(i,j,k,l)
!     if (imas.ne.100) return !----------------------------------------------
...
#ifdef __c_isotopes
        c13flux =c13flux +bgct2d(i,j,jc13flux)*dlxp(i,j)*dlyp(i,j)
        c14flux =c14flux +bgct2d(i,j,jc14flux)*dlxp(i,j)*dlyp(i,j)
#endif
...
!     & +sco2flux
     ...
     & +zburial(isssc13)+zburial(issso13)*rcar+zpror13+zprca13   &
#ifdef DIFFAT     
     & +zatm13c*ppm2con  
#else
!     & +c13flux
     ...
     & +zburial(isssc14)+zburial(issso14)*rcar+zpror14+zprca14   &
#ifdef DIFFAT     
     & +zatm13c*ppm2con  
#else
     & +atm_c14*atmacmol
!+atmacmol*(c14prosta-c14prod)
#endif
#endif /*__c_isotopes*/
      totalnitr=                                                      &
     &   (zocetrato(idet)+zocetrato(idoc)+zocetrato(iphy)             &
...
#endif     
       if(imas.eq.101) then
       write(0,*)'living pools',zocetrato(iphy),zocetrato(izoo),     &
     & zocetrato(idoc)
!      atm_c14=atm_c14+(c14inva-total14c)/atmacmol
!      c14prod=(c14inva-total14c)/atmacmol
!      c14inva=total14c
!      write(0,*)'c14production',c14prod
       endif
#ifdef __c_isotopes
      if(imas.eq.100) then
      total14c=                                                    &
...
#else
      WRITE(io_stdo_bgc,*) 'Global total[kmol] of kohlen   : ',       &
     & totalcarbon,ozkohl,atm_co2*atmacmol,imas
#endif



      write(io_stdo_bgc,*)'einzeln',ozkohl,c14prod*atmacmol,ozkohl-ozkoa        &
     &,atm_co2*atmacmol-atcoa,imas            
