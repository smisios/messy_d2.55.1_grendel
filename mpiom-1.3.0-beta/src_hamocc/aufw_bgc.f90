...
! ----------------------------------------------------------------------    
     ...
     &    9,'?????????',39 ,'apparent solubility product for calcite',  &
...
!!$      DO kt=1,12
!!$      DO kk=1,8
!!$         CALL gather_arr(chemcm(:,:,kk,kt),chemcm_g(:,:,kk,kt),p_io)
!!$      ENDDO
!!$      ENDDO
!!$      IF(p_pe==p_io) THEN
!!$        ncstat = NF_INQ_VARID(ncid,'chemcm',ncvarid)
!!$        IF ( ncstat .NE. NF_NOERR ) CALL stop_all(err_text)
!!$        err_text = 'NETDCF: Problem with put for chemcm'
!!$        ncstat = NF_PUT_VAR_DOUBLE(ncid,ncvarid,chemcm_g)
!!$        IF ( ncstat .NE. NF_NOERR ) CALL stop_all(err_text)
!!$        err_text = 'NETDCF: Problem with flush for chemcm'
!!$        ncstat = NF_SYNC(ncid)
!!$        IF ( ncstat .NE. NF_NOERR ) CALL stop_all(err_text)
!!$      ENDIF
