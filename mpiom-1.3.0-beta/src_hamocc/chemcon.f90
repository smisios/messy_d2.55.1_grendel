...
implicit none
      ...
      REAL :: rrr, rho,bor,p,cp,tc
      REAL :: a1,a2,a3,b1,b2,b3,atn2o,rs
        


       WRITE(io_stdo_bgc,*)  ' CHEMCON is called with modal,kpie,kpje,kpke,kplmonth,ptiestu:  ',  &
...
!      call bounds_exch('p+',ptho,'chemcon 1')
!      call bounds_exch('p+',psao,'chemcon 2')
!      call bounds_exch('p+',pddpo,'chemcon 3')


      IF(modal.LE.0) THEN
      DO 1 j=1,kpje
      DO 1 i=1,kpie
      IF(pddpo(i,j,1).GT.0.5) THEN           ! wet cell

!
!*        21.1 SET ABSOLUTE TEMPERATURE
!              ------------------------
      T=ptho(i,j,1)+TEMZER                     ! degC to K
      Q=T*PERC                                 ! perc=0.01
      S=MAX(25.,psao(i,j,1))                   ! minimum salinity 25
