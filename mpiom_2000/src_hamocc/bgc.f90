...
!**********************************************************************
      ...
      USE mo_timeser_bgc
      USE mo_param1_bgc
      USE mo_inventory , ONLY : inventory_bgc2

#ifdef __cpl_co2
      USE mo_commo1, ONLY : ndtday
#endif
#ifdef PDYNAMIC_BGC
      USE mo_dynamic
#endif /* PDYNAMIC_BGC */
...
implicit none
      INTEGER :: kpie,kpje,kpke,i,j!,l,k

      REAL(wp),intent(in) :: pfswr  (kpie,kpje)
      REAL(wp),intent(in) :: psicomo(kpie,kpje)
      REAL(wp),intent(in) :: pfu10(kpie,kpje)

      REAL(wp),intent(in) :: ptho (kpie,kpje,kpke)
      REAL(wp),intent(in) :: psao (kpie,kpje,kpke)
      REAL(wp),intent(in) :: pddpo(kpie,kpje,kpke)
      REAL(wp),intent(in) :: pdlxp(kpie,kpje),pdlyp(kpie,kpje)
      REAL(wp),intent(in) :: pdpio(kpie,kpje,kpke)
      REAL(wp),intent(in) :: pwo(kpie,kpje,kpke+1)
      REAL(wp),intent(in) :: ppao(kpie,kpje)
...
! Net solar radiation: multiply  with sea ice concentration
      ...
      DO  j=1,kpje
      DO  i=1,kpie
        strahl(i, j) = pfswr(i, j) * (1._wp - psicomo(i, j))
      ENDDO
      ENDDO
...
!!$      enddo
      ...
      ! Reset carbon inventory to 38500 Gt (number hardcoded in carreset)
      IF (invreset) THEN
         CALL carreset(kpie,kpje,kpke,pddpo,psao,ptho,psicomo,pfu10,1,1,1,pdlxp,pdlyp)
         invreset = .FALSE.
      END IF
...
!     Sediment module
     ...
     &   'Sediment shifting at runstep ',ldtrunbgc,' ...'
      ...
      CALL inventory_bgc2(kpie,kpje,kpke,kplyear,kplmon,kplday)
