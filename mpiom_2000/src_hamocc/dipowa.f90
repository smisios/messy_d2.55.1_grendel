      SUBROUTINE DIPOWA(kpie,kpje,kpke,pdlxp,pdlyp,pwo)
!
...
implicit none
      ...
      REAL(wp) :: sedb1(kpie,0:ks,npowtra)                ! ????
      REAL(wp) :: zcoefsu(0:ks),zcoeflo(0:ks)             ! diffusion coefficients (upper/lower)
!!!!      REAL(wp) :: TREDSY(kpie,0:kpke,3)                   ! redsy for 'reduced system'?
      REAL(wp) :: TREDSY(kpie,0:ks,3)                   ! redsy for 'reduced system'?
...
!ik needed for boundary layer ventilation in fast sediment routine
      ...
      REAL(wp) :: pwo(kpie,kpje,kpke+1)       ! vertical velocity at scalar points (not used)
      REAL(wp) :: bolven(kpie)                ! bottom layer ventilation rate
      ...
      ENDDO
      zcoeflo(ks) = 0.0_wp                                ! diffusion coefficient for bottom sediment layer
