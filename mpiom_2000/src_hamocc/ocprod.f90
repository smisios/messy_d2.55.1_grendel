...
! nocetra is the number of all BGC elements (size of ocetra(,,,l))
     ...
     &        docrem, opalrem, remin2o, aou, refra
      ...
      REAL(wp) :: dustinp, dustinp_volcano
      REAL(wp) :: fopa, fdet, fcal
      REAL(wp) :: dmsprod,dms_bac,dms_uv
      REAL(wp) :: detref, detrl
      INTEGER :: volcano
#ifdef __c_isotopes
      REAL(wp) :: rem13,rem14
      REAL(wp) :: rl13, rl14
...
!$OMP PARALLEL PRIVATE(i,j,k,l,dustinp,dustinp_volcano, &
!$OMP   avphy,avanut,avanfe,xa,xn,phosy,volcell,avsil,avgra,ya,yn, &
!$OMP   grazing,graton,gratpoc,grawa,bacfra,phymor, &
!$OMP   zoomor,excdoc,exud,export,delsil,delcar,dmsprod,dms_bac,dms_uv,   &
!$OMP   sterph,sterzo,remin,detref,docrem,opalrem,aou,refra,remin2o,detrl, &
!$OMP   pho,phofa,temfa,fopa,fdet,fcal &
#ifdef __c_isotopes
!$OMP  ,rem13,rem14,rl13,rl14,rocean13,rocean14,   &
!$OMP   flui13,flui14,fcal13,fcal14,d13C,d14C &
#endif
#ifdef AGG
!$OMP         ,fphy,avmass, avsize, avsmin1, avsmax1, avnos, anosloss , &
!$OMP         nosin1, nosde1, nosin2, nosde2, nosin3, nosde3, &
!$OMP         zmornos, avm, avn, eps, e1,e2,e3,e4,es1,es3, &
!$OMP         TopM, TopF, snow,fshear,sagg1,sagg2,sagg4, &
!$OMP         sett_agg,shear_agg,effsti,dfirst,dshagg,dsett, &
!$OMP         checksize,nacheck,flar1,flar2,flar3, &
!$OMP         fTSFac,fTMFac,fTopF,fTopM,wphy,wphyup,wnosup,wnos &
#endif
!$OMP    )
...
!$OMP DO
      ...
      DO j=2,kpje-1
         DO i=2,kpie-1
            IF (pddpo(i, j, k) .GT. 0.5_wp) THEN
               phofa=pi_alpha*strahl(i,j)*swr_frac(i,j,k)
               temfa= 0.6_wp * 1.066_wp**ptho(i,j,k)
               pho= dtb*phofa*temfa/SQRT(phofa**2 + temfa**2)
!!$               pho=pi_alpha*strahl(i,j)*swr_frac(i,j,k)        &        ! strahl =swr*(1-icecover_fraction) pi_alpha=0.02*dtb
...
!!$                                                                     ! emr: "optimum" 0.8, radiation ~400Wm^-2 ->.002*400=.8
               ...
               avanut = MAX(0._wp, MIN(ocetra(i, j, k, iphosph),                &  ! available nutrients [kmol P/m3] (phosphate
     &             rnoi * ocetra(i,j,k,iano3)))                             ! rnoi=1./16.                     + nitrate
               avanfe = MAX(0._wp, MIN(avanut,ocetra(i,j,k,iiron)/riron))       ! riron=6.1e-4                    iron limitation
               xa=avanfe                                             ! try 10% iron-independent plankton: .9*avanfe+.1*avanut???
               xn = xa / (1._wp + pho * avphy / (xa + bkphy))
               phosy = MAX(0._wp, xa-xn)

               volcell = pdlxp(i, j) * pdlyp(i, j) * pddpo(i, j, k) * 1.e-6_wp          ! why *1.e-6? pdlxp [m], pdlyp [m], pddpo [m]
...
! 13C, 14C 'removal' by exportproduction
         ...
         flui13 = MAX(rocean13 - 1._wp, 0._wp)                                    ! assumes rocean >1 in euphotic layer
         flui14 = MAX(rocean14 - 1._wp, 0._wp)                                    ! flui means what? (flui14 never used) flux...?
...
! DMS (js: slightly out of place)
     ...
     &            * (1._wp + 1._wp / (ptho(i, j, k) + dmsp1)**2)
         dms_bac = dmsp3 * ABS(ptho(i, j, k) + 3._wp) * ocetra(i, j, k, idms) &   ! bacterial consumption
...
!     &      + bifr14*rcar*( bacfra - phosy + graton + ecan*zoomor)
     ...
     &                        -(graton+ecan*zoomor)*ro2ut
         ...
         opalrem = dremopal * 0.1_wp &
              * (ptho(i, j, k) + 3.0_wp) * ocetra(i, j, k, iopal)
...
!$OMP DO
      DO 20 k=kwrbioz+1,kpke
         ...
         DO 201 i=1,kpie
            ...
            IF(ocetra(i,j,k,ioxygen).gt.5.e-8_wp) THEN                          ! remineralization of poc12 using oxygen
...
!***********************************************************************
     ...
     &                           + (remin + docrem) * 1.e-4_wp * ro2ut * refra
            ocetra(i,j,k,igasnit)=ocetra(i,j,k,igasnit)                &
     &                           -(remin+docrem) * 1.e-4_wp * ro2ut * refra
            ocetra(i,j,k,ioxygen)= ocetra(i,j,k,ioxygen)               &
     &                           -(remin+docrem)*1.e-4_wp*ro2ut*refra*0.5_wp
...
!$OMP DO
      DO 30 k=kwrbioz+1,kpke
         DO 30 j=1,kpje
         DO 30 i=1,kpie
            IF (ocetra(i, j, k, ioxygen) .LT. 5.e-7_wp) THEN                            ! denitrification
#ifdef AGG
               avmass = ocetra(i,j,k,iphy)+ocetra(i,j,k,idet)
#endif /*AGG*/

               remin = 0.05_wp * drempoc * MIN(ocetra(i, j, k, idet),               &       ! remineralization using NO3
     &                           0.5_wp * ocetra(i, j, k, iano3) / nitdem)

               detref = remin / (ocetra(i, j, k, idet) + 1.e-60_wp)                        ! P-units
#ifdef __c_isotopes
...
! proxies
     ...
     &                              - (rem14+rl14    )
               ...
               ocetra(i,j,k,iano3)  =ocetra(i,j,k,iano3)  -nitdem*remin + rnit*remin2o
               ocetra(i, j, k, igasnit) = ocetra(i, j, k, igasnit) &
...
!alkalinity is increased during denitrification due to consumption of H+ (see Wolf-Gladrow etal,2007)
               ocetra(i,j,k,ialkali)=ocetra(i,j,k,ialkali)+nitdem*remin - rnit*remin2o
               n2budget(i, j, k) = n2budget(i, j, k) + 2._wp * n2prod * remin
!denitrification produces water (H2O), the corresponding O2 uptake is budgeted in h2obudget
               h2obudget(i, j, k) = h2obudget(i, j, k) + 0.5_wp * n2prod * remin
...
!$OMP DO
      DO 301 k=kwrbioz+1,kpke
         DO 301 j=1,kpje
         DO 301 i=1,kpie
            IF (ocetra(i, j, k, ioxygen) .LT. 3.e-6_wp) THEN
#ifdef AGG
               avmass = ocetra(i,j,k,iphy) + ocetra(i,j,k,idet)
#endif /*AGG*/
               ...
               ocetra(i,j,k,ialkali) = ocetra(i,j,k,ialkali) - rnit * remin
...
! needed for the division
          IF (ABS(eps - 3._wp) .LT. 1.e-15_wp) eps = 3._wp + vsmall
          IF (ABS(eps - 4._wp) .LT. 1.e-15_wp) eps = 4._wp + vsmall
          IF (ABS(eps - 3. - SinkExp) .LT. 1.e-15_wp) eps = 3._wp + SinkExp + vsmall
          IF (ABS(eps - 1._wp - SinkExp - FractDim) .LT. 1.e-15_wp)    &
...
! SIZE DITRIBUTION
         ...
         eps = ((1._wp+ FractDim)*snow-avn*cellmass) /                    &  ! exponential coefficient of size distribution
     &           (snow-avn*cellmass)

         IF (ABS(eps - 3._wp) .LT. 1.e-15_wp) eps=3._wp + vsmall
         IF (ABS(eps - 4._wp) .LT. 1.e-15_wp) eps=4._wp + vsmall
         IF (ABS(eps - 3._wp - SinkExp) .LT. 1.e-15_wp) &
...
! SIZE DITRIBUTION
         ...
         eps = ((1._wp + FractDim) * snow - avn * cellmass) /          &
     &           (snow-avn*cellmass)

         IF (ABS(eps - 3._wp) .LT. 1.e-15_wp) eps = 3._wp + vsmall
         IF (ABS(eps - 4._wp) .LT. 1.e-15_wp) eps = 4._wp + vsmall
         IF (ABS(eps - 3._wp - SinkExp) .LT. 1.e-15_wp) eps = 3._wp + SinkExp + vsmall
         IF (ABS(eps - 1._wp - SinkExp - FractDim) .LT. 1.e-15_wp)           &
              eps = 1._wp + SinkExp + FractDim + vsmall
...
! first depth
         ...
         if(k1ts1(l).gt.0) then
             ...
             ts1(its1fdet,l+1,lts1) = ts1(its1fdet,l+1,lts1) -9999._wp
             ts1(its1fopa,l+1,lts1) = ts1(its1fopa,l+1,lts1) -9999._wp
             ts1(its1fcal,l+1,lts1) = ts1(its1fcal,l+1,lts1) -9999._wp
          endif
...
! second depth
         ...
         if(k2ts1(l).gt.0) then
             ...
             ts1(its2fdet,l+1,lts1) = ts1(its2fdet,l+1,lts1) -9999._wp
             ts1(its2fopa,l+1,lts1) = ts1(its2fopa,l+1,lts1) -9999._wp
             ts1(its2fcal,l+1,lts1) = ts1(its2fcal,l+1,lts1) -9999._wp
          endif
! third depth
         if(k3ts1(l).gt.0) then
             ...
             ts1(its3fdet,l+1,lts1) = ts1(its3fdet,l+1,lts1) -9999._wp
             ts1(its3fopa,l+1,lts1) = ts1(its3fopa,l+1,lts1) -9999._wp
             ts1(its3fcal,l+1,lts1) = ts1(its3fcal,l+1,lts1) -9999._wp
         endif
      ENDDO
!
...
! write out fluxes at 90 m (fixed settling velocity part)
         if (kbo(i,j).gt.n90depth) then
         bgcflux(i,j,kcoex90) =  ocetra(i,j,n90depth,idet)*wpoc/dtbgc      !CDI tinka
...
! write out fluxes at about 1000 m
         if (kbo(i,j).gt.n1000depth) then
         bgcflux(i,j,kcoex1000) =  ocetra(i,j,n1000depth,idet)*wpoc/dtbgc      !CDI tinka
...
! write out fluxes at about 1950 m
         if (kbo(i,j).gt.n2000depth) then
         bgcflux(i,j,kcoex2000) =  ocetra(i,j,n2000depth,idet)*wpoc/dtbgc      !CDI tinka
