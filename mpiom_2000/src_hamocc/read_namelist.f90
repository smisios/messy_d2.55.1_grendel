...
implicit none
      ...
      NAMELIST /BGCCTL/ rmasko,rmasks,kchck,isac                     &
                      ...
                      ,lspinbgc,fspinbgc,invreset                    &
                      ,deltacalc,deltaorg,deltasil,diffat
...
!
      IF(p_pe==p_io) THEN
        OPEN(io_stdi_bgc,FILE='NAMELIST_BGC',STATUS='UNKNOWN',           &
...
!        CLOSE(io_stdi_bgc)
      ...
      CALL p_bcast(fspinbgc,p_io)
      CALL p_bcast(invreset,p_io)
      CALL p_bcast(deltacalc,p_io)
      CALL p_bcast(deltaorg,p_io)
      CALL p_bcast(deltasil,p_io)
