...
implicit none
     ...
     &  meantime_2d,meancnt_bgc_2D,kpje,kpie,nbgcm2d
      ...
      ! FIXME: this continuous dividing is sick: replace with mult of inverse
      DO l=1,kdtot
      DO k=1,nbgcdyn
      DO j=1,kpje
      DO i=1,kpie
         bgcdyn(i, j, k, l) = bgcdyn(i, j, k, l) / REAL(meancnt_bgc_2D, wp)
      ENDDO
      ENDDO
      ENDDO
      ENDDO
      ...
      DO j=1,kpje
      DO i=1,kpie
         bgc_zmld(i, j) = bgc_zmld(i, j) / REAL(meancnt_bgc_2D, wp)
         bgc_nmld(i, j) = bgc_nmld(i, j) / REAL(meancnt_bgc_2D, wp)
      ENDDO
      ENDDO
