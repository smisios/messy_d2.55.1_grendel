...
!      ncstat = NF_CREATE('bgcmean_sed.nc',NF_CLOBBER, nc_sed_id)
      ncstat = NF_CREATE('bgcmean_sed.nc',IOR(NF_CLOBBER,NF_64BIT_OFFSET), nc_sed_id)
...
!-----------------------------------------------------------------------
      ...
      CALL NETCDF_DEF_VARDB(nc_sed_id,4,'time',1,ncdims,ncvarid,          &
...
!-----------------------------------------------------------------------
     ...
     &   8,'degree E',34,'2-d longitude of scalar grid cells',rmasko,20,io_stdo_bgc)
      icode=1
      ncstat = NF_PUT_ATT_INT(nc_sed_id,ncvarid, 'code', NF_INT, 1, icode)
