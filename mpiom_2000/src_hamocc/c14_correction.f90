...
!     satoxy is computed in chemcon (called before aufr, seems ok.)
      ...
      subroutine c14_correction(kpie,kpje,kpke,pdlxp,pdlyp,pddpo,psao,ptho)
#ifdef __c_isotopes
      USE mo_carbch
      USE mo_biomod
      USE mo_sedmnt
      ...
      REAL(wp)  aq,cq,sq                        ! global average alkalinity, 12C, salinity
      REAL(wp)  volcell
      REAL(wp)  totvol                          ! total volume of wet cells
      REAL(wp)  an (kpie,kpje,kpke)             ! salinity-normalised alkalinity
      REAL(wp)  cn (kpie,kpje,kpke)             ! salinity-normalised 12C
      REAL(wp)  orgrem(kpie,kpje,kpke)          ! remineralized organic carbon
      REAL(wp)  caldis(kpie,kpje,kpke)          ! remineralized carbonate shells
      totvol =0.
      ...
      do i=1,kpie
       ...
       if (pddpo(i,j,k).gt.0.5 .and. ptho(i,j,k).lt.10.) then               ! wet cells with T < 10 degC
