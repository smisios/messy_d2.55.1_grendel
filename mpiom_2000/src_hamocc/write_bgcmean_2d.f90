...
!**************************************************************************
      ...
      USE mo_commo1, ONLY: lyears, lmonts, ldays, ldtdayc, ndtday
      ...
      implicit none
      INTEGER :: kpie,kpje,kpke,i,j,k,nk
      REAL(wp) :: pddpo(kpie,kpje,kpke)

      REAL(wp) :: time2d,t_hour
...
!      IF ( ncstat .NE. NF_NOERR ) call STOP_ALL('WRITE_BGCMEAN_2D: Problem with netCDF103d')
      ...
      if(mean_2D_freq.ne.4) then
         ...
         t_hour=REAL(ldtdayc, wp)/REAL(ndtday, wp)
      endif
      time2d = REAL(LDAYS+LMONTS*100+LYEARS*10000, wp) + t_hour
...
!      IF ( ncstat .NE. NF_NOERR ) STOP 'WRITE_BGCMEAN_2D: Problem with netCDF205'
      ...
      CALL write_netcdf_var(nc_2d_id,'co214flux',bgct2d(1,1,jco214f),nk,meantime_2d)
      CALL write_netcdf_var(nc_2d_id,'co2flux',bgct2d(1,1,jco2flux),nk,meantime_2d)
      CALL write_netcdf_var(nc_2d_id,'n2oflux',bgct2d(1,1,jn2oflux),nk,meantime_2d)
      CALL write_netcdf_var(nc_2d_id,'prorca',bgct2d(1,1,jprorca),nk,meantime_2d)
      CALL write_netcdf_var(nc_2d_id,'prcaca',bgct2d(1,1,jprcaca),nk,meantime_2d)
      CALL write_netcdf_var(nc_2d_id,'silpro',bgct2d(1,1,jsilpro),nk,meantime_2d)
      CALL write_netcdf_var(nc_2d_id,'produs',bgct2d(1,1,jprodus),nk,meantime_2d)

#ifdef PANTHROPOCO2
      CALL write_netcdf_var(nc_2d_id,'co2fant',bgct2d(1,1,jco2fant),nk,meantime_2d)
      CALL write_netcdf_var(nc_2d_id,'cacadiff',bgct2d(1,1,jcacadiff),nk,meantime_2d)
#endif
#ifdef ANTC14
      CALL write_netcdf_var(nc_2d_id,'rantc14',rbomb(1,1),nk,meantime_2d)
#endif
