...
!     CALL read_netcdf_var(ncid,'aksp',aksp(1,1,1),kpke)
     ...
     IF(p_pe==p_io) THEN
        ...
        IF ( ncstat .NE. NF_NOERR ) THEN
           WRITE(io_stdo_bgc,*) 'AUFR - calcinpglint not found in restart file'
        ELSE
           ncstat = NF_GET_VAR_DOUBLE(ncid,ncvarid,calcinpglint)
           IF ( ncstat .NE. NF_NOERR ) CALL stop_all('AUFR: Problem reading calcinpglint')
        END IF
        ncstat = NF_INQ_VARID(ncid,'orginpglint',ncvarid)
        IF ( ncstat .NE. NF_NOERR ) THEN
           WRITE(io_stdo_bgc,*) 'AUFR - orginpglint not found in restart file'
        ELSE
           ncstat = NF_GET_VAR_DOUBLE(ncid,ncvarid,orginpglint)
           IF ( ncstat .NE. NF_NOERR ) CALL stop_all('AUFR: Problem reading orginpglint')
        END IF
        ncstat = NF_INQ_VARID(ncid,'silinpglint',ncvarid)
        IF ( ncstat .NE. NF_NOERR ) THEN
           WRITE(io_stdo_bgc,*) 'AUFR - silinpglint not found in restart file'
        ELSE
           ncstat = NF_GET_VAR_DOUBLE(ncid,ncvarid,silinpglint)
           IF ( ncstat .NE. NF_NOERR ) CALL stop_all('AUFR: Problem reading silinpglint')
        END IF

        WRITE(io_stdo_bgc,*) 'AUFR - Integrated continental input (calc,org,sil): ', &
                                        calcinpglint, orginpglint, silinpglint
     END IF
     CALL p_bcast(calcinpglint, p_io)
     CALL p_bcast(orginpglint,  p_io)
     CALL p_bcast(silinpglint,  p_io)
...
#else
!warn "!! Attention !! Update required !!!!!"
...
!
      ...
      IF (pddpo(i, j, k) .GT. 0.5_wp) THEN
         ocetra(i, j, k, l) = MAX(ocetra(i, j, k, l), 0._wp)
      ENDIF
      ENDDO
      ENDDO
      ENDDO
...
!ik         poc   (i,j,k)=MAX(poc   (i,j,k),0.)
         hi(i, j, k) = MAX(hi(i, j, k), 1.e-12_wp)
!ik         calciu(i,j,k)=MAX(calciu(i,j,k),0.)
      ...
      IF (kbo(i, j) .GT. 1) THEN
         powtra(i, j, k, ipowaic) = MAX(powtra(i, j, k, ipowaic), 0._wp)
#ifdef __c_isotopes
         powtra(i, j, k, ipowc13) = MAX(powtra(i, j, k, ipowc13), 0._wp)
         powtra(i, j, k, ipowc14) = MAX(powtra(i, j, k, ipowc14), 0._wp)
#endif
         powtra(i, j, k, ipowaal) = MAX(powtra(i, j, k, ipowaal), 0._wp)
         powtra(i, j, k, ipowaph) = MAX(powtra(i, j, k, ipowaph), 0._wp)
         powtra(i, j, k, ipowaox) = MAX(powtra(i, j, k, ipowaox), 0._wp)
         powtra(i, j, k, ipown2)   =MAX(powtra(i, j, k, ipown2)  ,0._wp)
         powtra(i, j, k, ipowno3) = MAX(powtra(i, j, k, ipowno3), 0._wp)
         powtra(i, j, k, ipowasi) = MAX(powtra(i, j, k, ipowasi), 0._wp)
         sedlay(i, j, k, issso12) = MAX(sedlay(i, j, k, issso12), 0._wp)
         sedlay(i, j, k, isssc12) = MAX(sedlay(i, j, k, isssc12), 0._wp)
#ifdef __c_isotopes
         sedlay(i, j, k, isssc13) = MAX(sedlay(i, j, k, isssc13), 0._wp)
         sedlay(i, j, k, isssc14) = MAX(sedlay(i, j, k, isssc14), 0._wp)
#endif
         sedlay(i, j, k, issssil) = MAX(sedlay(i, j, k, issssil), 0._wp)
         sedlay(i, j, k, issster) = MAX(sedlay(i, j, k, issster), 0._wp)
         sedhpl(i, j, k) = MAX(sedhpl(i, j, k), 1.e-12_wp)
      ENDIF
      ENDDO
      ENDDO
      ENDDO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE AUFR_MASK_BGC(kpie,kpje,kpke,pddpo,kplyear,kplmon     &
     &                    ,kplday,kpldtoce)
!$Source: /server/cvs/mpiom1/mpi-om/src_hamocc/aufr_bgc.f90,v $\\
!$Revision: 1.3.2.1.16.1.4.2 $\\
!$Date: 2006/04/06 10:01:07 $\\
...
!               call STOP_ALL('Stop : Restart file with different topography:     &
!     &         land mask values at wet points')
               IF (pddpo(i-1, j, 1) .GT. 0.5_wp) ocetra(i, j, k, l) &
                    = ocetra(i-1, j, 1, l)
               IF (pddpo(i+1, j, 1) .GT. 0.5_wp) ocetra(i, j, k, l) &
                    = ocetra(i+1, j, 1, l)
               IF (pddpo(i, j+1, 1) .GT. 0.5_wp) ocetra(i, j, k, l) &
                    = ocetra(i, j+1, 1, l)
               IF (pddpo(i, j-1, 1) .GT. 0.5_wp) ocetra(i, j, k, l) &
                    = ocetra(i, j-1, 1, l)
            END IF
