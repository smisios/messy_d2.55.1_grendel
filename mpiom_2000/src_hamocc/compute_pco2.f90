...
!**********************************************************************
      ...
      REAL(wp) psao(kpie,kpje,kpke)                 ! salinity (in)
      REAL(wp) pddpo(kpie,kpje,kpke)                ! cell thicknesses (in)
      REAL(wp) pco2(kpie,kpje)                      ! CO2 in ppm (out)

      REAL(wp) :: dddhhh,dadh,a,h,c,alk,t1,t2
      REAL(wp) :: akbi,ak2i,ak1i,dtja
...
! Calculate new hi concentration
         ...
         a = c * (2._wp + t2) / (1._wp + t2 + t2 * t1) + akw / h - h &
              + bt/(1._wp + h * akbi) - alk
         dadh = c * (1._wp / (ak2 * (1._wp + t2 + t2 * t1)) - (2._wp + t2) &
              * ((1._wp + 2._wp * t1) * ak2i)/  &
     &       (1._wp + t2 + t2 * t1)**2)                                      &
     &       - akw / h**2 - 1._wp - (bt * akbi) / (1._wp + h * akbi)**2
         ...
         t1=h*ak1i
         t2=h*ak2i
         a = c * (2._wp + t2) / (1._wp + t2 + t2 * t1) + akw/h &
              - h + bt/(1._wp + h * akbi) - alk
...
!
         pco2(i, j) = c / ((1._wp + ak1 * (1._wp + ak2 / h) / h) * ak0)
