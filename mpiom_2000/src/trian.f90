SUBROUTINE TRIAN
  ...
  REAL(wp) FAKE, FAKN, FAKS, FAKW, RE, RR1, WILKIN, ZZZ

  REAL(wp), ALLOCATABLE :: FW(:,:), FE(:,:), FN(:,:), FS(:,:)
  REAL(wp), ALLOCATABLE :: FW_G(:,:), FE_G(:,:)
  REAL(wp), ALLOCATABLE :: FN_G(:,:), FS_G(:,:)


  ALLOCATE ( fw(ie,je), fe(ie,je), fn(ie,je), fs(ie,je))

  IF (p_pe == p_io) THEN
     ALLOCATE ( fw_g(ie_g,je_g), fe_g(ie_g,je_g), fn_g(ie_g,je_g)  &
          , fs_g(ie_g,je_g))
  ELSE
          ALLOCATE ( fw_g(0,0), fe_g(0,0), fn_g(0,0), fs_g(0,0))
  ENDIF
  ...
  CALL gather(FW,FW_G,p_io)
  CALL gather(FE,FE_G,p_io)
  CALL gather(FN,FN_G,p_io)
  CALL gather(FS,FS_G,p_io)
