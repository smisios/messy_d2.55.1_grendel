...
SUBROUTINE rho1j(t, s, p, rh, j)
  ...
  REAL(dp), INTENT(INOUT) :: s(ie,je)
  REAL(dp), INTENT(IN) :: t(ie,je), p
  REAL(dp), INTENT(OUT) :: rh(ie,je)
  INTEGER, INTENT(IN) :: j
  ...
  DO i=1,ie
!!$ mz_bk_20110207+
!!$    IF (j .eq. 0) write(*,*) 'QQQQ: j==0!!'
    s(i,j)=MAX(s(i,j),28.0_dp)
!!$    write(*,*) 's(i,j): ', s(i,j), i, j ,ie, je
!!$  write(*,*) 's(i,j)**3: ', s(i,j)**3
!!$  write(*,*) 'SQRT(s(i,j)**3): ', SQRT(s(i,j)**3)
    s3h=SQRT(s(i,j)**3)
!!$  write(*,*) 's3h: ', s3h
!!$  write(*,*) 'p: ', p
!!$  write(*,*) 't(i,j): ', t(i,j), i, j, ie, je
!!$  write(*,*) 'test:' ,(1._wp - p/(p*(                                     &
