...
! Universal constants
  ...
  REAL(dp), PARAMETER :: aradtogra = 180._dp/api
  REAL(dp), PARAMETER :: agratorad = api/180._dp
...
! REAL(dp), PARAMETER :: ar    = 8.314e3_dp       ! universal gas constant in J/K/kmol
  ...
  REAL(dp), PARAMETER :: avogadro_num = 6.022045e23_dp   ! Avogadro constant in 1/mol
...
! Molar weights in g/mol
  ...
  REAL(dp), PARAMETER :: amso2 = 32.066_dp        ! molecular weight of SO2
...
!  REAL(wp), parameter :: ice_dmin = 0.10        ! minimal ice thicknessss
!  REAL(wp), parameter :: ice_rho  = 910.0       ! ice density
!  REAL(wp), parameter :: ice_cp   = 2106.0      ! specific ice heat capacity ??
!  REAL(wp), parameter :: ice_alpha = 2.1656     ! ??
!  REAL(wp), parameter :: ice_rici = 2.09e+06    ! volumetric heat capacity of ice [j/m**3/k]
!  REAL(wp), parameter :: ice_difiz = 12.e-07    ! temperature diffusivity of ice  [m**2/s]
!  REAL(wp), parameter :: sn_cond = 0.22         ! snow thermal conductivity [j/s/m/k]
!  REAL(wp), parameter :: sn_dens = 300.0        ! snow density              [kg/m**3]
!  REAL(wp), parameter :: sn_capa = 634500.0     ! snow vol. heat capacity   [j/m**3/k]
