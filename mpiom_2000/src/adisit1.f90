      ...
      USE MO_PARAM3
      IMPLICIT NONE
      REAL(wp) TH,SH,PA,TPO
      REAL(wp) :: PR, S, T, FNE, FST
!***********************************************************
...
!
      S = SH
      TPO=TH
      TH=(TH+A1*PR+B1*PR*(S-35._wp)+C1*PR**2                               &
     & -D*PR**2*(S-35._wp)-E1*PR**3)                                       &
     & /(1._wp-A2*PR+B2*PR*(S-35._wp)+C2*PR**2-E2*PR**3)
!
      T=TH
      FNE=T-PR*(A1+A2*T-A3*T**2+A4*T**3)-PR*(S-35._wp)*(B1-B2*T)           &
     &-PR**2*(C1-C2*T+C3*T**2) +D*(S-35._wp)*PR**2-PR**3*(-E1+E2*T)        &
     &    -TPO
       FST=1._wp-PR*(A2-2._wp*A3*T+3._wp*A4*T**2) +PR*(S-35._wp)*B2                 &
     & +PR**2*(C2-2._wp*C3*T) -E2*PR**3
      TH=TH-FNE/FST
