...
!lk                             , debug_parallel
  ...
  ! um_ak_20080527+
  !LOGICAL, PUBLIC, SAVE :: L_E5_STREAM_OUTPUT
  LOGICAL, PUBLIC, SAVE :: L_BM_ORIG_OUTPUT
  ! um_ak_20080527-
#endif
...
#ifdef MESSY
  ! ALLOW ORIGINAL ECHAM5 STREAM OUTPUT
  !IF (.NOT. L_E5_STREAM_OUTPUT) RETURN ! um_ak_20080630
  IF (.NOT. L_BM_ORIG_OUTPUT) RETURN    ! um_ak_20080630
#endif
...
!------------------------------------------------------------------------------
  SUBROUTINE out_streams
    ...
    USE mo_filename,     ONLY: out_filetype, GRIB!, NETCDF ! mz_pj_20050614
...
#else
    IF (ASSOCIATED (g_aps)) DEALLOCATE (g_aps)
#endif
...
#else
    USE mo_control,       ONLY: nlon, ngl, nsp, nvclev, lnwp
#endif
