MODULE mo_io
  ...
  USE mo_kind,          ONLY: dp
  USE mo_netcdf
  USE mo_filename,      ONLY: NETCDF, UNDEFINED ! mz_pj_20050531
  ...
  USE mo_linked_list,   ONLY: list_element, FOURIER, GAUSSIAN, SPECTRAL
...
! op_pj_20120319+
          ...
          write_mode = IOR(NF_CLOBBER,NF_64BIT_OFFSET)
#else
! op_pj_20120319-
...
!==============================================================================
  ...
  !
    ...
    TYPE (list_element) ,POINTER :: next
    REAL(dp)            ,POINTER :: zout(:,:,:,:), zptr(:,:,:,:)
    INTEGER :: fileID

    IF (p_pe == p_io) & ! mz_ak_20040726
         fileID     = fileinfo%file_id
    next => io_list
    DO WHILE (ASSOCIATED(next))
      IF (next%field%info%lrerun) THEN
...
!==============================================================================
  SUBROUTINE IO_init
    ...
    CALL TC_set(yr, mo, dy, hr, mi, se, date_nat)
    CALL TC_convert(date_nat, start_date)
!    CALL set_date_components(start_date,yr,mo,dy,hr,mi,se)
    CALL timer_get_date(status,'resume', yr, mo, dy, hr, mi, se)
    CALL TC_set(yr, mo, dy, hr, mi, se, date_nat)
    CALL TC_convert(date_nat, resume_date)
!    CALL set_date_components(resume_date,yr,mo,dy,hr,mi,se)

    IF (p_pe == p_io) fileID = header%file_id  ! needs to become obsolete ...
#endif
    IF (p_pe == p_io) THEN
...
#else
    ! um_ak_20080409+
    IF (.NOT. lresume) CALL ec_manager_init(IO_timestep,istep)
    ! um_ak_20080409-
#endif
