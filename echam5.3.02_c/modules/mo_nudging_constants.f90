...
!BOP
  ...
  ! Joachim Buchholz, MPI Mainz, 24. July 2003
  ! Aiko Voigt, MPI Hamburg, March 2011, 
  !   add inudgformat to chose between netcdf and cray-format
...
#ifdef MESSY
  LOGICAL :: lnudg_out = .FALSE.  ! output feasible? ! op_pj_20150331
#endif
!EOX
  ...
  ! CONSTANT
! mz_pj_20050615+
!  INTEGER, PARAMETER :: nmaxc = 80   ! maximal number of nudging weights 
!                                     ! for levels
  INTEGER, PARAMETER :: nmaxc = 200  ! maximal number of nudging weights 
                                     ! for levels
! mz_pj_20050615-
...
!EOX
  ...
  ! CONSTANT
  REAL(kind=dp), PARAMETER :: nfact = 1.e-5_dp 
  ...
  !REAL(kind=dp)    :: &
  !  nudgd(nmaxc) =  5.0_dp, &! nudging weight for divergence (5.6 hours)
  !  nudgv(nmaxc) = 10.0_dp, &! nudging weight for vorticity (2.8 hours)
  !  nudgt(nmaxc) =  1.0_dp, &! nudging weight for temperature (27.8 hours)
  !  nudgp        = 10.0_dp   ! nudging weight for log sfc pressure (2.8 hours)
  !
...
!BOX
  ...
  INTEGER, PUBLIC, SAVE  :: ndunit  ! unit for special diagnostics
!EOX
!EOC
