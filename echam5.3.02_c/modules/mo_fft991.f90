MODULE mo_fft991
  ...
  PUBLIC :: init_fft991
  PUBLIC :: cleanup_fft991
  PUBLIC :: fft991cy

  REAL(dp), ALLOCATABLE :: trigs(:)  !  array used by *fft991*.
  INTEGER :: ifax(10)                !  array used by *fft991*.

  !  nfft: maximum length of calls to *fft.

#if ( (defined CRAY) && (! defined _CRAYMPP) ) || ( defined _UNICOSMP )
#if (_MAXVL == 64)
...
CONTAINS
  ...
  END SUBROUTINE init_fft991

  SUBROUTINE fft991cy(a,zwork,inc,jump,n,lot,isign)
    ...
    REAL(dp) :: work(SIZE(zwork))

    !  Local scalars: 
    INTEGER :: i, ia, ibase, ierr, ierrg, ifac, igo, ii, istart, ix, iz, &
         j, jbase, jj, k, la, nb, nblox, nfax, nvex, nx
...
!CSD$ PARALLEL DO PRIVATE(ia,i,j,la,igo,ifac,ierr,work,nvex,&
!CSD$& ix,istart,ibase,jbase,jj)
!$OMP PARALLEL PRIVATE(ia,i,j,la,igo,ifac,ierr,work,nvex,&
!$OMP ix,istart,ibase,jbase,jj,nb)
!$OMP DO
    DO nb = 1, nblox
       nvex = lot - (nblox-1)*nfft
       istart=1
       IF ( nb>1 ) THEN
          istart=nvex*jump+(nb-2)*nfft*jump+1
          nvex=nfft
       ENDIF
       ia = istart
       i = istart
...
10     CONTINUE
       ...
       DO k = 1, nfax
          ...
          ierr = -1
          IF (igo==-1) GO TO 20
          CALL rpassm(a(ia),a(ia+la*inc),work(1),work(ifac*la+1),inc,1, &
               &          jump,nx,nvex,n,ifac,la,ierr)
          GO TO 30
20        CONTINUE
          CALL rpassm(work(1),work(la+1),a(ia),a(ia+ifac*la*inc),1,inc,nx, &
               &          jump,nvex,n,ifac,la,ierr)
30        CONTINUE
          IF (ierr /= 0) ierrg = ierr
          la = ifac*la
          igo = -igo
          ia = istart
       END DO
...
50  CONTINUE
    ierrg = 0
!CSD$ PARALLEL DO PRIVATE(ia,i,j,la,igo,ifac,ierr,work,nvex,&
!CSD$& ix,iz,istart,ibase,jbase,jj)
!$OMP PARALLEL PRIVATE(ia,i,j,la,igo,ifac,ierr,work,nvex,&
!$OMP ix,iz,istart,ibase,jbase,jj,nb)
!$OMP DO
    DO nb = 1, nblox
       nvex = lot - (nblox-1)*nfft
       istart=1
       ...
       DO k = 1, nfax
          ...
          ierr = -1
          IF (igo==-1) GO TO 60
          CALL qpassm(a(ia),a(ia+ifac*la*inc),work(1),work(la+1),inc,1, &
               &          jump,nx,nvex,n,ifac,la,ierr)
          GO TO 70
60        CONTINUE
          CALL qpassm(work(1),work(ifac*la+1),a(ia),a(ia+la*inc),1,inc,nx, &
               &          jump,nvex,n,ifac,la,ierr)
70        CONTINUE
          IF (ierr/=0) ierrg = ierr
          igo = -igo
...
100 CONTINUE
  ...
  SUBROUTINE qpassm(a,b,c,d,inc1,inc2,inc3,inc4,lot,n,ifac,la,ierr)
    ...
    !  Array arguments 
    ! REAL(dp) :: a(n),b(n),c(n),d(n),trigs(n)
    REAL(dp) :: a(*), b(*), c(*), d(*)

    !  Local scalars: 
    REAL(dp) :: a0, a1, a10, a11, a2, a20, a21, a3, a4, a5, a6, b0, b1, b10, b11, &
    ...
    !  Data statements 
    DATA sin36 /0.587785252292473_dp/, sin72 /0.951056516295154_dp/, &
         qrt5  /0.559016994374947_dp/, sin60 /0.866025403784437_dp/
...
!OCL NOVREC
       DO ijk = 1, lot
          c(ja+j) = (a(ia+i)+0.5_dp*(a(ic+i)-a(ie+i))) + sin60*(a(ib+i)-a(if+i))
          d(ja+j) = -(a(id+i)+0.5_dp*(a(ib+i)+a(if+i))) - sin60*(a(ic+i)+a(ie+i))
          c(jb+j) = a(ia+i) - (a(ic+i)-a(ie+i))
          d(jb+j) = a(id+i) - (a(ib+i)+a(if+i))
          c(jc+j) = (a(ia+i)+0.5_dp*(a(ic+i)-a(ie+i))) - sin60*(a(ib+i)-a(if+i))
          d(jc+j) = -(a(id+i)+0.5_dp*(a(ib+i)+a(if+i))) + sin60*(a(ic+i)+a(ie+i))
          i = i + inc3
          j = j + inc4
       END DO
       ibase = ibase + inc1
...
180 CONTINUE
  ...
  SUBROUTINE rpassm(a,b,c,d,inc1,inc2,inc3,inc4,lot,n,ifac,la,ierr)
    ...
    !  Local scalars: 
    REAL(dp) :: c1, c2, c3, c4, c5, qqrt5, qrt5, s1, s2, s3, s4, s5, sin36, sin45, &
    ...
    !  Local arrays: 
    REAL(dp) :: a10(nfft), a11(nfft), a20(nfft), a21(nfft), b10(nfft), b11(nfft), b20(nfft), &
         &      b21(nfft)
...
!OCL NOVREC
          DO ijk = 1, lot
             ...
             c(jb+j) = c1*((a(ia+i)-0.5_dp*(a(ib+i)+a(ic+i)))-(sin60*(b(ib+i)+ &
                  &            b(ic+i)))) - s1*((b(ia+i)-0.5_dp*(b(ib+i)-b(ic+i)))+(sin60*(a(ib+i)- &
                  &            a(ic+i))))
             d(jb+j) = s1*((a(ia+i)-0.5_dp*(a(ib+i)+a(ic+i)))-(sin60*(b(ib+i)+ &
                  &            b(ic+i)))) + c1*((b(ia+i)-0.5_dp*(b(ib+i)-b(ic+i)))+(sin60*(a(ib+i)- &
                  &            a(ic+i))))
             c(jc+j) = c2*((a(ia+i)-0.5_dp*(a(ib+i)+a(ic+i)))+(sin60*(b(ib+i)+ &
                  &            b(ic+i)))) - s2*((b(ia+i)-0.5_dp*(b(ib+i)-b(ic+i)))-(sin60*(a(ib+i)- &
                  &            a(ic+i))))
             d(jc+j) = s2*((a(ia+i)-0.5_dp*(a(ib+i)+a(ic+i)))+(sin60*(b(ib+i)+ &
                  &            b(ic+i)))) + c2*((b(ia+i)-0.5_dp*(b(ib+i)-b(ic+i)))-(sin60*(a(ib+i)- &
                  &            a(ic+i))))
             i = i + inc3
             j = j + inc4
          END DO
...
!OCL NOVREC
       DO ijk = 1, lot
          c(ja+j) = a(ia+i) + (a(ib+i)+a(ic+i))
          c(jb+j) = ((a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))+qrt5*(a(ib+i)-a(ic+i))) - &
               &          (sin72*b(ib+i)+sin36*b(ic+i))
          c(jc+j) = ((a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))-qrt5*(a(ib+i)-a(ic+i))) - &
               &          (sin36*b(ib+i)-sin72*b(ic+i))
          c(jd+j) = ((a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))-qrt5*(a(ib+i)-a(ic+i))) + &
               &          (sin36*b(ib+i)-sin72*b(ic+i))
          c(je+j) = ((a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))+qrt5*(a(ib+i)-a(ic+i))) + &
               &          (sin72*b(ib+i)+sin36*b(ic+i))
          i = i + inc3
          j = j + inc4
       END DO
...
!OCL NOVREC
          DO ijk = 1, lot
             ...
             a10(ijk) = (a(ia+i)-0.25_dp*((a(ib+i)+a(ie+i))+(a(ic+i)+a(id+i)))) + &
                  &            qrt5*((a(ib+i)+a(ie+i))-(a(ic+i)+a(id+i)))
             a20(ijk) = (a(ia+i)-0.25_dp*((a(ib+i)+a(ie+i))+(a(ic+i)+a(id+i)))) - &
                  &            qrt5*((a(ib+i)+a(ie+i))-(a(ic+i)+a(id+i)))
             b10(ijk) = (b(ia+i)-0.25_dp*((b(ib+i)-b(ie+i))+(b(ic+i)-b(id+i)))) + &
                  &            qrt5*((b(ib+i)-b(ie+i))-(b(ic+i)-b(id+i)))
             b20(ijk) = (b(ia+i)-0.25_dp*((b(ib+i)-b(ie+i))+(b(ic+i)-b(id+i)))) - &
                  &            qrt5*((b(ib+i)-b(ie+i))-(b(ic+i)-b(id+i)))
...
!OCL NOVREC
       DO ijk = 1, lot
          c(ja+j) = (a(ia+i)+a(ib+i)) + a(ic+i)
          c(jb+j) = (qrt5*(a(ia+i)-a(ib+i))+(0.25_dp*(a(ia+i)+a(ib+i))-a(ic+i))) - &
               &          (sin36*b(ia+i)+sin72*b(ib+i))
          c(je+j) = -(qrt5*(a(ia+i)-a(ib+i))+(0.25_dp*(a(ia+i)+a(ib+i))-a(ic+i))) - &
               &          (sin36*b(ia+i)+sin72*b(ib+i))
          c(jc+j) = (qrt5*(a(ia+i)-a(ib+i))-(0.25_dp*(a(ia+i)+a(ib+i))-a(ic+i))) - &
               &          (sin72*b(ia+i)-sin36*b(ib+i))
          c(jd+j) = -(qrt5*(a(ia+i)-a(ib+i))-(0.25_dp*(a(ia+i)+a(ib+i))-a(ic+i))) - &
               &          (sin72*b(ia+i)-sin36*b(ib+i))
          i = i + inc3
          j = j + inc4
       END DO
...
!OCL NOVREC
       DO ijk = 1, lot
          ...
          c(jb+j) = (2.0_dp*(a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))+qqrt5*(a(ib+i)-a(ic+ &
               &          i))) - (ssin72*b(ib+i)+ssin36*b(ic+i))
          c(jc+j) = (2.0_dp*(a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))-qqrt5*(a(ib+i)-a(ic+ &
               &          i))) - (ssin36*b(ib+i)-ssin72*b(ic+i))
          c(jd+j) = (2.0_dp*(a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))-qqrt5*(a(ib+i)-a(ic+ &
               &          i))) + (ssin36*b(ib+i)-ssin72*b(ic+i))
          c(je+j) = (2.0_dp*(a(ia+i)-0.25_dp*(a(ib+i)+a(ic+i)))+qqrt5*(a(ib+i)-a(ic+ &
               &          i))) + (ssin72*b(ib+i)+ssin36*b(ic+i))
          i = i + inc3
          j = j + inc4
       END DO
...
!OCL NOVREC
       DO ijk = 1, lot
          ...
          c(jb+j) = ((a(ia+i)-a(id+i))+0.5_dp*(a(ib+i)-a(ic+i))) - (sin60*(b(ib+ &
               &          i)+b(ic+i)))
          c(jf+j) = ((a(ia+i)-a(id+i))+0.5_dp*(a(ib+i)-a(ic+i))) + (sin60*(b(ib+ &
               &          i)+b(ic+i)))
          c(jc+j) = ((a(ia+i)+a(id+i))-0.5_dp*(a(ib+i)+a(ic+i))) - (sin60*(b(ib+ &
               &          i)-b(ic+i)))
          c(je+j) = ((a(ia+i)+a(id+i))-0.5_dp*(a(ib+i)+a(ic+i))) + (sin60*(b(ib+ &
               &          i)-b(ic+i)))
          i = i + inc3
          j = j + inc4
       END DO
...
!OCL NOVREC
       DO ijk = 1, lot
          ...
          c(jb+j) = (sin60*(a(ia+i)-a(ic+i))) - (0.5_dp*(b(ia+i)+b(ic+i))+b(ib+i))
          c(jf+j) = -(sin60*(a(ia+i)-a(ic+i))) - (0.5_dp*(b(ia+i)+b(ic+i))+b(ib+i))
          c(jc+j) = sin60*(b(ic+i)-b(ia+i)) + (0.5_dp*(a(ia+i)+a(ic+i))-a(ib+i))
          c(je+j) = sin60*(b(ic+i)-b(ia+i)) - (0.5_dp*(a(ia+i)+a(ic+i))-a(ib+i))
          i = i + inc3
          j = j + inc4
       END DO
       ibase = ibase + inc1
...
!OCL NOVREC
       DO ijk = 1, lot
          c(ja+j) = (2.0_dp*(a(ia+i)+a(id+i))) + (2.0_dp*(a(ib+i)+a(ic+i)))
          c(jd+j) = (2.0_dp*(a(ia+i)-a(id+i))) - (2.0_dp*(a(ib+i)-a(ic+i)))
          c(jb+j) = (2.0_dp*(a(ia+i)-a(id+i))+(a(ib+i)-a(ic+i))) - (ssin60*(b(ib+ &
               &          i)+b(ic+i)))
          c(jf+j) = (2.0_dp*(a(ia+i)-a(id+i))+(a(ib+i)-a(ic+i))) + (ssin60*(b(ib+ &
               &          i)+b(ic+i)))
          c(jc+j) = (2.0_dp*(a(ia+i)+a(id+i))-(a(ib+i)+a(ic+i))) - (ssin60*(b(ib+ &
               &          i)-b(ic+i)))
          c(je+j) = (2.0_dp*(a(ia+i)+a(id+i))-(a(ib+i)+a(ic+i))) + (ssin60*(b(ib+ &
               &          i)-b(ic+i)))
          i = i + inc3
          j = j + inc4
       END DO
...
!OCL NOVREC
       DO ijk = 1, lot
          c(ja+j) = 2.0_dp*(((a(ia+i)+a(ie+i))+a(ic+i))+(a(ib+i)+a(id+i)))
          c(je+j) = 2.0_dp*(((a(ia+i)+a(ie+i))+a(ic+i))-(a(ib+i)+a(id+i)))
          c(jc+j) = 2.0_dp*(((a(ia+i)+a(ie+i))-a(ic+i))-(b(ib+i)-b(id+i)))
          c(jg+j) = 2.0_dp*(((a(ia+i)+a(ie+i))-a(ic+i))+(b(ib+i)-b(id+i)))
          c(jb+j) = 2.0_dp*((a(ia+i)-a(ie+i))-b(ic+i)) + ssin45*((a(ib+i)-a(id+ &
               &          i))-(b(ib+i)+b(id+i)))
          c(jf+j) = 2.0_dp*((a(ia+i)-a(ie+i))-b(ic+i)) - ssin45*((a(ib+i)-a(id+ &
               &          i))-(b(ib+i)+b(id+i)))
          c(jd+j) = 2.0_dp*((a(ia+i)-a(ie+i))+b(ic+i)) - ssin45*((a(ib+i)-a(id+ &
               &          i))+(b(ib+i)+b(id+i)))
          c(jh+j) = 2.0_dp*((a(ia+i)-a(ie+i))+b(ic+i)) + ssin45*((a(ib+i)-a(id+ &
               &          i))+(b(ib+i)+b(id+i)))
          i = i + inc3
          j = j + inc4
       END DO
...
180 CONTINUE
  ...
  SUBROUTINE set991(n)
    ...
    del = 4.0_dp*ASIN(1.0_dp)/REAL(n,dp)
    nil = 0
    nhl = (n/2) - 1
    DO k = nil, nhl
       angle = REAL(k,dp)*del
...
40  CONTINUE
    ...
    ifax(10) = n
    RETURN
  END SUBROUTINE set991
!------------------------------------------------------------------------------
  SUBROUTINE cleanup_fft991
    IF (ALLOCATED(trigs)) DEALLOCATE(trigs)
  ENDSUBROUTINE cleanup_fft991
!------------------------------------------------------------------------------
END MODULE mo_fft991
