MODULE mo_memory_g2a
  ...
  ! mz_jb_20031217+: SAVE attributes added
  REAL(dp), SAVE, POINTER, PUBLIC :: um1(:,:,:)
  REAL(dp), SAVE, POINTER, PUBLIC :: vm1(:,:,:)
  REAL(dp), SAVE, POINTER, PUBLIC :: dudlm1(:,:,:)
  REAL(dp), SAVE, POINTER, PUBLIC :: dvdlm1(:,:,:)
  REAL(dp), SAVE, POINTER, PUBLIC :: dtlm1(:,:,:)
  REAL(dp), SAVE, POINTER, PUBLIC :: dtmm1(:,:,:)
  ! mz_jb_20031217-
...
CONTAINS
  ...
  SUBROUTINE construct_g2a (lnlon, lnlev, lngl, nlon, nlev, ngl)
    ...
    CALL add_stream_element (g2a, 'um1' , um1, dim1p, dim1, dimnames=dim1n, &
                                  lrerun=.TRUE., longname= 'um1 * cos(lat) ') ! mz_mt_20040114
    CALL add_stream_element (g2a, 'vm1' , vm1, dim1p, dim1, dimnames=dim1n, &
                                  lrerun=.TRUE., longname= 'vm1 * cos(lat) ') ! mz_mt_20040114
