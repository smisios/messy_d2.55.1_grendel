...
#define VECTOR 1
#endif
!#define FAST_BUT_DECOMPDEP

#ifdef MESSY
#ifdef MESSYMMD
#define _XNOZEROINITEND 1
#endif
#endif

!#define CHECK_TRANSPOSITION
!#define DEBUG
MODULE mo_tpcore
  ...
  USE messy_main_tendency_bi,   ONLY: mtend_get_handle,      &
                                      ...
                                      mtend_id_tracer
#else
#ifdef _XNOZEROINITEND
  USE messy_main_constants_mem, ONLY: dp
#endif                                 
#endif

  USE mo_control,       ONLY: lyaxt_transposition
  USE mo_echam_yaxt,    ONLY: yaxt_tr_gp_to_ffsl, yaxt_tr_ffsl_to_gp, &
       &                      yaxt_tr_gp_to_ffsl_2d, &
       &                      yaxt_tr_ffsl_to_gp_3d, yaxt_tr_ffsl_to_gp, &
       &                      yaxt_tr_gp_to_ffsl_3d, yaxt_ffsl_ghost_update_2d, &
       &                      yaxt_ffsl_ghost_update_3d, yaxt_ffsl_ghost_update_4d
...
#ifdef MESSYTENDENCY
  ! see also: messy_main_tendency_bi.f90, subroutine main_tendency_init_memory
  CHARACTER(len=*), PARAMETER, PUBLIC     :: modstr = 'advect'
  !variable for tendency budget
  integer,save                            :: my_handle
#endif

  ! have_j1(nghost): have jlat=1    within ghost-extended local latitudes
  ! have_jm(nghost): have jlat=nlat within ghost-extended local latitudes
  LOGICAL, SAVE :: have_j1(0:3) = .FALSE.
  LOGICAL, SAVE :: have_jm(0:3) = .FALSE.

  INTEGER, SAVE :: itimer_ghost_update = 0
...
CONTAINS
  ...
  SUBROUTINE init_tpcore
    ...
    USE messy_main_tracer_mem_bi, ONLY: ntrac_gp, ti_gp, ON, I_ADVECT
#endif
    ...
    lfirst = .false.
    ! REQUESTS need to be set in main_tendency_init_memory; here it
    ! is too late (for channel object memory assignment).
...
!!$  CALL mtend_register (my_handle,mtend_id_xi)
    ...
    number_advect = 0
    DO j=1, ntrac_gp
       IF (ti_gp(j)%tp%meta%cask_i(I_advect) == ON) THEN
          number_advect = number_advect + 1
       END IF
    END DO
    pcnst = jps + number_advect
#endif
...
!1
#ifndef E5SGHOST
    IF (.NOT. lyaxt_transposition) THEN
    ...
    IF (lyaxt_transposition) THEN
      itimer_ghost_update = new_timer('yaxt_ffsl_ghost_update')
      DO j = 0,3
        have_j1(j) = (jfirst - j <= 1)  .AND. (1  <= jfirst + j)
        have_jm(j) = (jlast  - j <= ldc%nlat) .AND. (ldc%nlat <= jlast  + j)
      ENDDO
    ELSE
      itimer_ghost_update = new_timer('orig_ffsl_ghost_update')
      have_j1(:) = (jfirst == 1)
      have_jm(:) = (jlast == ldc%nlat)
    ENDIF
  ...
  SUBROUTINE setup_tpcore
    ...
    REAL(dp), DIMENSION(SIZE(xtm1,1),SIZE(xtm1,2),SIZE(xtm1,4)) :: xtp1
#ifdef HAVE_YAXT
    REAL(dp), DIMENSION(SIZE(xtm1,1),SIZE(xtm1,2),SIZE(xtm1,4)) :: xtp2
    REAL(dp), DIMENSION(SIZE(xtm1,1),SIZE(xtm1,2),SIZE(xtm1,4)) :: xtp3
#endif
#endif 
#endif
...
!    INTEGER :: jl, jk, jt, jglat, l 
    ...
    IF (lyaxt_transposition) THEN
...
#ifndef _XNOZEROINITEND
      CALL yaxt_tr_gp_to_ffsl(u,v,qm1,xlm1,xim1, &
           &                  alpsm1,alps, &
           &                  ub,vb,fb(:,:,:,1),fb(:,:,:,2),fb(:,:,:,3),  &
           &                  ps1b, ps2b)
#ifdef CHECK_TRANSPOSITION
      CALL check3d_gp2ffsl('u->ub', u, ub)
      CALL check3d_gp2ffsl('v->vb', v, vb)
      CALL check3d_gp2ffsl('qm1->fb1', qm1, fb(:,:,:,1))
      CALL check3d_gp2ffsl('xlm1->fb2', xlm1, fb(:,:,:,2))
      CALL check3d_gp2ffsl('xim1->fb3', xim1, fb(:,:,:,3))
#endif

#else /* else case of ifndef _XNOZEROINITEND */

#ifndef MESSYTENDENCY
      xtp1(:,:,:) = qm1(:,:,:) + qte (:,:,:) * ztmst
      xtp2(:,:,:) = xlm1(:,:,:) + xlte (:,:,:) * ztmst
      xtp3(:,:,:) = xim1(:,:,:) + xite (:,:,:) * ztmst
#else
      CALL mtend_get_start_g(mtend_id_q, v0=xtp1)
      CALL mtend_get_start_g(mtend_id_xl, v0=xtp2)
      CALL mtend_get_start_g(mtend_id_xi, v0=xtp3)
#endif

      CALL yaxt_tr_gp_to_ffsl(u,v,xtp1,xtp2,xtp3, &
           &                  alpsm1,alps, &
           &                  ub,vb,fb(:,:,:,1),fb(:,:,:,2),fb(:,:,:,3),  &
           &                  ps1b, ps2b)
      ...
      CALL check3d_gp2ffsl('u->ub', u, ub)
      CALL check3d_gp2ffsl('v->vb', v, vb)
      CALL check3d_gp2ffsl('xtp1->fb1', xtp1, fb(:,:,:,1))
      CALL check3d_gp2ffsl('xtp2->fb2', xtp2, fb(:,:,:,2))
      CALL check3d_gp2ffsl('xtp3->fb3', xtp3, fb(:,:,:,3))
#endif

#endif

#endif /* end of ifdef HAVE_YAXT */

    ELSE ! (lyaxt_transposition)
...
#ifndef MESSYTENDENCY
    xtp1(:,:,:) = qm1(:,:,:) + qte (:,:,:) * ztmst
    CALL tr_gp_ffsl (ldc ,1 ,xtp1        ,fb(:,:,:,1 ))
    xtp1(:,:,:) = xlm1(:,:,:) + xlte (:,:,:) * ztmst
    CALL tr_gp_ffsl (ldc ,1 ,xtp1        ,fb(:,:,:,2 ))
    xtp1(:,:,:) = xim1(:,:,:) + xite (:,:,:) * ztmst
    CALL tr_gp_ffsl (ldc ,1 ,xtp1        ,fb(:,:,:,3 ))
#else
    CALL mtend_get_start_g(mtend_id_q, v0=xtp1)
    CALL tr_gp_ffsl (ldc ,1 ,xtp1        ,fb(:,:,:,1 ))
    CALL mtend_get_start_g(mtend_id_xl, v0=xtp1)
    CALL tr_gp_ffsl (ldc ,1 ,xtp1        ,fb(:,:,:,2 ))
    CALL mtend_get_start_g(mtend_id_xi, v0=xtp1)
    CALL tr_gp_ffsl (ldc ,1 ,xtp1        ,fb(:,:,:,3 ))
#endif
#endif

  ENDIF ! (lyaxt_transposition)
...
#else
    DO it = 1, ntrac_gp
       IF (ti_gp(it)%tp%meta%cask_i(I_advect) == ON) THEN
          ib = ib + 1
#ifndef _XNOZEROINITEND
          WHERE (ABS(xtm1(:,:,it,:)) < zlimit) xtm1(:,:,it,:) = 0.0_wp
          CALL tr_gp_ffsl (ldc ,1 ,xtm1(:,:,it,:), fb(:,:,:,ib))         
#else
#ifndef MESSYTENDENCY
          xtp1(:,:,:) = xtm1(:,:,it,:) + xtte(:,:,it,:) * ztmst
#else
! ub_ak_20181023 CALL mtend_get_start_g(mtend_id_tracer, v0=xtp1, idt=it)
          CALL mtend_get_start_g(it, v0=xtp1)
#endif
          WHERE (ABS(xtp1) < zlimit) xtp1 = 0.0_wp
          IF (lyaxt_transposition) THEN
            CALL yaxt_tr_gp_to_ffsl_3d(xtp1(:,:,:), fb(:,:,:,ib))
#ifdef CHECK_TRANSPOSITION
            CALL check3d_gp2ffsl('xtp1->fb', xtp1, fb(:,:,:,ib))
#endif
          ELSE
            CALL tr_gp_ffsl (ldc ,1 ,xtp1(:,:,:), fb(:,:,:,ib))
          ENDIF
#endif
       END IF
    END DO
...
!    CALL tr_gp_ffsl (ldc ,1 ,xtm1        ,fb(:,:,:,4:))
    IF (lyaxt_transposition) THEN
      ! nothing to do for alpsm1, alps (already transposed above)
    ELSE
...
!         pcnst, as, bs, ub, vb, ps1b, ps2b, psb, fb, 4, 4, 7, 0)
         pcnst, as, bs, ub, vb, ps1b, ps2b, psb, fb, 4, 4, 7, nmwicfi)
! mz_kk_20070411-
  ...
  SUBROUTINE tpcore_tendencies(psm1cor, pscor)
    ...
    ! mz_pj_20040330-
#ifdef MESSYTENDENCY
    real(dp),dimension(size(qm1,1),size(qm1,2),size(qm1,3))    :: start_buf
    real(dp),dimension(size(qm1,1),size(qm1,2),size(qm1,3))    :: tend_buf
    real(dp),dimension(size(xtm1,1),size(xtm1,2),size(xtm1,4)) :: trac_buf
#endif
#ifdef _XNOZEROINITEND
    REAL(dp), DIMENSION(SIZE(xtm1,1),SIZE(xtm1,2),SIZE(xtm1,4)) :: xtp1
#endif
...
#ifdef DEBUG
    REAL(wp) :: zpsz(ldc%ffsl%lats:ldc%ffsl%latn), &
         &      zps1z(ldc%ffsl%lats:ldc%ffsl%latn), &
         &      zps2z(ldc%ffsl%lats:ldc%ffsl%latn)
...
#else
    xtp1(:,:,:) = qm1(:,:,:)  + qte(:,:,:) * ztmst
    qte(:,:,:)  = qte(:,:,:)  + (fb_gp(:,:,1,:) -  xtp1(:,:,:)) / ztmst
    xtp1(:,:,:) = xlm1(:,:,:) + xlte(:,:,:) * ztmst
    xlte(:,:,:) = xlte(:,:,:) + (fb_gp(:,:,2,:) - xtp1(:,:,:)) / ztmst
    xtp1(:,:,:) = xim1(:,:,:)  + xite(:,:,:) * ztmst
    xite(:,:,:) = xite(:,:,:) + (fb_gp(:,:,3,:) - xtp1(:,:,:)) / ztmst
#endif
#endif
    ib = 3
#ifndef MESSY
    ...
    ! start values are required because tendencies already have been 
    ! modified in DYN

    call mtend_get_start_g (mtend_id_q, v0  = start_buf)
    tend_buf = (fb_gp(:,:,1,:) -  start_buf) / ztmst
    call mtend_add_g(my_handle, mtend_id_q, px = tend_buf)

    call mtend_get_start_g (mtend_id_xl, v0  = start_buf)
    tend_buf = (fb_gp(:,:,2,:) -  start_buf) / ztmst
    call mtend_add_g(my_handle, mtend_id_xl,  px = tend_buf)

    call mtend_get_start_g (mtend_id_xi, v0  = start_buf)
    tend_buf = (fb_gp(:,:,3,:) -  start_buf) / ztmst
    call mtend_add_g (my_handle, mtend_id_xi,  px = tend_buf)
#endif
    DO it = 1, ntrac_gp
       IF (ti_gp(it)%tp%meta%cask_i(I_advect) == ON) THEN
          ib = ib + 1
...
#endif
#else
!          call mtend_get_start_g (mtend_id_tracer, idt=it, v0 = trac_buf)
          call mtend_get_start_g (it, v0 = trac_buf)
          trac_buf = (fb_gp(:,:,ib,:)-trac_buf) / ztmst          
!          call mtend_add_g (my_handle, mtend_id_tracer, idt=it, px = trac_buf) 
          call mtend_add_g (my_handle, it, px = trac_buf) 
#endif
       END IF
    END DO
#endif
    ...
    jlast  = ldc%ffsl%latn
    IF (lyaxt_transposition) THEN
      CALL yaxt_tr_gp_to_ffsl_2d(ps_gp, psb)
#ifdef CHECK_TRANSPOSITION
      CALL check2d_gp2ffsl('ps_gp->ps', ps_gp, psb)
#endif
    ELSE
      CALL tr_gp_ffsl(ldc , 1, ps_gp, psb)
    ENDIF

    DO jlat = jfirst, jlast
  ...
  SUBROUTINE tpcore(dt, ae, im, jm, km, jfirst, jlast, nq,   &
    ...
    CALL timer_start(itimer_ghost_update)
    IF (lyaxt_transposition) THEN
      CALL yaxt_ffsl_ghost_update_2d(psm, ng)
      CALL yaxt_ffsl_ghost_update_2d(psn, mg)
    ELSE
      CALL ghost_update(psm, im, jm, 1, 1, jfirst, jlast, ng, ng)
      CALL ghost_update(psn, im, jm, 1, 1, jfirst, jlast, mg, mg)
    ENDIF
    CALL timer_stop(itimer_ghost_update)
#endif
...
!$OMP DO
      DO k=1,klev
        IF ( jfirst == 1 ) CALL xpavg(q(1,1,k,iq), im)
        IF ( jlast == jm ) CALL xpavg(q(1,jm,k,iq), im)
      ENDDO
!$OMP END DO
!$OMP END PARALLEL
!CSD$ END PARALLEL DO
    ...
    CALL timer_start(itimer_ghost_update)
    IF (lyaxt_transposition) THEN
      CALL yaxt_ffsl_ghost_update_4d(q2,ng)
    ELSE
...
!CSD$ END PARALLEL DO 
    ...
    IF (lyaxt_transposition) THEN
      CALL yaxt_tr_ffsl_to_gp(fb_gp, ps_gp, delp_gp, &
           &                  q, ps, delp)
#ifdef CHECK_TRANSPOSITION
      CALL check4d_ffsl2gp('q->fb_gp', q, fb_gp)
      CALL check2d_ffsl2gp('tpcore:ps->ps_gp', ps, ps_gp)
      CALL check3d_ffsl2gp('tpcore:delp->delp_gp', delp, delp_gp)
#endif
    ELSE
  ...
  SUBROUTINE air_mass_flux(im, jm, km, jfirst, jlast, iord, jord,    &
    ...
    CALL timer_start(itimer_ghost_update)
    IF (lyaxt_transposition) THEN
      CALL yaxt_ffsl_ghost_update_3d(v3,mg)
    ELSE
    ...
    CALL timer_start(itimer_ghost_update)
    IF (lyaxt_transposition) THEN
      CALL yaxt_ffsl_ghost_update_3d(cx,ng)
    ELSE
...
!!$        ENDDO
!!$      ENDDO
       CALL update_ps(0,ak(1),delp(1:im,jfirst:jlast,1:klev), &
            ps(1:im,jfirst:jlast))
! mz_kk_20070411-
...
!2
#ifdef E5SGHOST
    IF ((js2gng == 2).AND.(1 >= jfirst-ng)) THEN
      DO i=1,im
        adx(i, 1) = q(i,1)
      ENDDO
    ENDIF
!3
    IF ((jn2gng == jm-1).AND.(jm <= jlast+ng)) THEN
      DO i=1,im
        adx(i,jm) = q(i,jm)
      ENDDO
    ENDIF
...
!4
#ifdef E5SGHOST
      IF ((js2gng1 == 2).AND.(1 >= jfirst-ng+1)) THEN
#else
!!$   IF ( jfirst == 1 ) THEN
      IF ( have_j1(ng) ) THEN   
#endif
...
!5
#ifdef E5SGHOST
      IF ((jn2gng1 == jm-1).AND.(jm <= jlast+ng-1)) THEN
#else
!!$   IF ( jlast == jm ) THEN
      IF ( have_jm(ng) ) THEN
#endif
...
!8
#ifdef E5SGHOST
      IF ((js2g1 == 2).AND.(1 >= jfirst-1)) THEN
#else
!!$   IF ( jfirst == 1 ) THEN
      IF ( have_j1(1) ) THEN
#endif
...
!9
#ifdef E5SGHOST
      IF ((jn2g1 == jm-1).AND.(jm <= jlast+1)) THEN
#else
!!$   IF ( jlast == jm ) THEN
      IF ( have_jm(1) ) THEN
#endif
...
!CSD$ END PARALLEL DO
  ...
  SUBROUTINE map1_ppm_gp( km,   pe1,                               &
    ...
    REAL(wp), INTENT(inout) :: q2(kbdim,kn)     ! Field input/output
...
!lk    ENDDO
  ...
  SUBROUTINE adj_fx(im, jm, km, jfirst, jlast, ak, bk, ffsl,  &
    ...
    fx_iteration: DO it=1,n_adj
      ...
      CALL timer_start(itimer_ghost_update)
      IF (lyaxt_transposition) THEN
        CALL yaxt_ffsl_ghost_update_2d(ps, mg)
      ELSE
...
!!$        ENDDO
!!$      ENDDO
      CALL update_ps(it,ak(1),delp(1:im,jfirst:jlast,1:klev), &
           ps(1:im,jfirst:jlast))
! mz_kk_20070411-
...
! mz_kk_20070411+
  !-------------------------------------------------------------------------
#ifdef FAST_BUT_DECOMPDEP
  SUBROUTINE update_ps (it,ak1,delp,ps)

    ! update surface pressure      Klaus Ketelsen (April 2007)
    ! In case of multiple PROCB, update local level first and the global sum

    USE mo_mpi,           ONLY: p_pe, p_sum, p_communicator_a

    integer,intent(IN)                            :: it
    REAl(wp),intent(IN)                           :: ak1
    REAL(wp),intent(IN),dimension(:,:,:)          :: delp
    REAL(wp),intent(INOUT),dimension(:,:)         :: ps

    integer                                       :: i,j,k
    real(wp)                                      :: ak1_lo
    real(wp),dimension(size(ps))                  :: ps_lo

#ifdef NOMPI
    ak1_lo = ak1
#else
    if(ldc%set_b == 1)   then
      ak1_lo = ak1                      !Add ak(1) only once
    else
      ak1_lo = 0.0
    endif
#endif
...
#ifndef NOMPI
    ! there is no p_sum for 2-D arrays. reshape to call p_sum only once

    ps_lo = reshape(ps,(/size(ps_lo)/))
    ps = reshape(p_sum(ps_lo,p_communicator_a),(/size(ps,1),size(ps,2)/))
#endif
!kk    write(6,*) 'ps_sum ',ldc%set_b,p_pe,it,sum(ps)
  ...
  SUBROUTINE update_ps (it,ak1,delp,ps)
    ...
    integer,intent(IN)                            :: it
    REAl(wp),intent(IN)                           :: ak1
    REAL(wp),intent(INOUT),dimension(:,:,:)       :: delp
    REAL(wp),intent(INOUT),dimension(:,:)         :: ps

    integer                                       :: k
    real(wp) :: ps_gp(ldc%nproma,ldc%ngpblks)
    real(wp) :: delp_gp(ldc%nproma,ldc%nlev,ldc%ngpblks)
    integer  :: nproma, nlev, ngpblks
    ...
    ps = 0._wp
    IF (lyaxt_transposition) THEN
      CALL yaxt_tr_ffsl_to_gp_3d(delp_gp, delp)
#ifdef CHECK_TRANSPOSITION
      CALL check3d_ffsl2gp('update_ps:delp->delp_gp', delp, delp_gp) ! fails
#endif
    ELSE
      CALL tr_gp_ffsl(ldc, -1, delp_gp, delp)
    ENDIF
    ps_gp(:,:) = ak1
    DO k=1,nlev
      ps_gp(1:nproma,1:ngpblks) = ps_gp(1:nproma,1:ngpblks) + &
                                  delp_gp(1:nproma,k,1:ngpblks)
    ENDDO
    IF (lyaxt_transposition) THEN
      CALL yaxt_tr_gp_to_ffsl_2d(ps_gp, ps)
#ifdef CHECK_TRANSPOSITION
      CALL check2d_gp2ffsl('ps_gp->ps', ps_gp, ps)
#endif
    ELSE
      CALL tr_gp_ffsl(ldc, 1, ps_gp, ps)
    ENDIF
!!$    write(6,*) 'ps_sum ',ldc%set_b,p_pe,it,sum(ps)
...
#else
        sent = 0
        DO j = 1, ldc% ffsl% nb_s% length
          zones = min (ldc% ffsl% nlat, ng - sent)
            IF (zones <= 0) EXIT
          CALL p_isend (q (1, jfirst, k, i), ldc% ffsl% nb_s% pe (j), 1234, im * zones)
          sent = sent + zones
        END DO

        sent = 0
        DO j = 1, ldc% ffsl% nb_n% length
          zones = min (ldc% ffsl% nlat, ng - sent)
            IF (zones <= 0) EXIT
          CALL p_isend (q (1, jlast + 1 - zones, k, i), ldc% ffsl% nb_n% pe (j), 9876, im * zones)
          sent = sent + zones
        END DO
#endif
      ENDDO
...
!15
        ...
        received = 0
        DO j = 1, ldc% ffsl% nb_s% length
          zones = min (ldc% ffsl% nb_s% zones (j), ng - received)
            IF (zones <= 0) EXIT
          CALL p_irecv (q (1, jfirst - received - zones, k, i), ldc% ffsl% nb_s% pe (j), 9876, im * zones)
          received = received + zones
        END DO

        received = 0
        DO j = 1, ldc% ffsl% nb_n% length
          zones = min (ldc% ffsl% nb_n% zones (j), ng - received)
            IF (zones <= 0) EXIT
          CALL p_irecv (q (1, jlast + 1 + received, k, i), ldc% ffsl% nb_n% pe (j), 1234, im * zones)
          received = received + zones
        END DO
#endif
      ENDDO
...
!16
...
#else
    REAL(wp) :: q_send1(im,km,nq,ng)
    REAL(wp) :: q_send2(im,km,nq,ng)
    REAL(wp) :: q_recv1(im,km,nq,ng)
    REAL(wp) :: q_recv2(im,km,nq,ng)

    received = 0
    DO j = 1, ldc% ffsl% nb_s% length
      zones = min (ldc% ffsl% nb_s% zones (j), ng - received)
        IF (zones <= 0) EXIT
      CALL p_irecv (q_recv1 (1, 1, 1, 1 + received), ldc% ffsl% nb_s% pe (j), 9876, im * km * nq * zones)
      received = received + zones
    END DO
    ...
    DO j = 1, ldc% ffsl% nb_n% length
      zones = min (ldc% ffsl% nb_n% zones (j), ng - received)
        IF (zones <= 0) EXIT
      CALL p_irecv (q_recv2 (1, 1, 1, 1 + received), ldc% ffsl% nb_n% pe (j), 1234, im * km * nq * zones)
      received = received + zones
    END DO

    forall (j = 1 : min (ldc% ffsl% nlat, ng)) q_send1 (:, :, :, j) = q (:, jfirst + j - 1, :, :)
    sent = 0
    DO j = 1, ldc% ffsl% nb_s% length
      zones = min (ldc% ffsl% nlat, ng - sent)
        IF (zones <= 0) EXIT
      CALL p_isend (q_send1 (1, 1, 1, 1), ldc% ffsl% nb_s% pe (j), 1234, im * km * nq * zones)
      sent = sent + zones
    END DO

    forall (j = 1 : min (ldc% ffsl% nlat, ng)) q_send2 (:, :, :, j) = q (:, jlast + 1 - j, :, :)
    sent = 0
    DO j = 1, ldc% ffsl% nb_n% length
      zones = min (ldc% ffsl% nlat, ng - sent)
        IF (zones <= 0) EXIT
      CALL p_isend (q_send2 (1, 1, 1, 1), ldc% ffsl% nb_n% pe (j), 9876, im * km * nq * zones)
      sent = sent + zones
    END DO

    CALL p_wait

    forall (j = 1 : ng) q (:, jfirst - j, :, :) = q_recv1 (:, :, :, j)
    forall (j = 1 : ng) q (:, jlast  + j, :, :) = q_recv2 (:, :, :, j)
#endif
#endif
...
!   REAL(wp) :: q_recv1(im,ng,km,nq) 
!   REAL(wp) :: q_recv2(im,ng,km,nq)
    real(wp), dimension(:), allocatable, save :: q_send1, q_send2, q_recv1, q_recv2
!cdir gm_array(q_send1,q_send2,q_recv1,q_recv2)
    integer, save :: qsize_old = 0
    integer :: i, g, m, k, ii

    qsize = im*ng*km*nq

    if ( qsize .gt. qsize_old ) then
      if ( qsize_old .gt. 0 ) then
        deallocate (q_send1, q_send2, q_recv1, q_recv2)
      endif
      allocate ( q_send1(qsize), q_send2(qsize), &
                 q_recv1(qsize), q_recv2(qsize) )
      qsize_old = qsize
    endif
      ...
      do k = 1, nq
        do m= 1, km
          do g = 1, ng
            do i = 1, im
              ii = ii+1
              q(i, jfirst-ng-1+g, m, k) = q_recv1(ii)
            enddo
          enddo
        enddo
      enddo
...
#ifdef CHECK_TRANSPOSITION
 CHARACTER(len=40) FUNCTION aux_enum_label(label) RESULT(s)
   CHARACTER(len=*) :: label
   CHARACTER(len=16) :: x
   INTEGER, SAVE :: call_num = 0
   call_num = call_num + 1
   WRITE(x,'(I8)') call_num
   s=TRIM(ADJUSTL(label))//' ('//TRIM(ADJUSTL(x))//')'
 END FUNCTION aux_enum_label

 SUBROUTINE check2d_gp2ffsl(label, gp, ffsl, with_exp)
    USE mo_transpose,     ONLY: tr_gp_ffsl
    CHARACTER(len=*), INTENT(in) :: label
    REAL(wp), INTENT(in) :: gp(:,:)
    REAL(wp), INTENT(in) :: ffsl(:,:)
    LOGICAL, OPTIONAL, INTENT(in) :: with_exp

    REAL(wp) :: ref_gp(SIZE(gp,1), SIZE(gp,2))
    REAL(wp) :: ref_ffsl(SIZE(ffsl,1), SIZE(ffsl,2))
    LOGICAL :: lexp
    ...
    IF (lexp) ref_ffsl = EXP(ref_ffsl)

    IF (ANY(ffsl /= ref_ffsl)) CALL finish('mo_tpcore::check2d_gp2ffsl', TRIM(aux_enum_label(label)))
    IF (ldc%pe == 0) CALL message('mo_tpcore::check2d_gp2ffsl:',TRIM(aux_enum_label(label))//' passed')
  END SUBROUTINE check2d_gp2ffsl

  SUBROUTINE check3d_gp2ffsl(label, gp, ffsl)
    USE mo_transpose,     ONLY: tr_gp_ffsl
    CHARACTER(len=*), INTENT(in) :: label
    REAL(wp), INTENT(in) :: gp(:,:,:)
    REAL(wp), INTENT(in) :: ffsl(:,:,:)

    REAL(wp) :: ref_gp(SIZE(gp,1), SIZE(gp,2), SIZE(gp,3))
    REAL(wp) :: ref_ffsl(SIZE(ffsl,1), SIZE(ffsl,2), SIZE(ffsl,3))

    ref_gp = gp
    ref_ffsl = 0.0_dp
    CALL tr_gp_ffsl(ldc , 1 ,ref_gp, ref_ffsl)

    IF (ANY(ffsl /= ref_ffsl)) CALL finish('mo_tpcore::check3d_gp2ffsl', TRIM(aux_enum_label(label)))
    IF (ldc%pe == 0) CALL message('mo_tpcore::check3d_gp2ffsl:',TRIM(aux_enum_label(label))//' passed')
  END SUBROUTINE check3d_gp2ffsl

    SUBROUTINE check2d_ffsl2gp(label, ffsl, gp)
    USE mo_transpose,     ONLY: tr_gp_ffsl
    CHARACTER(len=*), INTENT(in) :: label
    REAL(wp), INTENT(in) :: ffsl(:,:)
    REAL(wp), INTENT(in) :: gp(:,:)
    ...
    IF (ANY(gp /= ref_gp)) CALL finish('mo_tpcore::check2d_ffsl2gp', TRIM(aux_enum_label(label)))
    IF (ldc%pe == 0) CALL message('mo_tpcore::check2d_ffsl2gp:',TRIM(aux_enum_label(label))//' passed')
  END SUBROUTINE check2d_ffsl2gp

  SUBROUTINE check3d_ffsl2gp(label, ffsl, gp)
    USE mo_transpose,     ONLY: tr_gp_ffsl
    CHARACTER(len=*), INTENT(in) :: label
    REAL(wp), INTENT(in) :: ffsl(:,:,:)
    REAL(wp), INTENT(in) :: gp(:,:,:)
    ...
    IF (ANY(gp /= ref_gp)) CALL finish('mo_tpcore::check3d_ffsl2gp', TRIM(aux_enum_label(label)))
    IF (ldc%pe == 0) CALL message('mo_tpcore::check3d_ffsl2gp:',TRIM(aux_enum_label(label))//' passed')
  END SUBROUTINE check3d_ffsl2gp

  SUBROUTINE check4d_ffsl2gp(label, ffsl, gp)
    USE mo_transpose,     ONLY: tr_gp_ffsl
    CHARACTER(len=*), INTENT(in) :: label
    REAL(wp), INTENT(in) :: ffsl(:,:,:,:)
    REAL(wp), INTENT(in) :: gp(:,:,:,:)

    REAL(wp) :: ref_ffsl(SIZE(ffsl,1), SIZE(ffsl,2), SIZE(ffsl,3), SIZE(ffsl,4))
    REAL(wp) :: ref_gp(SIZE(gp,1), SIZE(gp,2), SIZE(gp,3), SIZE(gp,4))

    ref_ffsl = ffsl
    ref_gp = 0.0_dp
    CALL tr_gp_ffsl(ldc ,-1 ,ref_gp, ref_ffsl)

    IF (ANY(gp /= ref_gp)) CALL finish('mo_tpcore::check4d_ffsl2gp', TRIM(aux_enum_label(label)))
    IF (ldc%pe == 0) CALL message('mo_tpcore::check4d_ffsl2gp:', TRIM(aux_enum_label(label))//' passed')
  END SUBROUTINE check4d_ffsl2gp
