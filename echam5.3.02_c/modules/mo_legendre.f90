MODULE mo_legendre
  ...
  !--------------------------------------------------
  ! mz_jb_20040506+: SAVE attributes added
  REAL(dp), TARGET,  ALLOCATABLE, SAVE :: pnmd(:)   ! direct legendre transform
  REAL(dp), TARGET,  ALLOCATABLE, SAVE :: anmd(:)
  REAL(dp), TARGET,  ALLOCATABLE, SAVE :: rnmd(:)
  ! mz_jb_20040506-
  !------------------------------------------------
...
!------------------------------------------------------------------------------
  SUBROUTINE legmod(kpass, pnmt, anmt, rnmt)
    ...
    ELSE 
      ...
      DO j=1, dc%nlat/2    
        z2w = 2.0_dp * gl_gw(j)
        ! mz_pj_20060316 only if PRESENT
        IF (PRESENT(pnmt)) pnmt(j,:) = pnm(:,j) * z2w 
        IF (PRESENT(anmt)) anmt(j,:) = anm(:,j) * z2w 
      END DO
      ...
      DO j = 1, dc%nlm
        ...
        DO jn = 1, inp
          fnnp = REAL(innp+jn,dp)
          IF (PRESENT(rnmt)) & ! mz_pj_20060316
  ...
  SUBROUTINE leginv(kpass, pnmit, anmit, pnmiuvt, anmiuvt)
    ...
    IF(PRESENT(kpass)) THEN
      ...
      DO jm = 1, dc%nlm
        ...
        DO jn = is, inp
          fnnp = REAL(innp+jn, kind=dp)
          pnmiuv(imp+jn) = pnmi(imp+jn)*a*innp / ((fnnp-1.0_dp)*fnnp)
          anmiuv(imp+jn) = anmi(imp+jn)*a*zcst / ((fnnp-1.0_dp)*fnnp)
        END DO
    ...
    ELSE
      ...
      DO jm = 1, dc%nlm
        ...
        DO jn = is, inp
          fnnp = REAL(innp+jn, kind=dp)
          IF (PRESENT(pnmiuvt)) & ! mz_pj_20040130
          pnmiuvt(:,imp+jn) = pnm(imp+jn,:)*a*innp       / ((fnnp-1.0_dp)*fnnp)
          IF (PRESENT(anmiuvt)) & ! mz_pj_20040130
      ...
      zsa = 1.0_dp / a
      IF (PRESENT(anmit)) & ! mz_pj_20040130
      anmit(:,:) = TRANSPOSE(anm(:,:)) * zsa
      IF (PRESENT(pnmit)) & ! mz_pj_20040130
      pnmit(:,:) = TRANSPOSE(pnm(:,:))
