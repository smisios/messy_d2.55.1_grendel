...
! !DATE:         August 2002
! !LAST CHANGE:  25. July 2003, Joachim Buchholz
...
!BOX
    IF ((nudgdsize < dtx1) .AND. (dtx1 < (1.0_dp-nudgdsize)) ) THEN
       wobs = nudgdamp
    ELSE
...
!BOX
    ...
    DO jk=1,nlev    ! 3-d fields
       nudgta(jk,:,:)=1.0_dp + flagn(jk,:,:)*(nudgt(jk)+wmod*nudgto(jk)-1.0_dp)
       nudgtb(jk,:,:)=         flagn(jk,:,:)*           wobs*nudgto(jk)

       nudgda(jk,:,:)=1.0_dp + flagn(jk,:,:)*(nudgd(jk)+wmod*nudgdo(jk)-1.0_dp)
       nudgdb(jk,:,:)=         flagn(jk,:,:)*           wobs*nudgdo(jk)

       nudgva(jk,:,:)=1.0_dp + flagn(jk,:,:)*(nudgv(jk)+wmod*nudgvo(jk)-1.0_dp)
       nudgvb(jk,:,:)=         flagn(jk,:,:)*           wobs*nudgvo(jk)
    END DO
    DO jk=1,nssfc   ! surface fields
       nudgsa(jk,:,:)=1.0_dp + flagn(nlevp1,:,:)*(nudgp+wmod*nudgpo-1.0_dp)
    ...
    ! mz_ak_20060731+
    IF ((SIZE(ldc%sm) > 0) .AND. (SIZE(ldc%snn0)>0)) THEN
    ! mz_ak_20060731-
       IF (ldc%sm(1) == 0 .AND. ldc%snn0(1) == 0 .AND. ldc%snsp /= 0) &
            buf_ref(:,2,1,:) = 0.0_dp
...
!BOX
    ...
    IF (lnudgwobs) THEN
       idano(:,:,:) = (1.0_dp-flagn(:nlev, :,:))*(sd (:,     :,:) &
                     - sdobs(:,     :,:))
       ivano(:,:,:) = (1.0_dp-flagn(:nlev, :,:))*(svo(:,     :,:) &
                     - svobs(:,     :,:))
       itano(:,:,:) = (1.0_dp-flagn(:nlev, :,:))*(stp(:nlev, :,:) &
                     - stobs(:nlev, :,:))
       isano(1,:,:) = (1.0_dp-flagn(nlevp1,:,:))*(stp(nlevp1,:,:) &
                     - stobs(nlevp1,:,:))
...
!BOX
       IF (lsite_n0) THEN
          ...
          stsite(:nlev, :,:) = stsite(:nlev, :,:)                 &
               - flagn(:nlev, :,:)*stten(:,:,:)                   &
               + ( stp  (:nlev,:,:) - st_m_n0(:nlev,:,:) )*rtwodt &
               - ( stobs(:nlev,:,:) - st_o_n0(:nlev,:,:) )*rtwodt

          stsite(nlevp1,:,:) = stsite(nlevp1,:,:)                   &
               - flagn(nlevp1,:,:)*ssten(1,:,:)                     &
       ...
       ! store in SITE detection mode observed/modelled values from the last
       ! step
