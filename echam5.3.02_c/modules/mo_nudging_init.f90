...
!BOP
  ...
  ! Joachim Buchholz, MPI Mainz, 05. August 2003
...
!BOX
  ...
  USE mo_nudging_io,   ONLY: ndg_file_vor, ndg_file_div, ndg_file_stp, &
                             ndg_file_nc, ndgfile, ndginc ! op_pj_20190930
...
!BOP
    ...
    NAMELIST /ndgctl/&
         ...
         ndg_file_nc,   &! netcdf-file of the nudging data           !! AV
         ndginc,        &! increment along time axis in netCDF file  ! op_pj_20190930
         inudgformat     ! switch to specify format of nudging data  !! AV
!EOP
CONTAINS
...
!BOP
  ...
  RECURSIVE SUBROUTINE NudgingInit(ifunction)
...
#else
         lnudg_run, lnudg_out, nudgmin, nudgmax, &
#endif
...
#else
         NudgingStreamInit, nudg, nudg_gp, NdgInitCounter
#endif
...
!BOX
    ...
    INTEGER            :: kfile
    REAL(kind=dp), POINTER :: flagn_global(:,:,:), flagn_local(:,:,:)
    ...
    EXTERNAL pbclose, pbopen, pbseek, util_i8toi4
    INTRINSIC allocated, int, max, min, real, trim
...
!!$          ! op_pj_20130910+
!!$          IF (nsstinc > 0) then
!!$             CALL finish('NudgingInit', &
!!$                  'sst-nudging not implemented for netCDF; set nsstinc=0 in NDGCTL and use appropriate standard SST')
!!$          ENDIF
!!$          ! op_pj_20130910-
      ...
      IF (ltdiag .AND. p_parallel_io) THEN
         ! search next free unit in range 90-99: 
         ndunit = find_next_free_unit(90,99) 
...
!BOX
      ...
      ELSE IF (lfirst_cycle) THEN
         ...
         IF (inudgformat /= 2) THEN ! op_pj_20131112 only, if not netCDF format
            WRITE(mess,'(f7.2)') ndg_freez
            CALL message(' Use sea ice detection limit NDG_FREEZ = ',mess)
         END IF ! op_pj_20131112
         !<<dod
      ENDIF
...
!BOX
      IF (lfirst_cycle) THEN
         ...
         DO
            ...
            DO i = 1,nmaxc
               IF (nudgd(i) >= 0.0_dp) THEN; linp_div = .TRUE.; EXIT; END IF
               IF (nudgv(i) >= 0.0_dp) THEN; linp_vor = .TRUE.; EXIT; END IF
               IF (nudgt(i) >= 0.0_dp) THEN; linp_tem = .TRUE.; EXIT; END IF
            END DO
...
!!$              ihead_nc(2) = INT((timevals(1)-ihead_nc(1))*24._dp)       ! ihead_nc(2) is HH
                 ihead_nc(2) = NINT((timevals(1)-ihead_nc(1))*24._dp)      ! ihead_nc(2) is HH
! op_pj_20180412-
                 DEALLOCATE (timevals)
               ENDIF

               ! op_pj_20180329+
               ! nts must at least be 2, see timevals below ...
               IF (p_parallel) CALL p_bcast(nts, p_io)
               IF (nts < 2) THEN
                  CALL finish('NudgingInit',&
                       'less than 2 timesteps in nudging data file')
               ENDIF
               ! op_pj_20180329-
...
!!$              ihead_nc(2) = INT((timevals(2)-ihead_nc(1))*24._dp)       ! ihead_nc(2) is HH
                 ihead_nc(2) = NINT((timevals(2)-ihead_nc(1))*24._dp)      ! ihead_nc(2) is HH
! op_pj_20180412-
                 DEALLOCATE (timevals)
...
!BOX
      ...
      IF (lnudgstop) THEN
         IF (nudg_stop < next_date) lnudg_out = .FALSE.
      END IF
      ! op_pj_20150331-
#endif
...
!BOX
      ...
      flagn_local => flagn; flagn = 0.0_dp

      ALLOCATE(flagn_global(nlevp1,2,nsp)); flagn_global(:,:,:) = 0.0_dp

      SELECT CASE(nudgtrun)
      CASE (NDG_WINDOW_ALL)
         !>>dod bugfix
         WRITE(mess,'(a14,i0,a29)') '  Truncation (',nudgtrun, &
              ') ... USE all meridional parts'
         !<<dod
         flagn_global(nudglmin:nudglmax,:,nudgmin:nudgmax) = 1.0_dp
         flagn_global(nlevp1           ,:,nudgmin:nudgmax) = 1.0_dp

      CASE (NDG_WINDOW_CUT)
         !>>dod bugfix
         WRITE(mess,'(a14,i0,a29)') '  Truncation (',nudgtrun, &
              ')... cut off meridional parts'
         !<<dod
         DO jm=nudgsmin,nudgsmax
            is1 = nmp(jm+1) + 1
            is2 = nmp(jm+1) + 1 + nudgsmax - jm
            flagn_global(nudglmin:nudglmax,:,is1:is2) = 1.0_dp
            flagn_global(nlevp1           ,:,is1:is2) = 1.0_dp
         END DO

      CASE (NDG_WINDOW_CUT0)
         !>>dod bugfix
         WRITE(mess,'(a14,i0,a29)') '  Truncation (',nudgtrun, &
              ')... cut off except wave 0'
         !<<dod
         DO jm=nudgsmin,nudgsmax
            IF (jm==0) THEN
               is1 = nmp(1) + 1
      ...
      DO i=1,nlevp1
         DO ii=1,nsp
            sum1 = INT( REAL(sum1,dp)                   &
                       +flagn_global(i,1,ii)            &
                       +flagn_global(i,2,ii))
            sum2 = INT( REAL(sum2,dp)                   &
                       +(1.0_dp-flagn_global(i,1,ii))   &
                       +(1.0_dp-flagn_global(i,2,ii)))
         END DO
      END DO
      ...
      DEALLOCATE(flagn_global, stat=ierr) ! not needed any more
      IF (ierr /= 0) CALL finish('NudgingInit', &
           'Error while deallocating flagn_global')
...
!BOX  
      ...
      linp_vor = .FALSE.
      DO i = 1,nlev
         IF (nudgt(i)>=0.0_dp) THEN; linp_tem = .TRUE.; ilev_t_max = i; END IF
         IF (nudgd(i)>=0.0_dp) THEN; linp_div = .TRUE.; ilev_d_max = i; END IF
         IF (nudgv(i)>=0.0_dp) THEN; linp_vor = .TRUE.; ilev_v_max = i; END IF
      END DO
      ...
      !-- find minimum level
      DO i = ilev_t_max,1,-1; IF (nudgt(i) < 0.0_dp) EXIT; ilev_t_min = i
      END DO
      DO i = ilev_d_max,1,-1; IF (nudgd(i) < 0.0_dp) EXIT; ilev_d_min = i
      END DO
      DO i = ilev_v_max,1,-1; IF (nudgv(i) < 0.0_dp) EXIT; ilev_v_min = i
      END DO
...
!BOX
      ...
      ELSE
         nudgp          = 1.0_dp-nudgpo
         nudgt(1:nlev)  = 1.0_dp-nudgto(:)
         nudgd(1:nlev)  = 1.0_dp-nudgdo(:)
         nudgv(1:nlev)  = 1.0_dp-nudgvo(:)
      END IF
             
      !--- convert coefficients for implicit nudging
      IF (lnudgimp) THEN
         !--- using implicit nudging reset weights
         nudgp         = 1.0_dp/(1.0_dp+nudgpo)
         nudgt(1:nlev) = 1.0_dp/(1.0_dp+nudgto(:))
         nudgd(1:nlev) = 1.0_dp/(1.0_dp+nudgdo(:))
         nudgv(1:nlev) = 1.0_dp/(1.0_dp+nudgvo(:))
      ...
      IF (lnudgimp) THEN
         ...
         DO i=1,nlev
            WRITE(mess,'(i5,3(e12.3,2f10.3))') i,     &
               (1.0_dp-nudgt(i))/(nudgt(i)*twodt),    &
               nudgt(i)*100.0_dp, nudgto(i)*100.0_dp, &
               (1.0_dp-nudgd(i))/(nudgd(i)*twodt),    &
               nudgd(i)*100.0_dp, nudgdo(i)*100.0_dp, &
               (1.0_dp-nudgv(i))/(nudgv(i)*twodt),    &
               nudgv(i)*100.0_dp, nudgvo(i)*100.0_dp
            CALL message('',mess)
         ENDDO
         WRITE(mess,'(a,e12.3,2f10.3)') ' surface fields = ', &
            (1.0_dp-nudgp)/(nudgp*twodt),                     &
            nudgp*100.0_dp,nudgpo*100.0_dp
         CALL message('',mess)
...
#ifdef MESSY
      CALL set_stream(nudg_gp, lpout=.TRUE.)  ! switch output stream on
#endif
      CALL NdgInitCounter
...
!BOX
      IF (lnudg_run) THEN
         ...
         lnudg_run = .FALSE.
#ifdef MESSY
         lnudg_out = .FALSE. ! op_pj_20150331
#endif
      ELSE
...
!BOX
      ...
      CALL CloseBlock
      IF (nsstinc > 0) THEN ! op_pj_20130905 bug-fix according to S.Rast
         ! mz_jb_20060316: only on p_parallel_io
         IF (p_parallel_io) CALL NudgingSSTClose(.TRUE.)
      END IF                ! op_pj_20130905 bug-fix according to S.Rast

      CALL message('','Close nudging data files.')
      ! switch output stream off: 
      CALL set_stream(nudg, lpout=.FALSE., lrerun=.FALSE.)  
#ifdef MESSY
      CALL set_stream(nudg_gp, lpout=.FALSE., lrerun=.FALSE.)  
#endif
      lnudge = .FALSE.
...
! ==============================================================================
 ...
 CONTAINS
   ...
   SUBROUTINE bc_nudging
     ...
     CALL p_bcast (ndg_file_nc,   p_io) !! AV
     ...
     CALL p_bcast (inudgformat,   p_io)  !! AV
