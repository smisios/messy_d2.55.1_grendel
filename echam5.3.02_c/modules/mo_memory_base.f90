...
!------------------------------------------------------------------------------
  ...
  USE mo_linked_list,  ONLY: memory_info, memory_type, t_stream, list_element,&
                             ...
                             print_memory_info, empty_info,                   &
                             GRIB, NETCDF,                                    &
                             BELOWSUR, SURFACE, ABOVESUR2, ABOVESUR10,        &
                             HYBRID, HYBRID_H, UNKNOWN,                       &
                             GAUSSIAN, FOURIER, SPECTRAL, HEXAGONAL, GRIDPOINT
...
!------------------------------------------------------------------------------
  ...
  PUBLIC :: GRIB, NETCDF           ! valid file types
  PUBLIC :: GAUSSIAN, FOURIER, SPECTRAL, HEXAGONAL, GRIDPOINT ! flag 'repr'
...
!------------------------------------------------------------------------------
  ...
  INTEGER      ,PARAMETER    :: maxstr   = 30    ! max number of output streams
...
!==============================================================================
  ...
  SUBROUTINE new_stream (stream, name, filetype,       &
                         post_suf, rest_suf, init_suf, &
                         lpost, lpout, lrerun, linit, interval)
...
!==============================================================================
  ...
  SUBROUTINE set_stream (stream, filetype,             &
                         post_suf, rest_suf, init_suf, &
                         lpost, lpout, lrerun, linit, interval)
...
!==============================================================================
  ...
  INTEGER          ,INTENT(in) ,OPTIONAL :: tracidx
    ...
    LOGICAL :: verb
    LOGICAL :: a
    INTEGER :: iklev, iklon, iklat, iktrac, iknsp, ikcpl
    LOGICAL :: linvalid
    ...
    IF (a) THEN
      ...
      !---------------------------
      iklev = 0; iklon = 0; iklat = 0; iktrac = 0; iknsp = 0; ikcpl = 0
    ...
    info% lreg = .false.
    info% dima = info% dim
    if (info% repr /= GAUSSIAN) info% lreg = .true.
    if (ldc % lreg)             info% lreg = .true.
    if (.not.info% lreg) then
      select case (info% ndim)
      case (2)
...
!-----------------------------------------------------------------------------
  ...
  LOGICAL          ,INTENT(in) ,OPTIONAL :: verbose       ! produce printout
    ...
    CALL set_memory_info (element% field% info                                &
     ,name=name  ,longname=longname ,units=units                              &
     ,ldims=ldims ,gdims=gdims ,ndim=2 ,alloc=alloc ,repr=repr                &
     ,lpost=lpost ,laccu=laccu, lmiss=lmiss, missval=missval                  &
     ,reset=reset, lrerun=lrerun                                              &
     ,contnorest=contnorest ,table=table ,code=code ,bits=bits                &
     ,leveltype=leveltype ,dimnames=dimnames                                  &
...
!-----------------------------------------------------------------------------
  SUBROUTINE add_stream_element_1d (stream, name, ptr, ldims, gdims, &
       code, table, laccu, lrerun, contnorest, dimnames)

    TYPE (t_stream)       ,INTENT(inout):: stream      ! list
    CHARACTER (*)         ,INTENT(in)   :: name        ! name of variable
    REAL(dp)              ,POINTER      :: ptr(:)      ! reference to field
    INTEGER               ,INTENT(in)   :: ldims(1)    ! shape of array 
...
!==============================================================================
  ...
  SUBROUTINE print_memory_use (stream)
    ...
    WRITE (nout,'(a,i10,a,i4,a)') &
         'Memory in use: ', stream%memory_used, ' bytes in ', &
