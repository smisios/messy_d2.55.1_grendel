...
!OCL NOALIAS
  ...
  SUBROUTINE setup_spitfire
    ...
    USE mo_time_control,          ONLY: ztmst => time_step_len
#endif
    USE messy_main_tracer_mem_bi, ONLY: ntrac_gp, ti_gp, ON, I_ADVECT
#endif
    ...
    DO i = 1, plat
      ...
      zrcst  = 1.0/gl_sqcst(jglat)                 ! 1./cos(latitude)
...
#else
          fb(jl,i,jk,1) = (qm1(jl,jk,l)  + qte(jl,jk,l)  *ztmst) *zpde(jl,jk,i)
          fb(jl,i,jk,2) = (xlm1(jl,jk,l) + xlte(jl,jk,l) *ztmst) *zpde(jl,jk,i)
          fb(jl,i,jk,3) = (xim1(jl,jk,l) + xite(jl,jk,l) *ztmst) *zpde(jl,jk,i)
#endif
        END DO
      END DO
...
#else
      DO jt = 1, ntrac_gp
        IF (ti_gp(jt)%tp%meta%cask_i(I_advect) == ON) THEN
           ita = ita + 1
           DO jk = 1, plev
              DO jl = 1, plon
#ifndef _XNOZEROINITEND
                 fb(jl,i,jk,jps+ita) = xtm1(jl,jk,jt,l)*zpde(jl,jk,i)
#else
                 fb(jl,i,jk,jps+ita) = (xtm1(jl,jk,jt,l)  &
                      + xtte(jl,jk,jt,l) * ztmst) *zpde(jl,jk,i)
#endif
              END DO
           END DO
        END IF
...
#else
          qte    (jl,jk,jlat) = (fb(jl,ilat,jk,1)- qm1(jl,jk,jlat))/ztmst
          xlte   (jl,jk,jlat) = (fb(jl,ilat,jk,2)-xlm1(jl,jk,jlat))/ztmst
          xite   (jl,jk,jlat) = (fb(jl,ilat,jk,3)-xim1(jl,jk,jlat))/ztmst
#endif
        END DO
      END DO
      !
...
END MODULE mo_spitfire
  ...
  FUNCTION minmod(a, b) RESULT(c)
    ...
    c = 0.5*(SIGN(1.0,a) + SIGN(1.0,b))*MIN(ABS(a),ABS(b))
  ...
  FUNCTION medan(a, b, c) RESULT(d)
    ...
    z1 = b-a
    z2 = c-a
    d = a + 0.5*(SIGN(1.0,z1) + SIGN(1.0,z2))*MIN(ABS(z1),ABS(z2))
