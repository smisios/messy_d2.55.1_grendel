...
! 
#if ! (defined (__ibm__) || defined (__XT3__) || defined (LF) || defined (__PGI) || defined(__INTEL_COMPILER))
...
!
#if defined (__ibm__) || defined (sun) || defined (NAG) || defined (CRAY) || defined(__PGI)
#define TR15581 1
#endif
!
...
! on vector machines
!
#if (defined (CRAY) && ! defined (_CRAYMPP)) || defined (__SX__) || defined (ES) || defined (__uxp__)  || defined(__PGI)
#define EXPLICIT 1
#endif
!
MODULE mo_transpose
...
!------------------------------------------------------------------------------
  ...
  INTEGER, OPTIONAL    ,INTENT(in)  :: source       ! source to gather from
    ...
    LOGICAL              :: lreg        ! flag for regular grid
    REAL(dp)    ,POINTER :: lcb (:,:,:) ! pointer/temporary buffer
    real(dp), pointer        :: recv_ptr(:,:,:)
! NE jh
!cdir gm_array(lcb_g)
    real(dp),allocatable,save,target:: lcb_g (:,:,:)
    integer,save :: nk_old = 0
    !
    ! set, allocate local variables
    !
    ! for parallel debugging mode
    ...
    ELSE
      if ( nk_old /= nk ) then
        ...
        ALLOCATE (lcb_g(gl_dc(imype)% nglon,&
                  nk,                 & 
                  gl_dc(imype)% nglat))
        nk_old = nk
      endif
    ...
    ELSE
      ...
      !
      IF (gl_dc(imype)%pe == p_io) THEN
! PSRs esd0960/hlrs0073 - NEC jh
!       gather_array(gl_dc(imype)%pe)%receive3d(:,1:nk,:) = lcb(:,1:nk,:)
        recv_ptr => gather_array(gl_dc(imype)%pe)%receive3d
        recv_ptr(:,1:nk,:) = lcb(:,1:nk,:)
      ENDIF
      DO i = 1, p_nprocs
        IF (gl_dc(i)%pe /= gl_dc(imype)%pe) THEN
          CALL p_irecv(gather_array(gl_dc(i)%pe)%receive3d, gl_dc(i)%pe, &
      ...
      DO i = 1, p_nprocs
        ...
        IF(src == -1 .OR. (src == 0 .AND. p_io == pe)      &
          ...
          !
! PSRs esd0960/hlrs0073 - NEC jh
          recv_ptr => gather_array(pe)%receive3d
!         gl (gl_dc(i)% glons(1) : gl_dc(i)% glone(1),1:nk,   &
!             gl_dc(i)% glats(1) : gl_dc(i)% glate(1)) = &
!           gather_array(pe)%receive3d(:,1:nk,:gl_dc(i)% nglh(1))
          gl (gl_dc(i)% glons(1) : gl_dc(i)% glone(1),1:nk,   &
              gl_dc(i)% glats(1) : gl_dc(i)% glate(1)) = &
            recv_ptr(:,1:nk,:gl_dc(i)% nglh(1))
...
!               gather_array(pe)%receive3d(:,1:nk,gl_dc(i)% nglat/2+1:)
              gl (gl_dc(i)% glons(2) : gl_dc(i)% glone(2),1:nk,   &
                  gl_dc(i)% glats(2) : gl_dc(i)% glate(2)) = &
                recv_ptr(:,1:nk,gl_dc(i)% nglat/2+1:)
            ELSE
              !
              ! unpack second segment, split in longitudes
              !
...
!                                        gl_dc(i)% nglat/2+1:)
              gl(gl_dc(i)%glons(2):nlon,                                      &
                 1:nk,                                                        &
                 gl_dc(i)%glats(2):gl_dc(i)%glate(2))                         &
              = recv_ptr(:nlon-gl_dc(i)%glons(2)+1, 1:nk, gl_dc(i)% nglat/2+1:)
              !
!             gl (1:gl_dc(i)%glone(2),                                        &
!                 1:nk,                                                       &
!                 gl_dc(i)%glats(2):gl_dc(i)%glate(2))                        &
...
!                                        gl_dc(i)% nglat/2+1:)
              gl (1:gl_dc(i)%glone(2),                                        &
                  1:nk,                                                       &
                  gl_dc(i)%glats(2):gl_dc(i)%glate(2))                        &
              = recv_ptr(gl_dc(i)%nglon-gl_dc(i)%glone(2)+1:, 1:nk, gl_dc(i)% nglat/2+1:)
            ENDIF
          ENDIF
        ENDIF
      END DO
...
!#ifdef EXPLICIT
#if defined (EXPLICIT) || defined (__INTEL_v11)
      INTEGER :: i1, i2, i3, it
...
! NEC jh
!gm_array(buf_g)
    REAL(dp) ,ALLOCATABLE,save :: buf_g (:,:,:)   ! receive buffer
    integer, save     :: nglon_old = 0, nglat_old = 0, nlev_old = 0
    REAL(dp)     ,POINTER :: lx_gp (:,:,:) ! pointer/temporary buffer
...
! NEC jh
        if (      nglon_old .ne. dc% nglon &
             .or. nglat_old .ne. dc% nglat &
             .or.  nlev_old .ne. dc%  nlev   ) then

          if (allocated (buf_g) ) then
            deallocate (buf_g)
          endif

          ALLOCATE (buf_g (dc% nglon, dc% nglat, dc% nlev))
        ...
        DO k=1,dc%nlev
          dest_set_b = MOD(k-1,dc%nprocb)+1 ! set_b of receiving PE for this k
          DO j=1,dc%nglat
            buf_g(:,j,k) = lx_gp(:,k,dc%nglat+1-j)
          ENDDO
          !
          ! For PEs with odd set_a:
          !   Northern Hemisphere sent to same set_a
          ...
          IF( MOD(dc%set_a,2) == 1 ) THEN
            idx = dc%mapmesh(dest_set_b,dc%set_a)
            CALL p_isend(buf_g(1,nlat2+1,k),gdc(idx)%pe,1000+k,nlat2*nlong)
            idx = dc%mapmesh(dest_set_b,MIN(dc%set_a+1,dc%nproca))
            CALL p_isend(buf_g(1,1,k),gdc(idx)%pe,1000+k,nlat2*nlong)
          ELSE
            idx = dc%mapmesh(dest_set_b,dc%set_a-1)
            CALL p_isend(buf_g(1,nlat2+1,k),gdc(idx)%pe,1000+k,nlat2*nlong)
            idx = dc%mapmesh(dest_set_b,dc%set_a)
