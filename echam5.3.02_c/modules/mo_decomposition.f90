MODULE mo_decomposition
...
#ifdef E5SGHOST
  PUBLIC :: neighbours           ! component of pe_ffsl
#endif
  !
  ! Module variables
  !
  ...
  TYPE neighbours
    ...
    INTEGER, DIMENSION (nghost) :: pe     ! pe of neighbour
    INTEGER, DIMENSION (nghost) :: zones  ! number of zones at neighbour
  END TYPE neighbours
#endif
  !
...
#ifdef E5SGHOST
    TYPE (neighbours) :: nb_s  ! neighbours in the south
    TYPE (neighbours) :: nb_n  ! neighbours in the north
#endif
  END TYPE pe_ffsl
...
CONTAINS
  ...
  SUBROUTINE decompose (global_dc, nproma, nproca, nprocb, nlat, nlon, nlev, &
       nm, nn, nk, nadvec, norot, debug, lfull_m, &
       lats_1d, lons_1d)
  !
  ...
  INTEGER ,OPTIONAL   ,INTENT(in)  :: lons_1d         ! lons (column model)
    ...
    IF (debug_parallel >= 0) THEN
       ...
       !
       CALL set_decomposition (global_dc(pe:pe), 0, pe, 1, 1, .TRUE., lfullm)
...
#ifdef E5SGHOST
        DO i = 0, UBOUND(dc,1)
          CALL decompose_ffsl_ghosting (dc, dc(i))
        END DO
...
!------------------------------------------------------------------------------
#ifdef E5SGHOST
  SUBROUTINE decompose_ffsl_ghosting (dc, pe_dc)
  TYPE (pe_decomposed), INTENT (IN)     :: dc (:)  ! global decomposition array
  TYPE (pe_decomposed), INTENT (IN OUT) :: pe_dc   ! local decomposition element 

    INTEGER :: i, j
    INTEGER :: pe
    INTEGER :: zones

    pe_dc% ffsl% nb_s% length = nghost
    pe_dc% ffsl% nb_n% length = nghost

    zones = 0
    pe = pe_dc% ffsl% pe_s
    DO i = 1, nghost
      IF (pe == - 1) THEN
        pe_dc% ffsl% nb_s% length             =   i - 1
        pe_dc% ffsl% nb_s% pe    (i : nghost) = - 1
        pe_dc% ffsl% nb_s% zones (i : nghost) =   0
        EXIT
      END IF

      pe_dc% ffsl% nb_s% pe    (i) = pe
      j = indx (pe)
      pe_dc% ffsl% nb_s% zones (i) = dc (j)% ffsl% nlat

      zones = zones + pe_dc% ffsl% nb_s% zones (i)
      IF (zones >= nghost) THEN
        pe_dc% ffsl% nb_s% length                 =   i
        pe_dc% ffsl% nb_s% pe    (i + 1 : nghost) = - 1
        pe_dc% ffsl% nb_s% zones (i + 1 : nghost) =   0
        EXIT
      END IF
    ...
    DO i = 1, nghost
      IF (pe == - 1) THEN
        pe_dc% ffsl% nb_n% pe    (i : nghost) = - 1
        pe_dc% ffsl% nb_n% zones (i : nghost) = 0
        pe_dc% ffsl% nb_n% length             = i - 1
        EXIT
      END IF

      pe_dc% ffsl% nb_n% pe    (i) = pe
      j = indx (pe)
      pe_dc% ffsl% nb_n% zones (i) = dc (j)% ffsl% nlat

      zones = zones + pe_dc% ffsl% nb_n% zones (i)
      IF (zones >= nghost) THEN
        pe_dc% ffsl% nb_n% pe    (i + 1 : nghost) = - 1
        pe_dc% ffsl% nb_n% zones (i + 1 : nghost) =   0
        pe_dc% ffsl% nb_n% length                 =   i
        EXIT
      END IF
...
!..............................................................................
    ...
    INTEGER             :: indx
      ...
      END DO
    END FUNCTION indx
  END SUBROUTINE decompose_ffsl_ghosting
#endif
!------------------------------------------------------------------------------
  ...
  TYPE (pe_decomposed) ,INTENT (in) :: dc (:) ! global decomposition array
    ...
    IF (p_pe == p_io) THEN
      IF(SIZE(dc)>1) THEN
        WRITE(nout,*) REPEAT('-',79)
      ENDIF
      WRITE(nout,*)
      WRITE(nout,*) &
      '  ffsl Flux-Form Semi-Lagrangian transport scheme decomposition'
      WRITE(nout,*)
      WRITE(nout,'(a)')&
      '  seta  setb    pe  pe_x  pe_n  pe_s  nlat  latn  lats nlatx'
#ifdef E5SGHOST
      WRITE(nout,'(a)')&
      ' pn(1) zn(1) pn(2) zn(2) pn(3) zn(3) ps(1) zs(1) ps(2) zs(2) ps(3) zs(3)'
      WRITE(nout,*)
#endif
      WRITE(nout,*)
      DO i=1, SIZE(dc)
        ...
        WRITE(nout,'(12i6)')  &
          dc(i)% ffsl% nb_n% pe    (1), &
          dc(i)% ffsl% nb_n% zones (1), &
          dc(i)% ffsl% nb_n% pe    (2), &
          dc(i)% ffsl% nb_n% zones (2), &
          dc(i)% ffsl% nb_n% pe    (3), &
          dc(i)% ffsl% nb_n% zones (3), &
          dc(i)% ffsl% nb_s% pe    (1), &
          dc(i)% ffsl% nb_s% zones (1), &
          dc(i)% ffsl% nb_s% pe    (2), &
          dc(i)% ffsl% nb_s% zones (2), &
          dc(i)% ffsl% nb_s% pe    (3), &
          dc(i)% ffsl% nb_s% zones (3)
#endif
      END DO
      WRITE(nout,*)
    ENDIF
