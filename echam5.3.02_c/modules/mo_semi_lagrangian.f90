...
#else
          fb(jl+nxpt,jk,1,k,ih) = qm1(jl,jk,l)  + qte(jl,jk,l) * ztmst
          fb(jl+nxpt,jk,2,k,ih) = xlm1(jl,jk,l) + xlte(jl,jk,l) * ztmst
          fb(jl+nxpt,jk,3,k,ih) = xim1(jl,jk,l) + xite(jl,jk,l) * ztmst
#endif
        END DO
      END DO
...
#else
              xtp1(jl,jk,l) = xtm1(jl,jk,jt,l) + xtte(jl,jk,jt,l) * ztmst
              IF (ABS(xtp1(jl,jk,l))<zlimit) xtp1(jl,jk,l) = 0.0_dp
              fb(jl+nxpt,jk,3+ita,k,ih) = xtp1(jl,jk,l)
#endif
            END DO
          END DO
        END IF
...
#if (! defined CRAY) || (defined __SX__) || (defined ES) || (defined _CRAYMPP)
