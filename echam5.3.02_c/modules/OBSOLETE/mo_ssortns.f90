...
! fb_mk_20130126+
  ...
  USE messy_main_data_bi, ONLY : gworo_du, gworo_dv, gworo_dt  & ! tendency due to ORO GW DRAG
...
! fb_mk_20130126-
  ...
  REAL(dp),DIMENSION(kbdim,klev)       :: lo_tte, lo_vom, lo_vol
#endif

#ifdef MESSY
! fb_mk_20130126+
  zdu_oro => gworo_du(1:kbdim,:,jrow)
  zdv_oro => gworo_dv(1:kbdim,:,jrow)
  zdt_oro => gworo_dt(1:kbdim,:,jrow)
  zdu_lif => gwlif_du(1:kbdim,:,jrow)
  zdv_lif => gwlif_dv(1:kbdim,:,jrow)
  zdt_lif => gwlif_dt(1:kbdim,:,jrow)
! fb_mk_20130126-
#endif
...
#ifdef MESSYTENDENCY
   if(first)   my_handle = mtend_get_handle ('ssodrag')
   first = .false.
...
!DIR$ PREFERSTREAM
     DO jl=1,kproma
        IF(ktest(jl) == 1) THEN        ! mz_jb_20040610
           IF (jk >= kknu(jl)) THEN    ! mz_jb_20040610
           znum(jl)=pnu(jl)
