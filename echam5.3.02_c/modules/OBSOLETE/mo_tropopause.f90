...
CONTAINS
  ...
  SUBROUTINE WMO_tropopause(kproma, kbdim, klev, &
    ...
    REAL(dp) :: zag, zbg, zasum, zamean, zdp

    INTEGER :: jl, jk, kcount, jj 
...
!$OMP THREADPRIVATE (linit)
    ...
    ! compute dt/dz
    zdtdz(:,:)= 0.0_dp ! mz_pj_20050613
    zpmk(:,:) = 0.0_dp ! mz_pj_20050613
    DO jk = iplimb+1, iplimt-1, -1
      DO jl = 1, kproma

        ! ztm       lineare Interpolation in p**kappa
        ! gamma       dt/dp = a * kappa + papm1(jl,jk)**(kappa-1.)

        zpmk(jl,jk) = 0.5_dp*(papm1(jl,jk-1)**zkappa+papm1(jl,jk)**zkappa)
        zpm(jl,jk)  = zpmk(jl,jk)**zzkap               ! p centre
        za(jl,jk)   = (ptm1(jl,jk-1)-ptm1(jl,jk)) &
             /(papm1(jl,jk-1)**zkappa-papm1(jl,jk)**zkappa)
        zb(jl,jk)   = ptm1(jl,jk)-(za(jl,jk)*papm1(jl,jk)**zkappa)
        ztm(jl,jk)  = za(jl,jk)*zpmk(jl,jk)+zb(jl,jk)      ! T centre
        
        zdtdz(jl,jk)=zfaktor*zkappa*za(jl,jk)*zpm(jl,jk)**zkappa/ztm(jl,jk)
      ENDDO
    ENDDO
