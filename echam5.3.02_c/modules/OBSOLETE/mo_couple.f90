...
!
  ...
  USE mo_memory_g3b,      ONLY: awsol, &
                                ...
                                seaice,tsw,siced,sni,alake,slf,&
                                ocu,ocv
...
!CSL        IF ( gl_alake(jl,jrow) > 0.0_wp ) THEN
...
! !USES:
  ...
  REAL(dp)             :: area(nlon,ngl)  ! 2dim array of grid cell areas
  INTEGER              :: mask(nlon,ngl)  ! 2dim array of land see mask 
...
!     depending on whether the major part of the grid cell is wet or dry 
!     (slm = 1, slf > 0.5 =: land ; compare ioinitial).
...
!----------------------------------------------------------------------
  ...
  SUBROUTINE water_budget_corr
    ...
    !  computes global mean p-e correction

    !  Author:
    !  U. Schlese, MPI, July 2003

    USE mo_control,       ONLY: ngl, nlon
    USE mo_memory_g3b,    ONLY: apmebco, awfre, slm
    ...
    REAL(dp), POINTER  :: zpmeb(:,:)
    REAL(dp), POINTER  :: zawfre(:,:)
    ...
    IF (p_pe == p_io) THEN 
      DO jlat=1,ngl
        zpmebz(jlat)=SUM(zpmeb(1:nlon,jlat))*gl_budw(jlat)
      END DO
      ...
      zcorr=zpmeb_glob/((1.-slm_glob)*rhoh2o*couple_a2o_time)
