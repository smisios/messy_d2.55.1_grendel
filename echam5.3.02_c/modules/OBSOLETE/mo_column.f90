...
!!! #ifdef MESSY
  ! mz_lg_20031116+
  PUBLIC :: lat_1d_deg       ! global latitude of column in degrees (E/W, N/S)
  PUBLIC :: lon_1d_deg       ! global longitude of column in degrees (E/W, N/S)
  ! mz_lg_20031116-
!!! #endif
...
!!! #ifdef MESSY
  ! mz_lg_2003116+
  REAL             :: lat_1d_deg(mc)= -999.9 ! global latitude of column in degrees
  REAL             :: lon_1d_deg(mc)= -999.9 ! global longitude of column in degrees
  ! mz_lg_20031116-
!!! #endif
...
!!! #endif
    ...
    ENDIF
    ncol = 0 ! mz_pj_20060720
    IF (comode == '') RETURN 
#ifdef MESSY
    ! mz_lg_20031116+ added a routine that calculates the longitude/latitude
    !    array index, lon_1d, lat_1d whenever in the column namelist those
    !    are not defined but instead the longitude/latitude are given
    !    in degrees west (-)/east (+) and south (-)/north (+)

    CALL mz_column_loc(mc,lon_1d_deg, lat_1d_deg, lon_1d, lat_1d)
...
!------------------------------------------------------------------------------
  ...
  !
    ...
    END DO ! PE loop
#ifdef MESSY
    ! mz_lg_20030901+ a slight modification is introduced to avoid
    !     problems whenever the column latitude/longitude, LAT_1D or
    !     LON_1D are defined such that they are outside the grid of
    !     the selected model resolution. Experiences have shown that
    !     this problem results in a model crash which is not easy to
    !     trace back and therefore a CALL FINISH has been introduced!

    !mz_rs_20040326+
    !qqq check what to do with p_ful!!!
!     IF (comode=='traject' .AND. p_ful == -1) THEN
!       WRITE(nout,*)'LAT_1D or LON_1D in columnctl.nml > max. LAT/LON'  
!        CALL finish('mo_column.f90','Run terminated')
!     ENDIF
    !mz_rs_20040326-
    ! mz_lg_20030901-
#endif
...
#ifdef MESSY
  ! mz_rs_20040329+
  USE messy_main_tracer_mem_bi, ONLY: ntrac_gp, ti_gp, ON, I_FORCE_COL
  ! mz_rs_20040329-
#endif
  !
    ...
    IF (comode/=' ') THEN
      ...
      IF(for_trt>0) THEN
        ...
        DO i = 1, ntrac_gp
          ...
          ! mz_ak_20031127+
          ! CALL process_col3 (xtte(:,:,i,:), name ,for_trt==1)
          CALL process_col3 (xtte(:,:,i,:), name , &
            for_trt==1 .and. ti_gp(i)%tp%meta%cask_i(I_force_col)==ON )
          ! mz_ak_20031127-
        END DO
        ! mz_rs_20040329-
#endif
...
#ifdef MESSY
  ! mz_rs_20040329+
  USE messy_main_tracer_mem_bi, ONLY: ON, ntrac_gp, ti_gp, I_FORCE_COL
  ! mz_rs_20040329-
#endif
  !
    ...
    IF (comode/=' ') THEN
      ...
      IF (for_xt >0) THEN
        ...
        DO i = 1, ntrac_gp
          ...
          ! mz_ak_20031127+
          ! CALL process_col3 (xt(:,:,i,:), name ,for_xt==1 , cnudgxt)
          CALL process_col3 (xt(:,:,i,:), name , &
            for_xt==1 .and. ti_gp(i)%tp%meta%cask_i(I_force_col)==ON, cnudgxt )
          ! mz_ak_20031127-
        END DO
        ! mz_rs_20040329-
#endif
...
!------------------------------------------------------------------------------
#ifdef MESSY
  SUBROUTINE mz_column_loc(mc,lon_1d_deg,lat_1d_deg,lon_1d,lat_1d)

    ! Routine to calculate the longitude and latitude array indices,
    ! Whenever in the namelist the longitude and latitude are given
    ! in degrees east (+)/west (-) and north (+)/south(-). This facilitates
    ! the selection of the locations for which the column calculations
    ! have to be performed and avoids problems related to defining lat/lon
    ! array indices which are beyond the domain of the applied resolution
    !
    ! Author: Laurens Ganzeveld, MPICH, 15-11-2003
    ...
    INTEGER, INTENT(INOUT) :: lon_1d(mc), lat_1d(mc) ! long/lat array indices
    REAL,    INTENT(IN)    :: lon_1d_deg(mc), lat_1d_deg(mc) ! long/lat in degress

    ! LOCAL

    REAL            ::  sinlon, sinlat, coslon, coslat, dlat, dlon
    REAL, PARAMETER ::  latmax=90. ! see below for explanation
    INTEGER         ::  iilon, nc

    DO nc=1,mc

       ! only when the lat/lon are defined perform the calculations
       IF ((lon_1d(nc)/=-1 .AND. lat_1d(nc)/= -1) .OR.  &
           (lon_1d_deg(nc)<-999. .AND. lat_1d_deg(nc)< -999.)) GOTO 999

       ! mz_lg_20031116+ in case of longitude/latitude given outside the 
       !     domain a CALL FINISH is being used.

       IF (lon_1d_deg(nc) < -180. .OR. lon_1d_deg(nc) > 180. .OR.  & 
           lat_1d_deg(nc) < -90.  .OR. lat_1d_deg(nc) > 90.)       &
           CALL finish('mz_column_loc','Define -180<lon_1d_deg<180 or -90<lat_1d_deg<90')

       ! mz_lg_20031115+ calculation of the sin/cosine of the long/lat

       sinlon = SIN(lon_1d_deg(nc)*pi/180.)
       coslon = COS(lon_1d_deg(nc)*pi/180.)
       sinlat = SIN(lat_1d_deg(nc)*pi/180.)
       coslat = COS(lat_1d_deg(nc)*pi/180.)

       ! mz_lg_20031116+==========================================================
       ! The value latmax is the latitude at half the top grid of echam5. echam4 
       ! didn't have its top boundary exactly at 90 deg N/S (see the ECHAM 
       ! manual). The offset is resolution dependent (e.g., 87.091 deg for T30), 
       ! DLAT is the latitude in degrees. It has still to be checked if the 
       ! max latitude in echam5 is less then 90 degrees. For the time being it
       ! has been set at 90 degrees
       ! =========================================================================

       dlat=ASIN(sinlat)*180./pi
       lat_1d(nc)=(latmax-dlat)/(2*latmax/(ngl-1))+1

       dlon=ASIN(sinlon)*180./pi
       IF (coslon < 0. .AND. sinlon > 0.) THEN
          dlon=180.-dlon
       ELSEIF (coslon < 0. .AND. sinlon < 0.) THEN
          dlon=-180.-dlon
       ENDIF

       ! the echam longitude starts at 0 degrees.!!

       iilon=(dlon+180.)/(2*180./nlon)+1
       IF (iilon <= nlon/2) lon_1d(nc)=iilon+nlon/2
       IF (iilon >  nlon/2) lon_1d(nc)=iilon-nlon/2

       WRITE(*,'(1a,2f6.1)')'mz_column_loc, lon/lat in degrees:',&
             lon_1d_deg(nc), lat_1d_deg(nc)
       WRITE(*,'(1a,2i4)')'Resembling the lon/lat array indices:',&
             lon_1d(nc), lat_1d(nc)
