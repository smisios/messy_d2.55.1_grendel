MODULE mo_time_control
  ...
  USE mo_control,         ONLY: lcouple, lhd, nn, lnwp, nlev, ldiagamip &
                              , lamip ! mz_pj_20070813
#else
  USE mo_control,         ONLY: nn, lnwp, nlev &
                              , lamip ! mz_pj_20070813
#endif
...
#ifdef MESSY
  USE messy_main_timer_bi, ONLY: BM_time, timer_get_rerun_event
  USE messy_main_timer,    ONLY: L_TRIGGER_RESTART                &
                               , messy_time_days => time_days     &
                               , timer_set_date, timer_get_date   &
                               , timer_get_no_cycles, timer_get_labort &
                               , l_rerun ! um_ak_20140326
  USE messy_main_timer_manager, ONLY: timer_rewind_manager
#endif
  ...
  INTEGER               :: timelabel_type = FLT_YMD
  ...
  LOGICAL               ::  l_arcrerun = .FALSE.
  TYPE(time_event),SAVE :: ev_arcrerun
  TYPE(io_time_event),SAVE ::    arcrerun          &! rerun-files saving interval
       = &
       io_time_event(1,TIME_INC_MONTHS,TRIG_NONE,0)
  ...
  ! mz_pj_20070813+
  ! *** AMIP2 update events
  LOGICAL :: l_readamip = .FALSE.
  TYPE(time_event),SAVE :: ev_readamip
  TYPE(io_time_event),SAVE ::    readamip         &! event at first step of a year
       = &
  ! mz_tb_20070814+
       io_time_event(1,TIME_INC_YEARS,TRIG_FIRST,0)
  ! mz_tb_20070814-
  ! mz_pj_20070813-
  ...
  LOGICAL               ::  l_diagrad = .FALSE.
  TYPE(time_event),SAVE :: ev_diagrad
  TYPE(io_time_event),SAVE ::    diagrad           &! freq. for radiation diag.
       = &
       io_time_event(5,TIME_INC_DAYS,TRIG_NONE,0)
#endif
  ! ----------------------------------------------------------------------------
  ...
  PUBLIC  :: construct_events ! () prepare memory for all ECHAM events
  ...
  PRIVATE :: dtime               ! get delta time from echam time manager
  ...
  ! mz_jb_20031211+
  INTRINSIC abs, int, max, min, mod, nint, present, real, sum, trim
  ! mz_jb_20031211-
...
!
  ...
  SUBROUTINE p_bcast_event (event, p_source, comm)  !***************************
    ...
    CALL p_bcast(event%counter,    p_source, comm)
    CALL p_bcast(event%unit,       p_source, comm)
    CALL p_bcast(event%adjustment, p_source, comm)
    CALL p_bcast(event%offset,     p_source, comm)

  END SUBROUTINE p_bcast_event

  ! ----------------------------------------------------------------------------
  !+
  SUBROUTINE construct_events               !***** called in CONTROL ***********

    ! initialize all ECHAM event structures
    !-
    INTEGER :: i
    ...
    IF (ldebugev) THEN
...
#ifdef OBSOLETE
      WRITE(m_text,*) 'ARCRERUN=   ',arcrerun,  ','; CALL message('',m_text)
      ...
      WRITE(m_text,*) 'LABORT=     ',labort,    ','; CALL message('',m_text)
#ifdef OBSOLETE
      ...
      WRITE(m_text,*) 'DIAGRAD=    ',diagrad,   ','; CALL message('',m_text)
#endif
  ...
  END SUBROUTINE construct_events
...
#ifdef MESSY
    USE messy_main_timer,    ONLY:  timer_set_delta_time &
                                 ,  timer_get_delta_time
#endif
...
!!$    TYPE(messy_time_days) :: messy_date_day
    REAL(dp) :: messy_time
#endif
!!    INTEGER           :: days, seconds
...
#ifdef MESSY
    CALL timer_get_delta_time(status, messy_time)
    delta_time = messy_time
#endif
...
#ifdef MESSY
    CALL timer_set_delta_time(status, delta_time)
#endif
...
#ifdef MESSY
    INTEGER :: yr, mo, dy, hr, mi, se, status
    TYPE(messy_time_days) :: messy_new_date
#endif

    CALL manager_init(echam_time, .FALSE.)   !  unlock time manager
    CALL rewind_manager(echam_time, new_date, .TRUE.) 
#ifdef MESSY
    CALL get_date_components(new_date,yr,mo,dy,hr,mi,se)
    CALL timer_set_date(status, messy_new_date,yr,mo,dy,hr,mi,se)
    CALL timer_rewind_manager(BM_time, messy_new_date, .TRUE.) 
#endif
...
#else
    END IF
    CALL timer_get_date(status,'stop',dt_stop(1), dt_stop(2), dt_stop(3), &
         dt_stop(4), dt_stop(5), dt_stop(6))
    CALL TC_set(&
         dt_stop(1), dt_stop(2), dt_stop(3), &
         dt_stop(4), dt_stop(5), dt_stop(6), date_nat)
    CALL TC_convert(date_nat, stop_date)
...
#ifdef MESSY
    CALL timer_get_no_cycles(no_cycles)
    CALL timer_get_labort(labort)
#endif
  ...
  SUBROUTINE init_events     !********* called in INITIALIZE *******************
...
#ifdef MESSY
    INTEGER                      :: status    
    CHARACTER(LEN=STRLEN_SHORT)  :: adjust_ev
    REAL(dp)                     :: Roffset    ! ub_ak_20190110
#endif

    ! *** check weather prediction mode ----------------------------------------

    IF (lnwp) THEN
      IF (trigfiles%unit /= TIME_INC_HOURS) THEN
        CALL message('','Only HOURS as interval in LNWP mode are valid.')
        trigfiles = io_time_event (6,TIME_INC_HOURS,TRIG_FIRST,0)
      END IF
      putdata = trigfiles
    END IF
    ...
    CASE default
      timelabel_type = FLT_YMD
      CALL message('','Use default time label in file names (YYYYMM_DD).')
    END SELECT

    ! *** History and postprocessing events ------------------------------------
#ifdef MESSY
    CALL timer_get_rerun_event(status, putrerun%counter, putrerun%unit &
         ! ub_ak_20190110+
!         , putrerun%adjustment, putrerun%offset, adjust_ev)
         , putrerun%adjustment, Roffset, adjust_ev)
    putrerun%offset = NINT(Roffset)
         ! ub_ak_20190110-
#endif
...
#ifndef MESSY
!mz_ap_20070920+
! only for OASIS coupler, not for MESSy
    IF ( lcouple .AND. (&
...
!mz_ap_20070920-
    ...
    CALL echam_ev_init(ev_arcrerun, arcrerun,'archive rerun files',EV_TLEV_NEXT)

    ! *** radiation ------------------------------------------------------------
    CALL echam_ev_init(ev_trigrad,trigrad,'radiation computation', EV_TLEV_PRES)
    CALL echam_ev_init(ev_diagrad,diagrad,'radiation diagnostics', EV_TLEV_PRES)
#endif

    ! mz_pj_20070813+
    ! *** AMIP update
    IF (lamip) &
         CALL echam_ev_init(ev_readamip,readamip, 'AMIP2 update',EV_TLEV_PRES)
    ! mz_pj_20070813-
  ...
  END SUBROUTINE init_events
    ...
    SUBROUTINE echam_ev_init (event, io_ev, ev_name, eval_date)  !**************
      ...
      ! mz_jb_20031211+: if present(eval_date) added
      if_present_eval_date: IF (present(eval_date)) THEN
      ...
      END IF
      ELSE if_present_eval_date
        CALL finish ('echam_ev_init in mo_time_control.f90', &
          'Optional dummy argument eval_date not present.')
      END IF if_present_eval_date
      ! mz_jb_20031211-
    ...
    SUBROUTINE convert_steps2inter (steps, dt, unit, count)  !****************
      ...
      isdt = NINT(dt*real(isteps,dp))   ! mz_jb_20031211: explicit typecast
      imdt = isdt /        60
      ihdt = isdt /    (60*60)
      iddt = isdt / (24*60*60)
...
!
  ...
  SUBROUTINE time_set    !********** called in STEPON **************************
    ...
    USE mo_param_switches,  ONLY: lrad
#endif
    INTEGER         :: istep, isj, iradhlen, iput
...
!ik_bugfix_R1.02a
    l_trigfiles = event_state(ev_trigfiles, next_date) .OR. lfirst_cycle
#ifndef MESSY
    l_putrerun  = event_state(ev_putrerun,  next_date)
#else
! um_ak_20140326+
!!$ l_putrerun  = event_state(ev_putrerun,  next_date) .OR. L_TRIGGER_RESTART
    l_putrerun  = l_rerun
! um_ak_20140326-
#endif
    ...
    l_arcrerun  = event_state(ev_arcrerun,  next_date)
    l_diagamip  = event_state(ev_diagamip,  next_date)
#endif
    ! mz_pj_20070813+
    IF (lamip) &
         l_readamip  = event_state(ev_readamip,  next_date)
    ! mz_pj_20070813-
    ...
    l_diagrad = event_state(ev_diagrad, current_date) .OR. lstart .OR. lresume
...
!      l2nd_day = next_date < my_day     ! new version
    ...
    IF (l_putrerun) THEN
      ...
      END IF
#ifdef MESSY
        lbreak = lbreak .OR. L_TRIGGER_RESTART
#endif
    ELSE
      lbreak = .FALSE.
    ...
    IF (ldebugev) THEN
      ...
      IF (l_diagrad)   CALL echam_ev_print(ev_diagrad)
...
#ifdef OBSOLETE
      IF (l_arcrerun)  CALL echam_ev_print(ev_arcrerun)
      DO isj = 1,nsub
      ...
      ! mz_pj_20070813+
      IF (lamip) THEN
         IF (l_readamip) CALL echam_ev_print(ev_readamip)
      END IF
      ! mz_pj_20070813-
#ifdef OBSOLETE
    ...
    IF (l_trigrad) THEN
      ...
      iradhlen = INT(0.5_dp*event_state(ev_trigrad,.TRUE.))
  ...
  CONTAINS
    ...
    SUBROUTINE time_weights  !**************************************************
      ...
      zdayl    = REAL(NDAYLEN,dp)              ! mz_jb_20031211: dp added
      zmohlfm1 = REAL(imlenm1,dp)*zdayl*0.5_dp ! mz_jb_20031211: explicit ...
      zmohlf   = REAL(imlen,dp)  *zdayl*0.5_dp ! mz_jb_20031211: ... typecast
      zmohlfp1 = REAL(imlenp1,dp)*zdayl*0.5_dp ! mz_jb_20031211: ...
...
!
  ...
  REAL(dp) FUNCTION set_delta_time (truncation,nlevel)
    ...
    CHARACTER(len=STR_LEN_B) :: m_text

    IF (nlevel == 19 .OR. nlevel == 11) THEN
      IF      (truncation == 21) THEN; set_delta_time = 2400.0_dp
#ifdef MESSY
      ELSE IF (truncation == 10) THEN; set_delta_time = 2400.0_dp
#endif
      ELSE IF (truncation == 31) THEN; set_delta_time = 2400.0_dp
      ELSE IF (truncation == 42) THEN; set_delta_time = 1800.0_dp
      ELSE IF (truncation == 63) THEN; set_delta_time = 1200.0_dp
      ELSE IF (truncation == 85) THEN; set_delta_time =  900.0_dp
      ELSE IF (truncation ==106) THEN; set_delta_time =  720.0_dp
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE IF (nlevel == 31) THEN
      IF      (truncation == 31) THEN; set_delta_time = 1800.0_dp
      ELSE IF (truncation == 42) THEN; set_delta_time = 1200.0_dp
      ELSE IF (truncation == 63) THEN; set_delta_time =  720.0_dp
      ELSE IF (truncation == 85) THEN; set_delta_time =  480.0_dp
      ELSE IF (truncation ==106) THEN; set_delta_time =  360.0_dp
      ELSE IF (truncation ==159) THEN; set_delta_time =  240.0_dp
      ELSE IF (truncation ==213) THEN; set_delta_time =  180.0_dp
      ELSE IF (truncation ==255) THEN; set_delta_time =  150.0_dp
      ELSE IF (truncation ==319) THEN; set_delta_time =  150.0_dp
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
      ! op_pj_20090922+
    ELSE IF (nlevel == 41) THEN
      IF      (truncation == 31) THEN; set_delta_time =  900.0_dp
      ELSE IF (truncation == 42) THEN; set_delta_time =  900.0_dp
      ELSE IF (truncation == 63) THEN; set_delta_time =  600.0_dp
      ELSE IF (truncation == 85) THEN; set_delta_time =  450.0_dp
      ELSE IF (truncation ==106) THEN; set_delta_time =  360.0_dp
      ELSE IF (truncation ==159) THEN; set_delta_time =  240.0_dp
      ELSE IF (truncation ==213) THEN; set_delta_time =  180.0_dp
      ELSE IF (truncation ==255) THEN; set_delta_time =  150.0_dp
      ELSE IF (truncation ==319) THEN; set_delta_time =  120.0_dp
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
      ! op_pj_20090922-
    ...
    ELSE IF (nlevel == 47) THEN  
      ! Middle atmosphere version
      IF      (truncation == 42) THEN; set_delta_time =  900.0_dp
      ELSE IF (truncation == 63) THEN; set_delta_time =  600.0_dp
      ELSE IF (truncation == 85) THEN; set_delta_time =  450.0_dp
      ELSE IF (truncation ==106) THEN; set_delta_time =  360.0_dp
      ELSE IF (truncation ==159) THEN; set_delta_time =  240.0_dp
    ...
    ELSE IF (nlevel == 49) THEN  
      ...
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE IF (nlevel == 60) THEN
      IF      (truncation ==106) THEN; set_delta_time =  360.0_dp
      ELSE IF (truncation ==159) THEN; set_delta_time =  240.0_dp
      ELSE IF (truncation ==213) THEN; set_delta_time =  180.0_dp
      ELSE IF (truncation ==255) THEN; set_delta_time =  150.0_dp
      ELSE IF (truncation ==319) THEN; set_delta_time =  120.0_dp
      ...
      END IF
! ka_sv_20170404+
    ELSE IF (nlevel == 74) THEN
      IF      (truncation ==42) THEN; set_delta_time =  180.0_dp      
      ELSE IF (truncation ==106) THEN; set_delta_time =  60.0_dp
      ELSE IF (truncation ==159) THEN; set_delta_time =  40.0_dp
      ELSE IF (truncation ==213) THEN; set_delta_time =  30.0_dp
      ELSE IF (truncation ==255) THEN; set_delta_time =  20.0_dp
      ELSE IF (truncation ==319) THEN; set_delta_time =  15.0_dp
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE IF (nlevel == 94) THEN
      IF      (truncation ==42) THEN; set_delta_time =  180.0_dp
      ELSE IF (truncation ==106) THEN; set_delta_time =  60.0_dp
      ELSE IF (truncation ==159) THEN; set_delta_time =  40.0_dp
      ELSE IF (truncation ==213) THEN; set_delta_time =  30.0_dp
...
! ka_sv_20180112-
    ELSE IF (nlevel == 87) THEN  
      ...
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE IF (nlevel == 90) THEN
      ! Middle atmosphere version
      IF (truncation == 42) THEN; set_delta_time =  900.0_dp
! op_dc_20141015+
      ELSE IF (truncation == 31) THEN; set_delta_time =  900.0_dp
! op_dc_20141015-
      ELSE IF (truncation == 63) THEN; set_delta_time =  600.0_dp
      ELSE IF (truncation == 85) THEN; set_delta_time =  450.0_dp
      ELSE IF (truncation ==106) THEN; set_delta_time =  360.0_dp
    ...
    ELSE IF (nlevel == 95) THEN  
      ...
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE IF (nlevel == 191) THEN
      ! Middle atmosphere version
      IF      (truncation ==106) THEN; set_delta_time =  240.0_dp
      ELSE IF (truncation ==159) THEN; set_delta_time =  180.0_dp
      ELSE IF (truncation ==213) THEN; set_delta_time =  144.0_dp
      ELSE IF (truncation ==255) THEN; set_delta_time =  120.0_dp
      ELSE IF (truncation ==319) THEN; set_delta_time =  100.0_dp
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE
...
! mz_jb_20031211+: explicit typecast 
    isteps = INT(REAL(iday*3600+isec,dp)/delta_time) 
! mz_jb_20031211-
  ...
  FUNCTION str_date_native (no, date) RESULT (label)  !*************************
    ...
    my_no = MAX(MIN(no,FLT_YMDHMS),FLT_YMD)  ! as minimum return year+month+day

    CALL TC_get(date,yr,mo,dy,hr,mn,se)

    SELECT CASE(my_no)
    CASE(FLT_YM)
      WRITE(label(1: 6),'(i4.4,i2.2)')         yr,mo
    CASE(FLT_YMD)
      WRITE(label(1: 9),'(i4.4,i2.2,a1,i2.2)') yr,mo,'.',dy
    CASE(FLT_YMDH)
      WRITE(label(1:12),'(i4.4,i2.2,2(a1,i2.2))') &
                                               yr,mo,'.',dy,'_',hr
    CASE(FLT_YMDHM)
      WRITE(label(1:15),'(i4.4,i2.2,3(a1,i2.2))') &
                                               yr,mo,'.',dy,'_',hr,':',mn
    CASE(FLT_YMDHMS)
      WRITE(label(1:18),'(i4.4,i2.2,4(a1,i2.2))') &
                                               yr,mo,'.',dy,'_',hr,':',mn,':',se
    CASE default
      WRITE(label(1:9),'(i4.4,i2.2,a1,i2.2)')  yr,mo,'.',dy
    END SELECT
