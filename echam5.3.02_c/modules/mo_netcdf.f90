...
!------------------------------------------------------------------------------
  ...
  INTEGER       ,SAVE         :: IO_ndim_ids  ! mz_pj_20030728 (added SAVE)
  TYPE (IO_dim) ,TARGET ,SAVE :: IO_dim_ids (max_dim_ids)

! mz_jb_20030805+: SAVE attributes added
  INTEGER, SAVE :: BELOWSUR   != 111
  INTEGER, SAVE :: SURFACE    !=   1
  INTEGER, SAVE :: ABOVESUR2  != 105
  INTEGER, SAVE :: ABOVESUR10 != 105
  INTEGER, SAVE :: HYBRID     != 109 ?
  INTEGER, SAVE :: HYBRID_H   != 110 ?
! mz_jb_20030805-
...
!------------------------------------------------------------------------------
  SUBROUTINE IO_info_construct(fileinfo)
    USE mo_filename,    ONLY: NETCDF    ! mz_pj_20050531
    ...
    fileinfo%file_name        = '?'
    fileinfo%title            = '?'
    fileinfo%format           = NETCDF ! mz_pj_20050531
...
!------------------------------------------------------------------------------
  ...
  INTEGER           ,INTENT(out) ,OPTIONAL :: indx      ! index in IO_dim_ids 
    ...
    if (idx == 0) then
      ...
      idx = IO_ndim_ids
      ! dim => IO_dim_ids(idx)        ! mz_pj_20031001
    endif
    dim => IO_dim_ids(idx)           ! mz_pj_20031001
    !---------------------------------------------------------
