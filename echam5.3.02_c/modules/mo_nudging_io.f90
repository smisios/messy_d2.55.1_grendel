...
!BOP
  ...
  ! Aiko Voigt, MPI Hamburg, March 2011, 
  !   implement netcdf-based reading of nudging data
...
!EOP
  ...
  ! op_pj_20190930+
  ! Note: To avoid &ndgctl namelist incompatibilites, this is always
  !       defined, i.e. also #ifndef MESSY ... (Yet, without MESSy
  !       this variable is meaningless.)
  INTEGER, PUBLIC, SAVE :: ndginc = 1 ! increment along time axis in netCDF file
#ifdef MESSY
  LOGICAL, SAVE         :: lclassic
#endif
  ! op_pj_20190930-
  ...
  INTEGER, PUBLIC :: &
       ndgstep0  = 0,      ndgstep1  = 0,      &
       ndgstep2  = 0,      ndgstep3  = 0
  LOGICAL, PUBLIC :: &
       lndgstep0 =.FALSE., lndgstep1 =.FALSE., &
       lndgstep2 =.FALSE., lndgstep3 =.FALSE.
...
!EOP
    ...
    IF (lnudgdbx) THEN
       WRITE(mess,'(a32,i0,a5,i0)') 'Interpolation between ... steps ' &
            , ndgstep1,' ... ',ndgstep2
...
!EOP
    ...
    EXTERNAL pbread, util_cray2ieee, util_i8toi4
       ...
       IF (lrd_check) THEN
          ...
          IF (linp_tem .OR. linp_lnp) THEN
             ...
             ! NB: If lnudgpat is set, the program runs NOT in parallel!
                ...
                IF (kret/=ino_t_lev*WORD_LEN) &
                   CALL finish('ReadOneBlock','alpha data fault TEM/LNP')
#ifdef LITTLE_ENDIAN
                CALL swap64(phbuf,ino_t_lev)
#endif
                CALL util_cray2ieee(phbuf,zhbuf,ino_t_lev)
                phbuf(1:ino_t_lev) = zhbuf(1:ino_t_lev)
                a_pat_tem(ilev_t_min:ilev_t_min+ino_t_lev-1,4) = &
                   phbuf(1:ino_t_lev)
                IF (linp_lnp) THEN
          ...
          WRITE(mess,'(a48,i0)') &
             'READ NEW nudging DATA, corresponding step is : ',ndgstep3
          CALL message('ReadOneBlock',mess)
    ...
    IF (ierr /= 0) CALL finish('ReadOneBlock', &
         'Error while deallocating zhbuf.')
    DEALLOCATE (phbuf, stat=ierr)
    IF (ierr /= 0) CALL finish('ReadOneBlock', &
         'Error while deallocating phbuf.')

    CASE(2)                  !! read netcdf data

#ifdef MESSY
       ! classical increment of 1
       lclassic = (ndginc == 1)
#endif
       ...
       IF (.NOT. files_open) THEN       
          ...
          IF (.NOT. lclassic) THEN
             ! reduce counter of what will be counted twice
             rndgcount = rndgcount - rndtsc
          END IF
#endif
       END IF
...
#ifdef MESSY
                IF (.NOT. lclassic) THEN
                   rndgcount = rndgcount + 1.0_dp
                   IF ( (NINT(rndgcount) /= 1) .AND. &
                        (MOD(NINT(rndgcount)-1,ndginc) /= 0) ) THEN
                      DEALLOCATE (timevals)
                      CYCLE
                   END IF
                END IF
...
!!$             ihead_nc(2) = INT((timevals(ndtsc)-ihead_nc(1))*24._dp)       ! ihead_nc(2) is HH 
                ihead_nc(2) = NINT((timevals(ndtsc)-ihead_nc(1))*24._dp)      ! ihead_nc(2) is HH 
! op_pj_20180412+
             END IF
             DEALLOCATE (timevals)
          END IF
          ...
          IF (linp_lnp) THEN
             IF (p_parallel_io) THEN
                CALL IO_INQ_VARID (ndgfile%file_id, 'lnsp', nvarid)
                COUNT(:) = (/ 2, nsp,1,1 /)
                start(:) = (/ 1, 1, 1,ndtsc /)
...
#ifdef MESSY
          ELSE
             WRITE(mess,'(a48,i0,1x,a20,i0,1x,i0)') 'READ NEW nudging DATA, corresponding step is : ',ndgstep3, '; nudging steps are ', NINT(rndgcount), ndtsc
          ENDIF
#endif
          CALL message('ReadOneBlock',mess)
          CALL write_date(ndg_date3,' Corresponding date: ')
  ...
  CONTAINS
    ...
    IF (p_parallel_io) THEN
      DEALLOCATE(feld_global, stat=ierr)
      IF (ierr /= 0) CALL finish('ReadOneBlock', &
           'Error while deallocating feld_global.')
    END IF
...
!BOP
  ...
  SUBROUTINE OpenOneBlock
    ...
    USE mo_mpi,               ONLY: p_parallel_io, &
                                    p_bcast, p_parallel, p_io ! op_pj_20180329
...
!EOP
    ...
    IF (files_open) THEN
       CALL CloseBlock
       CALL message('','Nudging data blocks were closed in open.')
    ...
    CALL message('OpenOneBlock',mess)
      ...
      CASE(2)                  ! read netcdf-data
       ...
       IF (p_parallel_io) THEN
           ...
           lreopen = (TRIM(cfile) == TRIM(old_file)) ! op_pj_20180329
