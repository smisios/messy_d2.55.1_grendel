...
SUBROUTINE tf2
...
#else
  ! mz_rs_20040329+
  USE messy_main_tracer_mem_bi, ONLY: ON, ntrac_gp, ti_gp, ntrac_lg, ti_lg &
                                    , I_integrate
  ! mz_rs_20040329-
#endif
  ...
  INTRINSIC dot_product, epsilon   ! mz_jb_20040830
    ...
    IF (jrow == 1) THEN
      DO jt = 1, ntrac_lg
        IF (ti_lg(jt)%tp%meta%cask_i(I_integrate) == ON) THEN
          xtm1_a(:,:,jt,:) = xtm1_a(:,:,jt,:) + zeps*xt_a(:,:,jt,:)
        END IF
      END DO
    END IF
    ! mz_pj_20030930-
...
!CSD$ END PARALLEL DO
  ...
  IF (ltdiag) THEN
   DO jrow = 1, ngpblks
      IF (.NOT. lset_fh1) THEN
        lset_fh1 = DOT_PRODUCT(ptfh2(1:nproma,jrow),ptfh2(1:nproma,jrow)) &
                  > EPSILON(1.0_dp)
      END IF
      IF (lset_fh1) THEN
        ptfh1(1:nproma,:,1,jrow) = vom1  (1:nproma,:,jrow) &
                                  - ptfh1(1:nproma,:,1,jrow)
        ptfh1(1:nproma,:,2,jrow) = dm1   (1:nproma,:,jrow) &
                                  - ptfh1(1:nproma,:,2,jrow)
        ptfh1(1:nproma,:,3,jrow) = tm1   (1:nproma,:,jrow) &
                                  - ptfh1(1:nproma,:,3,jrow)
        ptfh2(1:nproma    ,jrow) = alpsm1(1:nproma,jrow)   &
                                  - ptfh2(1:nproma    ,jrow)
      ELSE
        ptfh1(1:nproma,:,:,jrow) = 0.0_dp
        ptfh2(:    ,jrow) = 0.0_dp
      ENDIF
      ! accumulate time filter part
      pdiga(1:nproma,:,21:23,jrow) = pdiga(1:nproma,:,21:23,jrow) &
                                    +ptfh1(1:nproma,:,  1:3,jrow)
      pdsga(1:nproma,1,    2,jrow) = pdsga(1:nproma,1,    2,jrow) &
                                    +ptfh2(1:nproma        ,jrow)
     END DO
   ENDIF
   ! mz_jb_20040829-
