...
!OCL NOVREC, NOALIAS
    DO jm = 1, nmp1
      ixr(2*jm-1+(jlev-1)*2*nmp1)  = jlev+(jm-1)*2*(nlev+0.5)
      ixr(2*jm+(jlev-1)*2*nmp1)    = jlev+nlev+(jm-1)*2*(nlev+0.5)
      ixrp(2*jm-1+(jlev-1)*2*nmp1) = jlev+(jm-1)*2*(nlevp1+0.5)
      ixrp(2*jm+(jlev-1)*2*nmp1)   = jlev+nlevp1+(jm-1)*2*(nlevp1+0.5)
    ENDDO
  ENDDO
#endif
