SUBROUTINE initemp(krow)
  ...
  !
  ! *initemp* is called from *gpc* at the first timestep only
  !
  ! 
  ...
  USE mo_memory_g3b,    ONLY: slf, tsoil, tsw, tsi, seaice, siced      &
                            , alake, tsl, tslm, tslm1
...
!
  DO jl = 1,nproma
    ...
    ELSE
      ...
      tsi(jl,jrow)=MIN(ztsl(jl),tmelt)
    END IF
    IF (alake(jl,jrow).EQ.0.0_dp .AND. slf(jl,jrow).LE.0.5_dp) THEN          !  ocean
