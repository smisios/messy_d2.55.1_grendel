SUBROUTINE inictl
  ...
  USE mo_control,       ONLY: lamip, lcouple, lipcc, ldebug, lmidatm, lmlo,   &
                              ...
                              njin, njout, subjob_cmd,                        &
                              stdout_redir, stderr_redir, lsolc, lreff        &
                              control_lyaxt_transposition => lyaxt_transposition
#else
  USE mo_control,       ONLY: lamip, lcouple, ldebug, lmidatm,                &
                              lnmi, lnudge, lnwp, ltdiag,                     &
                              lprint_m0, ldebugio, ldebugmem,                 &
                              ldebughd, loldrerun, ltimer, ltctest,           &
                              maxrow, ngl, nlev, nproca, nprocb, nproma,      &
                              numfl1, numfl2,nflu, na2stre,                   &
                              nisp, nigp, ndiahdf, nist,  nice, na2stat,      &
                              nhf1, nhg1, nhg2, nhg3, nhgl1, nfl1, nfl2,      &
                              njin, njout,                                    &
  ...
  USE mo_time_control,  ONLY: delta_time, no_days, no_steps, no_cycles,       &
                              ...
                              puthd, gethd,                                   &
                              trigjob, nsub, lsub, subflag, arcrerun,         &
  ...
  USE mo_time_control,  ONLY: delta_time, no_days, no_steps, no_cycles,       &
                              ...
                              labort,                                         &
                              putdata, putrerun,                              &
                              p_bcast_event, trigfiles,                       &
                              lresume, ldebugev, lfirst_cycle
#endif
  USE mo_advection,     ONLY: iadvec, nmwicfi ! mz_pj_20070411
  ...
  maxrow    = ngl
  lyaxt_transposition = control_lyaxt_transposition
...
!-- 2. Read namelist runctl
  ...
  IF (p_parallel) THEN
...
#ifdef HAVE_YAXT
     control_lyaxt_transposition = lyaxt_transposition
     CALL p_bcast (control_lyaxt_transposition, p_io)
#endif
     ...
     CALL p_bcast_event(arcrerun, p_io)
