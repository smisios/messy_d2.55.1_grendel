SUBROUTINE rad_int(                   &
  ...
  USE mo_constants ,ONLY: api, rd, g,   & ! some constants
                          ...
                          amw, amd        ! 
  USE mo_control   ,ONLY: nn, nlev, lmidatm, lcouple, lso4, lipcc
...
! mz_jb_20031216+: 
! - kind parameter dp used for declarations and constants
! - line lengths reduced, 
! - unused variables eliminated: 
!   * ztclear (clear sky fraction of total column), 
!   * zzdp (pressure thickness in Pa), 
!   * klev (number of levels), kaer (number of Tanre aerosols), 
!   * ztave (full level temperature [K]), 
!   * zpmb (half level pressure in hPa)
! - INTRINSIC statement
! - EXTERNAL statement
! mz_jb_20031216-
  ...
  INTRINSIC  epsilon, max, min, size, exp, log10
  EXTERNAL sw, rrtm_rrtm_140gp
...
!
  ...
  ! =================
     ...
     ! cloud cover
     ztclear(1:kproma)=1.0_dp-ptclc(1:kproma)   ! mz_jb_20031216
     DO jl=1,kproma
     ...
     CASE (1,2,3,4)
       DO jk = 1, klev
         ...
         DO jl = 1, kproma
           ...
           ztauaerl(jl,jk, 1) = ztauaer(jl,1)
           ztauaerl(jl,jk, 2) = ztauaer(jl,5)
           ztauaerl(jl,jk, 3) = ztauaer(jl,2)
           ztauaerl(jl,jk, 4) = ztauaer(jl,2)
           ztauaerl(jl,jk, 5) = ztauaer(jl,2)
           ztauaerl(jl,jk, 6) = ztauaer(jl,3)
           ztauaerl(jl,jk, 7) = ztauaer(jl,4)
           ztauaerl(jl,jk, 8) = ztauaer(jl,3)
           ztauaerl(jl,jk, 9) = ztauaer(jl,1)
           ztauaerl(jl,jk,10) = ztauaer(jl,1)
           ztauaerl(jl,jk,11) = ztauaer(jl,1)
           ztauaerl(jl,jk,12) = ztauaer(jl,1)
           ztauaerl(jl,jk,13) = ztauaer(jl,1)
           ztauaerl(jl,jk,14) = ztauaer(jl,1)
           ztauaerl(jl,jk,15) = ztauaer(jl,1)
           ztauaerl(jl,jk,16) = ztauaer(jl,1)
         END DO
       END DO
     CASE default
  ...
  ! =================
     ...
     ENDIF
              ...
              ztau(jl,2,jk)=zto2l*zinhoml(jl)+zto2i*zinhomi
              ztau(jl,3,jk)=zto3l*zinhoml(jl)+zto3i*zinhomi
              ztau(jl,4,jk)=zto4l*zinhoml(jl)+zto4i*zinhomi

              zomega(jl,1,jk)=zomgmx1
              zomega(jl,2,jk)=zomgmx2
              zomega(jl,3,jk)=zomgmx3
              zomega(jl,4,jk)=zomgmx4
     ...
     CALL SW ( kidia  ,kfdia   ,kbdim,   klev                          &
             , kaer   ,knewaer ,iaerh                                  &
             , zsct   ,zcardi  ,zpsol,   zalbd,    zalbp, zwv,    zqs  &
