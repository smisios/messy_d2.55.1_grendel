SUBROUTINE setrad
  ...
  IF (lrad) THEN
     ...
     CASE(1)
       IF (p_parallel_io) &
            WRITE(nerr,*) 'ico2 = 1 --> transported CO2 is not yet implemented'
       CALL finish('setrad','Run terminated')
     CASE(2)
     ! in case of coupled run overwrite default or namelist values
       IF(lcouple) THEN
