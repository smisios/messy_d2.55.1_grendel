SUBROUTINE subjob(kjob)
  ...
  ! Description:
  !
  ! Transfer information from model into a shell-script and submit a job.
  !
  ! Authors:
  !
  ...
  USE mo_time_control,    ONLY: previous_date, current_date, next_date,        &
                                ...
                                lstop, get_date_components, lsub
...
20 CONTINUE
  ...
  ! submit the filtered subjobs

  IF (lsub) THEN

     IF (lstop .AND. labort) THEN
      CALL message('','No script submitted at the end of the run.')

     ELSE
      WRITE(sub_cmd,'(a,a,a)') TRIM(subjob_cmd),' ',TRIM(yoofil)
      istat = util_system(sub_cmd)

      IF (istat==0) THEN
        WRITE (message_text,*) ' Job ', TRIM(yoofil), ' submitted.'
      ELSE
        WRITE (message_text,*) ' Error submitting ', TRIM(yoofil), &
  ...
  ELSE
     WRITE (message_text,*) ' Job ', TRIM(yoofil), ' filtered.'
