...
USE mo_specfun, ONLY: merge
  ...
  REAL(dp):: rdcpd, zsecfrw, zdthdp, zcor, zbetai0, zbetai1, zrhc      &
           , zsat, zqr
!   Input arrays
  INTEGER::  ktype(kbdim)
  REAL(dp) ::    pfrw(kbdim)
...
!
    ...
    ELSE
      zbetai0=(ztt-ixidx)*tbetai0(iqidx_l1(jl),ixidx+1)                &
           +(ixidx+1.-ztt)*tbetai0(iqidx_l1(jl),ixidx)
      zbetai1=(ztt-ixidx)*tbetai1(iqidx_l1(jl),ixidx+1)                &
           +(ixidx+1.-ztt)*tbetai1(iqidx_l1(jl),ixidx)
    ENDIF
        ...
        ELSE
          ...
          ELSE
            zbetai0=(ztt-ixidx)*tbetai0(iqidx(jl,jk),ixidx+1)          &
                 +(ixidx+1.-ztt)*tbetai0(iqidx(jl,jk),ixidx)
          ENDIF
