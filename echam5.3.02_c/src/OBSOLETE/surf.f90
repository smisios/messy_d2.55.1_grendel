...
USE messy_main_tendency_bi,   ONLY: mtend_get_handle,       &
 ...
 REAL(dp),DIMENSION(kbdim,klev)       :: lo_tte,lo_tm1
...
#ifdef MESSYTENDENCY
  IF(lfirst) THEN
     my_handle=mtend_get_handle('surf')
     lfirst=.FALSE.
  ENDIF
#endif
  zdtime = delta_time
...
#else
            zexpt=MAX(0._dp,lo_tm1(jl,klev)+3._dp-tmelt)*zdtime/zc2
#endif
...
#else
            lo_tte(jl,klev) = - (zsncmelt(jl)*zsncfac*zlfdcp(jl)/zdp(jl))
#endif
!
