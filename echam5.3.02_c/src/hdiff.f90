SUBROUTINE hdiff
  ...
  USE mo_control,       ONLY: lmidatm, ltdiag, nk, nkp1, nlev, nlevp1, &
                              ndiahdf
#ifdef MESSY
  USE mo_control,         ONLY: nvclev, vct ! ka_sv_20170405
  ...
  REAL(dp):: zcor1(nkp1,nlev), zcor2(nkp1,nlev), znd(nkp1,2*nlev)      &
           ...
           , zhigh(nkp1,nlev)
#ifdef MESSY
  REAL(dp):: zph(nlevp1), zpfull(nlev), zfact(nlev) ! ka_sv_20170405
#endif
...
#ifdef MESSY
! ka_sv_20170404+
  IF (ledith) THEN ! op_pj_20180725
     ! Calculate approximate full pressure levels
     DO jk=1,nlevp1
        za=vct(jk)
        zb=vct(jk+nvclev)
        zph(jk)=za+zb*100000._dp
     END DO
     DO jk = 1, nlev
        zpfull(jk)=(zph(jk)+zph(jk+1))*0.5_dp
     END DO
  END IF           ! op_pj_20180725
! ka_sv_20170404-
#endif
...
#ifdef MESSY
! ka_sv_20170404+
     ! Factor of increase of horizontal diffusion with height.
     ! Only used above 1 Pa to keep HAMMONIA equal to MIDATM below.
     IF (ledith) THEN ! op_pj_20180725
        IF (zpfull(jk) < 1._dp) THEN
           zfact(jk) = 1._dp+24.5_dp*(1._dp+TANH((-7._dp*LOG(  &
                zpfull(jk)/1.E5_dp)-115._dp)/12._dp))
           ! further increase for HAMMONIA by H. Schmidt
           ! op_pj_20180725: Note: This hard-wired level-index based condition
           !                       is really dangerous, because it depends on
           !                       the number of levels used. It would be better
           !                       to replace this by a pressure based 
           !                       condition.
           IF (jk < 6) THEN
              zfact(jk) = zfact(jk)*2.5_dp/REAL(jk)
           ENDIF
        ELSE
           zfact(jk)=1._dp
        ENDIF
...
#ifdef MESSY
     ELSE
        zdifd = difd*zztemp*zfact(jk)
        zdifvo = difvo*zztemp*zfact(jk)
        zdift = dift*zztemp*zfact(jk)
     ENDIF
#endif
