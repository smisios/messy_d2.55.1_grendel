...
MODULE messy_jval_si
  ...
  PUBLIC :: jval_init_coupling
  PUBLIC :: jval_global_start
  PUBLIC :: jval_physc
  PUBLIC :: jval_global_end
  PUBLIC :: jval_free_memory
  ! PRIVATE :: jval_read_nml_cpl
...
CONTAINS
  ...
  SUBROUTINE jval_initialize
    ...
    ! local
    CHARACTER(LEN=*), PARAMETER :: substr = 'jval_initialize'
    ...
    IF (p_parallel_io) THEN
       ...
       CALL jval_read_nml_cpl(status, io_unit)
       IF (status /= 0) CALL error_bi('error in jval_read_nml_cpl',substr)
    ENDIF
    ! broadcast results
  ...
  SUBROUTINE jval_init_memory
    ...
    CHARACTER(LEN=*), PARAMETER :: substr = 'jval_init_memory'
...
!!#D attila -
  ...
  SUBROUTINE jval_init_coupling
    ...
    ! LOCAL
    CHARACTER(LEN=*), PARAMETER   :: substr = 'jval_init_coupling'
    ...
    IF (status /=  953) THEN  !E 953 : DIMVAR does not exist
       ...
       IF (SIZE(plevs) /= nlev_h) THEN
          write(iouerr,*) 'JVAL: ', nlev_h, SIZE(plevs)
          CALL error_bi(&
    ...
    CALL new_attribute(status, TRIM(cname2), 'aer_asca', &
         'long_name', &
         c='scattering extinction of JVAL internal aerosol (per layer)' )
    CALL channel_halt(substr, status)
    CALL new_attribute(status, TRIM(cname2), 'aer_asca', &
         'units', c=' - ' )
    CALL channel_halt(substr, status)
    ...
    CALL new_attribute(status, TRIM(cname2), 'aer_aabs', &
         'long_name', &
         c='absorbing extinction of JVAL internal aerosol (per layer)' )
    CALL channel_halt(substr, status)
    CALL new_attribute(status, TRIM(cname2), 'aer_aabs', &
         'units', c=' - ' )
    CALL channel_halt(substr, status)
    ...
    CALL new_attribute(status, TRIM(cname2), 'aer_ga', &
         'long_name', &
         c='asymmetry factor of JVAL internal aerosol (per layer)' )
    CALL channel_halt(substr, status)
    CALL new_attribute(status, TRIM(cname2), 'aer_ga', &
         'units', c=' - ' )
    CALL channel_halt(substr, status)
    ...
    DO ji=1,MAXWAV
       ...
       CALL new_attribute(status, TRIM(cname2), 'aer_asca_b'//str_num, &
            'long_name', &
     c='scattering extinction of JVAL internal aerosol (per layer) for band '//str_num )
       CALL channel_halt(substr, status)
       CALL new_attribute(status, TRIM(cname2), 'aer_asca_b'//str_num, &
            'units', c=' - ' )
       CALL channel_halt(substr, status)
       ...
       CALL new_attribute(status, TRIM(cname2), 'aer_aabs_b'//str_num, &
            'long_name', &
     c='absorbing extinction of JVAL internal aerosol (per layer) for band '//str_num )
       CALL channel_halt(substr, status)
       CALL new_attribute(status, TRIM(cname2), 'aer_aabs_b'//str_num, &
            'units', c=' - ' )
       CALL channel_halt(substr, status)
       ...
       CALL new_attribute(status, TRIM(cname2), 'aer_ga_b'//str_num, &
            'long_name', &
         c='asymmetry factor of JVAL internal aerosol (per layer) for band '//str_num )
       CALL channel_halt(substr, status)
       CALL new_attribute(status, TRIM(cname2), 'aer_ga_b'//str_num, &
            'units', c=' - ' )
       CALL channel_halt(substr, status)
  ...
  END SUBROUTINE jval_init_coupling

  !***************************************************************************

  ! new routine for HALOE input, V3_H column above lev jk
  SUBROUTINE jval_global_start
  ...
  END SUBROUTINE jval_global_start
  ...
  SUBROUTINE jval_physc
    ...
    CHARACTER(LEN=*), PARAMETER :: substr = 'jval_physc'
    INTEGER :: jk, jp, jt, jn
...
!!#D attila -
#endif
  END SUBROUTINE jval_global_end
...
!!#D attila -
  ...
  SUBROUTINE jval_read_nml_cpl(status, iou)
    ...
    ! LOCAL
    CHARACTER(LEN=*), PARAMETER       :: substr = 'jval_read_nml_cpl'
...
!!#D attila -
  ...
  END SUBROUTINE jval_read_nml_cpl

!*****************************************************************************

END MODULE messy_jval_si
