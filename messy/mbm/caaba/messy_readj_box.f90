!*****************************************************************************
!                Time-stamp: <2018-02-15 17:31:54 sander>
!*****************************************************************************
...
CONTAINS
  ...
  SUBROUTINE readj_init
    ...
    USE caaba_mem,                ONLY: input_readj, input_readj_index
    ...
    PRINT *, HLINE2
    basename = TRIM(input_readj(INDEX(input_readj, '/', back=.TRUE.)+1:))
    PRINT *, 'subroutine readj_init'
    jx(:) = -999. ! negative dummy value
    CALL open_input_file(ncid_j, input_readj) ! get ID for input file
    CALL nf(nf90_inquire(ncid_j, n_dim, n_var))
    PRINT *, 'The file '//TRIM(input_readj)//' contains', n_var, 'variables'
      ...
      IF (INDEX(name_x,'J_') == 1) THEN ! only variables starting with 'J_'
        ...
        DO jp = 1, IP_MAX
          ...
          IF (TRIM(name_j) == TRIM(name_x)) THEN
            ! mz_rs_20110727+
            ! index works only after removing lon,lat,lev=1,1,1 in start:
            !CALL nf(nf90_get_var(ncid_j, varid_x, jx(jp), &
            ! start= (/1,1,1,input_readj_index/)))
            CALL nf(nf90_get_var(ncid_j, varid_x, jx(jp), &
              start= (/input_readj_index/)))
            ! mz_rs_20110727-
            l_initialized = .TRUE.
            EXIT
          ENDIF
