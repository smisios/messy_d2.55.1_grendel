#!/usr/bin/env python3
# -*- coding: utf-8 -*- Time-stamp: <2019-06-03 17:13:11 sander>
...
def get_elements_spc(spcfilename):
    ...
    for line in iter(SPCFILE): # loop over *.spc file
        ...
        print('%20s =  ' % (species), end='', file=LOGFILE)
        ...
        for chunk in spc_composition.split('+'): # split into atoms
            ...
            print('%g * %s + ' % (count, element), end='', file=LOGFILE)
    ...
    for line0 in iter(EQNFILE): # loop over *.eqn file
        ...
        if reaction:
            ...
            for chunk in products:
                ...
                for key, value in list(elements[product].items()):
