! If you want to change this file, edit scav/template_messy_scav_kpp.f90 
! (and maybe kp4/templates/initialize_kpp_ctrl_template.f90 or
! kp4/src/create_kpp_module.C), but not smcl/messy_scav_kpp.f90 !!!
...
MODULE messy_scav_kpp
  USE messy_scav_kpp_function,   ONLY: A, CalcStoichNum
  USE messy_scav_kpp_global      ! c, rconst, temp, press, dt, JX, khet_*,
                                  ! nsubsteps, logsteps, substep, method,
                                  ! rtol, atol, mcexp, ...
  USE messy_scav_kpp_initialize, ONLY: initialize
  USE messy_scav_kpp_integrator, ONLY: integrate, IERR_NAMES
  USE messy_scav_kpp_parameters  ! ind_*
  USE messy_scav_kpp_rates,      ONLY: update_rconst
  USE messy_scav_kpp_monitor,    ONLY: SPC_NAMES, EQN_NAMES, EQN_TAGS
  USE messy_scav_kpp_util,       ONLY: initialize_indexarrays

  IMPLICIT NONE
  SAVE

  ! the variables for which fill routines exist cannot be public:
  PRIVATE :: jx, cair, press, temp, C, cv_l, lwc, k_exf, k_exb, &
    k_exf_N2O5, k_exf_ClNO3, k_exf_BrNO3
  !mz_hr_20130409+: fix variable leak of 'C' to scav_init
  PRIVATE :: FIX, VAR
  !mz_hr_20130409- 
  ...
  ! - "USE messy_main_tools" is in Module_header of messy_scav_kpp.f90
  ...
  LOGICAL, PUBLIC, PARAMETER :: ivec=.FALSE.
...
SUBROUTINE error_output(C,ierr,PE)
  ...
  WRITE(filename,*) 'scav_PE'//TRIM(STRPE)//'_'//TRIM(STRNUM)//'.txt'
...
!qqq  REAL(dp), INTENT(INOUT), OPTIONAL       :: rstatus(:) ! mz_rs_20080113
  ...
  IF(PRESENT(ierrf)) ierrf(:) = 0
  IF(PRESENT(xNacc)) xNacc(:) = 0
  IF(PRESENT(xNrej)) xNrej(:) = 0
  C(:) = Conc(is,:)
  ...
  CONTAINS
    ...
    SUBROUTINE montecarlo_check
      ...
      PRINT *, 'scav/template_messy_scav_kpp.f90'
      STOP 1
