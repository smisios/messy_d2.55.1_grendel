! ******************************************************************
! ------------------------------------------------------------------
MODULE MESSY_NCREGRID_TOOLS_C1
! ------------------------------------------------------------------
...
! ******************************************************************
  ...
  ! READ IN OPTION
  INTEGER, PARAMETER, PUBLIC :: RGREAD_NCVAR =1 ! USE RGTOOL_C1_READ_NCVAR
  INTEGER, PARAMETER, PUBLIC :: RGREAD_NCFILE=2 ! USE RGTOOL_C1_READ_NCFILE
  ...
  ! - DATA IMPORT
  ! op_pj_20120510+
  INTERFACE RGTOOL_C1_READ_NCVAR
     MODULE PROCEDURE RGTOOL_C1_READ_NCVAR_4D
     MODULE PROCEDURE RGTOOL_C1_READ_NCVAR_5D
  END INTERFACE
  ! op_pj_20120510-
  PUBLIC  :: RGTOOL_C1_READ_NCVAR  ! READ ONE FIELD WITH NCREGRID
  ! op_pj_20120510+
  INTERFACE RGTOOL_C1_READ_NCFILE
     MODULE PROCEDURE RGTOOL_C1_READ_NCFILE_4D
     MODULE PROCEDURE RGTOOL_C1_READ_NCFILE_5D
  END INTERFACE
  ! op_pj_20120510-
  PUBLIC  :: RGTOOL_C1_READ_NCFILE ! READ ONE FILE WITH NCREGRID
...
! ------------------------------------------------------------------
SUBROUTINE RGTOOL_C1_READ_NCVAR_4D(modstr, vname, t, dat       &
  ...
  ! LOCAL
  CHARACTER(LEN=*), PARAMETER :: substr = 'RGTOOL_C1_READ_NCVAR'
...
END SUBROUTINE RGTOOL_C1_READ_NCVAR_4D
! ------------------------------------------------------------------

! ------------------------------------------------------------------
SUBROUTINE RGTOOL_C1_READ_NCVAR_5D(modstr, vname, t, dat       &
                               , lrg, lrgx, lrgy, lrgz, lok &  ! OPTIONAL
                               , hyam, hybm, p0, ps         &  ! OPTIONAL
                               , hyai, hybi                 &  ! OPTIONAL
                               , latm, lonm, lati, loni     &  ! OPTIONAL
...
END SUBROUTINE RGTOOL_C1_READ_NCVAR_5D
! ------------------------------------------------------------------

! ------------------------------------------------------------------
SUBROUTINE RGTOOL_C1_READ_NCFILE_4D(modstr, fname, t, dat, vars   &
                               , lrg, lrgx, lrgy, lrgz, lok    &  ! OPTIONAL
                               , hyam, hybm, p0, ps            &  ! OPTIONAL
                               , hyai, hybi                    &  ! OPTIONAL
                               , latm, lonm, lati, loni        &  ! OPTIONAL
  ...
  ! LOCAL
  CHARACTER(LEN=*), PARAMETER :: substr = 'RGTOOL_C1_READ_NCFILE_4D'
  INTEGER                             :: iou        ! logical I/O unit
!!$  INTEGER                             :: status     ! memory status
...
END SUBROUTINE RGTOOL_C1_READ_NCFILE_4D
! ------------------------------------------------------------------

! ------------------------------------------------------------------
SUBROUTINE RGTOOL_C1_READ_NCFILE_5D(modstr, fname, t, dat, vars   &
                               , lrg, lrgx, lrgy, lrgz, lok    &  ! OPTIONAL
                               , hyam, hybm, p0, ps            &  ! OPTIONAL
                               , hyai, hybi                    &  ! OPTIONAL
                               , latm, lonm, lati, loni        &  ! OPTIONAL
  ...
  ! LOCAL
  CHARACTER(LEN=*), PARAMETER :: substr = 'RGTOOL_C1_READ_NCFILE_5D'
  INTEGER                             :: iou        ! logical I/O unit
!!$  INTEGER                             :: status     ! memory status
  TYPE(ncvar), DIMENSION(:), POINTER  :: var        ! nc-variable list
  TYPE(geohybgrid)                    :: grid       ! grid-structure
...
END SUBROUTINE RGTOOL_C1_READ_NCFILE_5D
! ------------------------------------------------------------------
...
!!$  INTEGER                           :: status   ! memory status
  ...
  CASE(RGREAD_NCVAR)
     CALL RGTOOL_C1_READ_NCVAR(modstr                   &
          ,TRIM(actstr), tnc, field, lrg=lrg, lok=lok)

  CASE(RGREAD_NCFILE)
     IF (p_parallel_io) THEN
        CALL RGMSG(substr, RGMLW, &
             '2D-FIELD READ VIA RGTOOL_C1_READ_NCFILE!')
     END IF
     !
     CALL RGTOOL_C1_READ_NCFILE(modstr                         &
...
! ******************************************************************
! ------------------------------------------------------------------
END MODULE MESSY_NCREGRID_TOOLS_C1
! ------------------------------------------------------------------
! ******************************************************************
