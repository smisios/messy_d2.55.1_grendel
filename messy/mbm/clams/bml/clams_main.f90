...
!!$                                    , messy_tracer_afteradv
  USE messy_main_timer,           ONLY: lbreak, lresume, lstop      &
                                      , YEAR, MONTH, DAY, HOUR, MINUTE, SECOND, lfirst_cycle
  ...
  ! ju_nt_20199314: messy_setup already called in p_start !
...
!!$     CALL messy_tracer_afteradv
     ...
     if (p_pe==0) THEN
       ...
       'Begin time_loop:',YEAR,'-',MONTH,'-',DAY,' ',HOUR,':', MINUTE,':',SECOND
        WRITE(*,*) '-------------------------------------------- '
     endif
  ...
  CALL messy_finalize  ! close output files, deallocate fields

  IF (lstop) THEN
     CALL messy_blather_endfile_bi('SIMULATION FINISHED','clams_main')
  ELSE
     CALL info_bi('SIMULATION INTERRUPTED','clams_main')
  ENDIF
