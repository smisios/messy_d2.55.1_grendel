MODULE MESSY_CLAMSCHEM_DATA_UST
  ...
  TYPE(CHCH_T), DIMENSION(:), ALLOCATABLE :: chch_defs_ust
  TYPE(RATB_T), DIMENSION(:), ALLOCATABLE :: ratb_defs_ust
  TYPE(RATT_T), DIMENSION(:), ALLOCATABLE :: ratt_defs_ust
  TYPE(RATJ_T), DIMENSION(:), ALLOCATABLE :: ratj_defs_ust
  TYPE(RATH_T), DIMENSION(:), ALLOCATABLE :: rath_defs_ust
  
CONTAINS

SUBROUTINE CLAMS_CHEM_INIT_UST()
  ...
  ALLOCATE(chch_defs_ust(jpspec))
  ALLOCATE(ratb_defs_ust(jpbk))
  ALLOCATE(ratj_defs_ust(jppj))
  ALLOCATE(ratt_defs_ust(jptk))
  ALLOCATE(rath_defs_ust(jphk))
...
chch_t(42, 'N2O       ',  1, 'TR        ','          ',  0,  0,  0),  &
chch_t(43, 'NOystar   ',  1, 'TR        ','          ',  0,  0,  0)  &
/)
...
.60, 4.40E-32, -1.30,       .0, 7.50E-11,  0.20,       .0, 0.000, 0.000)  &  
  /)
   ...
   END SUBROUTINE CLAMS_CHEM_INIT_UST


   SUBROUTINE CLAMS_CHEM_CLEAN_UST

     implicit none

     DEALLOCATE(chch_defs_ust)
     DEALLOCATE(ratb_defs_ust)
     DEALLOCATE(ratj_defs_ust)
     DEALLOCATE(ratt_defs_ust)
     DEALLOCATE(rath_defs_ust)

   END SUBROUTINE CLAMS_CHEM_CLEAN_UST


END MODULE MESSY_CLAMSCHEM_DATA_UST
