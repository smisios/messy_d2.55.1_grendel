Module messy_clamschem_asad_bedriv
...
!
      ...
      USE messy_clamschem_asad_mod_clams, ONLY: jpctr, jpspec, jppj, jptk, &
                                                theta_field_size, mype, &
...
! Total production, loss and intermediate terms in budget calculation
      REAL(PREC), DIMENSION(theta_field_size) :: pdn
      REAL(PREC), DIMENSION(theta_field_size) :: l
      REAL(PREC), DIMENSION(theta_field_size) :: r1
      REAL(PREC), DIMENSION(theta_field_size) :: r2
      REAL(PREC), DIMENSION(theta_field_size) :: l1
      REAL(PREC), DIMENSION(theta_field_size) :: l2
      REAL(PREC), DIMENSION(theta_field_size) :: l3
...
! Fractional production coefficient
      REAL(PREC) :: fraction(jpspec, prodmax)
...
! ju_nt_20140213: in asad_ftoy: second argument is intent(inout)!
!      CALL asad_ftoy(.false.,1, n_points)
      ihelp = 1
      CALL asad_ftoy(.false.,ihelp, n_points)
      IF (nstst  /=  0) THEN 
...
end Module messy_clamschem_asad_bedriv
