Module messy_clamschem_asad_prls

contains

!
!     prls  -  Computes product and loss terms for required species
!
...
!     prod and slos as pointers in UM code - see routine messy_clamschem_asad_mod_init.]
!
...
!
        ...
        USE messy_clamschem_asad_mod, ONLY: prod, slos, y, rk, prk,       &
                                            dpd, dpw, pd,                 &
                                            nuni, nspi, ngrp, nprdx1,     &
                                            nprdx2, nprdx3, ntabfp, frpx, &
                                            nldepx, nnfrp
        use messy_clamschem_global,         only: missing_index
        USE messy_clams_global,             ONLY: prec
        USE messy_clamschem_asad_mod_clams, ONLY: jpspec, jpnr
...
!       2.2  Reactions with two reactants.
        ...
        DO jr = iunip1, jpnr
          ir1 = nspi(jr,1)
          ir2 = nspi(jr,2)
          ! ju_nt_20140211: jug following if statement added 02/99
          if (ir1 > 0) then
             DO jl = 1, kl
...
!!!!!!! Check if there are missing values:
      ...
      End Module messy_clamschem_asad_prls
