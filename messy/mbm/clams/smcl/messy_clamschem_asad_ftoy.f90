Module messy_clamschem_asad_ftoy

contains

!     asad_ftoy - sets species concentrations, partitions families
!
...
!
        ...
        USE messy_clamschem_asad_mod, ONLY: f, y, prod, slos, ratio, qa,     &
        ...
        USE messy_clamschem_asad_mod_clams, ONLY: jpspec, theta_field_size, failed
        USE messy_clamschem_asad_dummy,  ONLY: ereport
        USE messy_clamschem_asad_fyfixr, ONLY: ASAD_FYFIXR
        USE messy_clamschem_asad_fyinit, ONLY: ASAD_FYINIT
        USE messy_clamschem_asad_fyself, ONlY: ASAD_FYSELF
...
!       Local variables
        ...
        REAL(PREC)     :: zthresh
        REAL(PREC)     :: sl
        REAL(PREC)     :: zy(theta_field_size,jpspec)
        REAL(PREC)     :: zb(theta_field_size)
        REAL(PREC)     :: zc(theta_field_size)
        REAL(PREC)     :: zd(theta_field_size)
...
! ju_nt_20140217:
! following limit included to avoid unrealistic high values of ratio 
! typical values of ratio for minor species should be below 1
! J.U. Grooss 01.03.1999
                IF (ratio(jl,js) > 1.E+6) ratio(jl,js)=1.E+6
...
!     Check for NaNs and unlikely large values (jug, 04/2013)
                if ( y(jl,js) /= y(jl,js) ) failed(jl)=.true.
                ! if ( y(jl,js) .gt. tnd(jl) ) failed(jl)=.true.
              ENDDO
            IF ( .NOT. gconv ) EXIT
          ENDDO
          IF (gconv) THEN 
...
!       9. Convergence achieved or max. iterations reached.
      ...
      END Module messy_clamschem_asad_ftoy
