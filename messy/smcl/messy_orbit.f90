MODULE messy_orbit
  ...
  CHARACTER(LEN=*), PARAMETER :: modstr = 'orbit'
  CHARACTER(LEN=*), PARAMETER :: modver = '0.9'
  ...
  REAL(dp), PARAMETER :: dtor = pi/180._dp
  REAL(dp), PARAMETER :: stor = dtor/3600._dp
  REAL(dp), PARAMETER :: sectot = 1.0_dp/(86400.0_dp*36525.0_dp)

  ! Orbital parameters (namelist)
  ! -----------------------------
  REAL(DP), SAVE ::      &
       cecc   =  0.016715_dp     &
       ,cobld  =  23.441_dp      &
       ,clonp  =  282.7_dp
  !
  ! .TRUE. : orbit routine from vsop87
  ! .FALSE.: orbit routine pcmdi (AMIP)
  LOGICAL, PUBLIC, SAVE :: l_orbvsop87 = .TRUE.
...
CONTAINS
  ...
  SUBROUTINE sunlonandecc (t, lon, e)
    ...
    c = e*(2._dp-0.25_dp*e2)*SIN(m)+1.25_dp*e2*SIN(2._dp*m) &
         +1.0833333333_dp*e*e2*SIN(3._dp*m)
    lon = l+c
  ...
  SUBROUTINE recttosph (r, s)
    ...
    s(1) = ATAN2(r(2),r(1))
    IF (s(1) < 0.0_dp) s(1) = s(1)+2._dp*pi
  ...
  FUNCTION siderealtime(t) RESULT (sidtime)
    ...
    Theta = T*(360.98564736629_dp*36525._dp &
         +T*(0.000387933_dp-T/38710000._dp))
    sidtime = MODULO (((280.46061837_dp+Theta)*DToR), 2._dp*pi)
  ...
  SUBROUTINE nutationconst(t, nutlon, nutobl)
    ...
    om    = MODULO ((evalpoly (pom,    4, t)*dtor),2._dp*pi)
    msun  = MODULO ((evalpoly (pmmoon, 4, t)*dtor),2._dp*pi)
    mmoon = MODULO ((evalpoly (pmsun,  4, t)*dtor),2._dp*pi)
    f     = MODULO ((evalpoly (pf,     4, t)*dtor),2._dp*pi)
    d     = MODULO ((evalpoly (pd,     4, t)*dtor),2._dp*pi)

    n1 = (-0.01742_dp * t - 17.1996_dp) * SIN(om)            &
         + (-1.3187_dp - 0.00016_dp * t) * SIN(2._dp*f-2._dp*d+2._dp*om) &
         + (-0.2274_dp - 0.00002_dp * t) * SIN(2._dp*f+2._dp*om)     &
         + (0.2062_dp + 0.00002_dp * t) * SIN(2._dp*om)          &
         + (0.1426_dp - 0.00034_dp * t) * SIN(msun)          &
         + (0.0712_dp + 0.00001_dp * t) * SIN(mmoon)
    n2 = + (-0.0517_dp + 0.00012_dp * t) * SIN(msun+2._dp*f-2._dp*d+2._dp*om) &
         + (-0.0386_dp - 0.00004_dp * t) * SIN(2._dp*f+om)            &
         -0.0301_dp * SIN(mmoon+2._dp*f+2._dp*om)                         &
         + (0.0217_dp - 0.00005_dp * t) * SIN(-msun+2._dp*f-2._dp*d+2._dp*om) &
         -0.0158_dp * SIN(mmoon-2._dp*d)                              &
         + (0.0129_dp + 0.00001_dp * t) * SIN(2._dp*f-2._dp*d+om)
    n3 = +0.0123_dp * SIN(-mmoon+2._dp*f+2._dp*om)                &
         +0.0063_dp * SIN(2._dp*d)                            &
         + (0.0063_dp + 0.00001_dp * t) * SIN(mmoon+om)   &
         -0.0059_dp * SIN(-mmoon+2._dp*f+2._dp*d+2._dp*om)            &
         + (-0.0058_dp - 0.00001_dp * t) * SIN(-mmoon+om) &
         -0.0051_dp * SIN(mmoon+2._dp*f+om)
    n4 = +0.0048_dp * SIN(2._dp*mmoon-2._dp*d)      &
         +0.0046_dp * SIN(-2._dp*mmoon+2._dp*f+om)  &
         -0.0038_dp * SIN(2._dp*f+2._dp*d+2._dp*om)     &
         -0.0031_dp * SIN(2._dp*mmoon+2._dp*f+2._dp*om) &
         +0.0029_dp * SIN(2._dp*mmoon)          &
         +0.0029_dp * SIN(mmoon+2._dp*f-2._dp*d+2._dp*om)
    n5 = +0.0026_dp * SIN(2._dp*f)                        &
         -0.0022_dp * SIN(2._dp*f-2._dp*d)                    &
         +0.0021_dp * SIN(-mmoon+2._dp*f+om)              &
         + (0.0017_dp - 0.00001_dp * t) * SIN(2._dp*msun) &
         +0.0016_dp * SIN(-mmoon+2._dp*d+om)              &
         + (-0.0016_dp + 0.00001_dp * t) * SIN(2._dp*msun+2._dp*f-2._dp*d+2._dp*om)
    n6 = -0.0015_dp * SIN(msun+om)           &
         -0.0013_dp * SIN(mmoon-2._dp*d+om)      &
         -0.0012_dp * SIN(-msun+om)          &
         +0.0011_dp * SIN(2._dp*mmoon-2._dp*f)       &
         -0.0010_dp * SIN(-mmoon+2._dp*f+2._dp*d+om) &
         -0.0008_dp * SIN(mmoon+2._dp*f+2._dp*d+2._dp*om)
    n7 = +0.0007_dp * SIN(msun+2._dp*f+2._dp*om)  &
         -0.0007_dp * SIN(mmoon+msun-2._dp*d) &
         -0.0007_dp * SIN(-msun+2._dp*f+2._dp*om) &
         -0.0007_dp * SIN(2._dp*f+2._dp*d+om)     &
         +0.0006_dp * SIN(mmoon+2._dp*d)      &
         +0.0006_dp * SIN(2._dp*mmoon+2._dp*f-2._dp*d+2._dp*om)
    n8 = +0.0006_dp * SIN(mmoon+2._dp*f-2._dp*d+om) &
         -0.0006_dp * SIN(-2._dp*mmoon+2._dp*d+om)  &
         -0.0006_dp * SIN(2._dp*d+om)           &
         +0.0005_dp * SIN(mmoon-msun)       &
         -0.0005_dp * SIN(-msun+2._dp*f-2._dp*d+om) &
         -0.0005_dp * SIN(-2._dp*d+om)
    n9 = -0.0005_dp * SIN(2._dp*mmoon+2._dp*f+om)      &
         -0.0003_dp * SIN(-2._dp*mmoon+2._dp*f+2._dp*om)   &
         +0.0004_dp * SIN(2._dp*mmoon-2._dp*d+om)      &
         +0.0004_dp * SIN(msun+2._dp*f-2._dp*d+om)     &
         -0.0003_dp * SIN(mmoon-msun+2._dp*f+2._dp*om) &
         -0.0003_dp * SIN(-mmoon-msun+2._dp*f+2._dp*d+2._dp*om)
    n10 =-0.0003_dp * SIN(3._dp*mmoon+2._dp*f+2._dp*om)   &
         -0.0003_dp * SIN(-msun+2._dp*f+2._dp*d+2._dp*om) &
         -0.0003_dp * SIN(mmoon-msun-d)       &
         -0.0004_dp * SIN(mmoon-d)            &
         -0.0004_dp * SIN(msun-2._dp*d)           &
         +0.0004_dp * SIN(mmoon-2._dp*f)
    n11 =-0.0004_dp * SIN(d)          &
         -0.0003_dp * SIN(mmoon+msun) &
         +0.0003_dp * SIN(mmoon+2._dp*f)

    nutlon = n1+n2+n3+n4+n5+n6+n7+n8+n9+n10+n11

    n1 =   (0.00089_dp * t + 9.2025_dp) * COS(om)           &
         + (0.5736_dp - 0.00031_dp * t) * COS(2._dp*f-2._dp*d+2._dp*om) &
         + (0.0977_dp - 0.00005_dp * t) * COS(2._dp*f+2._dp*om)     &
         + (-0.0895_dp + 0.00005_dp * t) * COS(2._dp*om)        &
         + (0.0054_dp - 0.00001_dp * t) * COS(msun)         &
         -0.0007_dp * COS(mmoon)
    n2 = + (0.0224_dp - 0.00006_dp * t) * COS(msun+2._dp*f-2._dp*d+2._dp*om)   &
         +0.0200_dp * COS(2._dp*f+om)                               &
         + (0.0129_dp - 0.00001_dp * t) * COS(mmoon+2._dp*f+2._dp*om)      &
         + (-0.0095_dp + 0.00003_dp * t) * COS(-msun+2._dp*f-2._dp*d+2._dp*om) &
         -0.0070_dp * COS(2._dp*f-2._dp*d+om)                           &
         -0.0053_dp * COS(-mmoon+2._dp*f+2._dp*om)
    n3 = -0.0033_dp * COS(mmoon+om)            &
         +0.0026_dp * COS(-mmoon+2._dp*f+2._dp*d+2._dp*om) &
         +0.0032_dp * COS(-mmoon+om)           &
         +0.0027_dp * COS(mmoon+2._dp*f+om)        &
         -0.0024_dp * COS(-2._dp*mmoon+2._dp*f+om)     &
         +0.0016_dp * COS(2._dp*f+2._dp*d+2._dp*om)
    n4 = +0.0013_dp * COS(2._dp*mmoon+2._dp*f+2._dp*om)    &
         -0.0012_dp * COS(mmoon+2._dp*f-2._dp*d+2._dp*om)  &
         -0.0010_dp * COS(-mmoon+2._dp*f+om)       &
         -0.0008_dp * COS(-mmoon+2._dp*d+om)       &
         +0.0007_dp * COS(2._dp*msun+2._dp*f-2._dp*d+2._dp*om) &
         +0.0009_dp * COS(msun+om)
    n5 = +0.0007_dp * COS(mmoon-2._dp*d+om)       &
         +0.0006_dp * COS(-msun+om)           &
         +0.0005_dp * COS(-mmoon+2._dp*f+2._dp*d+om)  &
         +0.0003_dp * COS(mmoon+2._dp*f+2._dp*d+2._dp*om) &
         -0.0003_dp * COS(msun+2._dp*f+2._dp*om)      &
         +0.0003_dp * COS(-msun+2._dp*f+2._dp*om)
    n6 = +0.0003_dp * COS(2._dp*f+2._dp*d+om)           &
         -0.0003_dp * COS(2._dp*mmoon+2._dp*f-2._dp*d+2._dp*om) &
         -0.0003_dp * COS(mmoon+2._dp*f-2._dp*d+om)     &
         +0.0003_dp * COS(-2._dp*mmoon+2._dp*d+om)      &
         +0.0003_dp * COS(2._dp*d+om)               &
         +0.0003_dp * COS(-msun+2._dp*f-2._dp*d+om)
    n7 = +0.0003_dp * COS(-2._dp*d+om) &
         +0.0003_dp * COS(2._dp*mmoon+2._dp*f+om)
  ...
  FUNCTION approxdeltat(t) RESULT (deltat)
    ...
    ELSE 
      ...
      ELSE
        ...
        ind = INT((y-1620._DP)/2._DP)+1
        IF (ind > 186) ind = 186
        y = y/2._DP-REAL(ind, DP)-810._DP
...
!----------------------------------------------------------------------
  ...
  SUBROUTINE orbit_vsop87 (jde, ra, dec, dis, gha)
    ...
    USE messy_orbit_vsop87, ONLY: earth_position
...
!----------------------------------------------------------------------
  ...
  SUBROUTINE orbit_pcmdi (pvetim, pdisse, pdec, pra, status)
    !
    ! Description:
    !
    ...
    zeve = 2._DP*ATAN(TAN(0.5_dp*zlonpr)/zsqecc)

    ! calculation of true anomaly of vernal equinox (Kepler)

    ztlonpr = zeve-cecc*SIN(zeve)

    ! use Newtons method for determing the eccwntric anomaly for the 
    ! actual true anomaly. 
    ...
    IF ( cecc > 0.975_dp ) THEN  
      IF ( ABS(zm) < 0.52359_dp) THEN            ! M < ~30 deg in radians
        ...
        zz = SIGN(zz,zb+zsqrt) 
        zs0 = zz-0.5_dp*za
        zs = zs0-(0.078*zs0**5)/(1.0_dp+cecc)
    ...
    DO
      ...
      IF (iter >= 25) THEN
         WRITE(*,*) 'orbit_pcmdi: Eccentric anomaly not found!'
         status = 1
         RETURN
      END IF
  ...
  ELEMENTAL SUBROUTINE solang(coslon,sinlon, &
       &            ptim1,ptim2,ptim3,  &
       &            pamu0, pamu0c, prdayl)
    ...
    ! INPUT
    ! -----
    REAL(dp), INTENT(in) :: coslon ! cos(longitude)
    REAL(dp), INTENT(in) :: sinlon ! sin(longitude)
    ...
    ! OUTPUT
    ! ------
    REAL(dp) ,INTENT(out) :: pamu0   ! 
    REAL(dp) ,INTENT(out) :: pamu0c  ! 
    REAL(dp) ,INTENT(out) :: prdayl  ! 

    !  Executable statements 
    
    pamu0 = ptim1 + ptim2*coslon + ptim3*sinlon
    
    IF (pamu0 >= 0.0_dp) THEN
       pamu0c  = pamu0
       prdayl  = 1.0_dp
    ELSE  
       pamu0c  = 0.0_dp
  ...
  SUBROUTINE orbit_read_nml_ctrl(status, iou)
    ...
    NAMELIST /CTRL/ cecc, cobld, clonp, l_orbvsop87
  ...
  END SUBROUTINE orbit_read_nml_ctrl

END MODULE messy_orbit
