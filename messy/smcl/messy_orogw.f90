...
MODULE messy_orogw
  ...
  CHARACTER(LEN=*), PARAMETER, PUBLIC :: modstr = 'orogw'
...
CONTAINS
  !======================================================================
  SUBROUTINE sugwd(klon, klev, nvclev, vct, nn, l_truncation_error)

  IMPLICIT NONE

  !  Scalar arguments with intent(In):
  INTEGER,                INTENT (IN) :: klon, klev, nn, nvclev
  REAL(dp), DIMENSION(:), INTENT(IN)  :: vct
  ! local scalar
  INTEGER :: jk
  REAL(dp)    :: zstra, zsigt, zpm1r, zpr
  LOGICAL, INTENT(INOUT) :: l_truncation_error
...
SUBROUTINE orodrag( kproma,  kbdim,  klev,            &
     ...
     pvom,   pvol,   pte,                             &
! op_mr_20180404+
! For orographic cirrus in messy_cloud_kuebbeler14
...
! op_mr_20180404-
  ...
  ! Diagnostic output
  REAL(dp), INTENT(out) :: pgwd(kbdim, klev)
  REAL(dp), INTENT(out) :: pampl(kbdim, klev)
  REAL(dp), INTENT(out) :: plength(kbdim)
  REAL(dp), INTENT(out) :: peff(kbdim)
  REAL(dp), INTENT(out) :: pangle(kbdim)
  ...
  REAL(dp) :: dep_b(kbdim), pvpf(kbdim,klev), w_gwd(kbdim,klev) ! op_mr_20180404
  ...
  CALL gwstress( kproma, kbdim, klev,                                  &
       ...
       ztau,   zdmod, peff) ! op_mr_20180404
  ...
  CALL gwprofil(  kproma, kbdim, klev,                         &
       ...
       zri,     ztau,                                          &
       zdmod,   psig,  pstd, pampl) ! op_mr_20180404
  !
...
!DIR$ CONCURRENT
     DO 523 jl=1,kgwd
        ...
        IF(jk > ikenvh(ji)) THEN
           ...
           !
           zdudt(ji)=zdudt(ji)*(zbet/(1.0_dp+zbet))
           zdvdt(ji)=zdvdt(ji)*(zbet/(1.0_dp+zbet))
        END IF
        pvom(ji,jk)=zdudt(ji)
        pvol(ji,jk)=zdvdt(ji)
        ...
        ! Calculation of l(z) from Lott and Miller (1997), Eq. (11)
        plength(ji) = 2._dp * max( &
             abs(pstd(ji)/psig(ji) * cos(zpsi(ji,klev+1))), &
             abs(pstd(ji)/(pgam(ji)*psig(ji)) * sin(zpsi(ji,klev+1)))) &
             * dep_b(ji)

        ! Calculation of vertical velocity
        IF (plength(ji) .EQ. 0._dp) THEN
          w_gwd(ji,jk) = 0._dp
        ELSE
           w_gwd(ji,jk) = 2._dp*api/(2._dp*plength(ji))*sqrt(pampl(ji,jk)) &
                * max(0._dp,pvpf(ji,jk))
        END IF
...
SUBROUTINE orosetup                                           &
     ...
     , pnu  ,  pd1  ,  pd2  ,pdmod, pdep_b, angle, vpf )
  !
  !**** *gwsetup*
  !
  !     purpose.
  ...
  !-----------------------------------------------------------------------
  !
  USE messy_main_constants_mem,  ONLY: g, cpd=>cp_air, rd
  ...
  REAL(dp), INTENT(in) :: pum1(kbdim,klev)     ! zonal wind (t-dt)
  REAL(dp), INTENT(in) :: pvm1(kbdim,klev)     ! meridional wind (t-dt)
  REAL(dp), INTENT(out) :: pdep_b(kbdim), angle(kbdim), vpf(kbdim,klev)
...
2110 END DO
  ...
  DO 2112 jl=1,kproma
     ...
     angle(jl) = ppsi(jl,klev+1) ! op_mr_20180404
  ...
  DO 254  jk=ilevh,klev
     DO 253  jl=1,kproma
        IF(ktest(jl) == 1) THEN
           ...
           pdep_b(jl) = MIN(SQRT(pzdep(jl,kkenvh(jl))), 1._dp)
           IF (kkenvh(jl) == klev .OR. kkenvh(jl) == klev+1) THEN
              pdep_b(jl) = 1._dp
           ENDIF
...
SUBROUTINE gwstress( kproma, kbdim, klev,                           &
  ...
  REAL(dp), INTENT(out) :: peff_fld(kbdim)
  ...
  DO 301 jl=1,kproma
     IF(ktest(jl) == 1) THEN
        ...
        peff_fld(jl)=ppic(jl)-pval(jl)
        IF(kkenvh(jl) < klev)THEN
           zeff=MIN(gfrcrit*pvph(jl,klev+1)/SQRT(pstab(jl,klev+1))        &
                ,zeff)
           peff_fld(jl)=MIN(gfrcrit*pvph(jl,klev+1)/SQRT(pstab(jl,klev+1)) &
                ,zeff)
        ENDIF
...
SUBROUTINE gwprofil( kproma, kbdim, klev,                    &
  ...
  REAL(dp), INTENT(out) :: ampl(kbdim,klev)
  ...
  !  Local arrays: 
  REAL(dp) :: zdz2 (kbdim,klev) , znorm(kbdim) , zoro(kbdim), ztau (kbdim,klev+1), zdz2_sat(kbdim,klev)
  !
  !  Executable statements  
     ...
     DO 442 ji=1,kgwd
        ...
        IF(jk < kkcrith(jl)) THEN
           ...
           ELSE
              ...
              zriw=pri(jl,jk)*(1.0_dp-zalfa)/(1.0_dp+zalfa*zsqr)**2
              IF(zriw < grcrit) THEN
                 ...
                 zdz2_sat(jl,jk)=(pvph(jl,jk)*zalpha)**2/pstab(jl,jk)
                 ampl(jl,jk)=zdz2_sat(jl,jk)
...
SUBROUTINE orolift( kproma, kbdim, klev,               &
  ...
  USE messy_main_constants_mem, ONLY: g, omega, rd
  ...
  REAL(dp), INTENT(IN) :: delta_time, twomu_2d(kbdim)
     ...
     DO 601 jl=1,kproma
        IF(ktest(jl) == 1) THEN
           DO jk=klev,iknub(jl),-1
              ...
              zdudt(jl)=-pum1(jl,jk)/ztmst/(1.0_dp+zbet**2)
              zdvdt(jl)=-pvm1(jl,jk)/ztmst/(1.0_dp+zbet**2)
