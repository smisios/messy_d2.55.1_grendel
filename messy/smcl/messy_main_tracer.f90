...
! **********************************************************************
  ...
  USE messy_main_constants_mem, ONLY: DP, SP &
       , STRLEN_MEDIUM, STRLEN_VLONG, STRLEN_XLONG &
       , STRLEN_ULONG &
       , BIG_DP, MH, MC, MN, MF, MO, MS, MCl, MBr, MI, MHg &
       , MFe, MNa, MCa, MK, MMg
  ...
  CHARACTER(len=*), PARAMETER, PUBLIC :: modver = '2.6'
  ...
  ! =================================================================
#include "messy_main_tracer_chemprop.inc"
  ! =================================================================
  ...
  INTEGER, PARAMETER, PUBLIC :: I_DRYDEP     =  MAX_CASK_I_CHEMPROP +  4  ! DRY DEPOSITION
  INTEGER, PARAMETER, PUBLIC :: I_SEDI       =  MAX_CASK_I_CHEMPROP +  5  ! SEDIMENTATION
  INTEGER, PARAMETER, PUBLIC :: I_SCAV       =  MAX_CASK_I_CHEMPROP +  6  ! SCAVENGING
  INTEGER, PARAMETER, PUBLIC :: I_MIX        =  MAX_CASK_I_CHEMPROP +  7  ! TURBULENT MIXING
  INTEGER, PARAMETER, PUBLIC :: I_FORCE_COL  =  MAX_CASK_I_CHEMPROP +  8  ! FORCING IN COLUMN MODE
  INTEGER, PARAMETER, PUBLIC :: I_INTEGRATE  =  MAX_CASK_I_CHEMPROP +  9  ! TIME INTEGRATION
  INTEGER, PARAMETER, PUBLIC :: I_TIMEFILTER =  MAX_CASK_I_CHEMPROP + 10  ! TIME FILTER
  INTEGER, PARAMETER, PUBLIC :: I_FORCE_INIT =  MAX_CASK_I_CHEMPROP + 11  ! FORCE INIT AFTER RESTART
  INTEGER, PARAMETER, PUBLIC :: I_AEROSOL_METHOD = MAX_CASK_I_CHEMPROP + 12 ! MODAL OR BIN
  INTEGER, PARAMETER, PUBLIC :: I_AEROSOL_MODE   = MAX_CASK_I_CHEMPROP + 13 ! MODE OR BIN NUMBER
  INTEGER, PARAMETER, PUBLIC :: I_AEROSOL_SOL    = MAX_CASK_I_CHEMPROP + 14 ! SOLUBLE ON/OFF
  INTEGER, PARAMETER, PUBLIC :: I_AEROSOL_HETICE = MAX_CASK_I_CHEMPROP + 15 ! HIGHER ICE SCAV. FRAC.
  INTEGER, PARAMETER, PUBLIC :: I_HDIFF          = MAX_CASK_I_CHEMPROP + 16 ! HORIZONTAL DIFFUSION
  INTEGER, PARAMETER, PUBLIC :: I_RELAX          = MAX_CASK_I_CHEMPROP + 17 ! BOUNDARY DATA AVAILABLE
                                                                            ! i.e. relaxation possible
  INTEGER, PARAMETER, PUBLIC :: I_MMD_INIT       = MAX_CASK_I_CHEMPROP + 18
  INTEGER, PARAMETER, PUBLIC :: I_TAG_REG_IDT    = MAX_CASK_I_CHEMPROP + 19 ! id of associated regular
                        !                                                   ! species
  INTEGER, PARAMETER, PUBLIC :: I_TAG_SPECIFIC   = MAX_CASK_I_CHEMPROP + 20 ! flag for special
  !                                                                         ! treatment
  INTEGER, PARAMETER, PUBLIC :: I_INITIAL        = MAX_CASK_I_CHEMPROP + 21 ! initialisation of trac
                                                                            ! treatment 
  INTEGER, PARAMETER, PUBLIC :: I_LBC            = MAX_CASK_I_CHEMPROP + 22 ! lateral boundary cond.
                                                                            ! treatment
  INTEGER, PARAMETER, PUBLIC :: I_DAMP           = MAX_CASK_I_CHEMPROP + 23 ! require damping?
  INTEGER, PARAMETER, PUBLIC :: I_GRIBTAB        = MAX_CASK_I_CHEMPROP + 24 ! grib table number
  INTEGER, PARAMETER, PUBLIC :: I_GRIBPARAM      = MAX_CASK_I_CHEMPROP + 25 ! grib parameter number
  !
  INTEGER, PARAMETER, PUBLIC :: I_MTSKIP         = MAX_CASK_I_CHEMPROP + 26 ! handled by MTSKIP?
  INTEGER, PARAMETER, PUBLIC :: I_BBC            = MAX_CASK_I_CHEMPROP + 27 ! bottom boundary condition
  INTEGER, PARAMETER, PUBLIC :: I_SPPTPERT       = MAX_CASK_I_CHEMPROP + 28 ! PERTURBED PHYSICS
  INTEGER, PARAMETER, PUBLIC :: I_BLOCKMEM       = MAX_CASK_I_CHEMPROP + 29 ! BLOCKED MEMORY REQUIRED? 
   INTEGER, PARAMETER, PUBLIC :: I_FEEDBACK      = MAX_CASK_I_CHEMPROP + 30 ! ICON: feedback between patches
  ...
  INTEGER, DIMENSION(MAX_CASK_I_PROCESS), PARAMETER, PUBLIC :: &
       ...
       (/ ON, ON, ON, OFF, OFF, OFF, ON, OFF, ON, ON, OFF &
       , MODAL, 0, ON, OFF, OFF, T_RELAX_FULL, OFF, 0, 0 &
       , T_INI_FILE, T_LBC_FILE, ON , -1 , -1 &
       , OFF, T_BBC_ZEROFLUX, T_SPPTPERT_OFF, T_BLOCKMEM_OFF &
       , ON /)
  ...
  INTERFACE set_tracer
     ...
     MODULE PROCEDURE set_tracer_special
  END INTERFACE
  ...
  TYPE t_ident    ! IDENTIFICATION
     ...
     CHARACTER(LEN=STRLEN_ULONG)      :: longname    = '' ! opt. long name
     CHARACTER(LEN=STRLEN_ULONG)      :: standardname= '' ! opt. standard name
     CHARACTER(LEN=STRLEN_MEDIUM)     :: unit        = '' !
     CHARACTER(LEN=STRLEN_MEDIUM)     :: submodel    = '' ! requesting submodel
     INTEGER                          :: idx              ! tracer index in set
     INTEGER                          :: idxblck     = 0  ! tracer index in blck set
  ...
  TYPE T_TRACERSET
     ...
     INTEGER                                  :: ntrac  = 0
     ! NUMBER OF BLOCKED TRACERS IN SET (required for COSMO)
     INTEGER                                  :: nblck  = 0
     !
...
CONTAINS
  ...
  SUBROUTINE new_tracer(status, setname, basename, submodel                 &
       , idx, subname, longname, standardname, unit, medium, quantity, type &
       , cask_i, cask_r, cask_s, refspec)
    ...
    CHARACTER(LEN=*), INTENT(IN), OPTIONAL  :: refspec
    ...
    INTEGER                        :: i
    CHARACTER(LEN=STRLEN_FNAME)    :: specname
    ...
    DO
       ...
       IF (TRIM(ti%info%ident%fullname) == TRIM(fullname)) THEN
          IF (PRESENT(idx)) idx = ti%info%ident%idx
    ...
    IF (PRESENT(longname)) THEN
       IF (LEN(longname) > STRLEN_ULONG) THEN
    ...
    IF (PRESENT(standardname)) THEN
       IF (LEN(standardname) > STRLEN_ULONG) THEN
          status = 209 ! ERROR: standardname too long
          RETURN
       END IF
       ti%info%ident%standardname  = TRIM(standardname)
    END IF
    !
    ...
    DO i = 1, N_CHEMPROP
       IF (TRIM(chemprop(i)%kppname) == TRIM(specname)) THEN
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE set_tracer_special(status, setname, idx, refspec, standardname)
    ...
    IF (PRESENT(standardname)) THEN
       ti%info%ident%standardname = TRIM(ADJUSTL(standardname))
    END IF
    
    status = 0

  END SUBROUTINE set_tracer_special
  ! -------------------------------------------------------------------

  ! -------------------------------------------------------------------
  SUBROUTINE get_tracer_set_by_name(status, setname, trlist, ti, ntrac &
     , xt, xtte, xtm1, xmem, xt_tl, l_tfstd, l_init, l_enable &
     , ntracblck,xtblck, xtteblck)
    ...
    REAL(DP), DIMENSION(:,:,:,:,:),  POINTER,     OPTIONAL :: xt_tl
    ...
    IF (PRESENT(xt_tl))  xt_tl  => trset(zid)%xt_tl
    IF (PRESENT(ntracblck)) ntracblck =  trset(zid)%nblck
    IF (PRESENT(xtblck))    xtblck    => trset(zid)%xtblck
    IF (PRESENT(xtteblck))  xtteblck  => trset(zid)%xtteblck
  ...
  SUBROUTINE get_tracer_set_by_id(status, setid, setname, trlist, ti, ntrac &
    ...
    IF (PRESENT(xt_tl))   xt_tl     => trset(setid)%xt_tl
    IF (PRESENT(ntracblck)) ntracblck =  trset(setid)%nblck
    IF (PRESENT(xtblck))    xtblck    => trset(setid)%xtblck
    IF (PRESENT(xtteblck))  xtteblck  => trset(setid)%xtteblck
  ...
  SUBROUTINE setup_tracer_set(status, setname, dim, nt, l_tfstd, l_init &
    ...
    LOGICAL,               INTENT(IN), OPTIONAL :: l2tls
  ...
  SUBROUTINE print_tracer_set
    ...
    WRITE(form,*) '(1x,i3,1x,a15,1x,a8,1x,3(i1,1x),',MAX_CASK_I,'(1x,i6))'
  ...
  SUBROUTINE get_tracer_by_name(status, setname, basename &
       , subname, idx, fullname, longname, standardname   &
       , unit, submodel &
       , medium, quantity, type                           &
       , trinfo, pxt, pxtm1, pxtte, pxmem                 &
       , idxblck, pxtblck, pxtteblck)
    ...
    IF (PRESENT(standardname)) standardname  = ''
    ...
    IF (PRESENT(standardname)) THEN
       IF (LEN(standardname) > STRLEN_ULONG) THEN
          ...
          RETURN
       END IF
       standardname = TRIM(ti%info%ident%standardname)
    END IF
    !
    IF (PRESENT(unit)) THEN              ! UNIT
       IF (LEN(unit) > STRLEN_MEDIUM) THEN
  ...
  SUBROUTINE get_tracer_by_id(status, setname, idtrac, basename & 
       , subname, fullname, longname, standardname, unit, submodel &
       , medium, quantity, type                              &
       , trinfo, pxt, pxtm1, pxtte, pxmem                    &
       , idx, idxblck, pxtblck, pxtteblck)
...
!!$    ti%info%meta%linit = linit
  ...
  FUNCTION tracer_error_str(status)
    ...
    CASE('0209')
       tracer_error_str = 'E'//echar//': TRACER STANDARDNAME TOO LONG'
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE set_tracer_properties(extstatus, QTPROP, lprint, dom_id)

    USE messy_main_tools, ONLY: strcrack, ucase, split_name_domain, int2str
    ...
    nmlprop: DO i=1, nmax
       ...
       ntrac: DO j=1, n
          ...
          nset: DO k=1, m
             ...
             ! SPECIAL HOOK TO TAKE INTO ACCOUNT THE DOM_ID, IF ANY
             ! 1) check, if tracer_set is already specific with dom_id
             CALL split_name_domain(status, TRIM(trs(k)), zts, ireqpatch)
             IF (status == 0) THEN
                ! tracer set name is specific for patch ireqpatch
                IF (PRESENT(dom_id)) THEN
                   ! requested patch and actual patch mismatch
                   IF (ireqpatch /= dom_id) CYCLE
                ELSE
             ...
             IF (.NOT. lfound) THEN
                ...
                CASE('standardname','standard_name')
                   CALL set_tracer(status, TRIM(zts), idx &
                        , standardname = ZTPROP(i)%cont)
                   IF (status /= 0 .AND. llprint) &
                        WRITE(*,*) substr//' (9): '//&
                        &tracer_error_str(status)//&
                        &' [ set/tracer = '//TRIM(zts)//'/',idx,' ]'
  ...
  SUBROUTINE main_tracer_read_nml_ctrl(status, iou)
    ...
    NAMELIST /CTRL/ l_family, l_pdef, TPROP
