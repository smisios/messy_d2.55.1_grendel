...
! **********************************************************************
  ...
  PUBLIC :: loc_dimvar          ! locate pointer to attribute
...
CONTAINS
  ...
  SUBROUTINE add_dimvar_att(status, list, name, aname, i, c, r &
    ...
    CALL add_attribute(status, dimvar%att, aname, i, c, r &
         , loverwrite, iflag, lout)
    IF (status /= 0) RETURN
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE loc_dimvar(status, list, name, dimvar, l1stnamepart)
    ...
    CHARACTER(LEN=*),       INTENT(IN)           :: name
    TYPE(t_dimvar),         POINTER              :: dimvar
    LOGICAL,                INTENT(IN), OPTIONAL :: l1stnamepart
    ...
    LOGICAL                      :: lpart
    INTEGER                      :: nlen
    CHARACTER(LEN=STRLEN_MEDIUM) :: nam

    ! INIT
    lex = .FALSE.
    IF (PRESENT(l1stnamepart)) THEN
       lpart = l1stnamepart
    ELSE
       lpart =.FALSE.
    END IF
    ...
    ELSE
       ...
       DO
          IF (.NOT. ASSOCIATED(ai)) EXIT
          IF (TRIM(nam) == ai%this%name(1:nlen) .AND. &
               ai%this%name(nlen+1:nlen+1) == '_' ) THEN
             lex = .TRUE.
             EXIT
          END IF
          ae => ai
