MODULE messy_aeropt_tanre

!- Description:
!  This module contains a global aerosol climatology following Tanre(). 
!  
!  It is extracted from the ECHAM5 routines for the Tanre aerosol routines
!
...
!  H. Tost, Uni Mainz, Aug 2011 extraction into aeropt submodel


!  This module further contains:
!
!  A) the constant naer = 5 = number of aerosol mixture types
!  B) spectral coefficients for the horizontal spatial distribution
!     of sea, land, urban and desert aerosols
...
! A)
  ...
  INTEGER, PUBLIC, PARAMETER :: naer=5 ! number of aerosol mixture types
                               ! 1: land + desert + tropospheric background
                               ! 2: sea
                               ! 3: urban
                               ! 4: volcanic background
...
! D)
  ...
  REAL(dp),DIMENSION(4,naer) :: taua=RESHAPE((/&
       & 0.730719_dp, 0.730719_dp, 0.730719_dp, 0.730719_dp,&
       & 0.912819_dp, 0.912819_dp, 0.912819_dp, 0.912819_dp,&
       & 0.725059_dp, 0.725059_dp, 0.725059_dp, 0.725059_dp,&
       & 0.745405_dp, 0.745405_dp, 0.745405_dp, 0.745405_dp,&
       & 0.682188_dp, 0.682188_dp, 0.682188_dp, 0.682188_dp /)&
       &,SHAPE=(/4,naer/))

  ! sinle scattering albedo
  REAL(dp),DIMENSION(4,naer) :: piza=RESHAPE((/&
       & 0.872212_dp, 0.872212_dp, 0.872212_dp, 0.872212_dp,&
       & 0.982545_dp, 0.982545_dp, 0.982545_dp, 0.982545_dp,&
       & 0.623143_dp, 0.623143_dp, 0.623143_dp, 0.623143_dp,&
       & 0.944887_dp, 0.944887_dp, 0.944887_dp, 0.944887_dp,&
       & 0.997975_dp, 0.997975_dp, 0.997975_dp, 0.997975_dp /)&
       &,SHAPE=(/4,naer/))

  ! assymetry factor
  REAL(dp),DIMENSION(4,naer) :: cga=RESHAPE((/&
       & 0.647596_dp, 0.647596_dp, 0.647596_dp, 0.647596_dp,&
       & 0.739002_dp, 0.739002_dp, 0.739002_dp, 0.739002_dp,&
       & 0.580845_dp, 0.580845_dp, 0.580845_dp, 0.580845_dp,&
       & 0.662657_dp, 0.662657_dp, 0.662657_dp, 0.662657_dp,&
       & 0.624246_dp, 0.624246_dp, 0.624246_dp, 0.624246_dp /)&
       &,SHAPE=(/4,naer/))
...
!=======================================================================
  ...
  REAL(dp), POINTER, PUBLIC :: zaes_x(:,:) => NULL()
  REAL(dp), POINTER, PUBLIC :: zael_x(:,:) => NULL()
  REAL(dp), POINTER, PUBLIC :: zaeu_x(:,:) => NULL()
  REAL(dp), POINTER, PUBLIC :: zaed_x(:,:) => NULL()

  REAL(dp), POINTER, PUBLIC :: zaes(:) => NULL()
  REAL(dp), POINTER, PUBLIC :: zael(:) => NULL()
  REAL(dp), POINTER, PUBLIC :: zaeu(:) => NULL()
  REAL(dp), POINTER, PUBLIC :: zaed(:) => NULL()
  
  LOGICAL, PUBLIC :: tanre_init_su = .FALSE.
  LOGICAL, PUBLIC :: tanre_init    = .FALSE.

  PUBLIC :: su_aero_tanre,   calc_aero_tanre,  &
            init_aero_tanre, cleanup_aero_tanre, &
            init_aero_tanre_dc
  PUBLIC :: sw_optics_tanre,  lw_optics_tanre
!=======================================================================
!
! C)
...
!
! F)
  FUNCTION calc_aero_tanre(jrow,kproma,klev,pdp,ppf,pth)
  ...
  REAL(dp), INTENT(in) , DIMENSION(kproma,klev)    :: pdp,ppf
  REAL(dp), INTENT(in) , DIMENSION(kproma,klev+1)  :: pth
  REAL(dp), DIMENSION(kproma,klev,naer) :: calc_aero_tanre

  REAL(dp), DIMENSION(kproma,klev,naer) :: zaero
  ...
  zaes  => zaes_x  (1:kproma,jrow)
  zael  => zael_x  (1:kproma,jrow)
  zaeu  => zaeu_x  (1:kproma,jrow)
  zaed  => zaed_x  (1:kproma,jrow)

  ! output: aerosol longitude height sections in calc_aero_tanre
  ...
  DO jk = 1, klev
    DO jl = 1, kproma
      ...
      ELSE
         zaetrn(jl) = zaetro(jl)*(MIN(1._dp,pth(jl,jk)/pth(jl,jk+1)))**ctrpt
  ...
  calc_aero_tanre(1:kproma,:,:)=MAX(zaero(1:kproma,:,:),0._dp) ! aerosol is not negative

  END FUNCTION calc_aero_tanre

!==============================================================================================

  SUBROUTINE init_aero_tanre(nglon, nglat, iglat, ilof, &
                             coslon, sinlon, gl_twomu,  &
                             laes, lael, laeu, laed )

    INTEGER,  INTENT(IN)                             :: nglon, nglat
    INTEGER,  INTENT(IN), DIMENSION(NGLAT)           :: iglat, ilof
    REAL(dp), INTENT(IN)                             :: coslon(:)
    REAL(dp), INTENT(IN)                             :: sinlon(:)
    REAL(dp), INTENT(IN)                             :: gl_twomu(:)

    REAL(dp), INTENT(INOUT), DIMENSION(NGLON, NGLAT) :: laes
    REAL(dp), INTENT(INOUT), DIMENSION(NGLON, NGLAT) :: lael
    REAL(dp), INTENT(INOUT), DIMENSION(NGLON, NGLAT) :: laeu
    REAL(dp), INTENT(INOUT), DIMENSION(NGLON, NGLAT) :: laed
    ...
    INTEGER :: jmm, imm, imnc, imns, jnn, jl, jlat, jglat, nlof

    !  legtri subroutine copied into this module
  ...
  DO jlat = 1, nglat
    ...
    jglat = iglat(jlat)
    nlof  = ilof(jlat) ! longitude offset to global field
    ...
    DO jl = 1, nglon
      ...
      laes(jl,jlat) = zfaes(1) + 2._dp*(zfaes(2)*zcos1+zfaes(3)*zsin1+zfaes(4)*zcos2+   &
                      ...
                      zfaes(21)*zsin10)
      lael(jl,jlat) = zfael(1) + 2._dp*(zfael(2)*zcos1+zfael(3)*zsin1+zfael(4)*zcos2+   &
                      ...
                      zfael(21)*zsin10)
      laeu(jl,jlat) = zfaeu(1) + 2._dp*(zfaeu(2)*zcos1+zfaeu(3)*zsin1+zfaeu(4)*zcos2+   &
                      ...
                      zfaeu(21)*zsin10)
      laed(jl,jlat) = zfaed(1) + 2._dp*(zfaed(2)*zcos1+zfaed(3)*zsin1+zfaed(4)*zcos2+   &
  ...
  END SUBROUTINE init_aero_tanre
!------------------------------------------------------------------------------
  SUBROUTINE init_aero_tanre_dc(kproma, coslon, sinlon, sinlat,  &
                             laes, lael, laeu, laed )

    INTEGER,  INTENT(IN)                             :: kproma
    REAL(dp), INTENT(IN)                             :: coslon(:)
    REAL(dp), INTENT(IN)                             :: sinlon(:)
    REAL(dp), INTENT(IN)                             :: sinlat(:)

    REAL(dp), INTENT(INOUT), DIMENSION(kproma) :: laes
    REAL(dp), INTENT(INOUT), DIMENSION(kproma) :: lael
    REAL(dp), INTENT(INOUT), DIMENSION(kproma) :: laeu
    REAL(dp), INTENT(INOUT), DIMENSION(kproma) :: laed
    ...
    DO jl = 1, kproma
       ...
       ! Fourier transform
          ...
          laes(jl) = zfaes(1) + 2._dp*(zfaes(2)*zcos1+zfaes(3)*zsin1+zfaes(4)*zcos2+   &
               zfaes(5)*zsin2+zfaes(6)*zcos3+zfaes(7)*zsin3+zfaes(8)*zcos4+      &
               zfaes(9)*zsin4+zfaes(10)*zcos5+zfaes(11)*zsin5+zfaes(12)*zcos6+   &
               zfaes(13)*zsin6+zfaes(14)*zcos7+zfaes(15)*zsin7+zfaes(16)*zcos8+  &
               zfaes(17)*zsin8+zfaes(18)*zcos9+zfaes(19)*zsin9+zfaes(20)*zcos10+ &
               zfaes(21)*zsin10)
          lael(jl) = zfael(1) + 2._dp*(zfael(2)*zcos1+zfael(3)*zsin1+zfael(4)*zcos2+   &
               zfael(5)*zsin2+zfael(6)*zcos3+zfael(7)*zsin3+zfael(8)*zcos4+      &
               zfael(9)*zsin4+zfael(10)*zcos5+zfael(11)*zsin5+zfael(12)*zcos6+   &
               zfael(13)*zsin6+zfael(14)*zcos7+zfael(15)*zsin7+zfael(16)*zcos8+  &
               zfael(17)*zsin8+zfael(18)*zcos9+zfael(19)*zsin9+zfael(20)*zcos10+ &
               zfael(21)*zsin10)
          laeu(jl) = zfaeu(1) + 2._dp*(zfaeu(2)*zcos1+zfaeu(3)*zsin1+zfaeu(4)*zcos2+   &
               zfaeu(5)*zsin2+zfaeu(6)*zcos3+zfaeu(7)*zsin3+zfaeu(8)*zcos4+      &
               zfaeu(9)*zsin4+zfaeu(10)*zcos5+zfaeu(11)*zsin5+zfaeu(12)*zcos6+   &
               zfaeu(13)*zsin6+zfaeu(14)*zcos7+zfaeu(15)*zsin7+zfaeu(16)*zcos8+  &
               zfaeu(17)*zsin8+zfaeu(18)*zcos9+zfaeu(19)*zsin9+zfaeu(20)*zcos10+ &
               zfaeu(21)*zsin10)
          laed(jl) = zfaed(1) + 2._dp*(zfaed(2)*zcos1+zfaed(3)*zsin1+zfaed(4)*zcos2+   &
               zfaed(5)*zsin2+zfaed(6)*zcos3+zfaed(7)*zsin3+zfaed(8)*zcos4+      &
               zfaed(9)*zsin4+zfaed(10)*zcos5+zfaed(11)*zsin5+zfaed(12)*zcos6+   &
               zfaed(13)*zsin6+zfaed(14)*zcos7+zfaed(15)*zsin7+zfaed(16)*zcos8+  &
               zfaed(17)*zsin8+zfaed(18)*zcos9+zfaed(19)*zsin9+zfaed(20)*zcos10+ &
  ...
  END SUBROUTINE init_aero_tanre_dc
...
!------------------------------------------------------------------------------
  SUBROUTINE lw_optics_tanre(ptauaer, klev, kproma, paer)

   !============================================================================
   !
   !  Description:
   !
   !  Calculate aerosol optical thickness for Tanre aerosol climatology
   !  for each spectral band (longwave) of the ECHAM5 radiation scheme.
   !
   !  Code adopted from ECHAM5 SUBROUTINE RAD_INT.
   !
   !============================================================================

   USE messy_aeropt_mem,       ONLY: jpband ! # of spectral bands in E5
                                            ! longwave radiation sheme

   ! Parameters (in)

   INTEGER,  INTENT(in) :: klev     ! # of levels
   INTEGER,  INTENT(in) :: kproma   ! # of grid cells in arrays currently in use

   REAL(dp), INTENT(in) :: paer(kproma,klev,naer) ! aerosol loading

   ! Parameters (out)

   REAL(dp), INTENT(out) :: ptauaer(kproma,klev,jpband) ! AOT

   ! Local variables

   INTEGER  :: jk, jaer, jl ! loop indices

   REAL(dp) :: ztauaer(kproma,5)  ! aerosol optical properties are given in
                                  ! 5 bands of the JJM scheme

   ! Calculations start here...

   ! ... calculate aerosol optical thickness

   DO jk = 1, klev
      ztauaer(1:kproma,:) = 0.0_dp
      ! loop over Tanre aerosol types
      DO jaer = 1, naer
         DO jl = 1, kproma
            ! optical thickness of naer Tanre aerosols in 5 JJM bands
      ...
      DO jl = 1, kproma
         ...
         ptauaer(jl,jk, 1) = ztauaer(jl,1)
         ptauaer(jl,jk, 2) = ztauaer(jl,5)
         ptauaer(jl,jk, 3) = ztauaer(jl,2)
         ptauaer(jl,jk, 4) = ztauaer(jl,2)
         ptauaer(jl,jk, 5) = ztauaer(jl,2)
         ptauaer(jl,jk, 6) = ztauaer(jl,3)
         ptauaer(jl,jk, 7) = ztauaer(jl,4)
         ptauaer(jl,jk, 8) = ztauaer(jl,3)
         ptauaer(jl,jk, 9) = ztauaer(jl,1)
         ptauaer(jl,jk,10) = ztauaer(jl,1)
         ptauaer(jl,jk,11) = ztauaer(jl,1)
         ptauaer(jl,jk,12) = ztauaer(jl,1)
         ptauaer(jl,jk,13) = ztauaer(jl,1)
         ptauaer(jl,jk,14) = ztauaer(jl,1)
         ptauaer(jl,jk,15) = ztauaer(jl,1)
         ptauaer(jl,jk,16) = ztauaer(jl,1)
      END DO
   END DO

 END SUBROUTINE lw_optics_tanre

! ******************************************************************************

 SUBROUTINE sw_optics_tanre(ptauaz, ppizaz, pcgaz, klev,  &
                            kproma, paer)
   ...
   !  Description:
   !
   !  Calculate aerosol optical thickness, single scattering albedo, and
   !  assymetry factor for Tanre aerosol climatology for each spectral band
   ! (shortwave) of the ECHAM5 radiation scheme.
   !
   !  Code adopted from ECHAM5 SUBROUTINE SWCLR.
   !
   !============================================================================


   USE messy_aeropt_mem,       ONLY: nsw  ! # of spectral bands in ECHAM5
                                          ! shortwave radiation sheme
   ...
   REAL(dp), INTENT(out) :: ptauaz(kproma,klev,nsw) ! AOT
   REAL(dp), INTENT(out) :: ppizaz(kproma,klev,nsw) ! single scattering albedo
   REAL(dp), INTENT(out) :: pcgaz(kproma,klev,nsw)  ! assymetry factor

   ! Local variables

   INTEGER jk, jl, jae ! loop indices
   INTEGER ikl, inu
   ...
   DO INU = 1, NSW
      DO JK = 1, KLEV
         IKL=KLEV+1-JK
         DO JL = 1, KPROMA
            PCGAZ(JL,JK,INU)  = 0._DP
            PPIZAZ(JL,JK,INU) = 0._DP
            PTAUAZ(JL,JK,INU) = 0._DP
         END DO
         DO JAE = 1, NAER
            DO JL = 1, KPROMA
               PTAUAZ(JL,JK,INU) = PTAUAZ(JL,JK,INU) + PAER(JL,IKL,JAE)  &
                                   * TAUA(INU,JAE)
               PPIZAZ(JL,JK,INU) = PPIZAZ(JL,JK,INU) + PAER(JL,IKL,JAE)  &
                                   * TAUA(INU,JAE) * PIZA(INU,JAE)
               PCGAZ(JL,JK,INU) =  PCGAZ(JL,JK,INU) + PAER(JL, IKL ,JAE) &
                                   * TAUA(INU,JAE) * PIZA(INU,JAE) * CGA(INU,JAE)
            END DO
         END DO

         DO JL = 1, KPROMA
            PCGAZ(JL,JK,INU)  = PCGAZ(JL,JK,INU)  / PPIZAZ(JL,JK,INU)
            PPIZAZ(JL,JK,INU) = PPIZAZ(JL,JK,INU) / PTAUAZ(JL,JK,INU)
         END DO
      END DO
   END DO

 END SUBROUTINE sw_optics_tanre
