...
!qqq-
  ...
  SUBROUTINE ch_pnetcdf_write_header(status, IOMODE, channel, dim_time, att)
    ...
    USE messy_main_channel_attributes, ONLY: t_attribute_list
    USE messy_main_channel,            ONLY: update_cell_methods_attribute &
                                           , delete_cell_methods_attribute
    ...
    CALL new_attribute(status, channel%att, 'channel_netcdf_lib' &
         , c = NFMPI_inq_libvers(), loverwrite=.TRUE., lout=.TRUE.)
    IF (status /= 0) RETURN
    ...
    object_loop1: DO
       ...
       DO i=1, IRANK
          IF (ASSOCIATED(object%repr%dim(i)%ptr)) THEN
             IF (object%repr%dim(i)%ptr%lreduceio) CYCLE
    ...
    object_loop2: DO
       ...
       iorank = 0
       DO i=1, nrank
          IF (object%int%netcdf(IOMODE)%dimid(object%repr%order_mem2out(i)) &
               == NC_ID_UNDEF) CYCLE
          iorank = iorank + 1
          dimvec(iorank) = object%int%netcdf(IOMODE)%dimid( &
               object%repr%order_mem2out(i) )
       END DO

       IF (IOMODE == IOMODE_OUT) THEN
          IF (object%lstatic) THEN
             it = 0
          ELSE
             it = 1
             dimvec(iorank+1) = channel%int%netcdf(IOMODE)%dimid_time
          END IF
          XNF90_PREC = PNCOUT_PREC
          IF (object%ldp) XNF90_PREC = NF_DOUBLE
       ELSE
          it = 0
          XNF90_PREC = NF_DOUBLE
       END IF
       ...
       DO jsnd=1, SND_MAXLEN
          ...
          IF (IOMODE ==IOMODE_OUT) &
               CALL update_cell_methods_attribute(status &
               , object%att, jsnd, object%lstatic)
          IF (status /= 0) RETURN

          ! WRITE VARIABLE ATTRIBUTES
          CALL pnetcdf_write_attribute_list(status, ncid, varid, &
               object%att, XNF90_PREC)
          IF (status /= 0) RETURN

          ! update attribute list
          IF (IOMODE ==IOMODE_OUT) &
               CALL delete_cell_methods_attribute(status, object%att)
          IF (status /= 0) RETURN

          ! WRITE SPECIAL ATTRIBUTES FOR SECONDARY DATA
          IF ((jsnd == SND_CNT) .OR. (jsnd == SND_CAV)) THEN
             SELECT CASE(XNF90_PREC)
             CASE(NF_DOUBLE)
                CALL pnf(NFMPI_PUT_ATT_DOUBLE(ncid, varid, 'range' &
                     , NF_DOUBLE, INT(2, MPI_OFFSET_KIND) &
                     , object%io%range) &
                     , status, substr)
                IF (status /= 0) RETURN
                CALL pnf(NFMPI_PUT_ATT_DOUBLE(ncid, varid, '_FillValue' &
                     , NF_DOUBLE, INT(1,MPI_OFFSET_KIND) &
                     , FLAGGED_BAD) &
                     , status, substr)
                IF (status /= 0) RETURN
             CASE(NF_FLOAT)
                CALL pnf(NFMPI_PUT_ATT_REAL(ncid, varid, 'range' &
                     , NF_FLOAT, INT(2, MPI_OFFSET_KIND) &
                     , REAL(object%io%range)) &
                     , status, substr)
                IF (status /= 0) RETURN
                CALL pnf(NFMPI_PUT_ATT_REAL(ncid, varid, '_FillValue' &
                     , NF_FLOAT, INT(1,MPI_OFFSET_KIND) &
                     , REAL(FLAGGED_BAD)) &
                     , status, substr)
                IF (status /= 0) RETURN
             END SELECT
  ...
  SUBROUTINE ch_pnetcdf_write_data(status, IOMODE, channel, object &
    ...
    DO i=1, nrank
       lio(i) = &
            (object%int%netcdf(IOMODE)%dimid(object%repr%order_mem2out(i)) &
            /= NC_ID_UNDEF)
       IF (.NOT. lio(i)) CYCLE
       iorank = iorank + 1
    END DO
    ...
    CASE(IOMODE_OUT)
       ...
       ELSE
          it = 1
          start(iorank + 1) = channel%int%ntpfcnt  ! OUTPUT STEP IN netCDF FILE
          cnt(iorank + 1)   = 1                    ! 1 TIME STEP
       END IF
    CASE(IOMODE_RST)
...
!!$       cnt(1:nrank)   = object%repr%pdecomp%cnt(iseg,1:nrank)
          ...
          DO i=1,nrank
             IF (.NOT. lio(i)) CYCLE
             ii = ii + 1
             start(ii) = object%repr%pdecomp%start(iseg,i)
             cnt(ii)   = object%repr%pdecomp%cnt(iseg,i)
          END DO
...
!!$                  , cnt(1:nrank+it)     &
             ...
             ELSE
                IF (l_cheat) THEN
                   IF (I_VERBOSE_LEVEL >= 1) &
                        ...
                        ,'    WARNING: REQUIRED RESTART VARIABLE ', &
                        'NOT PRESENT! ' &
                        ,'HOWEVER: SKIPPED FOR TRACER INIT FROM RESTART FILE'
                   status = 0
                ELSE
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE pnetcdf_write_attribute_list(status, ncid, varid, list, prec)
    ...
    DO
       ...
       lskip = (.NOT. att%lout) .AND. &
               (att%iflag /= AF_RST_CMP) .AND. (att%iflag /= AF_RST_INP)
       IF (lskip) THEN
          ai => ai%next
          CYCLE
       ENDIF
       ...
       CASE(TYPE_REAL_DP)
          SELECT CASE(prec)
          CASE(NF_FLOAT)
             CALL pnf(NFMPI_PUT_ATT_REAL(ncid, varid &
                  , TRIM(att%name), NF_FLOAT, INT(1,MPI_OFFSET_KIND) &
                  , REAL(att%r)) &
                  , status, substr)
          CASE(NF_DOUBLE)
  ...
  SUBROUTINE pnf(sin, sout, substr)
    ...
    IF (sin /= NF_NOERR) THEN
       WRITE(iouerr,*) TRIM(substr),': *** parallel-netCDF ERROR on PE ',p_pe &
            ,' : ',NFMPI_STRERROR(sin)
