...
CONTAINS
  ...
  SUBROUTINE manager_init_days (manager, name, date, incr, message)
    ...
    IMPLICIT NONE
    INTRINSIC :: LEN, MIN, PRESENT, TRIM
    !
  ...
  SUBROUTINE manager_reinit_days (manager, new_date, ierr, message)
    ...
    REAL(dp)         :: zsecs
    INTEGER          :: iday1, isec1, iday2, isec2, ims1, ims2, new_step
    CHARACTER(LEN=STRLEN_ULONG) :: message_text
    ...
    ELSE
       ...
       CALL date_get (current_date, iday1, isec1, ims1, ierr)
       IF (ierr /=0 ) RETURN
       CALL date_get (new_date,     iday2, isec2, ims2, ierr)
       IF (ierr /=0 ) RETURN
       zsecs = OneDay*REAL((iday1-iday2),dp) + REAL((isec1-isec2),dp) &
            + REAL((ims1-ims2),dp)/1000._dp

       ! get new manager step
       new_step = INT((zsecs+0.00001_dp)/manager%delta_time)
  ...
  SUBROUTINE manager_reinit_incr (manager, incr, ierr, message)
    ...
    ELSE
      ...
      idays = INT((zsecs+0.00001_dp)/OneDay)
      zsecs = zsecs - REAL(idays,dp)*OneDay
      ...
      CALL add_date (idays, zsecs, current_date, ierr)
      IF (ierr /=0 ) RETURN
  ...
  SUBROUTINE manager_any_date (manager, date, point, ierr)
    ...
    IF (point < 0) THEN
      zsecs = REAL(point,dp)*manager%delta_time
    ...
    CALL add_date (idays, zsecs, date,  ierr)
  ...
  SUBROUTINE manager_step_at_date (manager, current_date, steps, lfit, ierr)
    ...
    INTEGER          :: iday1, isec1, iday2, isec2, ims1, ims2, iday
    REAL(dp)         :: zsecs, rsecond

    ierr = 0
    ! calculate time difference in seconds
    CALL date_get (current_date,       iday1, isec1,ims1,ierr)
    IF (ierr /=0 ) RETURN 
    CALL date_get (manager%start_date, iday2, isec2,ims2,ierr)
    IF (ierr /=0 ) RETURN 
    ...
    IF (zsecs < 0.0_dp) THEN
      steps = INT( (zsecs-0.0001_dp) /manager%delta_time)
    ELSE
      steps = INT( (zsecs+0.0001_dp)/manager%delta_time)
    END IF
    ...
    IF (zsecs < 0.0_dp) THEN
       iday    = INT( (zsecs- 0.0001_dp)/OneDay)
    ELSE
       iday    = INT( (zsecs+ 0.0001_dp)/OneDay)
    END IF
    rsecond = zsecs - REAL(iday,dp) *OneDay

    CALL copy_date(manager %start_date, check_date, ierr)
    IF (ierr /= 0) RETURN

    CALL add_date (iday, rsecond, check_date,ierr)
    IF (ierr /= 0) RETURN
...
! ------------------------------------------------------------------------
  ...
  SUBROUTINE timer_rewind_manager (manager, new_date, fix_start_date, ierr &
    ...
    ! LOCAL
    INTEGER          :: isecs, ims, idays, new_step, old_step, offset
    TYPE (time_days) :: wrk_day
    LOGICAL          :: lfit, learlier
    INTEGER          :: ierror
    REAL(dp)         :: zsecs ! ub_ak_20181219
    ...
    ELSE
       ...
       IF (ierror /= 0) THEN
          ...
          RETURN
       ENDIF
       CALL date_get(wrk_day, idays, isecs, ims, ierror)
       IF (ierror /= 0) THEN
          IF (PRESENT(ierr)) ierr=ierror
          RETURN
       ENDIF
       ...
       zsecs = REAL(isecs,dp) + REAL(ims,dp) / 1000._dp

       CALL add_date (-idays, -zsecs, wrk_day, ierror) 
       IF (ierror /= 0) THEN
          IF (PRESENT(ierr)) ierr=ierror
          RETURN
       ENDIF
       ...
       CALL add_date (idays,zsecs,manager%start_date, ierror)
       IF (ierror /= 0) THEN
          IF (PRESENT(ierr)) ierr=ierror
          RETURN
       ENDIF
