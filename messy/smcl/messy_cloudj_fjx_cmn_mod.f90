...
! !INTERFACE:
!
      MODULE messy_cloudj_fjx_cmn_mod

        USE messy_cmn_photol_mem, ONLY: JVN_ => IP_MAX
      implicit none
      public
...
!---- Variables in file 'FJX_spec.txt' (RD_XXX)
      ...
      ! TITLEJX: Title (short & long) for supplied cross sections, from 'FJX_spec.txt'
      CHARACTER*6  TITLEJX(X_)
      CHARACTER*16 TITLEJL(X_)
      ! SQQ: Flag for supplied cross sections, from 'FJX_spec.txt'
      CHARACTER*1  SQQ(X_)

!---- Variables in file 'FJX_scat-aer.txt' (RD_MIE)
...
!---- Variables in file 'FJX_scat-cld.txt' (RD_CLD)
...
!---- Variables in file 'FJX_scat-UMa.txt' (RD_CLD)
...
!---- Variables in file 'atmos_std.txt' (RD_PROF)
...
!
      integer, parameter :: &
!               LPAR   = NLEV   &   ! set by CTM code
!             , LWEPAR = NLEV -3  &    ! this can be set by CTM code
!             , L_     = LPAR  &
...
!--- trtr next line has to be changed 
!            , 
               ! mz_rs_20160927+
               ! get JVN_ from  IP_MAX in messy_cmn_photol_mem
               ! JVN_   = 108 &      ! max no. of J-values
               ! mz_rs_20160927-
...
!-------data to set up the random number sequence for use in cloud-JX
      ...
      END MODULE messy_cloudj_fjx_cmn_mod
