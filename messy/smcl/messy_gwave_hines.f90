...
MODULE messy_gwave_hines
  ...
  ! mz_ab_20100829+
  CHARACTER(LEN=*), PARAMETER, PUBLIC :: submodstr = 'gwave_hines'
  CHARACTER(LEN=*), PARAMETER, PUBLIC :: submodver = '5.302'
  ! mz_ab_20100829-
  ...
  LOGICAL :: lozpr    = .FALSE.
  INTEGER :: iheatcal =   0
  LOGICAL :: lrmscon_lat = .FALSE. ! gr_sm_20160810
  ...
  REAL(dp) :: emiss_press = 64290.0_dp   ! mz_jb_20040107
  ...
  ! gr_sm_20160810+
  ! latitude dependent rmscon
  REAL(dp) :: lat_rmscon_lo = 1.0_dp
  REAL(dp) :: lat_rmscon_hi = 1.0_dp
  REAL(dp) :: rmscon_lo = 20.0_dp
  REAL(dp) :: rmscon_hi = 20.0_dp
  ! gr_sm_20160810-

  ! ka_sv_20170710+: now namelist parameters
  INTEGER  :: icutoff    = 0
  ...
  INTEGER  :: emiss_lev   = -9999   ! mz_jb_20040107
  INTEGER  :: naz   = 8
  ...
  ! gr_sm_20160810+
  REAL(dp), DIMENSION(:), ALLOCATABLE :: rmscon_lat ! latitude dependent rmscon
  ! gr_sm_20160810-

  !-----------------------------------------------------------------

  PUBLIC :: rms_front, front_thres, kstar, &
       m_min, emiss_lev, pcrit, pcons, lextro, &
       lfront, rmscon, emiss_press, lozpr, iheatcal, &
       lrmscon_lat, rmscon_lat, rmscon_lo, rmscon_hi, & ! gr_sm_20160810
       lat_rmscon_lo, lat_rmscon_hi, &                  ! gr_sm_20160810
       icutoff, alt_cutoff                              ! ka_sv_20170710

  PUBLIC :: gwspectrum
  PUBLIC :: hines_read_nml_ctrl
...
!       ,ptte,        pvol,        pvom           &
    ...
    !
    !
    USE messy_main_constants_mem, ONLY: cpd=>cp_air, rd, rhoh2o=>rho_H2O, g
!!$    USE mo_constants,         ONLY: cpd, rd, rhoh2o, g
!!$    USE mo_gwspectrum,        ONLY: lextro, lfront, lozpr, iheatcal, &
!!$                                    emiss_lev, rmscon, kstar, naz,  &
!!$                                    pcrit, pcons
!!$    USE messy_main_data_bi,   ONLY: gwdrag_u, gwdrag_v, gwflux_u, gwflux_v &
!!$                                  , jrow
    ...
    ! op_pj_20180724+
    ! Output 2D
    REAL(dp) ,INTENT(OUT) :: diffco(kproma,klev) ! diffusion coefficient (m^2/s)
    ! op_pj_20180724-
...
!!$    REAL(dp) ,INTENT(INOUT) :: pvom(kbdim,klev)  ! tendency of zonal wind
    ...
    REAL(dp), DIMENSION(:,:,:), POINTER :: gwdrag_u, gwdrag_v, gwheat &
                                         , gwflux_u, gwflux_v
    REAL(dp), DIMENSION(:,:,:), INTENT(IN) :: vom1, dm1 &
                                            , dtlm1, dtmm1, dudlm1, dvdlm1
    REAL(dp), DIMENSION(:,:),   INTENT(IN) :: dalpslm1, dalpsmm1, alpsm1
    REAL(dp), DIMENSION(:),     INTENT(IN) :: gl_twomu, gl_sqcst, vct
    INTEGER,  DIMENSION(:,:),   INTENT(IN) :: ilat
    INTEGER,                    INTENT(IN) :: nvclev
    ...
    ! zonal tend, gravity wave spectrum (m/s^2)
    REAL(dp), DIMENSION(:,:), POINTER :: utendgw
    ! merid tend, gravity wave spectrum (m/s^2)
    REAL(dp), DIMENSION(:,:), POINTER :: vtendgw

    ! mz_ab_20100902+
!!$    REAL(dp) :: ttendgw(kproma,klev) ! temperature tend, gravity wave spectrum (K/s)
    REAL(dp), DIMENSION(:,:), POINTER :: ttendgw ! temperature tend, gravity wave spectrum (K/s)
    ! mz_ab_20100902-
! op_pj_20180724+: diffco is now formal parameter
!!$ REAL(dp) :: diffco(kproma,klev)  ! diffusion coefficient (m^2/s)
! op_pj_20180724-

    ! zonal momentum flux (pascals)
    REAL(dp), DIMENSION(:,:), POINTER :: flux_u
    ! meridional momentum flux (pascals)
    REAL(dp), DIMENSION(:,:), POINTER :: flux_v
    ...
    LOGICAL  :: ledith     ! op_pj_20180724

    status = 1 ! ERROR

    ! op_pj_20180724+: extension above 0.1 Pa ? -> ionosphere/themrmosphere
    ...
    IF (lextro) THEN
       ...
       !
       ! mz_pj_20071126+
       utendgw => gwdrag_u(1:kproma,:,krow)
       vtendgw => gwdrag_v(1:kproma,:,krow)
       flux_u  => gwflux_u(1:kproma,:,krow)
       flux_v  => gwflux_v(1:kproma,:,krow)
       ! mz_pj_20071126-
       ttendgw => gwheat(1:kproma,:,krow) ! mz_ab_20100902
...
!!$    visc_mol(:,:) = 1.5e-5_dp
       IF (.NOT. ledith) THEN
          visc_mol(:,:) = 1.5e-5_dp 
       ELSE
          ! adopting approach from HAMMONIA
          DO jk=1,klev
             DO jl=1,kproma
                visc_mol(jl,jk) = 3.90E-7_dp*ptm1(jl,jk)**.69_dp &
                     / density(jl,jk)
             ENDDO
          ENDDO
       ENDIF
! ka_sv_20180712-
       ...
       DO jl=1,kproma
!!$       rmswind(jl) = rmscon ! gr_sm_20160810
       ...
       IF (lfront) THEN
          CALL gw_fronts(krow, kproma, nazmth, rmswind, anis, &
               ! mz_ab_20100829+
               gl_twomu, gl_sqcst, nvclev, vct, klev,       &
               dalpslm1, dalpsmm1, vom1, dm1, alpsm1, ptm1, &
               dtlm1, dtmm1, pum1, pvm1, dudlm1, dvdlm1,      &
               ilat)
               ! mz_ab_20100829-
       ENDIF
       ...
       CALL hines_extro ( kproma, klev, nazmth,                          &
                          ...
                          levbot, lorms, status) ! mz_ab_20100829
       ! mz_ab_20100829+
       IF (status /= 0) RETURN
       ! mz_ab_20100829-


! mz_ab_20100902 tendencies now updated in SMIL
!!$       !   update tendencies:
!!$       !
!!$       DO jk=1, klev
!!$          DO jl=1,kproma
...
! mz_ab_20100902
  ...
  SUBROUTINE hines_extro ( nlons, nlevs, nazmth,                          &
                           ...
                           rmswind, anis, k_alpha, sigsqmcw,               &
                           m_alpha,  mmin_alpha, sigma_t, sigmatm,        &
                           lev2,  lorms, status) ! mz_ab_20100829
    !
...
!!$                                 icutoff, alt_cutoff, smco, nsmax
    ...
    INTEGER, INTENT(OUT) :: status ! error status ! mz_ab_20100829
    !-----------------------------------------------------------------------
    !

    status = 1 ! ERROR ! mz_ab_20100829
    ...
    IF (icutoff.EQ.1)  THEN
       CALL hines_exp ( drag_u, bvfb, alt, alt_cutoff,  &
                        il1, il2, lev1, lev2, nlons, nlevs, status )
       IF (status /= 0) RETURN
       CALL hines_exp ( drag_v, bvfb, alt, alt_cutoff,  &
                        il1, il2, lev1, lev2, nlons, nlevs, status )
       IF (status /= 0) RETURN ! mz_ab_20100829
    END IF
    !
...
!!$    USE mo_gwspectrum,   ONLY: kstar, m_min, slope, f1, f2, f3, naz
    ...
    IF (levbot > levtop)  THEN
       ...
       lincr  = -1
    ELSE
       WRITE (*,*) ' Error: level index not increasing downward '
       ! mz_ab_20100829+
       status = 1001
       RETURN
...
!CDIR NODEP
  ...
  SUBROUTINE hines_heat ( heat, diffco,                                 &
    ...
    !     * losigma_t   = .true. for total sigma not zero
    !
    USE messy_main_constants_mem, ONLY: cpd=>cp_air
...
! Akmaev (JGR, 2001) and Akmaev et al. (Ann. Geoph., 1997)
             diffco(i,l) = MAX ( diffco(i,l) - visc_mol(i,l), 0. )
! ka_sv_20170410-
             heat(i,l)   = heatng / cpd
          ENDIF
       END DO
...
!CDIR NODEP
  ...
  SUBROUTINE hines_intgrl (i_alpha,                                     &
                           ...
                           lorms, do_alpha, status) ! mz_ab_20100829
    !
...
!CDIR NODEP
    ...
    DO n = 1,naz
       ...
       IF (ANY(lerror)) THEN
          WRITE (*,*)
          WRITE (*,*) '******************************'
          WRITE (*,*) 'hines integral i_alpha < 0 '
          WRITE (*,*) '  longitude i=',i
          WRITE (*,*) '  azimuth   n=',n
          WRITE (*,*) '  level   lev=',lev
          WRITE (*,*) '  i_alpha =',i_alpha(i,n)
          WRITE (*,*) '  v_alpha =',v_alpha(i,lev,n)
          WRITE (*,*) '  m_alpha =',m_alpha(i,lev,n)
          WRITE (*,*) '  q_alpha =',v_alpha(i,lev,n)*rbvfb(i)
          WRITE (*,*) '  qm      =',v_alpha(i,lev,n)*rbvfb(i)*m_alpha(i,lev,n)
          WRITE (*,*) '******************************'
          WRITE (*,*) ' Error: Hines i_alpha integral is negative  '
          ! mz_ab_20100829+
!!$          CALL finish(' hines_intgrl','Run terminated')
          status = 1002 ! ERROR: Hines i_alpha integral is negative
          RETURN
          ! mz_ab_20100829-
       END IF
...
!!$    USE mo_doctor, ONLY: nout
    ...
    DO i = ilprt1,ilprt2
       ...
       IF (iu_print.EQ.1)  THEN
          WRITE (*,*)
          WRITE (*,'(a,i3)') 'hines gw (east-west) at longitude i =',i
          WRITE (*,6005)
6005      FORMAT (15x,' u ',2x,'sig_e',2x,'sig_t',3x,'m_e',  &
               &            4x,'m_w',4x,'fluxu',5x,'gwdu')
          DO l = levprt1,levprt2
             WRITE (*,6701) alt(i,l)/1.e3_dp, v_alpha(i,l,n_east),   &
...
6701      FORMAT (' z=',f7.2,1x,3f7.1,2f7.3,f9.4,f9.3)
       ...
       IF (iv_print.EQ.1)  THEN
          WRITE(*,*)
          WRITE(*,'(a,i3)') 'hines gw (north-south) at longitude i =',i
          WRITE(*,6006)
6006      FORMAT (15x,' v ',2x,'sig_n',2x,'sig_t',3x,'m_n',   &
               &            4x,'m_s',4x,'fluxv',5x,'gwdv')
          DO l = levprt1,levprt2
             WRITE (*,6701) alt(i,l)/1.e3_dp, v_alpha(i,l,n_north),    &
  ...
  SUBROUTINE hines_exp (darr, data_zmax, alt, alt_exp,     &
                        il1, il2, lev1, lev2, nlons, nlevs, status) ! mz_ab_20100829
    !
...
!!$       CALL finish('hines_exp','Run terminated')
       status = 1003 ! ERROR: level index not increasing downward
       RETURN
       ! mz_ab_20100829-
    END IF
    !
  ...
  SUBROUTINE  gw_fronts(krow, kproma, nazmth, rmswind, ani, &
       ! mz_ab_20100829+
       gl_twomu, gl_sqcst, nvclev, vct, nlev,      &
       dalpslm1, dalpsmm1, vom1, dm1, alpsm1, tm1, &
       dtlm1, dtmm1, um1, vm1, dudlm1, dvdlm1,     &
       ilat)
       ! mz_ab_20100829-
    !
    !
...
!!$    USE mo_gwspectrum,     ONLY: rmscon, rms_front, front_thres, naz
    ...
    REAL(dp), DIMENSION(:,:,:), INTENT(IN) :: vom1, dm1 &
                                            , dtlm1, dtmm1, dudlm1, dvdlm1
    REAL(dp), DIMENSION(:,:),   INTENT(IN) :: dalpslm1, dalpsmm1, alpsm1, tm1, um1, vm1
    REAL(dp), DIMENSION(:),     INTENT(IN) :: gl_twomu, gl_sqcst, vct
    INTEGER,  DIMENSION(:,:),   INTENT(IN) :: ilat
    INTEGER,                    INTENT(IN) :: nvclev, nlev
    ! mz_ab_20100829-
    !
    !  internal variables.
    !
...
!!$    ttm1(:) = tm1 (:,iplev-1,jrow)
  ...
  SUBROUTINE hines_read_nml_ctrl(status, iou, modstr, nn)
    ...
    NAMELIST /CTRL_HINES/ lextro, lfront, lozpr, iheatcal, rmscon, emiss_press, &
         kstar, m_min, rms_front, front_thres,  pcrit,  pcons, &
         ! gr_sm_20160810+
         lrmscon_lat, rmscon_lo, rmscon_hi, lat_rmscon_lo, lat_rmscon_hi, &
         ! gr_sm_20160810-
         icutoff, alt_cutoff ! ka_sv_20170710-
    ...
    CALL read_nml_open(lex, substr, iou, 'CTRL_HINES', modstr)
    IF (.not.lex) RETURN    ! <modstr>.nml does not exist

    READ(iou, NML=CTRL_HINES, IOSTAT=fstat)
    CALL read_nml_check(fstat, substr, iou, 'CTRL_HINES', modstr)
    IF (fstat /= 0) RETURN  ! error while reading namelist

    ! DIAGNOSE NAMELIST
    IF (.NOT.lextro) THEN
       WRITE(*,*)    'lextro = .FALSE. --> no hines gw spectrum'
    ENDIF
    ...
    IF (lextro) THEN
       ...
       IF (lfront) THEN
          WRITE(*,*)    'lfront = .TRUE.  --> gravity waves from fronts'
          IF (nn==31)  THEN
             front_thres  = 0.10_dp
          ENDIF
          IF ( (nn /= 31) .AND. (nn /= 42) ) THEN
             status = 1004 ! ERROR: LFRONT=.true. not implemented for nn=', nn
             RETURN
          ENDIF
       ENDIF

       IF (lozpr) THEN
          status = 1005 ! ERROR: LOZPR=.TRUE. not implemented for maecham5
          RETURN
       ENDIF

       SELECT CASE(iheatcal)
       CASE(0)
          WRITE(*,*) 'standard maecham5: momentum flux deposition ONLY'
       CASE(1)
          WRITE(*,*) ' iheatcal = 1 : momentum deposion, heating and diff coeff'
          IF ( ABS(slope-1._dp) > EPSILON(1._dp) ) THEN
             status = 1006 ! ERROR: Heat and coeff for slope not 1 not implemented
             RETURN
          ENDIF
       END SELECT

       IF ( naz /= 8 )  then
             status = 1007 ! ERROR: naz not 8 not implemented
             RETURN
       ENDIF

       IF (ABS(slope-1._dp) > EPSILON(1._dp)  ) then
             status = 1008 ! ERROR: slope not 1. not implemented
             RETURN
       ENDIF

       IF ( (m_min > EPSILON(1._dp)) .AND. (ABS(slope-1._dp) > EPSILON(1._dp)) ) then
          status = 1009 ! ERROR:  m_min not zero for slope not 1. not implemented
          RETURN
       ENDIF

       IF ( smco < 1._dp ) then
          status = 1010 ! ERROR: smco less than 1. not implemented
          RETURN
       ENDIF
       ! gr_sm_20160810+
       ! set latitude dependent GW source
       IF ( lrmscon_lat ) THEN
          WRITE(*,*) "GWAVE: Latidudinal dependence of rmscon is enabled."
          IF (lfront .OR. lozpr) THEN
             WRITE(*,*) 'GWAVE: Latitude dependent GW source not supported'// &
  ...
  END SUBROUTINE hines_read_nml_ctrl

END MODULE messy_gwave_hines
