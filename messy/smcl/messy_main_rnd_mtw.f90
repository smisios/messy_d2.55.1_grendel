...
!  integer( kind = wi )  :: mti = n + 1_wi ! mti==N+1 means mt[N] is not initialized
  integer( kind = wi ), public  :: mt(n)          ! the array for the state vector
  logical( kind = wi ), public  :: mtinit = .false._wi ! means mt[N] is not initialized
  integer( kind = wi ), public  :: mti = n + 1_wi ! mti==N+1 means mt[N] is not initialized
! mz_pj_20060217-
  ...
  function genrand_int32( ) result( y )
    ...
    intrinsic :: iand, ishft, ieor, btest, mvbits
