...
! **********************************************************************
  ...
  INTEGER, SAVE :: NCOUT_PREC = NF90_FLOAT
...
CONTAINS
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE ch_netcdf_init_io(status, IOMODE, channel, AMODE, att, lclose)
...
!            , NF90_CLOBBER                           &  ! overwrite
  ...
  SUBROUTINE ch_netcdf_write_header(status, IOMODE, channel, dim_time, att)
    ...
    CALL new_attribute(status, channel%att, 'channel_netcdf_lib' &
         , c = nf90_inq_libvers(), loverwrite=.TRUE., lout=.TRUE.)
    IF (status /= 0) RETURN
    ...
    object_loop2: DO
       ...
       IF (IOMODE == IOMODE_OUT) THEN
          ...
          END IF
          XNF90_PREC = NCOUT_PREC
          IF (object%ldp) XNF90_PREC = NF90_DOUBLE
       ELSE
          it = 0
          XNF90_PREC = NF90_DOUBLE
       END IF
...
!!$            , dimvec(1:nrank+it)        & ! netCDF dim-IDs
               , dimvec(1:iorank+it)       & ! netCDF dim-IDs
               , varid)                    & ! netCDF var-ID
               , status, substr )
          IF (status /= 0) RETURN
          ...
          IF ((jsnd == SND_CNT) .OR. (jsnd == SND_CAV)) THEN
             ...
             CASE(NF90_DOUBLE)
                CALL nf(NF90_PUT_ATT(ncid, varid, 'range', object%io%range) &
                     , status, substr)
                IF (status /= 0) RETURN
                CALL nf(NF90_PUT_ATT(ncid, varid, '_FillValue', FLAGGED_BAD) &
                     , status, substr)
                IF (status /= 0) RETURN
             CASE(NF90_FLOAT)
                CALL nf(NF90_PUT_ATT(ncid, varid, 'range' &
                     , REAL(object%io%range))             &
                     , status, substr)
                IF (status /= 0) RETURN
                CALL nf(NF90_PUT_ATT(ncid, varid, '_FillValue' &
                     , REAL(FLAGGED_BAD)) &
                     , status, substr)
                IF (status /= 0) RETURN
             END SELECT
...
!!$         , cnt(1:nrank+it)   ) &
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE netcdf_write_attribute_list(status, ncid, varid, list, prec)
    ...
    DO
       ...
       CASE(TYPE_REAL_DP)
          SELECT CASE(prec)
          CASE(NF90_FLOAT)
             CALL nf(NF90_PUT_ATT(ncid, varid, TRIM(att%name), REAL(att%r)) &
                  , status, substr)
          CASE(NF90_DOUBLE)
  ...
  SUBROUTINE netcdf_define_dimvar_list(status, ncid, dimid, var, PREC)
    ...
    DO
       ...
       CALL netcdf_write_attribute_list(status, ncid, varid, dv%att, PREC)
       IF (status /= 0) RETURN
       ! ---------------------------------------------
...
!!$#endif
    ...
    IF (sin /= NF90_NOERR) THEN
       ...
       WRITE(iouerr,*) TRIM(substr),': *** netCDF ERROR: ',NF90_STRERROR(sin)
