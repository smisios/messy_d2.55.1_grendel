...
! **********************************************************************
  ...
  TYPE t_attribute
     CHARACTER(LEN=STRLEN_LONG)   :: name    = ''
...
!     PRIVATE
  ...
  TYPE t_att_ptr
     TYPE(t_attribute), DIMENSION(:), POINTER :: ptr => NULL()
  END TYPE t_att_ptr
...
CONTAINS
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE add_attribute(status, list, name, i, c, r, loverwrite, iflag, lout)
  ...
  ! -------------------------------------------------------------------
  SUBROUTINE return_attribute(status, list, name, i, c, r, iflag, lout)
  ...
  SUBROUTINE loc_attribute(status, list, name, att)
    ...
    ! CHECKS
    IF (LEN_TRIM(ADJUSTL(name)) > STRLEN_LONG) THEN
