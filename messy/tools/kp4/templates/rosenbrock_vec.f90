...
#ifdef FULL_ALGEBRA
   ERROR STOP COMPILATION! FULL ALGEBRA NOT IMPLEMENTED IN VECTOR MODE!
#else
...
!!$ REAL(kind=dp) :: Err, Yerr(NVAR)
   ...
   INTEGER :: Pivot(VL_DIM,NVAR), Direction, ioffset, j, istage
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   REAL(kind=dp)                 :: deltat
!kk logical,parameter     :: fixed_step=.   
...
TimeLoop: DO 
   ...
   IF ( Nstp > Max_no_steps ) THEN  ! Too many steps
      ...
      RETURN
   END IF
   WHERE ( ((T(1:VL)+0.1_dp*H(1:VL)) == T(1:VL)) .OR. (H(1:VL) <= Roundoff) )  ! Step size too small
      IERRV(1:VL) = -7
   END WHERE
