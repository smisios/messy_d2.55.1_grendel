! mz_rs_20081219+
! original code is from John Burkardt, see:
! http://people.scs.fsu.edu/~burkardt/f_src/spline/spline.html
! new web page:
! https://people.sc.fsu.edu/~jburkardt/f_src/spline/spline.html
! changed into a MESSy-compatible Fortran90 module by Rolf Sander, 2009
! 
module messy_main_math_spline
USE messy_main_constants_mem,      ONLY: DP ! mz_ab_20110317  
implicit none
private :: dp
contains
! mz_rs_20081219-
...
!
  if ( tval < tdata(2) ) then
    ...
    a = beta2 + 4.0_dp * beta1 + 4.0_dp * beta1 * beta1 &
      + 6.0_dp * ( 1.0_dp - beta1 * beta1 ) &
      - 3.0_dp * ( 2.0_dp + beta2 + 2.0_dp * beta1 ) &
      + 2.0_dp * ( 1.0_dp + beta2 + beta1 + beta1 * beta1 )

    b = - 6.0_dp * ( 1.0_dp - beta1 * beta1 ) &
        + 6.0_dp * ( 2.0_dp + beta2 + 2.0_dp * beta1 ) &
        - 6.0_dp * ( 1.0_dp + beta2 + beta1 + beta1 * beta1 )

    c = - 3.0_dp * ( 2.0_dp + beta2 + 2.0_dp * beta1 ) &
        + 6.0_dp * ( 1.0_dp + beta2 + beta1 + beta1 * beta1 )

    d = - 2.0_dp * ( 1.0_dp + beta2 + beta1 + beta1 * beta1 )
    ...
    b = - 6.0_dp * beta1 * ( 1.0_dp - beta1 * beta1 )

    c = - 3.0_dp * ( beta2 + 2.0_dp * beta1**2 + 2.0_dp * beta1**3 )

    d = 2.0_dp * ( beta2 + beta1 + beta1**2 + beta1**3 )

    yval = ( ( d * u + c ) * u + b ) * u + a

  else if ( tval < tdata(5) ) then

    yval = 2.0_dp * beta1**3 * ( 1.0_dp - u )**3

  end if

  yval = yval / ( 2.0_dp + beta2 + 4.0_dp * beta1 + 4.0_dp * beta1**2 &
    + 2.0_dp * beta1**3 )
...
!
  ...
  mbasis(1,1) = - 2.0_dp * beta1 * beta1 * beta1
  mbasis(1,2) =   2.0_dp * beta2 &
    + 2.0 * beta1 * ( beta1 * beta1 + beta1 + 1.0_dp )
  mbasis(1,3) = - 2.0_dp * ( beta2 + beta1 * beta1 + beta1 + 1.0_dp )
  mbasis(1,4) =   2.0_dp

  mbasis(2,1) =   6.0_dp * beta1 * beta1 * beta1
  mbasis(2,2) = - 3.0_dp * beta2 &
    - 6.0_dp * beta1 * beta1 * ( beta1 + 1.0_dp )
  mbasis(2,3) =   3.0_dp * beta2 + 6.0_dp * beta1 * beta1
  mbasis(2,4) =   0.0_dp

  mbasis(3,1) = - 6.0_dp * beta1 * beta1 * beta1
  mbasis(3,2) =   6.0_dp * beta1 * ( beta1 - 1.0_dp ) * ( beta1 + 1.0_dp )
  mbasis(3,3) =   6.0_dp * beta1
  mbasis(3,4) =   0.0_dp

  mbasis(4,1) =   2.0_dp * beta1 * beta1 * beta1
  mbasis(4,2) =   4.0_dp * beta1 * ( beta1 + 1.0_dp ) + beta2
  mbasis(4,3) =   2.0_dp
  mbasis(4,4) =   0.0_dp
...
!
  ...
  mbasis(1,1) = - ( 1.0_dp - alpha ) * ( 1.0_dp - alpha ) / alpha
  mbasis(1,2) =   beta + ( 1.0_dp - alpha ) / alpha
  mbasis(1,3) =   alpha - 1.0_dp / ( 1.0_dp - beta )
  mbasis(1,4) =   beta * beta / ( 1.0_dp - beta )

  mbasis(2,1) =   2.0_dp * ( 1.0_dp - alpha ) * ( 1.0_dp - alpha ) / alpha
  mbasis(2,2) = ( - 2.0_dp * ( 1.0_dp - alpha ) - alpha * beta ) / alpha
  mbasis(2,3) = ( 2.0_dp * ( 1.0_dp - alpha ) &
    - beta * ( 1.0_dp - 2.0_dp * alpha ) ) / ( 1.0_dp - beta )
  mbasis(2,4) = - beta * beta / ( 1.0_dp - beta )

  mbasis(3,1) = - ( 1.0_dp - alpha ) * ( 1.0_dp - alpha ) / alpha
  mbasis(3,2) =   ( 1.0_dp - 2.0_dp * alpha ) / alpha
  mbasis(3,3) =   alpha
  mbasis(3,4) =   0.0_dp
...
!
  ...
  mbasis(1,1) =   1.0_dp / alpha
  mbasis(1,2) = - 1.0_dp / ( alpha * ( 1.0_dp - alpha ) )
  mbasis(1,3) =   1.0_dp / ( 1.0_dp - alpha )

  mbasis(2,1) = - ( 1.0_dp + alpha ) / alpha
  mbasis(2,2) =   1.0_dp / ( alpha * ( 1.0_dp - alpha ) )
  mbasis(2,3) = - alpha / ( 1.0_dp - alpha )
...
!
  ...
  mbasis(1,1) =   1.0_dp / beta
  mbasis(1,2) = - 1.0_dp / ( beta * ( 1.0_dp - beta ) )
  mbasis(1,3) =   1.0_dp / ( 1.0_dp - beta )

  mbasis(2,1) = - ( 1.0_dp + beta ) / beta
  mbasis(2,2) =   1.0_dp / ( beta * ( 1.0_dp - beta ) )
  mbasis(2,3) = - beta / ( 1.0_dp - beta )
...
!
  ...
  mbasis(1,1) = - 1.0_dp / 2.0_dp
  mbasis(1,2) =   3.0_dp / 2.0_dp
  mbasis(1,3) = - 3.0_dp / 2.0_dp
  mbasis(1,4) =   1.0_dp / 2.0_dp

  mbasis(2,1) =   2.0_dp / 2.0_dp
  mbasis(2,2) = - 5.0_dp / 2.0_dp
  mbasis(2,3) =   4.0_dp / 2.0_dp
  mbasis(2,4) = - 1.0_dp / 2.0_dp

  mbasis(3,1) = - 1.0_dp / 2.0_dp
  mbasis(3,2) =   0.0_dp
  mbasis(3,3) =   1.0_dp / 2.0_dp
  mbasis(3,4) =   0.0_dp

  mbasis(4,1) =   0.0_dp
  mbasis(4,2) =   2.0_dp / 2.0_dp
  mbasis(4,3) =   0.0_dp
  mbasis(4,4) =   0.0_dp
...
!
  ...
  if ( left == 1 ) then
    arg = 0.5_dp * ( tval - tdata(left) )
    first = left
  else if ( left < ndata - 1 ) then
    arg = tval - tdata(left)
    first = left - 1
  else if ( left == ndata - 1 ) then
    arg = 0.5_dp * ( 1.0_dp + tval - tdata(left) )
    first = left - 1
  end if
!
!  TVEC(I) = ARG**(N-I).
...
!
  ...
  if ( n == 0 ) then
    ...
    do i = 2, n
      bern(i) = x * bern(i-1)
      do j = i-1, 1, -1
        bern(j) = x * bern(j-1) + ( 1.0_dp - x ) * bern(j)
      end do
      bern(0) = ( 1.0_dp - x ) * bern(0)
    end do
...
!
  ...
  next(2) = 0
  ! mz_rs_20090610+
!!$! zero must have the same data type (kind=8) as h:
  ! zero must have the same data type (kind=dp) as h:
  ! xmi = min ( 0.0_dp, h )
  ! xma = max ( 0.0_dp, h )
!!$xmi = min ( 0.0_8, h )
!!$xma = max ( 0.0_8, h )
...
!
  ...
  rn0 = ntab
  c(0) = y_sum / real ( ntab, dp )

  ptab(1:ntab) = y_sum / real ( ntab, dp )

  if ( ndeg == 0 ) then
    eps = sum ( ptab(1:ntab) - ytab(1:ntab) )**2
    eps = sqrt ( eps / real ( ntab, dp ) )
    return
  end if
!
!  The polynomial is of degree at least 1.
!
  b(1) = sum ( xtab(1:ntab) ) / real ( ntab, dp )
...
!
  ...
  integer  j
  real(dp) p
  real(dp) pj(point_num)
  real(dp) pjm1(point_num)
  real(dp) s(nterms)
...
!
  pjm1(1:point_num) = 0.0_dp
  pj(1:point_num) = 1.0_dp
!
...
!
  ...
  ! mz_rs_20090610+
!!$  ! zero must have the same data type (kind=8) as arg1 and arg2:
  ! zero must have the same data type (kind=dp) as arg1 and arg2:
  ! pchst = sign ( 1.0_8, arg1 ) * sign ( 1.0_8, arg2 )
!!$pchst = sign ( 1.0_8, arg1 ) * sign ( 1.0_8, arg2 )
...
!
  ...
  if ( 0 < left-1 ) then
    ...
    yval = yval + ( 2.0_dp * ydata(1) - ydata(2) ) * bval
  end if
!
...
!
  ...
  if ( right+1 <= ndata ) then
    ...
    yval = yval + ( 2.0_dp * ydata(ndata) - ydata(ndata-1) ) * bval
  end if
...
!  evaluated in its 4th interval.
!
  bval = 2.0_dp * ( beta1 * ( 1.0_dp - u ) )**3  / delta
...
!  evaluated in its third interval.
!
  a = beta2 + ( 4.0_dp + 4.0_dp * beta1 ) * beta1

  b = - 6.0_dp * beta1 * ( 1.0_dp - beta1 ) * ( 1.0_dp + beta1 )
...
!
  ...
  c = 3.0_dp * beta2 + 6.0_dp * beta1 * beta1
...
!
  ...
  real(dp) bernstein_val(0:cubic)
  real(dp) data_val(dim_num,cubic*interval_num+1)
  integer  dim
  integer  interval
  integer  offset
  integer  point
  real(dp) point_t(point_num)
  real(dp) point_val(dim_num,point_num)
  real(dp) t
  real(dp) t_01
...
!
  if ( ibcbeg == 0 ) then
    ...
    a(2,1) = ( t(2) - t(1) ) / 3.0_dp 
    a(1,2) = ( t(2) - t(1) ) / 6.0_dp
...
!
  do i = 2, n-1
    ypp(i) = ( y(i+1) - y(i) ) / ( t(i+1) - t(i) ) &
           - ( y(i) - y(i-1) ) / ( t(i) - t(i-1) )
    a(3,i-1) = ( t(i) - t(i-1) ) / 6.0_dp
    a(2,i) = ( t(i+1) - t(i-1) ) / 3.0_dp
    a(1,i+1) = ( t(i+1) - t(i) ) / 6.0_dp
  end do
!
...
!
  if ( ibcend == 0 ) then
    ...
    a(3,n-1) = ( t(n) - t(n-1) ) / 6.0_dp
    a(2,n) = ( t(n) - t(n-1) ) / 3.0_dp
...
!
  ...
  do i = 1, ndata-1
    ...
    divdif3 = c(2,i) + c(2,i+1) - 2.0_dp * divdif1
...
!
  ...
  spval = c(2,left) + dt * ( 2.0_dp * c(3,left) + dt * 3.0_dp * c(4,left) )
...
!
  tval = ( a_copy + tdata(a_right) ) / 2.0_dp
  ...
  do i_left = a_right, b_left - 1
    ...
    tval = ( tdata(i_left+1) + tdata(i_left) ) / 2.0_dp
  ...
  tval = ( tdata(b_left) + b_copy ) / 2.0_dp
...
!
  data_x(1:n) = 0.5_dp * ( int_x(1:n) + int_x(2:n+1) )
!
!  Set up C, D, E, the coefficients of the linear system.
!
  a(3,1:n-2) = 1.0_dp &
    - ( 0.5_dp * ( data_x(2:n-1) + int_x(2:n-1) ) - data_x(1:n-2) ) &
  ...
  a(2,2:n-1) = 1.0_dp &
    + ( 0.5_dp * ( data_x(2:n-1) + int_x(2:n-1) ) &
    - data_x(1:n-2) ) &
    / ( data_x(2:n-1) - data_x(1:n-2) ) &
    - ( 0.5_dp * ( data_x(2:n-1) + int_x(3:n) ) - data_x(2:n-1) ) &
...
!
  ...
  integer  left
  real(dp) mbasis(4,4)
  real(dp) mbasis_l(3,3)
  real(dp) mbasis_r(3,3)
  integer  right
  real(dp) tdata(ndata)
  real(dp) tval
  real(dp) ydata(ndata)
...
!
  ...
  ypval = dif1 + dif2 * ( 2.0_dp * tval - t1 - t2 )
...
! mz_rs_20081219+
end module messy_main_math_spline
! mz_rs_20081219-
