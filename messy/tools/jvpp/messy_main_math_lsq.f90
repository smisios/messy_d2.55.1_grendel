! mz_rs_20090209+
! original code is from Alan J. Miller, see:
! http://jblevins.org/mirror/amiller
! http://jblevins.org/mirror/amiller/lsq.f90
...
SUBROUTINE poly_fit(x, y, m, beta, r2, io_log)

REAL(DP), DIMENSION(:),           INTENT(IN)  :: x, y
INTEGER,                           INTENT(IN)  :: m
REAL(DP), DIMENSION(:),           INTENT(OUT) :: beta
REAL(DP),               OPTIONAL, INTENT(OUT) :: r2
INTEGER,                 OPTIONAL, INTENT(IN)  :: io_log

!!$REAL(DP)  :: mini, maxi
REAL(DP)  :: var, totalSS
REAL(DP), DIMENSION(0:m) :: sterr, xrow
LOGICAL   :: lindep(0:m)
INTEGER   :: i, j, ier
INTEGER, PARAMETER :: DIMCOV=231 ! (21*22)/2
REAL(DP), DIMENSION(DIMCOV) :: covmat

!!$mini = MINVAL(y)
!!$maxi = MAXVAL(y)
!!$IF (maxi-mini<TINY(0.)) THEN
!!$  beta(:) = 0.
!!$  beta(0) = maxi
!!$  IF (maxi>0.) THEN
!!$    r2 = 1.
...
CALL cov(m+1, var, covmat, DIMCOV, sterr, ier)

! Calculate r2:
totalSS = rss(1)
IF (ABS(totalSS)>TINY(0._dp)) THEN
  r2 = (totalSS - rss(m+1))/totalSS
ELSE
  r2 = 0._dp
ENDIF

IF (PRESENT(io_log)) THEN
!!$  WRITE(io_log,*) 'Least-squares coefficients & std. errors'
!!$  WRITE(io_log,*) 'Power  Coefficient          Std.error      Resid.sum of sq.'
!!$  DO i = 0, m
!!$    WRITE(io_log,'(I4,G20.12,"   ",G14.6,"   ",G14.6)')  &
!!$      i, beta(i+1), sterr(i), rss(i+1)
!!$  ENDDO
!!$  WRITE(io_log,'(A,G20.12)') ' Residual standard deviation = ', SQRT(var)
  IF (ABS(totalSS)>TINY(0._dp)) THEN
    WRITE(io_log,'(A,G20.12)') ' R^2 = ', r2
  ELSE
    WRITE(io_log,'(A)') ' R^2 = undefined'
  ENDIF
ENDIF
...
INTEGER    :: col, row, pos
REAL(DP)   :: eps1, ten = 10.0_dp, total, work(ncol)
...
! mz_rs_20090209+
END MODULE messy_main_math_lsq
! mz_rs_20090209-
