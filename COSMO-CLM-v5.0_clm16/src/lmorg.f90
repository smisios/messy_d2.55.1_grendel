...
! V5_0_clm8 2016-01-29  H.-J. Panitz, Irina Fast, DKRZ
!  For a correct representation of the value of real variables
!   being converted to Integer, the "int_dp" parameter has been included in
!   the corresponding conversion statements
! V5_0_clm9 2016-05-11  H.-J. Panitz, IMK/KIT
! change sequence of calculation for nzdays in case of lbdclim=T
!   -  necessary to get a meaningful value for very large ntstep
! V5.0_clm12  2018/01/15 Ronny Petrik, HZG
!  add the nullification of a new maximum wind statistics VABS_10M_SECMAX and 
...
USE data_parameters,     ONLY:   ireals, iintegers, int_dp
!HJP 2016-01-29 End  
!
...
USE data_runcontrol,     ONLY:                                               &
       ...
       lphys, lconv, luseobs, l2tls, lsemi_imp, lgsp,                        &
...
!DL 1.12.2014 change to allow itype_albedo=3 in climate mode
!      l2dim, nincsn, itype_aerosol, l_2mom
       l2dim, nincsn, itype_aerosol, l_2mom, itype_albedo
!DL end change
...
#ifdef TWOMOM_SB
REAL (KIND=ireals), POINTER ::                        &
  qni_now    (:,:,:)  => NULL(), & ! QNICE at nnow
  qni_nx0    (:,:,:)  => NULL()    ! QNICE at nx0
#endif
...
comp_pe: IF (lcompute_pe) THEN
  ...
  IF( lasync_io .AND. nc_asyn_io>0 ) THEN 
    ...
    IF (izerror /= 0) THEN
      ierrstat = 3321
      yzerrmsg = ' ERROR    *** Allocation of space for isend_buffer failed *** ' // yzerrmsg
...
! AF Put l_cosmo_art to false if you don't want to calculate the ART part
    IF (l_cosmo_art_nl) THEN
      l_cosmo_art=(l_cosmo_art_nl .AND. ((ntstep*dt/3600.) >= artstart ))
    ENDIF

    ! Initialization of COSMO_ART
    IF (l_cosmo_art .AND. (ntstep*dt/3600.) == artstart) THEN
...
!
!HJP 2016-01-29 Begin
! change sequence of calculation -  necessary to get a meaningful value for very
! large ntstep
!         nzdays  = NINT ((ntstep+1)*dt) / 86400
          nzdays  = NINT ((REAL((ntstep+1),ireals)/86400._ireals)*dt,int_dp)
!HJP 2016-01-29 End
...
!         nzhours = NINT ((ntstep+1)*dt) / 3600
          nzhours = NINT ((ntstep+1)*dt, int_dp) / 3600
!HJP 2016-01-29 End
...
! nehddt = NINT ( 3600.0_ireals / dt )
  nehddt = NINT ( 3600.0_ireals / dt, iintegers )
!HJP 2016-01-29 End
...
!CDIR COLLAPSE
        ! CK 20101204 ART uses its own interpolation weights
        caero_new(:,:,:,:) = z1_art*caero_bd (:,:,:,:,nbd1_art) + &
...
!DL 1.12.2014 change to allow itype_albedo=3 in climate mode
          IF (itype_albedo == 3) THEN
            alb_dif(i,j) = z1*alb_dif_bd(i,j,nbd1) + z2*alb_dif_bd(i,j,nbd2)
          ENDIF
!DL end change
...
!-------------------------------------------------------------------------------
!  Section 4: Reinitialize aggregated variables related to wind
!-------------------------------------------------------------------------------

  IF (ntstep-1 == nnextmxu) THEN
    vmax_10m  (:,:)        = 0.0_ireals
    vabsmx_10m(:,:)        = 0.0_ireals
    vgust_dyn (:,:)        = 0.0_ireals
    vgust_con (:,:)        = 0.0_ireals
    vabs_10m_secmax(:,:,:) = 0.0_ireals
    turb_intens(:,:,:)     = 0.0_ireals
...
!   nnextmxu = NINT (hnextmxu * 3600.0_ireals / dt)
    nlastmxu = NINT (hlastmxu * 3600.0_ireals / dt, iintegers)
    nnextmxu = NINT (hnextmxu * 3600.0_ireals / dt, iintegers)
!HJP 2016-01-29 Begin
  ENDIF
