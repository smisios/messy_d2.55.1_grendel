...
! V5.00_clm12  2018-10-30  B. Rockel, HZG
!   Replace 4 by max_gribrep
!
! Code Description:
!   Language:           Fortran 90.
...
! -----------------------------------------------------
    ...
    nvscoc       ,& ! no. of bits occ. by each indicator                "
...
! -------------------
    ...
    nbsqer       ,& ! error of observed rel. humidity
    nbsper       ,& ! error of observed prec.amount
...
!===============================================================================
  ...
  REAL (KIND=ireals)       , PARAMETER  :: &
    tmxdif(4) = (/ 0.251_ireals   ,& ! accepted time diff. between obs time
                   0.251_ireals   ,& ! and analysis time  of the parameters:
                   0.01_ireals    ,& ! t2m, rh2m, precipitation, ff10m
                   0.251_ireals /),& ! 
    rmxhds    =    400._ireals    ,& ! max. height difference (obs.-model)
                                     ! for precipitation observations
    rmxhdt    =    300._ireals    ,& ! max. height difference (obs.-model)
                                     ! for T2m and RH2m observations
    rmxhdv    =    300._ireals    ,& ! max. height difference (obs.-model)
                                     ! for ff10m observations
    rmprcs(3) = (/ 40000._ireals  ,& ! precipitation scan radius in m.
                   70000._ireals  ,& 
                  110000._ireals/),& 
    rmt2ms(3) = (/200000._ireals  ,& ! 2m temp. scan radius in m
                  100000._ireals  ,&
                   50000._ireals/),&
    rmr2ms(3) = (/200000._ireals  ,& ! 2m hum. scan radius in m.
                  100000._ireals  ,&
                   50000._ireals/),&
    rmfffs(3) = (/200000._ireals  ,& ! 10m wind  scan radius in m.
                  100000._ireals  ,&
                   50000._ireals/)
...
!   -------------------------
  ...
  INTEGER (KIND=iintegers)                , PRIVATE :: &
   ...
   maxdep         ! maximum number of increments within a model row
                  !   = MAX (NINT(maxobs / (0.4_ireals*je_tot)), 20)
                  ! ( = maxobs / (je_tot **0.75) )
...
! -------------
  ...
  INTEGER (KIND=iintegers) ::  &
    ...
    iisdim           , & ! number of integer elements sent to each processor
    lentot           , & ! total length of observational data available
    lenact           , & ! actual length of data to be gathered
...
! -------------------------
  ...
  INTEGER (KIND=iintegers)   ::       &
    ...
    iidispl(num_compute) , & ! displacement of incoming data within 'bufall'
    iirdim (num_compute)     ! number of elements received from each process
...
! This is done at the very first model step
  IF (ntstep == nstart) THEN
    ht2next  = ht2a
    nt2next  = NINT (ht2next  * 3600.0_ireals / dt)
    hh2next  = hh2a
    nh2next  = NINT (hh2next  * 3600.0_ireals / dt)
    hffnext  = hffa
    nffnext  = NINT (hffnext  * 3600.0_ireals / dt)
    hprcnext = hprc
    nprcnext = NINT (hprcnext * 3600.0_ireals / dt)
  ENDIF
...
#ifdef GRIBDWD
        CALL copen (isfco, TRIM(ydatname), 'w  ', istat)
        IF (istat /= 0) THEN
          yerrmsg = 'opening DWDGRIB file lansfc failed'
          CALL model_abort (my_cart_id, 2022, yerrmsg, yroutine)
        ENDIF
#endif
      ELSEIF (pp_lansfc%yform_write(1:3) == 'api') THEN
#ifdef GRIBAPI
        CALL grib_open_file(isfco, TRIM(ydatname), 'w', istat)
        IF (istat /= GRIB_SUCCESS) THEN
          yerrmsg = 'opening grib_api file lansfc failed'
          CALL model_abort (my_cart_id, 2022, yerrmsg, yroutine)
        ENDIF
#endif
      ENDIF
...
! maximum number of observations used (depends on 'ztdiff' resp. 'tmxdif')
  maxobs  = maxsgo * (1.0_ireals + ztdiff)

! Maximum number of increments within a model row
  maxdep  = MAX ( NINT( maxobs / (0.4_ireals*je_tot)) , 20)
! maxdep  = NINT( maxobs / SQRT(SQRT(REAL(je_tot*je_tot*je_tot, ireals))) )
  ...
  DO i = 1,ie_tot
    ...
    IF (alon(i) > 180.0_ireals) THEN
      alon(i) = alon(i) - 360.0_ireals
    ENDIF
  ENDDO
...
!   check observation time
    IF (ABS( osghed(iob,nhtime)-acthr ) <= ztdiff+epsy) THEN
      ...
      iobrtr = IBITS( mosgbd(iob,nbscfw), nvtrbp, nvtroc )
      IF (iobrtr >= 5) THEN
        iobrtr = iobrtr - 4
      ELSE
        iobrtr = iobrtr * 6
...
!     check if observation contains usable data        
      IF (     (osgbdy(iob,nbst  ) /= rmdi)                                    &
          ...
          .OR.((osgbdy(iob,nbspr ) /= rmdi) .AND. (ABS( REAL (iobrtr, ireals)  &
                                                       -raintp) <= epsy))) THEN
...
!       check height difference between station altitude and model orography
        ...
        IF (zhdif > MAX (170.0_ireals,osghed (iob,nhalt)/5.0_ireals))   THEN
...
!       observation longitude and latitude
        bufloc (noblon,nobpro) = NINT (osghed (iob,nhilon)*100._ireals)
        bufloc (noblat,nobpro) = NINT (osghed (iob,nhjlat)*100._ireals)

!       station altitude
        bufloc (nstalt,nobpro) = NINT (osghed (iob,nhalt))

!       type of assigned gridpoint (land/sea indicator)
        IF (fr_land  (igp_ob,jgp_ob) >= 0.5) THEN
...
!       adapt observed temperature to model orography height
        IF (osgbdy (iob, nbst  ) /= rmdi) THEN
          bufloc (nt2mob,nobpro) = NINT ( (osgbdy (iob, nbst  )+              &
                                   0.006_ireals*zhdif)*100._ireals)
          bufloc (nt2oer,nobpro) = NINT (0.8_ireals *                         &
                        (1._ireals + 0.003_ireals*ABS(zhdif))*100._ireals)
        ELSE
...
!       screen level humidity and observation error
        IF (osgbdy (iob, nbsrh ) /= rmdi) THEN
          bufloc (nh2mob,nobpro) = NINT (osgbdy (iob, nbsrh )*100._ireals)
          bufloc (nh2oer,nobpro) = NINT (0.08_ireals *                        &
                        (1._ireals + 0.003_ireals*ABS(zhdif))*100._ireals)
        ELSE
...
!       precipitation amount and observation error
        IF ((osgbdy (iob,nbspr ) /= rmdi) .AND. (ABS( REAL (iobrtr, ireals)    &
                                                     -raintp) <= epsy)) THEN
          bufloc (nprcob,nobpro) = NINT (osgbdy (iob, nbspr )*100._ireals)
          bufloc (nprcer,nobpro) = NINT (1.0_ireals * 100._ireals) 
        ELSE
...
!       10m wind speed and observation error
        IF (osgbdy (iob, nbsu  ) /= rmdi .AND. osgbdy (iob, nbsv  ) /= rmdi)   &
          ...
          bufloc (nf10ob,nobpro) = NINT (                                      &
           SQRT (osgbdy (iob, nbsu)**2 + osgbdy (iob, nbsv)**2 )*100._ireals)
          bufloc (nf10er,nobpro) = NINT (1.4_ireals*                           &
                        (1._ireals + 0.003_ireals*ABS(zhdif))*100._ireals)
        ELSE
...
!-------------------------------------------------------------------------------
  ...
  pp_lansfc%plev(:)          = -1.0     ! not used
  pp_lansfc%zlev(:)          = -1.0     ! not used
...
! variables for reorganisation of loops, to enhance efficiency on vector proces.
   ...
   REAL (KIND=ireals), DIMENSION(10000) ::  zdistm_, zrormx_, wwa_, zmodor_    &
...
!  station altitude
   zstalt = REAL  (bufall(nstalt,iob), ireals)
...
!  observed value
   IF (bufall(iobval,iob) == imdi .OR.                                       &
      ...
      CYCLE All_obs
   ELSE
     zobval = bufall(iobval,iob) *0.01_ireals

!    observation error
     zoberr = bufall(ioberr,iob) *0.01_ireals
   ENDIF
...
!    Station characteristics
     ...
     ! The longitude values have to be limited to the range (-180.0,+180.0)
     IF (rpalon(idep,jrow) > 180.0_ireals) THEN
       rpalon(idep,jrow) = rpalon(idep,jrow) - 360.0_ireals
     ENDIF

     rpalto(idep,jrow) = bufall(noblat,iob) *0.01_ireals
     rpalno(idep,jrow) = bufall(noblon,iob) *0.01_ireals
     rpahgt(idep,jrow) = REAL  (bufall(nstalt,iob), ireals)
...
!     observations for given latitude row
        ...
        zdistm    = MAX(ACOS(MIN(zdisop,1._ireals)),1.e-5_ireals) * r_earth
...
!       Calculate weights
        IF (zdistm <= zmscan) THEN
          IF (lprecip_) THEN
            wwa (jdep2_) = .5_ireals*(COS(zrormx*pi)+                 &
                                 1._ireals)/(1._ireals+0.3_ireals*zrormx)
          ELSE
            wwa (jdep2_) = (zrsmrs-zdissq) /(zrsmrs+zdissq)
          ENDIF
        ELSE
            wwa (jdep2_) = 0._ireals
        ENDIF

        IF (lwonl .AND. ldiasa .AND. (jlat == jonl) .AND. (jlon == ionl)       &
                  .AND. (wwa(jdep2_) > 0.001_ireals))   THEN
           j_ = j_ + 1
           zdistm_(j_) = zdistm
           zrormx_(j_) = zrormx
           wwa_(j_)    = wwa(jdep2_) 
...
!         Calculate weights
          IF (zavdis <= zvdmax) THEN
            wwa (jdep2_) = 0.5_ireals*(COS(zvdovx*pi)+1._ireals)/     &
                          (1._ireals+0.8_ireals*zvdovx)* wwa(jdep2_)
          ELSE
            wwa (jdep2_) = 0._ireals
          ENDIF
          ...
          IF (lwonl .AND. ldiasa .AND. (jlat == jonl) .AND. (jlon == ionl)     &
                    .AND. (wwa(jdep2_) > 0.001_ireals))   THEN
             j_ = j_ + 1
             zwi_(j_)    = 0.5_ireals *COS(zavdis/zvdmax*pi) + 0.5_ireals
             zmodor_(j_) = zmodor
...
!        Take into account observation errors
         wwa(jdep2_) = MIN( ABS(1-islgp-npagpt(jdep,jlb)) , 1) *         &
...
!----------------------------------------------------------------------------
   ...
   IF (ww   <= 0._ireals)                                CYCLE Model_lon

   IF (yvar == 'TOT_PREC') THEN
     IF (ww >  0.20_ireals)                    THEN
        rsurin(jlon,jlat) = w/ww
        indexv(jlon,jlat) = 1
     ELSE IF(jscan == nprcsc .AND. ww >  0.02_ireals)      THEN
        rsurin(jlon,jlat) = w/ww
        indexv(jlon,jlat) = 1
     ELSE
        rsurin(jlon,jlat) = 0._ireals
        CYCLE Model_lon
     ENDIF
     IF (rsurin(jlon,jlat) <  0.04_ireals) rsurin(jlon,jlat) = 0._ireals

   ELSEIF (yvar /= 'TOT_PREC') THEN
     rsurin(jlon,jlat) = w/(ww+1._ireals)
     indexv(jlon,jlat) = 1
   ENDIF  ! yvar
...
!CDIR NODEP
       DO ii_ = 1, i_
          ...
          jlb    = indxj_(ii_)
          jdep2_ = indxk_(ii_)
          IF (wwa(jdep2_) >= 0.001_ireals) THEN
...
! Preset statistics variables
  ...
  zsd      =  0._ireals
  zmaxch   =  0._ireals
  zmaxvl   =  -999999._ireals
  irowmx   =  0
  ipoimx   =  0
  zminvl   =  +999999._ireals
  irowmn   =  0
  ipoimn   =  0
...
! Local scalars:
  ...
  INTEGER  (KIND=iintegers)            :: ndgb, mmgb, jjgb, nhgb, jcgb,  &
                                          ivar, iv, i, j, k, igrl, ierrf,&
                                          nmingb, i1, i2, i3, izerror, igrbid
  ...
  INTEGER  (KIND=iintegers)            :: jj,mm,dd,hh,mi, izntri

  INTEGER (KIND=int_ga)                :: irecord_lga      ! length of grib record

  REAL     (KIND=ireals)               :: pollon_grib, zstartlon, zendlon
  REAL     (KIND=ireals)               :: zbias, zgribfactor
...
! ELEMENTAL REAL FUNCTION fpvsw  ( zt, b1, b2w, b3, b4w )
  !------------------------------------------------------
! REAL    (KIND=ireals)     , INTENT (IN)  ::  zt, b1, b2w, b3, b4w
  !----------------------------------------------------------------------------
