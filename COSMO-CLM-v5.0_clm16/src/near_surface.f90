...
!  manage the computation of the averaged fluxes for direct solar normalized 
!    radiation, i.e. 'aswdir_sn(2)';
!  use the normalized solar flux swdir_sn2 for computation of sunshine duration
!    not sodwddm
!  introduce the computation of new wind diagnostics (direction, mean and sector
!   statistics)
!  introduce the computation of 3D-fields of mean wind components 'u' and 'v'
!  introduce the computation of maximum turbulence intensity estimate
!
! Code Description:
! Language: Fortran 90.
...
USE data_fields,        ONLY:                                        &
       u, v, w, t, pp, p0, hsurf, hhl, gz0, tcm, tch, u_10m,         &
       ...
       t_2m_av, td_2m_av, u_10m_av, v_10m_av, dursun, clct, clct_av, &
       swdir_s, swdir_sn, swdir_sno, swdifd_s, swdifu_s, lwd_s,      &
       lwu_s, aswdir_s, aswdir_sn, aswdir_sno,                       &
       ...
       hail_gsp, prh_gsp, vabs_av, vabs_10m_av, wdirgeo_10m,         &
       ...
       wdirgeo_10m_freq  ! frequency distribution of wind direction ( 1 )
...
USE data_turbulence,    ONLY:   akt

USE data_io,            ONLY:                                        &
  lbdclim, undefgrib, nwdirsec
...
USE utilities,          ONLY : uvrot2uv_vec
...
REAL (KIND=ireals)         ::    &
  ...
  zf2m, zf10m, z0m, zdzg, zval, zfac, zepsi, zdtr, esat, zt, sec, ha_sun

INTEGER (KIND=iintegers)   ::     &
  zi_bra(ie,je)                     ! for the computation of Brasseur; values 0 or 1

INTEGER (KIND=iintegers)   ::    &
  i, j, k, ii, jj, ntl, kso, izerror, sectors
...
REAL (KIND=ireals)         ::     &
  ...
  zhori(ie,je,nhori),             & !
  zumrot(ie,je),                  & ! derotated u-wind, grid_eastward_wind 
  zvmrot(ie,je),                  & ! derotated v-wind, grid_northward_wind 
  ztke3d(ie,je,ke1),              & ! 3d field of TKE
  zhelpwdir(ie,je)                  ! helper field for wind direction classification
...
REAL (KIND=ireals) ::    &
  zrpi18 = 57.2957795_ireals        ! conversion between radians and degree


REAL (KIND=ireals), PARAMETER ::  &
zpa_swlim_dursun = 120.0,         & ! minimum normalized radiation required to count the sunshine duration
zpa_wsplim_wdir  = 0.3              ! minimum wind speed required to calculate a wind direction
...
!------------------------------------------------------------------------------
  ...
  zhll(:,:) = 0.5_ireals*( hhl(:,:,ke)   + hhl(:,:,ke1) )
  zhlt(:,:) = 0.5_ireals*( hhl(:,:,ke-1) + hhl(:,:,ke)  )
  ...
  DO j = jstartpar,jendpar
    DO i = istartpar,iendpar
      ...
      zumold    = 0.5 * (u(i,j,ke,nnow) + u(ii,j,ke,nnow))
      zvmold    = 0.5 * (v(i,j,ke,nnow) + v(i,jj,ke,nnow))
      zvpb(i,j) = MAX ( SQRT (zumold**2 + zvmold**2) , zvmin )

      zumold_t  = 0.5 * (u(i,j,ke-1,nnow) + u(ii,j,ke-1,nnow))
      zvmold_t  = 0.5 * (v(i,j,ke-1,nnow) + v(i,jj,ke-1,nnow))

      IF     (itype_diag_gusts == 1 .OR. itype_diag_gusts == 4) THEN
        ! new computation of gusts
        z10  = hsurf(i,j) + 10.0_ireals
      ...
      zum1(i,j) = 0.5 * ( u(i,j,ke,nx) + u(ii,j,ke,nx) )
      zvm1(i,j) = 0.5 * ( v(i,j,ke,nx) + v(i,jj,ke,nx) )
      zum2(i,j) = 0.5 * ( u(i,j,ke-1,nx) + u(ii,j,ke-1,nx) )
      zvm2(i,j) = 0.5 * ( v(i,j,ke-1,nx) + v(i,jj,ke-1,nx) )
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        IF ( (lphys .EQV. .FALSE.) .OR. (ltur .EQV. .FALSE.) ) THEN
          zcm       = 1.0E-4_ireals
          zch       = 1.0E-4_ireals
          gz0 (i,j) = MAX ( gz0(i,j) , 1.0E-3_ireals )
        ELSE
          zcm       = MAX ( tcm(i,j) , 5.0E-4_ireals )
          zch       = MAX ( tch(i,j) , 4.0E-5_ireals )
        ENDIF
        ! other variables
        ...
        IF (zhs <= zh) THEN
          ...
          zh05m      = zhs + 0.25 * (zh - zhs)
          zh2m       = zh05m + 1.5 * g * (zh - zh05m) / (zh1 - 0.5*g)
        ELSE
          ! unstable case
          zsqcm     = MAX ( zsqcm  , 0.01_ireals )
          zchdcm    = MAX ( zchdcm , 0.01_ireals )

          zdz       = MAX(zh1-z10g, 0.0_ireals)
          zlnz1     = 1.0 - zsqcm/akt * LOG ( 1.0 + (EXP ( akt/zsqcm ) - 1.0) &
                    *gz0(i,j) * zdz / (zh1*(z10g + gz0(i,j))))
          zdz       = MAX(zh1 - z2g, 0.0_ireals)
          zh2m      = zhs + (zh - zhs) * (1.0 - zchdcm *                     &
                      LOG ( 1.0 + (EXP ( 1.0/zchdcm) - 1.0) * (gz0(i,j) *    &
                            zdz / (zh1 * (z2g + gz0(i,j))) ) ))
        ENDIF
        ...
        zp2m        = ps(i,j,nx) * (1.0 - z2g / &
                      (r_d * t_2m(i,j) * (1.0 + rvd_m_o* qv_nx(i,j,ke))))
        zew2m       = esat(t_2m(i,j))
  ...
  IF (.NOT.llm .AND. itype_synd == 2  &
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
         ...
         IF (fr_land(i,j).LT.0.5_ireals) THEN
            gz0(i,j) = MAX ( gz0(i,j) , 1.0E-3_ireals )
         END IF
         z0m = gz0(i,j)/g
         ...
         k   = ke
         zk  = 0.5_ireals * ( hhl(i,j,ke) - hsurf(i,j) )
         zk1 = 0.0_ireals
         DO WHILE ( zk < 2.0_ireals .AND. k > 1 )
           k   = k - 1
           zk1 = zk
           zk  = 0.5_ireals*( hhl(i,j,k)+hhl(i,j,k+1) ) - hsurf(i,j)
         END DO
         zdzg = ( 2.0_ireals - zk1 )*g
         IF ( k == ke ) THEN
           zfac = tfh (i,j)
           ztk1 = t_g (i,j,nx)
           zqk1 = qv_s(i,j,nx)
         ...
         zf2m = LOG( (2.0_ireals+z0m)/(zk1+z0m) ) / zfk1
         t_2m (i,j) = t (i,j,k,nx) + ( zf2m - 1.0_ireals ) &
                                   * ( t (i,j,k,nx) - ztk1 )*zfac
         qv_2m(i,j) = qv_nx(i,j,k) + ( zf2m - 1.0_ireals ) &
                                   * ( qv_nx(i,j,k) - zqk1 )*zfac
         zval = zpk1 * EXP( -zdzg / ( r_d*t_2m(i,j) &
                                   *(1.0_ireals+rvd_m_o*qv_nx(i,j,k)) ) ) !P_2m
         zval = zval/(rdv/MAX(qv_2m(i,j),zepsi)-rdv+1.0_ireals)           !e_2m
         rh_2m(i,j) = 100.0_ireals * MIN ( zval/esat(t_2m(i,j)), 1.0_ireals)
         zfac = LOG( zval/b1 )
         td_2m(i,j) = MIN ( ( b2w*b3 - b4w*zfac ) / ( b2w - zfac ) , t_2m(i,j) )

         ! Wind at 10m screen-level 

         DO WHILE ( zk < 10.0_ireals .AND. k > 1 )
           k   = k - 1
           zk1 = zk
           zk  = 0.5_ireals*( hhl(i,j,k)+hhl(i,j,k+1) ) - hsurf(i,j)  
         END DO
         ii  = MAX( i-1, 1 )
         jj  = MAX( j-1, 1 )
         zuk = 0.5_ireals * ( u(i,j,k,nx) + u(ii,j,k,nx) )
         zvk = 0.5_ireals * ( v(i,j,k,nx) + v(i,jj,k,nx) )
         IF ( k == ke ) THEN
           zfac = tfm(i,j)
           zuk1 = 0.0_ireals
           zvk1 = 0.0_ireals
         ELSE
           zfac = 1.0_ireals
           zuk1 = 0.5_ireals * ( u(i,j,k+1,nx) + u(ii,j,k+1,nx) )
           zvk1 = 0.5_ireals * ( v(i,j,k+1,nx) + v(i,jj,k+1,nx) )
         END IF
         zfk1       = LOG( (zk+z0m) / (zk1+z0m) )
         zf10m      = LOG( (10.0_ireals+z0m) / (zk1+z0m) ) / zfk1
         u_10m(i,j) = zuk + ( zf10m - 1.0_ireals )*( zuk - zuk1 )*zfac
         v_10m(i,j) = zvk + ( zf10m - 1.0_ireals )*( zvk - zvk1 )*zfac
...
!------------------------------------------------------------------------------
  ...
  IF ( llm ) THEN
    ...
    u_10m     (:,:) = 99.9_ireals
    v_10m     (:,:) = 99.9_ireals
    vmax_10m  (:,:) = 99.9_ireals
    vabsmx_10m(:,:) = 99.9_ireals
    vgust_dyn (:,:) = 99.9_ireals
    vgust_con (:,:) = 99.9_ireals
    t_2m      (:,:) = t (:,:,ke,nx)
    qv_2m     (:,:) = qv_nx(:,:,ke)
    td_2m     (:,:) = 0.0_ireals
    rh_2m     (:,:) = 0.0_ireals
...
!------------------------------------------------------------------------------
  ...
  !                    (1._ireals + 3._ireals*2.4_ireals*zsqcm)
  !
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        vgust_dyn(i,j) = MAX ( vgust_dyn(i,j) ,                             &
                zvp10(i,j) + 3.0_ireals * 2.4_ireals * zsqcm * zvp10(i,j) )
      ENDDO
    ENDDO
  ...
  ELSEIF (itype_diag_gusts == 2) THEN
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        zsqcm = SQRT (zcm)
        vgust_dyn(i,j) = MAX ( vgust_dyn(i,j) ,                             &
                zvp30(i,j) + 3.0_ireals * 2.4_ireals * zsqcm * zvpb(i,j) )
      ENDDO
    ENDDO
  ...
  ELSEIF (itype_diag_gusts == 3) THEN
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        ztke_bot  = 0.5_ireals * tke(i,j,ke+1,ntke)**2
        ztke_top  = 0.01_ireals * ztke_bot
        zpeb  = 0.0_ireals
        zitke = 0.0_ireals
        z_p   = 0.0_ireals
        ...
        DO k=ke,2,-1      !! Loop over levels
          ...
          IF (zi_bra(i,j) == 0 ) THEN
            ...
            zpil   = (1.0E-5_ireals * (p0(i,j,k  ) + pp(i,j,k  ,nx)))**rdocp
            zpiu   = (1.0E-5_ireals * (p0(i,j,k-1) + pp(i,j,k-1,nx)))**rdocp
            zthvl  = t(i,j,k  ,nx)*(1.0+rvd_m_o*qv_nx(i,j,k  ))/zpil
            zthvu  = t(i,j,k-1,nx)*(1.0+rvd_m_o*qv_nx(i,j,k-1))/zpiu
  
            ! compute vertical integral of potential energy of buoyancy (zpeb)
            z_h    = hhl(i,j,k) - hhl(i,j,k+1)
            !zthvd  = abs(zthvu - zthvl)
            zthvd  = zthvu - zthvl
            zthv1  = 0.5_ireals * (zthvl + zthvu)
            zbuon  = g*zthvd/zthv1
            zpeb   = zpeb  + zbuon * z_h
  
            ! compute vertical integral of tke (zitke)
            z_p    = z_p + z_h
            !ztke   = (0.5_ireals * tke(i,j,k,ntke)**2+0.5_ireals * tke(i,j,k-1,ntke)**2)*0.5_ireals
            ztke   = 0.5_ireals * tke(i,j,k,ntke)**2
            zitke  = zitke + ztke * z_h
  
            ! compute wind
            zum1(i,j) = 0.5 * ( u(i,j,k,nx) + u(MAX(1,i-1),j,k,nx) )
            zvm1(i,j) = 0.5 * ( v(i,j,k,nx) + v(i,MAX(1,j-1),k,nx) )
            zum2(i,j) = 0.5 * ( u(i,j,k-1,nx) + u(MAX(1,i-1),j,k-1,nx) )
            zvm2(i,j) = 0.5 * ( v(i,j,k-1,nx) + v(i,MAX(1,j-1),k-1,nx) )
            zumk   = 0.5_ireals * (zum1(i,j) + zum2(i,j))
            zvmk   = 0.5_ireals * (zvm1(i,j) + zvm2(i,j))
  ...
  ELSEIF (itype_diag_gusts == 4) THEN
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        vgust_dyn(i,j) = MAX ( vgust_dyn(i,j) ,                             &
                zvp10(i,j)                                                  &
              + (3.0_ireals * 2.4_ireals + 0.09_ireals * zvp10(i,j))        & ! gust factor
              * zsqcm * zvp10(i,j) )
      ENDDO
    ENDDO
...
!------------------------------------------------------------------------------
  ...
  ! derotate the 10m winds in order to realize wind direction calculations
  zumrot = u_10m
  zvmrot = v_10m
  CALL uvrot2uv_vec(zumrot, zvmrot, rlat*zrpi18, rlon*zrpi18, pollat, pollon, ie, je)
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        IF (lbdclim) THEN
          ...
          clct_av (i,j)    = clct_av (i,j) + clct (i,j)
        ENDIF

        vabs_10m_av(i,j)   = vabs_10m_av(i,j) + SQRT( u_10m(i,j)**2.0_ireals + v_10m(i,j)**2.0_ireals )
        wdirgeo_10m(i,j)   = ( raddeg * ATAN2 (zumrot(i,j), zvmrot(i,j)) + 180.0_ireals )
        ! repair the wind direction
        WHERE ( ABS(wdirgeo_10m-360.0_ireals) <= 0.01 ) wdirgeo_10m = 0.0_ireals
        WHERE ( SQRT( u_10m(:,:)**2.0_ireals + v_10m(:,:)**2.0_ireals ) < zpa_wsplim_wdir ) &
           wdirgeo_10m(:,:) = undefgrib

        DO k = 1, ke
          ii = MAX(i-1,1)
          jj = MAX(j-1,1)
          zumk    = 0.5_ireals * (u(i,j,k,ntl) + u(ii,j,k,ntl))
          zvmk    = 0.5_ireals * (v(i,j,k,ntl) + v(i,jj,k,ntl))
          zwmk    = 0.5_ireals * (w(i,j,k+1,ntl) + w(i,j,k,ntl))
          ztke    = 0.5_ireals * (ztke3d(i,j,k+1) + ztke3d(i,j,k))
          !
          vabs_av(i,j,k) = vabs_av(i,j,k) + SQRT( zumk**2.0_ireals + zvmk**2.0_ireals )
          turb_intens(i,j,k) =   MAX(turb_intens(i,j,k), SQRT( 2.0_ireals*ztke ) / &
                         SQRT( zumk**2.0_ireals + zvmk**2.0_ireals + zwmk**2.0_ireals ) )
        ENDDO
        ...
        ! from Burkhardt Rockel, Ronny Petrik HZG
        IF (swdir_sno(i,j) > zpa_swlim_dursun) THEN
!rp 2017-08-10        IF (sodwddm(i,j) > 120.0_ireals) THEN
          dursun(i,j) = dursun(i,j) + dt
        ENDIF

        ! maximum possible sunshine duration buz
        sec=(360.0_ireals/nhori)
        ...
        ! ( 1.0E-7_ireals=securi due to zsmu0 > 1.0E-9_ireals)
        ha_sun = 1.0E-7_ireals + (zhori(i,j,k+1)*(sun_azi(i,j)-sec*ii)+ &
        ...
        IF (dursun_m(i,j) > 0.0_ireals) THEN
            dursun_r(i,j) = 100.0_ireals*dursun(i,j)/dursun_m(i,j)
        ENDIF
    ...
    ! statistics related to wind direction
    zsecwidth=360.0_ireals/REAL(nwdirsec,ireals)
    zhelpwdir = wdirgeo_10m
    WHERE( zhelpwdir > (360.0_ireals - 0.5_ireals*zsecwidth) ) zhelpwdir = zhelpwdir - 360.0_ireals
    DO sectors = 1, nwdirsec
      WHERE( zhelpwdir >= (sectors-1.5_ireals)*zsecwidth .AND. zhelpwdir <= (sectors-0.5_ireals)*zsecwidth )
        vabs_10m_secav(:,:,sectors) = vabs_10m_secav(:,:,sectors) + SQRT( u_10m(:,:)**2.0_ireals + v_10m(:,:)**2.0_ireals )
        vabs_10m_secmax(:,:,sectors) = MAX(vabs_10m_secmax(:,:,sectors), SQRT( u_10m(:,:)**2.0_ireals + v_10m(:,:)**2.0_ireals )) 
        wdirgeo_10m_freq(:,:,sectors) = wdirgeo_10m_freq(:,:,sectors) + 1.0_ireals
      ENDWHERE
    ENDDO
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        aswdir_sn(i,j) = aswdir_sn(i,j)  + swdir_sn(i,j)
        aswdir_sno(i,j)= aswdir_sno(i,j) + swdir_sno(i,j)
