...
! V5.10_cclm,  2017-07-14, Ronny Petrik, HZG
!  adapt the method 'lininterp2D_xinter1D_vec' for handling interpolation to height 
!   levels which vary in west-east-direction (i-direction), i.e. the method has two
!   optional parameters now: one for a one-dimensional level-variable 
!   (p-interpolation or z-interpolation to levels above sea level) and one for a 
!   two-dimensional level-variable (z-interpolation to levels with constant vertical 
...
USE data_parameters , ONLY :   &
    ...
    int_dp       ! KIND-type parameter fo  double precision integers
...
! start of the forecast (if any) + the forecast length in seconds
  insec = NINT(ntsteps*dt, int_dp) + INT(imi, int_dp) * 60_int_dp             &
                                   + INT(iss, int_dp) *  1_int_dp
! inhour is the same in (full) hours
  IF (insec >= 0_iintegers) THEN
     inhour = INT(insec/3600_int_dp, iintegers)
  ELSE
     inhour = INT(FLOOR(REAL(insec,ireals)/3600_ireals),iintegers)
  ENDIF
...
! irsec are the seconds left between the last full hour and the actual time
  irsec    = INT(MODULO(insec,  3600_int_dp), iintegers)
  imin     = INT(irsec/60, iintegers)
  isec     = MODULO(irsec,  60)
! add the time between the last full hour and the actual time
  acthour  = REAL (ihour, ireals) +  REAL (irsec, ireals) / 3600.0_ireals
...
! --------------------
  ...
  INTEGER (KIND=iintegers), INTENT(IN)    ::  nn          ! length of 'data'
! REAL    (KIND=ireals)   , INTENT(IN)    ::  idata (nn)  ! array to be sorted
  INTEGER (KIND=iintegers), INTENT(IN)    ::  idata (nn)  ! array to be sorted
  INTEGER (KIND=iintegers), INTENT(INOUT) ::  iindex (nn) ! indices for 'idata'
...
! REAL    (KIND=ireals)    ::  datap  ! value of pivot element
  INTEGER (KIND=iintegers) ::  datap  ! value of pivot element
  INTEGER (KIND=iintegers) ::  &
...
!===============================================================
  ...
  SUBROUTINE mittel_integral(xwerte, fwerte, anz, xgrenzen, finter, anzinter, ierr)
    ...
    INTEGER(KIND=iintegers), INTENT(in) :: anz, anzinter
    REAL(KIND=ireals), INTENT(in) :: xwerte(anz), fwerte(anz), xgrenzen(anzinter+1)
    INTEGER(KIND=iintegers), INTENT(out) :: ierr
    REAL(KIND=ireals), INTENT(out) :: finter(anzinter)

    INTEGER(KIND=iintegers) :: i, j, fehler
    REAL(KIND=ireals) :: xtmpu(1), ftmpu(1), xtmpo(1), ftmpo(1), xtmp, ftmp


    ierr = 0_iintegers

    finter = -9999.99_ireals
    ...
    DO i=1, anzinter
      ...
      DO
        ...
        IF (xwerte(j) > xtmpu(1)) THEN
          finter(i) = finter(i) + 0.5 * (fwerte(j) + ftmp) * (xwerte(j) - xtmp)
          ftmp = fwerte(j)
          xtmp = xwerte(j)
        END IF
        j = j + 1
      END DO
      finter(i) = finter(i) + 0.5 * (ftmpo(1) + ftmp) * (xtmpo(1)-xtmp)
      IF ( xtmpo(1)-xtmpu(1) > 0.0 ) THEN
...
!===============================================================
  ...
  SUBROUTINE mittel_integral_vec(xwerte, fwerte, anz, xgrenzen, finter, anzinter, ierr)
    ...
    INTEGER(KIND=iintegers) :: i, j, fehler, maxiter, iorig_low(anzinter+1)
    REAL(KIND=ireals) :: xtmpu(anzinter), ftmpu(anzinter), ftmp(anzinter+1)
    ...
    DO WHILE ( ANY(nocheine) .AND. j <= maxiter )
      DO i=1, anzinter
        IF ( nocheine(i) ) THEN
          finter(i) = finter(i) + 0.5 * (fwerte(iorig_low(i)+j) + &
    ...
    DO i=1, anzinter
      finter(i) = finter(i) + 0.5 * (ftmp(i+1) + ftmpu(i)) * (xgrenzen(i+1)-xtmpu(i))   
      IF ( xgrenzen(i+1) - xgrenzen(i) > 0.0 ) THEN
...
!===============================================================
  ...
  SUBROUTINE linear_interpol(xwerte, fwerte, anz, xinter, finter, anzinter, ierr, missingval)
    ...
    INTEGER(KIND=iintegers), INTENT(in) :: anz, anzinter
    REAL(KIND=ireals), INTENT(in) :: xwerte(anz), fwerte(anz), xinter(anzinter)
    REAL(KIND=ireals), INTENT(in), OPTIONAL :: missingval
    INTEGER(KIND=iintegers), INTENT(out) :: ierr
    REAL(KIND=ireals), INTENT(out) :: finter(anzinter)

    INTEGER(KIND=iintegers) :: i, j, cnt
    REAL(KIND=ireals) :: xtmp, xmax, xmin
...
!===============================================================
  ...
  SUBROUTINE linear_interpol_vec(xwerte, fwerte, anz, &
    ...
    REAL(KIND=ireals), INTENT(in) :: xwerte(anz), fwerte(anz), xinter(anzinter)
    REAL(KIND=ireals), INTENT(in), OPTIONAL :: missingval
    INTEGER(KIND=iintegers), OPTIONAL, INTENT(out) :: iorig_low(anzinter)
    INTEGER(KIND=iintegers), INTENT(out) :: ierr
    REAL(KIND=ireals), INTENT(out) :: finter(anzinter)

    INTEGER(KIND=iintegers) :: i, j, cnt(anzinter), ju(anzinter)
    REAL(KIND=ireals) :: xmax, xmin
    ...
    IF (.NOT.PRESENT(missingval)) THEN
      DO i=1, anzinter
        IF (xinter(i) > xmax) THEN
          cnt(i) = cnt(i) + 1_iintegers
          finter(i) = fwerte(anz)
          ju(i) = anz + 1_iintegers
...
!===============================================================
  ...
  SUBROUTINE lininterp2D_xinter1D_vec(xwerte, fwerte, ni, anz, &
    ...
    INTEGER(KIND=iintegers), INTENT(in) :: ni, anz, anzinter
    REAL(KIND=ireals), INTENT(in) :: xwerte(ni, anz), fwerte(ni, anz), xinter(ni,anzinter)
    REAL(KIND=ireals), INTENT(in), OPTIONAL :: missingval
    INTEGER(KIND=iintegers), INTENT(out) :: ierr
    REAL(KIND=ireals), INTENT(out) :: finter(ni, anzinter)
        
    
    INTEGER(KIND=iintegers) :: i, j, k, cnt(ni)
    REAL(KIND=ireals) :: xmax(ni), xmin(ni)
    ...
    DO i=1, anzinter
      ...
      IF (PRESENT(missingval)) THEN
        ...
        DO j=1,ni
          IF (xinter(j,i) > xmax(j) .OR. xinter(j,i) <= xmin(j)) THEN
            cnt(j) = cnt(j) + 1
      ...
      ELSE
        ...
        DO j=1,ni
          IF (xinter(j,i) > xmax(j)) THEN
            cnt(j) = cnt(j) + 1
            finter(j,i) = fwerte(j,anz)
          ELSEIF (xinter(j,i) <= xmin(j)) THEN
            cnt(j) = cnt(j) + 1
      ...
      intervallsuchen: DO k=1, anz-1
        IF (ALL(xinter(:,i) <= xwerte(:,k))) EXIT intervallsuchen
        DO j=1,ni
          IF ( xinter(j,i) > xwerte(j,k) .AND. xinter(j,i) <= xwerte(j,k+1)) THEN
            cnt(j) = cnt(j) + 1
            finter(j,i) = fwerte(j,k) + (fwerte(j,k+1)-fwerte(j,k)) / &
                 (xwerte(j,k+1)-xwerte(j,k)) * (xinter(j,i)-xwerte(j,k))
          END IF
        END DO
      END DO intervallsuchen
