...
!   add new variables related to normalized solar fluxes at the surface, i.e.
!   (a)swdir_sn, (a)swdir_sno
!   add variables related to new wind diagnostics (mean, maximum, 
!    statistics per wind sector, 3D mean u- and v-component, turbulence
!    intensity)
!
! Code Description:
! Language: Fortran 90.
...
!   fields from the radiation scheme
    ...
    clct_av(:,:),   & ! time mean in total cloud cover                  --   
...
! ------------------------------------------
  ...
  REAL  (KIND=ireals), POINTER             ::           &
    ...
    wdirgeo_10m(:,:), &  ! wind direction at 10m (unprojected systen) ( °   )
...
!DL 1.12.2014 change to allow itype_albedo=3 in climate mode
  ...
  REAL  (KIND=ireals), POINTER             ::           &
#endif
    alb_dif_bd(:,:,:)   ! boundary field for diffuse albedo           ( --  ) 
!DL end change
