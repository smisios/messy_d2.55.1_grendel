...
! V5.0_clm12   2017/08/10 Ronny Petrik, HZG
!  adapt the character lengths (related to variable names) to the variable 
!    'clen' from data_io.f90';
!  reorganize the reset of aggregated variables, i.e. done within organized_data
!    and not within_organize_output; new type resetvar to handle the reset-process
!  the restart file was not filled with variables implemented for wind diagnostics,
!    normalized radiation, ... -> add aggregated variables to restart
!  new definitions of vertical level types according to wind diagnostics
!
! Code Description:
! Language: Fortran 90.
...
USE data_parameters,    ONLY:  ireals, iintegers, iwlength, intgribf, int_ga

!------------------------------------------------------------------------------

#ifdef POLLEN
USE data_pollen,        ONLY:  lpollenini, lpollenbd, l_pol_species
...
USE data_io,            ONLY:                              &
    clen,           & ! maximum length for variable short_names
...
USE data_fields, ONLY: rain_gsp, snow_gsp, grau_gsp, hail_gsp, rain_con, snow_con, wdirgeo_10m_freq
!==============================================================================
...
! Type and Pointer for the variables to reset
TYPE resetvar
  INTEGER               :: id1, id2, id3
  LOGICAL               :: reset
  CHARACTER (len=clen)  :: yzname
  TYPE(resetvar), POINTER :: next
END TYPE resetvar
TYPE(resetvar), POINTER :: varresinit, varresnow
...
INTEGER                        :: next_in_step, next_out_step
...
!<--  rp 2018-12-10
LOGICAL, ALLOCATABLE                  :: zfsecav(:), zfwdirfreq(:)  !appearance of output variables
...
!<--  rp 2018-12-10
  ! this output is looked for belonging together
  ALLOCATE (zfsecav (ngribout)); zfsecav = .FALSE.
  ALLOCATE (zfwdirfreq (ngribout));  zfwdirfreq = .FALSE.
...
!<--  rp 2018-12-10
    DO iz3 = 1, now%nyvar_m
      SELECT CASE ( TRIM(now%yvarml(iz3)) )
         CASE ('WDIRGEO_10M_FREQ')
            zfsecav(ngout) = .TRUE.
         CASE ('VABS_10M_SECAV')
            zfwdirfreq(ngout) = .TRUE.
      END SELECT
    ENDDO
...
!  HJP 2018-12-14 Begin
  IF (my_world_id == 0) THEN
!<--  rp 2018-12-10
   IF (ngout > 1_iintegers) THEN
     IF ( ( COUNT(zfsecav) > 1 ) .OR. ( COUNT(zfwdirfreq) > 1 ) ) THEN
!      yerrmsg = '*    ERROR    *** while reading namelist /GRIBOUT/:   WDIRGEO_10M_FREQ and VABS_10M_SECAV appearing at two different gribouts ***'
       yerrmsg = '*    ERROR    *** namelist /GRIBOUT/:   WDIRGEO_10M_FREQ and VABS_10M_SECAV have to be in the same GRIBOUT ***'
!      WRITE (yerrmsg(49:50),'(I2)') ngout
       ierror   = 6900
       RETURN
     ENDIF
     IF ( ( COUNT(zfsecav) == 1 ) .AND. ( COUNT(zfwdirfreq) == 1 ) .AND. ( COUNT(zfsecav .AND. zfwdirfreq) == 0 ) ) THEN
!      yerrmsg = '*    ERROR    *** while reading namelist /GRIBOUT/:    WDIRGEO_10M_FREQ and VABS_10M_SECAV are not in the same gribout ***'
       yerrmsg = '*    ERROR    *** namelist /GRIBOUT/:   WDIRGEO_10M_FREQ and VABS_10M_SECAV have to be in the same GRIBOUT ***'
!      WRITE (yerrmsg(49:50),'(I2)') ngout
       ierror   = 6900
       RETURN
...
ELSEIF (yaction == 'init') THEN
  ...
  ylevltypes1(333) = 'WindSectorClass'
  ...
  ylevltypes2(333) = 'WindSectorClass'
...
!<--  rp 2018-07-25
  ...
  !----------------------------------------------------------------------------
  ! Section 2.2: Initialize MPE_IO (has to be done by all PEs)
  !----------------------------------------------------------------------------
  ...
  IF ( (lasync_io .OR. (num_compute > 1) ) .AND. (nc_asyn_io < 1) ) THEN
    ...
    ibuf_len = compute_grib_intbuffer_length(ie_tot, je_tot, iwlength)
#ifdef I2CINC
  ...
  pp_restart%outsteps         = ((nhour_restart(2) - nhour_restart(1)) &
                                / nhour_restart(3)) + 1
  ALLOCATE (pp_restart%ngrib(pp_restart%outsteps), STAT=istat)
  pp_restart%ngrib(1)         = NINT (3600.0_ireals * nhour_restart(1) /dt)-1
  IF (pp_restart%outsteps > 1) THEN
    DO irestart = 2,pp_restart%outsteps
      pp_restart%ngrib(irestart) = pp_restart%ngrib(irestart-1) + &
      NINT(3600.0_ireals * nhour_restart(3) / dt)   
    ENDDO
  ENDIF
  !END KOchange
...
!END KOchange
  ...
  pp_restart%yform_write      = yform_restart  ! not 'grb1'
...
!HJP 2017-12-20 Begin
  pp_restart%lzint_above_ground=.FALSE.     ! not used
!HJP 2017-12-20 End  
...
ELSEIF (yaction == 'start') THEN
  ...
  IF (my_world_id == 0_iintegers) THEN
    ...
    DO n = 1, nyvar_i
       WRITE (nuspecif, '(T8,A,I3,A,T21,A,T58,A)') 'yvarini(',n,')',     &
                                                              yvarini(n),'C10'
    ENDDO
    WRITE (nuspecif, '(A2)')  '  '
  ENDIF
...
! Variables for boundary data
  ...
  yvarbd ( 3) = 'T         '; yvarbd ( 4) = 'PP        '
  yvarbd ( 5) = 'T_SNOW    '; yvarbd ( 6) = 'W_SNOW    '
  yvarbd ( 7) = 'QV_S      '
  nyvar_b = 7
...
!DL end change
  ...
  IF (my_world_id == 0_iintegers) THEN
    ...
    DO n = 1, nyvar_b
       WRITE (nuspecif, '(T8,A,I3,A,T21,A,T58,A)') 'yvarbd(',n,')',     &
                                                               yvarbd(n),'C10'
    ENDDO
    WRITE (nuspecif, '(A2)')  '  '
  ENDIF
  ...
  IF (lbdclim) THEN
    ...
    pp_restart%yvarml(pp_restart%nyvar_m + 5)  = 'CLCT_AV   '
    pp_restart%nyvar_m     = pp_restart%nyvar_m + 5
  ENDIF
  ...
  pp_restart%yvarml(pp_restart%nyvar_m + 1)  = 'ASWDIR_SN       '
  pp_restart%yvarml(pp_restart%nyvar_m + 2)  = 'ASWDIR_SNO      '
  pp_restart%yvarml(pp_restart%nyvar_m + 3)  = 'TURB_INTENS     '
  pp_restart%yvarml(pp_restart%nyvar_m + 4)  = 'VABS_AV         '
  pp_restart%yvarml(pp_restart%nyvar_m + 5)  = 'VABS_10M_AV     '
  pp_restart%yvarml(pp_restart%nyvar_m + 6)  = 'WDIRGEO_10M     '
  pp_restart%yvarml(pp_restart%nyvar_m + 7)  = 'VABS_10M_SECAV  '
  pp_restart%yvarml(pp_restart%nyvar_m + 8)  = 'VABS_10M_SECMAX '
  pp_restart%yvarml(pp_restart%nyvar_m + 9)  = 'WDIRGEO_10M_FREQ'
...
!   ENDIF
  ...
  IF (lartif_data .EQV. .FALSE.) THEN
    ...
    ELSE
      ...
      CALL organize_input (.FALSE., nufile, 'restart', nlgw, nlgw_ini, list_res_o,           &
                           pp_restart%nyvar_m, yform_restart, 'o')
      IF (.NOT. l2tls) THEN
        CALL organize_input (.FALSE., nufile, 'restart', nlgw, nlgw_ini, list_res_n,         &
                             pp_restart%nyvar_p, yform_restart, 'n')
      ENDIF
    ENDIF
  ...
  gribout_loop_init: DO
    ...
    now%nyvar_c = n1
    DO n = 1, now%nyvar_c
      now%yvarc (n)(1:clen) = yzloclist(n)(1:clen)
    ENDDO
...
!             ENDIF
    ...
    now%nyvar_m = n1
    DO n = 1, now%nyvar_m
      now%yvarml(n)(1:clen) = yzloclist(n)(1:clen)
    ENDDO
    ...
    now%nyvar_p = n1
    DO n = 1, now%nyvar_p
      now%yvarpl(n)(1:clen) = yzloclist(n)(1:clen)
    ENDDO
    ...
    now%nyvar_z = n1
    DO n = 1, now%nyvar_z
      now%yvarzl(n)(1:clen) = yzloclist(n)(1:clen)
    ENDDO
...
!             ENDIF
    ...
    now%nyvar_s = n1
    DO n = 1, now%nyvar_s
      now%yvarsl(n)(1:clen) = yzloclist(n)(1:clen)
    ENDDO
 ...
 iout = 0
  ...
  gribout_loop: DO
    ...
    IF ((nstep == now%ngrib(now%nextstep) .AND. L_BM_ORIG_OUTPUT) &
      ...
      ! aggregated variables under reset
      ALLOCATE( varresinit, STAT=izmemstat)
      IF (izmemstat /= 0) THEN
        yerrmsg = ' ERROR allocating varres pointer'
        ierror   = 6010
        RETURN
      ENDIF
      varresnow => varresinit
      ...
      IF (luse_rttov .AND. now%nyvar_s > 0 .AND. isynsat_stat == 0) THEN
        ...
        lwriteready = .TRUE.
        DO n = 1, now%nyvar_s
          varresnow%id1    = now%ilist_sl(1,n);
          varresnow%id2    = now%ilist_sl(2,n);
          varresnow%id3    = now%ilist_sl(3,n);
          varresnow%yzname = now%yvarsl(n)
          IF (n == now%nyvar_s) THEN
            NULLIFY (varresnow%next)
          ELSE
            ALLOCATE(varresnow%next, STAT=izmemstat)
            varresnow => varresnow%next
          ENDIF
        ENDDO
      ENDIF
#endif
      ...
      IF (now%nyvar_p > 0) THEN
        ...
        lwriteready = .TRUE.
        DO n = 1, now%nyvar_p
          varresnow%id1    = now%ilist_pl(1,n);
          varresnow%id2    = now%ilist_pl(2,n);
          varresnow%id3    = now%ilist_pl(3,n);
          varresnow%yzname = now%yvarpl(n)
          IF (n == now%nyvar_p) THEN
            NULLIFY (varresnow%next)
          ELSE
            ALLOCATE(varresnow%next, STAT=izmemstat)
            varresnow => varresnow%next
      ...
      IF (now%nyvar_z > 0) THEN
        ...
        lwriteready = .TRUE.
        DO n = 1, now%nyvar_z
          varresnow%id1    = now%ilist_zl(1,n);
          varresnow%id2    = now%ilist_zl(2,n);
          varresnow%id3    = now%ilist_zl(3,n);
          varresnow%yzname = now%yvarzl(n)
          IF (n == now%nyvar_z) THEN
            NULLIFY (varresnow%next)
          ELSE
            ALLOCATE(varresnow%next, STAT=izmemstat)
            varresnow => varresnow%next
      ...
      IF (now%nyvar_m > 0) THEN
        ...
        lwriteready = .TRUE.
        DO n = 1, now%nyvar_m
          varresnow%id1    = now%ilist_ml(1,n);
          varresnow%id2    = now%ilist_ml(2,n);
          varresnow%id3    = now%ilist_ml(3,n);
          varresnow%yzname = now%yvarzl(n)
          IF (n == now%nyvar_m) THEN
            NULLIFY (varresnow%next)
          ELSE
            ALLOCATE(varresnow%next, STAT=izmemstat)
            varresnow => varresnow%next
...
#ifndef MESSY
      ! Ronny Petrik, 20.06.2018
      ! reset aggregation variables at the end of gribout=n and NOT at the end of 
      ! each part of gribout=n; 
      ! RESET OF ACCU ELEMENTS IS DONE IN MESSY CHANNEL
      ! Uwe Boehm 09.08.2007
      ! avoid reset of total precipitation components before adding them up
      IF (lbdclim .AND. (now%nyvar_m + now%nyvar_p + now%nyvar_z) > 0) THEN
        varresnow => varresinit
        reset_loop: DO
          ...
          CASE(2)
            ! reset summation and mean variables 
            ! min, max variables are controlled by hincmxt/u and were nullified at other places
          ...
          ! reset 3D-variables
            IF ( (var(i1,i2,i3)%ntri == 3) .OR. (var(i1,i2,i3)%ntri == 4) ) THEN
              IF (ASSOCIATED(var(i1,i2,i3)%p3)) THEN
                var(i1,i2,i3)%p3(:,:,:) = 0.0_ireals
              ENDIF
            ENDIF
            IF ( TRIM(var(i1,i2,i3)%name) == 'VABS_10M_SECAV' ) THEN
                 wdirgeo_10m_freq(:,:,:) = 0.0_ireals
            END IF
          !
          END SELECT
    
          IF (ASSOCIATED(varresnow%next)) THEN
            varresnow => varresnow%next
          ELSE
            EXIT reset_loop
          ENDIF
      ...
      ENDIF
      !
      DEALLOCATE( varresinit )
#endif
    ENDIF  !output needed for gribout=n
...
!BEGIN KOchange
 ...
 IF ((nstep == pp_restart%ngrib(pp_restart%nextstep)) .AND.                                   &
      (pp_restart%ngrib(pp_restart%nextstep) <= NINT (3600.0_ireals * nhour_restart(2) / dt))) &
                                                                         THEN
    fc_hour = REAL(pp_restart%ngrib(pp_restart%nextstep), ireals)
...
! Begin KOchange
   ...
   IF (pp_restart%nextstep .NE. SIZE(pp_restart%ngrib)) THEN
     pp_restart%nextstep = pp_restart%nextstep + 1
   ENDIF
! End KOchange
#ifdef MESSY
    ENDIF
  ...
  IF(my_cart_id == 0) THEN
    ...
    DO k = 1, ke+1
      WRITE(nuspecif,'(1X,I5,F12.4,F12.4)' ) &
         k, vcoord%vert_coord(k), vcoord%sigm_coord(k)*refatm%p0sl*0.01
    ENDDO
    ...
    IF (refatm%irefatm == 1) THEN
      ...
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Pressure at Sea-Level:            ', refatm%p0sl * 0.01_ireals
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Temperature at Sea-Level:         ', refatm%t0sl-273.15_ireals
      WRITE(nuspecif,'(A37,F15.4)')                                            &
...
! Variables for default values
  ...
  CHARACTER (LEN=  4) ::  &
    ...
    yform_restart_d  ! format of the restart files
  CHARACTER (LEN=200)  ::    &
...
IF (nproc > 1) THEN
  ...
  IF (my_world_id == 0) THEN
    ...
    charbuf( 2) = ytrans_out(1:100)
    charbuf( 3) = ymode_read
    charbuf( 4) = ymode_write
    charbuf( 5) = yform_read
    charbuf( 6) = yform_restart
    charbuf( 7) = yncglob_institution(1:100)
    charbuf( 8) = yncglob_title(1:100)
    charbuf( 9) = yncglob_source(1:100)
    charbuf(10) = yncglob_project_id(1:100)
    charbuf(11) = yncglob_experiment_id(1:100)
    charbuf(12) = yncglob_contact(1:100)
    charbuf(13) = yncglob_references(1:100)
    charbuf(14) = ydir_restart
    charbuf(15) = ytunit_restart
    charbuf(16) = ytrans_in (101:200)
    charbuf(17) = ytrans_in (201:250)
    charbuf(18) = ytrans_out(101:200)
    charbuf(19) = ytrans_out(201:250)
    charbuf(20) = yncglob_institution (101:200)
    charbuf(21) = yncglob_title (101:200)
    charbuf(22) = yncglob_source (101:200)
    charbuf(23) = yncglob_project_id (101:200)
    charbuf(24) = yncglob_experiment_id (101:200)
    charbuf(25) = yncglob_contact (101:200)
    charbuf(26) = yncglob_references (101:200)
  ENDIF
  ...
  CALL distribute_values (charbuf,26, 0, imp_character, icomm_world, ierr)
  ...
  IF (my_world_id /= 0) THEN
    ...
    ytrans_out(1:100)     = charbuf( 2)
    ymode_read            = charbuf( 3)
    ymode_write           = charbuf( 4)
    yform_read            = charbuf( 5)
    yform_restart         = charbuf( 6)
    yncglob_institution(1:100)   = charbuf( 7)
    yncglob_title(1:100)         = charbuf( 8)
    yncglob_source(1:100)        = charbuf( 9)
    yncglob_project_id(1:100)    = charbuf(10)
    yncglob_experiment_id(1:100) = charbuf(11)
    yncglob_contact(1:100)       = charbuf(12)
    yncglob_references(1:100)    = charbuf(13)
    ydir_restart          = charbuf(14)
    ytunit_restart        = charbuf(15)
    ytrans_in (101:200)   = charbuf(16)
    ytrans_in (201:250)   = charbuf(17)
    ytrans_out(101:200)   = charbuf(18)
    ytrans_out(201:250)   = charbuf(19)
    yncglob_institution (101:200)   = charbuf(20)
    yncglob_title (101:200)         = charbuf(21)
    yncglob_source(101:200)         = charbuf(22)
    yncglob_project_id(101:200 )    = charbuf(23)
    yncglob_experiment_id (101:200) = charbuf(24)
    yncglob_contact(101:200)        = charbuf(25)
    yncglob_references(101:200)     = charbuf(26)
  ENDIF
...
IF (my_world_id == 0) THEN
  ...
  WRITE (nuspecif, '(T8,A,T26,  A)')                                        &
                                'ytrans_in (C*250)',         TRIM(ytrans_in)
  WRITE (nuspecif, '(T8,A,T26,  A)')                                        &
                               'ytrans_out (C*250)'        ,TRIM(ytrans_out)
  WRITE (nuspecif, '(T8,A,T21,  A  ,T40,A    ,T59,A)')                       &
  ...
  WRITE (nuspecif, '(T8,A,T21,  A  ,T40,A    ,T59,A)')                       &
                     'yform_restart ',yform_restart, yform_restart_d  ,'C*4'
  WRITE (nuspecif, '(T8,A,T30,A)')                                           &
...
! Variables for default values
  ...
  CHARACTER (LEN=250) :: ydirhhl_d     ! directory with HHL file
  CHARACTER (LEN= 50) :: ynamhhl_d     ! name of HHL file
...
! Define the namelist group
  NAMELIST /gribin/ ydirini   , lchkini   , nlgw_ini  ,                    &
                    ...
                    lbdana    , ydirhhl   , ynamhhl   , hincbound ,        &
...
!------------------------------------------------------------------------------
  ...
  nzylen = LEN_TRIM (ydirhhl)
  IF( nzylen > 0 ) THEN
    IF( ydirhhl(nzylen:nzylen) /= '/') THEN
      IF( nzylen < LEN(ydirhhl) ) THEN
        ydirhhl = ydirhhl (1:nzylen)//'/'
      ELSE
        PRINT *,' ERROR    *** ydirhhl is too long *** '
        ierrstat = 1002
      ENDIF
    ENDIF
  ENDIF
...
IF (nproc > 1) THEN
  ...
  IF (my_world_id == 0) THEN
    ...
    charbuf ( 4)(  1:100) = ydirhhl(  1:100)
    charbuf ( 5)(  1:100) = ydirhhl(101:200)
    charbuf ( 6)(  1: 50) = ydirhhl(201:250)
    charbuf ( 7)(  1: 50) = ynamhhl(  1: 50)
    charbuf ( 8)(  1:100) = ydirbd (  1:100)
    charbuf ( 9)(  1:100) = ydirbd (101:200)
    charbuf (10)(  1: 50) = ydirbd (201:250)
    charbuf (11) = ytunitbd
  ...
  CALL distribute_values (charbuf, 11, 0, imp_character, icomm_world, ierr)
  ...
  IF (my_world_id /= 0) THEN
    ...
    ydirhhl(  1:100) = charbuf ( 4)(  1:100)
    ydirhhl(101:200) = charbuf ( 5)(  1:100)
    ydirhhl(201:250) = charbuf ( 6)(  1: 50)
    ynamhhl(  1: 50) = charbuf ( 7)(  1: 50)
    ydirbd (  1:100) = charbuf ( 8)(  1:100)
    ydirbd (101:200) = charbuf ( 9)(  1:100)
    ydirbd (201:250) = charbuf (10)(  1: 50)
    ytunitbd         = charbuf (11)
  ENDIF
ENDIF
...
IF (my_world_id == 0) THEN
  ...
  WRITE (nuspecif, '(T8,A,T26,  A                  )')                       &
                                        'ydirhhl (C*250)   ',TRIM(ydirhhl)
  WRITE (nuspecif, '(T8,A,T26,  A                  )')                       &
                                        'ynamhhl (C* 50)   ',TRIM(ynamhhl)
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A)')                       &
...
!    l_fi_pmsl_smooth,      l_fi_pmsl_smooth_d   !  add. smoothing of fi,pmsl in moutains
     l_fi_pmsl_smooth,      l_fi_pmsl_smooth_d,& !  add. smoothing of fi,pmsl in moutains
     lzint_above_ground,    lzint_above_ground_d !  to choose z-int to levels above ground
!HJP 2017-12-20 End

!  inputvariables for the unit of microphysics output:
   LOGICAL                      ::             &
     loutput_q_densities,  loutput_q_densities_d ! determ. unit of QX and QNX on output
...
!HJP 2017-12-20 Begin
   lzint_above_ground=  lzint_above_ground_d
!HJP 2017-12-20 End 
...
! check the longitude coordinates
  zendlon_tot = startlon_tot + REAL(ie_tot-1, ireals) * dlon
  ...
  IF (slon > elon) THEN
    ...
    ELSE
      ...
      ELSE
        ...
        ierrstat = 10
      ENDIF
      IF (elon > zendlon_tot + 0.0001_ireals) THEN
...
! check the latitude coordinates
  zendlat_tot = startlat_tot + REAL(je_tot-1, ireals) * dlat
  IF (slat < startlat_tot) THEN
    PRINT *, ' ERROR: start-lat is outside total domain: ', slat, startlat_tot
    ierrstat = 10
  ENDIF
  IF (elat > zendlat_tot + 0.0001_ireals) THEN
...
!HJP 2017-12-20 Begin
  outblock%lzint_above_ground  = lzint_above_ground
!HJP 2017-12-20  End
...
!   ENDDO
    ...
    DO i=1,nzmxml
      charbuf(7+       i)(1:clen) = outblock%yvarml(i)(1:clen)
    ENDDO
    DO i=1,nzmxpl
      charbuf(7+nzmxml+i)(1:clen) = outblock%yvarpl(i)(1:clen)
    ENDDO
    DO i=1,nzmxzl
      charbuf(7+nzmxml+nzmxpl+i)(1:clen) = outblock%yvarzl(i)(1:clen)
    ENDDO
    DO i=1,nzmxzl
      charbuf(7+nzmxml+nzmxpl+nzmxzl+i)(1:clen) = outblock%yvarsl(i)(1:clen)
    ENDDO
    DO i=1,nzmxc
      charbuf(7+nzmxml+nzmxpl+2*nzmxzl+i)(1:clen) = outblock%yvarc(i)(1:clen)
    ENDDO
...
!HJP 2017-12-20 Begin
    logbuf(12) = outblock%lzint_above_ground
!HJP 2017-12-20 End
...
! CALL distribute_values (logbuf , 11, 0, imp_logical,  icomm_world, ierr)
  CALL distribute_values (logbuf , 12, 0, imp_logical,  icomm_world, ierr)
!HJP 2017-12-20 End
...
!   ENDDO
    ...
    outblock%ysuffix(1:LEN_TRIM(outblock%ysuffix))  = charbuf(4)
    outblock%ytunit (1:LEN_TRIM(outblock%ytunit))   = charbuf(5)
    outblock%ydomain(1:LEN_TRIM(outblock%ydomain))  = charbuf(6)
    outblock%yform_write(1:4)                       = charbuf(7)(1:4)
    DO i=1,nzmxml
      outblock%yvarml(i)(1:clen) = charbuf(7+i)(1:clen)
    ENDDO
    DO i=1,nzmxpl
      outblock%yvarpl(i)(1:clen) = charbuf(7+nzmxml+i)(1:clen)
    ENDDO
    DO i=1,nzmxzl
      outblock%yvarzl(i)(1:clen) = charbuf(7+nzmxml+nzmxpl+i)(1:clen)
    ENDDO
    DO i=1,nzmxzl
      outblock%yvarsl(i)(1:clen) = charbuf(7+nzmxml+nzmxpl+nzmxzl+i)(1:clen)
    ENDDO
    DO i=1,nzmxc
      outblock%yvarc (i)(1:clen) = charbuf(7+nzmxml+nzmxpl+2*nzmxzl+i)(1:clen)
    ENDDO
...
!HJP 2017-12-20 Begin
    outblock%lzint_above_ground  = logbuf(12)
!HJP 2017-12-20 End
  ENDIF
ENDIF
...
!HJP 2017-12-20 Begin
  WRITE(nuspecif,'(T7,A,T21,L6,T39,L6,T58,A)')                            &
      'lzint_above_ground', outblock%lzint_above_ground, lzint_above_ground_d,'L'
!HJP 2017-12-20 Begin
  WRITE(nuspecif,*)
