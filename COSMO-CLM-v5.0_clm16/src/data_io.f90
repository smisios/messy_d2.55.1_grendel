...
!  added new Namelist parameter lzint_above_ground to choose z-interpolation to height levels
!   above ground
...
!  increase the length of short names by changing variable 'clen' from 10 to 13 
!     -> allow output of solar angles
!  increase the length of short names by changing variable 'clen' from 13 to 16
!     -> allow output for wind statistics
!  increase size of idims_id_out to allow for new dimension (wind sector class)
!  treat the implementation of new variables in grib- and restart-files
!    (new vertical levels)  
! V5_0_clm14   2018/12/05 Ronny Petrik, HZG
!  move the definition of nwdirsec from data_fields.f90 to data_io.f90
!
! Code Description:
! Language: Fortran 90.
...
#else
INTEGER(KIND=iintegers), PARAMETER :: clen = 16
#endif
...
CHARACTER(LEN=30)  :: ylevltypes1(500), ylevltypes2(500)
...
INTEGER (KIND=iintegers), PARAMETER   :: &
  ...
  ndims_id_out=16    ! number of dimensionID's for netCDF formatted output
...
INTEGER (KIND=iintegers)              :: &
  idims_id_out (ndims_id_out)
                  ...
                  !  14: ID for ke_snow
                  !  15: ID for soil2 (needed for t_so)             "soil2"
                  !  16: ID for classes of wind direction
...
INTEGER (KIND=iintegers) :: nwdirsec = 16 !number of windsectors to consider for statistics
...
INTEGER (KIND=iintegers), PARAMETER :: &
  ...
  max_gribrep  =   5, & ! maximum number of allowed repetition of the same
...
! ------------
  ...
  CHARACTER (LEN= 250) :: ydirhhl         ! catalog-name of the HHL file
  CHARACTER (LEN=  50) :: ynamhhl         ! name of the HHL file
...
! ------------------------------------
  ...
  CHARACTER (LEN=250) :: ydir_restart       ! directory of restart file
...
TYPE ar_des
  ...
  REAL(KIND=ireals)              :: factor
  REAL(KIND=ireals)              :: bias
  INTEGER(KIND=intgribf)         :: ntri
  INTEGER(KIND=intgribf)         :: rank
  INTEGER(KIND=intgribf)         :: idimvert
  REAL(KIND=ireals), POINTER     :: p4   (:,:,:,:)
  REAL(KIND=ireals), POINTER     :: p4_bd(:,:,:,:)
  REAL(KIND=ireals), POINTER     :: p3   (:,:,:)
  REAL(KIND=ireals), POINTER     :: p3_bd(:,:,:)
  REAL(KIND=ireals), POINTER     :: p2   (:,:)
...
! INTEGER (KIND=iintegers)   :: n_num              ! current grid number
  ...
  REAL    (KIND=ireals)      :: slon               ! left longitude
  REAL    (KIND=ireals)      :: slat               ! botton latitude
  REAL    (KIND=ireals)      :: elon               ! right longitude
  REAL    (KIND=ireals)      :: elat               ! top latitude
...
! CHARACTER (LEN= 10)        :: ydbpw              ! database password
  REAL    (KIND=ireals)      :: plev(nlevels)      ! pressure levels
  REAL    (KIND=ireals)      :: zlev(nlevels)      ! z-levels
  ...
  LOGICAL                    :: loutput_q_densities! switch for unit of QX, QNX:
