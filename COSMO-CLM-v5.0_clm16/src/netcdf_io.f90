...
!
! Current Code Owner: C2SM, Carlos Osuna
!  phone:  
!  fax:    
...
! V5_00_clm8   2018/12/05  B. Rockel (_br), HZG
!   netcdf attributes for height levels counting from surface height
!   axis attribute added to z-coordinate
...
!  adapt the 'write_nc_gdefs' to the option for interpolating not only to 
!   z-levels above sea level but also above Earth Surface (variable 'Height'
!   instead of variable 'altitude')
!  add some fill values to wind statistics related variables 'VABSDIR...'
!  introduce the dimension definition for the wind sectors and its boundaries
!  reorganize the definition of 3D-variables to manage cases with third 
!   dimension not equal vertical coordinate
!  treat the output variable TURB_INTENS
!
! Code Description:
! Language: Fortran 90.
...
!   stepDate = list_yakdat(1)
  ...
  CALL write_nc_gdefs (nuedat, outblock, icomm_asynio, nc_asyn_io,       &
                         yextension, NF90_FLOAT, istep, errmsg, izerror)
  ...
  IF (yextension == 'c' ) THEN
    ...
    CALL write_nc_vdefs (nuedat, outblock%nyvar_c, outblock%ilist_c, ivar_id,  &
                           ...
                           yextension, NF90_FLOAT, errmsg, izerror)
...
DO WHILE( keeplooking  )
  ...
  IF( minamongvars .NE. -1 .AND.    &
     ( (minamongvars < nstop) .OR. (minamongvars==nfinalstop) ) ) THEN !_br 09.09.16
!     ( (minamongvars < nstop - nstart) .OR. (minamongvars==nfinalstop - nstart) ) ) THEN !_br 09.09.16

     keeplooking = .TRUE. 
     nextmin = minamongvars
!     total_n_outsteps = total_n_outsteps +1 !_br 09.09.16
     IF (minamongvars >= nstart) total_n_outsteps = total_n_outsteps +1 !_br 09.09.16
...
!    ((minamongvars < nstop - nstart) .OR. (minamongvars==nfinalstop - nstart)) ) THEN !_br 09.09.16
    ...
    IF (minamongvars >= nstart) THEN !_br 09.09.16
    tstep = tstep + 1

!    CALL get_utc_date(minamongvars+nstart, ydate_ini, dt, itype_calendar, ydate1_l,    & !_br 09.09.16
    CALL get_utc_date(minamongvars, ydate_ini, dt, itype_calendar, ydate1_l,    &      !_br 09.09.16
                      ydate2_l, nactday_l, acthour_l)

    list_yakdat( tstep ) = ydate1_l
    list_out_nsteps( tstep ) = minamongvars !_br 09.09.16
!    list_out_nsteps( tstep ) = minamongvars + nstart !_br 09.09.16
    ENDIF !_br 09.09.16
...
INTEGER (KIND=iintegers) :: i,j,record_size 
...
! Scalar arguments with intent(in):
  ...
  INTEGER (KIND=iintegers)      , INTENT(IN)     :: &
    out_type    ! = NF90_FLOAT for output and NF90_DOUBLE for restart
...
INTEGER (KIND=iintegers)  :: &
    ...
    jwindsecVarID,  & ! NetCDF ID for the wind sector !Ronny Petrik
    jwindsecbdsID,  & ! NetCDF ID for the wind sector bounds !Ronny Petrik
...
REAL (KIND=irealgrib) :: &
    ...
    zsecwidth,                &      ! width of the wind sector classes
    zwdirsec_vals(nwdirsec),  &      ! wind direction of sector classes
    zwdirsec_bnds(nbnds,nwdirsec)    ! boundaries of sector classes  
...
IF( (lasync_io .AND. (yextension /= 'o')) .OR. my_comm_id == 0 ) THEN

! some pre-settings
  ierror = 0
!  IF( lasync_io ) THEN
  IF( lasync_io .AND. (yextension /= 'o')) THEN
    IF( step == 0 ) THEN
...
!_br 23.01.12
  ...
  IF ((yextension == 'o') .OR. (yextension == 'n')) THEN
    ierror=nf90_put_att(ncid, NF90_GLOBAL,  "psm0", psm0)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror=nf90_put_att(ncid, NF90_GLOBAL,  "dsem0", dsem0)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror=nf90_put_att(ncid, NF90_GLOBAL,  "msem0", msem0)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror=nf90_put_att(ncid, NF90_GLOBAL,  "kem0", kem0)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror=nf90_put_att(ncid, NF90_GLOBAL,  "qcm0", qcm0)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror=nf90_put_att(ncid, NF90_GLOBAL,  "ntke", ntke)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
...
!             list_yakdat(step)(7:8)//' '//list_yakdat(step)(9:10)//':'//list_yakdat(step)(11:12)//':'  &
             list_yakdat(step)(7:8)//'T'//list_yakdat(step)(9:10)//':'//list_yakdat(step)(11:12)//':'  &  !_ br 31.01.18
             //list_yakdat(step)(13:14)//'Z'
  ELSE
    ydate = 'seconds since '// ydate_ini(1:4)//'-'//ydate_ini(5:6)//'-'  &
!             //ydate_ini(7:8)//' '//ydate_ini(9:10)//':'//ydate_ini(11:12)//':'& 
             //ydate_ini(7:8)//'T'//ydate_ini(9:10)//':'//ydate_ini(11:12)//':'&  !--br 31.01.18
             //ydate_ini(13:14)//'Z'
  ENDIF
  ...
  !BEGIN KOCHANGE
!  IF ((yextension == 'o') .OR. (yextension == 'n')) THEN
!    ierror=nf90_put_att(ncid, jtimeID,  "ntke", ntke)
!    IF (ierror /= NF90_NOERR) THEN
!      yerrmsg = TRIM(NF90_strerror(ierror))
!      RETURN
!    ENDIF
  ...
  IF (lbdclim) THEN
     ...
     IF (outblock%nextstep /= 1) THEN
       time_bnds(1,1) = time(1) - &
           (outblock%ngrib(outblock%nextstep) - outblock%ngrib(outblock%nextstep-1))*dt
     !BEGIN KOchange
     ELSEIF (SIZE(outblock%ngrib) == 1) THEN
       time_bnds(1,1) = 0._ireals
     !END KOchange
     ELSE
...
! define the attributes of longitude and latitude
  ...
  ! rotated longitude
  ierror=nf90_def_var(ncid, "rlon", out_type, (/ idims_id_out(1) /),     &
                                                jrlonVarID)
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ...
  ! rotated latitude
  ierror=nf90_def_var(ncid, "rlat", out_type, (/ idims_id_out(2) /),      &
                                                jrlatVarID)
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ...
  IF (.NOT. outblock%luvmasspoint .AND. yextension /= 'c') THEN  
    ...
    ierror=nf90_def_var(ncid, "srlon", out_type, (/ idims_id_out(9) /),     &
                                                 jsrlonVarID)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ...
    ierror=nf90_def_var(ncid, "srlat", out_type, (/ idims_id_out(10) /),    &
                                                 jsrlatVarID)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
...
! geographic longitude
  ierror=nf90_def_var(ncid, "lon", out_type, (/ idims_id_out(1),        &
  ...
  ! geographic latitude
  ierror=nf90_def_var(ncid, "lat", out_type, (/ idims_id_out(1),         &
  ...
  IF (.NOT. outblock%luvmasspoint .AND. yextension /= 'c') THEN  
    ...
    ierror=nf90_def_var(ncid, "slonu", out_type,                         &
    ...
    ierror=nf90_def_var(ncid, "slatu", out_type,                         &
    ...
    ierror=nf90_def_var(ncid, "slonv", out_type,                           &
    ...
    ierror=nf90_def_var(ncid, "slatv", out_type,                           &
...
! axis accordingly
  IF (yextension == 'p') THEN   ! pressure co-ordinate
    ...
    IF (ierror /= NF90_NOERR) THEN
      ...
      RETURN
    ENDIF
    ierror=nf90_def_var(ncid, "pressure", out_type,                    &
...
!--br 23.01.18
    IF (outblock%lzint_above_ground) THEN
      ierror=nf90_def_dim(ncid,"height",outblock%kezin,idims_id_out(3))
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
      ierror=nf90_def_var(ncid, "height", out_type,                    &
                         (/ idims_id_out(3) /), jvcVarID)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
    ...
    ELSE
      ...
      IF (ierror /= NF90_NOERR) THEN
        ...
        RETURN
      ENDIF
      ierror=nf90_def_var(ncid, "altitude", out_type,                  &
          (/ idims_id_out(3) /), jvcVarID)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
...
!_br 25.11.08
  ELSE ! model coordinate
      ...
      IF (ierror /= NF90_NOERR) THEN
        ...
        RETURN
      ENDIF
      ierror=nf90_def_var(ncid, "vcoord", out_type,                     &
      ...
      IF (lradtopo) THEN
         ...
         IF (ierror /= NF90_NOERR) THEN
            ...
            RETURN
         ENDIF
         ierror=nf90_def_var(ncid, "nhori", out_type,                 &
      ...
      IF (yextension /= 'c') THEN
        ...
        zheight_2m  = 2._ireals
        zheight_10m = 10._ireals
        zheight_toa = hhl(1,1,1)
        zwbtemp_13c = 1.3_ireals

        ierror=nf90_def_var(ncid, "height_2m", out_type, jh2VarID)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
        ...
        ENDIF
!--br 05.02.18
        ierror=nf90_put_att(ncid, jh2VarID, "axis", "Z")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
!--br 05.02.18 end
        ...
        IF (ierror /= NF90_NOERR) THEN
          ...
          RETURN
        ENDIF
        ierror=nf90_def_var(ncid, "height_10m", out_type, jh10VarID)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
        ...
        ENDIF
!--br 05.02.18
        ierror=nf90_put_att(ncid, jh10VarID, "axis", "Z")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
!--br 05.02.18 end
        ...
        IF (ierror /= NF90_NOERR) THEN
          ...
          RETURN
        ENDIF
        ierror=nf90_def_var(ncid, "height_toa", out_type, jhtoaVarID)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
        ...
        ENDIF
!--br 05.02.18
        ierror=nf90_put_att(ncid, jhtoaVarID, "axis", "Z")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
...
! Due to consistency reason change name of wet bulb temperature to wbtemp_13c
!       ierror=nf90_def_var(ncid, "wbt_13c", out_type, jwbt13VarID)
        ierror=nf90_def_var(ncid, "wbtemp_13c", out_type, jwbt13VarID)
!HJP 2015-09-10 End
!
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
        ...
        ENDIF
!--br 05.02.18
        ierror=nf90_put_att(ncid, jwbt13VarID, "axis", "Z")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
...
! define fields of variable multi layer soil model
  IF (lmulti_layer .AND. &
    ...
    ENDIF
    !KOCHANGE T_SO
    IF (out_type == NF90_DOUBLE) THEN
      ierror=nf90_def_dim(ncid, "soil2", ke_soil+2, idims_id_out(15))
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
    ...
    ierror=nf90_def_var(ncid, "soil1", out_type, (/ idims_id_out(8) /),  &
                                                   jsoilVarID)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ...
    ierror=nf90_def_var(ncid, "soil1_bnds", out_type,                    &
    ...
    IF (lmulti_snow) THEN
       ...
       IF (ierror /= NF90_NOERR) THEN
         ...
         RETURN
       ENDIF
      ierror=nf90_def_var(ncid, "snow_layer", out_type, (/ idims_id_out(14) /),  &
                                                     jsnowVarID)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
  ...
  IF (luse_rttov .AND. yextension == 's') THEN
    ...
    zmsgchan_wave = (/ 3.9_ireals, 6.2_ireals, 7.3_ireals, 8.7_ireals, 9.7_ireals, 10.8_ireals, 12.0_ireals, 13.4_ireals /)
    ierror=nf90_def_var(ncid, "msgchan_wave", out_type, (/ idims_id_out(13) /),  &
                                                   jmsgchanVarID)
    IF (ierror /= NF90_NOERR) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
  ...
  ! defining fields related to wind sector statistics
  ierror=nf90_def_dim(ncid, "windsector", nwdirsec, idims_id_out(16))
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF
  zsecwidth=360.0_ireals/REAL(nwdirsec,ireals)
  DO zsectwdir = 1, nwdirsec
    zwdirsec_vals(zsectwdir)    = (zsectwdir-1.0_ireals)*zsecwidth
    zwdirsec_bnds(1, zsectwdir) = zwdirsec_vals(zsectwdir) - 0.5_ireals*zsecwidth
    zwdirsec_bnds(2, zsectwdir) = zwdirsec_vals(zsectwdir) + 0.5_ireals*zsecwidth
  ENDDO
  ierror=nf90_def_var(ncid, "windsector", out_type, (/ idims_id_out(16) /),  &
                                               jwindsecVarID)
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ...
  ierror=nf90_put_att(ncid, jwindsecVarID, "long_name", "wind direction of sectors")
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF
  ierror=nf90_put_att(ncid, jwindsecVarID, "units", "degrees")
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF
  ierror=nf90_put_att(ncid, jwindsecVarID, "bounds", "windsector_bnds")
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF

  ierror=nf90_def_var(ncid, "windsector_bnds", out_type,    &
     (/ idims_id_out(6), idims_id_out(16) /), jwindsecbdsID)
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF
  ierror=nf90_put_att(ncid, jwindsecbdsID, "long_name",       &
                     "boundaries of wind sectors")
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF
  ierror=nf90_put_att(ncid, jwindsecbdsID, "units", "degrees")
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
  RETURN
  ENDIF
...
! write the values of longitude, latitude and vertical axis to file
  ...
  ! write the values for wind sectors to file
  ierror=nf90_put_var(ncid, jwindsecVarID, zwdirsec_vals, start=(/ 1 /),   &
        count=(/ nwdirsec /))
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF
  ierror=nf90_put_var(ncid, jwindsecbdsID, zwdirsec_bnds, start=(/ 1,1 /),   &
        count=(/ nbnds, nwdirsec /))
  IF (ierror /= NF90_NOERR) THEN
    yerrmsg = TRIM(NF90_strerror(ierror))
    RETURN
  ENDIF
...
!IF(llasync_io.OR. my_comm_id == 0) THEN
IF((lasync_io .AND. (yextension /= 'o')).OR. my_comm_id == 0) THEN
  DEALLOCATE (zvcoord)
ENDIF
...
SUBROUTINE write_nc_vdefs (ncid, numlist, ilist, var_id, luvmasspoint,      &
                           icomm, npes, yextension, out_type, yerrmsg, ierror)
...
!    IF(lasync_io) THEN
    ...
    CASE(4)
      ...
      IF ( (yextension == 'p') .OR. (yextension == 'z') .OR.               &
                            (vardim3_p4 == ke) ) THEN
        ierror = nf90_def_var(ncid, TRIM(var(i1,i2,i3)%name), out_type,  &
      ...
      ELSEIF ( vardim3_p4 == ke1) THEN
        ...
        ENDIF
        !KOCHANGE T_SO
      ELSEIF ( TRIM(var(i1,i2,i3)%name) == 'T_SO' .AND. out_type == NF90_DOUBLE) THEN
        ierror = nf90_def_var(ncid, TRIM(var(i1,i2,i3)%name), out_type,  &
                          (/ idims_id_out(iid), idims_id_out(jid),         &
    ...
    CASE(3)
      ...
      IF (yextension == 'p' .OR. yextension == 'z') THEN
        ...
        IF (ierror /= NF90_NOERR) THEN
          ...
          RETURN
        ENDIF
      ELSE ! output variables on model levels
        ...
        ! for the synthetic satellite images
          ...
          ELSE
            ...
            ENDIF
          ENDIF
        ELSEIF (var(i1,i2,i3)%levtyp == 333) THEN
          ! for the wind sector statistics
          ierror = nf90_def_var(ncid, TRIM(var(i1,i2,i3)%name), out_type,&
                          (/ idims_id_out(iid), idims_id_out(jid),         &
...
! set the attributes of the variable
    ...
    IF(  (TRIM(var(i1,i2,i3)%name) == 'HTOP_CON')     .OR.      &
         ...
         (TRIM(var(i1,i2,i3)%name) == 'VABS_10M_SECAV')  .OR.   &
         (TRIM(var(i1,i2,i3)%name) == 'VABS_10M_SECMAX') .OR.   &
         (TRIM(var(i1,i2,i3)%name) == 'WDIRGEO_10M')     .OR.   &
!--br 06.08.19 -->
         (TRIM(var(i1,i2,i3)%name) == 'CIN_MU')     .OR.   &
         (TRIM(var(i1,i2,i3)%name) == 'CIN_ML')     .OR.   &
         (TRIM(var(i1,i2,i3)%name) == 'CAPE_MU')     .OR.   &
         (TRIM(var(i1,i2,i3)%name) == 'CAPE_ML')     .OR.   &
!--br 06.08.19 <--
...
! instantaneous values
    CASE(0)
      ierror = nf90_put_att (ncid, var_id(n), "cell_methods", "time: point")
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
    ...
    CASE(2)
      ...
      ELSE IF(var(i1,i2,i3)%name == 'VGUST_DYN') THEN
        ...
        IF (ierror /= NF90_NOERR) THEN
          ...
          RETURN
        ENDIF
      ELSE IF(var(i1,i2,i3)%name == 'VABS_10M_SECMAX') THEN
        ierror = nf90_put_att (ncid, var_id(n), "cell_methods", "time: maximum")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
      ELSE IF(var(i1,i2,i3)%name == 'TURB_INTENS') THEN
        ierror = nf90_put_att (ncid, var_id(n), "cell_methods", "time: maximum")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
...
! Set attribute for layer means of the soil layers
    IF (  vardim3_p4  /= -1) THEN
      IF ( (var(i1,i2,i3)%rank == 4) .AND.                                 &
        ...
        ierror = nf90_put_att (ncid, var_id(n), "cell_methods", "time: point soil1: mean")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
