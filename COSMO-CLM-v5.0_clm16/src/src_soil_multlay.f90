...
! V5_00_clm2   2014/07/04 Jan-Peter Schulz, DWD
!  bug fix in implicit solver of heat conduction equation
!  change results only if itype_heatcond=2
! V5_00_clm3   2014/11/24 Ekaterina Machulskaya(DWD) /H.-J. Panitz (IMK-TRO, KIT)
...
! V5_00_clm3   2014/11/27 and 2015/01/23 B. Rockel, HZG (_br) /H.-J. Panitz (IMK-TRO, KIT)
...
! V5_00_clm3a   2015/02/05 H.-J. Panitz (IMK-TRO, KIT), Felix Ament (look for !FEA NP89)
!  new option for parameterization of bare soil evapoaration according to 
!   Noilhan and Planton (1989); the new scheme (ISBA) can be activated by Namelist parameter itype_evsl
!   itype_evsl=3
...
!   corrected
! V5_00_clm8   2016/01/29 J.-P. Schulz (JPS)/DWD
...
! include resistance version of bare soil evaporation suggested by J.-P. Schulz, DWD
!  to be chosen by itype_evsl=4
! V5_00_clm9   2016/07/06 H.-J. Panitz, IMK/KIT
!  bug fix related to ISBA version of bare soil evaporation (itype_evsl=3)
! V5_00_clm10  2016/09/07 H.-J. Panitz, IMK/KIT
!  delete factor 0.5 infront of PI to be in accordance to
!  ICON coding
!
! Code Description:
! Language: Fortran 90.
...
!FEA NP89 (bare soil evaporation following Noilhan and Planton 1989)
    pi,           &
!FEE
...
!J.P. Schulz 2014-07-04 Begin
    zakb1          , & ! utility variable
    zakb2          , & ! utility variable
!J.P. Schulz 2014-07-04 End   
...
! kos ETHZ, 2015/06/05 multiply cdw0 with tuning parameter soilhyd
!        zdw   (i,j)  = cdw0  (mstyp)
        zdw   (i,j)  = cdw0  (mstyp)*soilhyd
! kos ETHZ end
        zdw1  (i,j)  = cdw1  (mstyp)
! kos ETHZ, 2015/06/05 multiply ckw0 with tuning parameter soilhyd
!        zkw   (i,j)  = ckw0  (mstyp)
        zkw   (i,j)  = ckw0  (mstyp)*soilhyd
! kos ETHZ end
        zkw1  (i,j)  = ckw1  (mstyp)
        zik2  (i,j)  = cik2  (mstyp)
...
! kos ETHZ, 2015/06/05 multiply rootdp with tuning parameter fac_rootdp2
!        zbwt(i,j)   = MAX(0.001_ireals,rootdp(i,j))! Artificial minimum value
        zbwt(i,j)   = MAX(0.001_ireals,rootdp(i,j)*fac_rootdp2)! Artificial 
                                                ! minimum value for root depth
! kos ETHZ end
        ...
        DO i = istarts, iends
          IF (llandmask(i,j)) THEN             ! for land-points only
            IF (m_styp(i,j).ge.3) THEN
              ...
              w_so (i,j,kso,:) = MIN(w_so(i,j,kso,:), 1.0_ireals*zporv(i,j)*zdzhs(kso) )
            ELSE
          ...
          IF (w_snow(i,j,nx) <=1.0E-6_ireals) THEN
            ...
            IF(lmulti_snow) THEN
              ...
              DO ksn = 1, ke_snow
                ...
                w_snow_mult  (i,j,ksn,nx) = 0.0_ireals
                rho_snow_mult(i,j,ksn,nx) = 0.0_ireals
                dzh_snow_mult(i,j,ksn,nx) = 0.0_ireals
              END DO
            END IF
          ELSE
...
!The "old" snow density
        ...
        DO   j = jstarts, jends
          DO i = istarts, iends
            zicount1(i,j) = COUNT(w_snow_mult(i,j,:,nx).gt.zepsi)
            zicount2(i,j) = COUNT(dzh_snow_mult(i,j,:,nx).gt.zepsi)
          END DO
        END DO
        DO ksn = 1, ke_snow
          DO   j = jstarts, jends
  ...
  DO   j = jstarts, jends
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN          ! land-points only
        ...
        fr_snow(i,j) = zrss*zsf_heav(zwsnow(i,j) - zepsi)
        fr_wi  (i,j) = zrww*zsf_heav(zwin  (i,j) - zepsi)
...
!       of average snow density of snow layer
        zdz_snow (i,j) =  zdz_snow (i,j)/MAX(0.01_ireals,fr_snow(i,j))
        ...
        IF (llandmask(i,j)) THEN             ! land-points only
          IF (zep_s(i,j) < 0.0_ireals) THEN  ! upwards directed potential
            ...
            zesoil(i,j) = zevap*zbeta*zep_s(i,j)      & ! evaporation
                          *(1._ireals - fr_wi  (i,j)) & ! not water covered
                          *(1._ireals - fr_snow(i,j)) & ! not snow covered
        ...
        IF (llandmask(i,j)) THEN       ! land points only
          IF (zep_s(i,j) < 0.0_ireals) THEN   ! upwards directed potential
            ...
            IF (m_styp(i,j).ge.3) THEN ! Computations not for ice and rocks
              ...
              IF(zw_fr(i,j,1)+zevapor*(1.0_ireals - fr_wi(i,j))   &
                                     *(1.0_ireals - fr_snow(i,j)) &
            ...
            zesoil(i,j) = zevap*zbeta*zep_s(i,j)       & ! evaporation
                          *(1.0_ireals - fr_wi  (i,j)) & ! not water covered
                          *(1.0_ireals - fr_snow(i,j)) & ! not snow covered
                          * eai(i,j)/sai(i,j) ! relative source surface
                                              ! of the bare soil
            lhfl_bs(i,j) = lh_v * zesoil(i,j)
          END IF  ! upwards directed potential evaporation
...
!FEA NP89
  !----------------------------------------------------------------------------
  ! Section I.4.2c: Bare soil evaporation, ISBA version
  !----------------------------------------------------------------------------

  IF (itype_evsl.EQ.3) THEN   ! ISBA version
    ! Calculation of bare soil evaporation after Noilhan & Planton (1989).
    ! Determination of mean water content relative to volume of voids
    DO   j = jstarts, jends
      DO i = istarts, iends
        IF (llandmask(i,j)) THEN       ! land points only
...
!
!HJP Begin 2016-07-06
!    To my opinion there was a bug in the calculation if saturation vapor
!    pressure  since the "dummy" variables z2iw and z4iw depend on (i,j)
!    Thus, using the values for z2iw and z4iw from above, where they are
!    calculated in dependence on (i,j), is wrong since here always the values
!    from the latest (i,j) pair is used
!
!    Thus, z2iw and z4iw have to be recalculated here.
!    The coefficients b2 for water or ice, and b4 for water
!    or ice are chosen depending on surface temperature 
        ...
        IF (llandmask(i,j)) THEN       ! land points only
          IF (zep_s(i,j) < 0.0_ireals) THEN   ! upwards directed potential
            ...
            IF (m_styp(i,j).GE.3) THEN ! Computations not for ice and rocks
               ...
               zbeta  = 1.0_ireals                                            &
                      / (1.0_ireals + zrhoch(i,j)*zalpha/zrho_atm(i,j))
            END IF ! Computations not for ice and rocks
          ...
          IF (llandmask(i,j)) THEN     ! land-points only
            IF (zep_s(i,j) < 0.0_ireals  &    ! potential evaporation and
              ...
              ztrang(i,j,kso) =  zr_root*zbeta             & ! reduction
                                   ...
                                   * (1. - fr_wi(i,j))     & ! non-water
                                   * (1. - fr_snow(i,j))   & ! non-snow
...
!US             zustar     = zuv*SQRT(tcm(i,j))
                zrla       = 1.0_ireals/MAX(cdash*SQRT(ustar_fv(i,j)),zepsi)
              ELSE
                zrla       = 0._ireals
              ENDIF
...
!JPS 2016-01-29 Begin: J. P. Schulz' method
                IF(zw_fr(i,j,kso)+ztrang(i,j,kso)*zdtdrhw/zdzhs(kso) .lt. zpwp(i,j)) THEN
                  ztrang(i,j,kso) = MIN(0.0_ireals,(zpwp(i,j)-zw_fr(i,j,kso))*zdzhs(kso)/zdtdrhw)
                END IF
!JPS 2016-01-29 End J. P. Schulz' method
    ...
    DO i = istarts, iends
      IF(llandmask(i,j))THEN     ! land-points only
        ...
        zverbo(i,j) = zdwidt(i,j) + zesoil(i,j) + ztrangs(i,j) +              &
                        (1._ireals-fr_snow(i,j))*(zrr(i,j) + zrs(i,j))
        ...
        zinfmx = zinfmx*(1._ireals - fr_snow(i,j))
  ...
  DO   j = jstarts, jends
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN          ! land-points only
        ...
        IF(lmulti_snow) THEN
          zgstr = sigma*(1._ireals - Ctalb) * ( (1._ireals - fr_snow(i,j))* &
        ...
        zrnet_s(i,j) = (1._ireals - fr_snow(i,j))*(sobs(i,j)+zthsoi(i,j))

        zshfl_s(i,j) = (1._ireals - fr_snow(i,j))*cp_d*zrhoch(i,j)*  &
                                                      (zth_low(i,j) - zts(i,j))
...
!EM        ELSEIF ((1._ireals-ztsnow_pm(i,j))*zrr(i,j) > 0.0_ireals) THEN
        ELSEIF ((1._ireals-zts_pm(i,j))*zrr(i,j) > 0.0_ireals) THEN
...
!       Influence of heatflux through snow on total forcing:
        ...
        IF (zwsnew(i,j).GT.zepsi) THEN
          ...
          IF(zdwsndt(i,j)-zrs(i,j)-zrr(i,j).GT.0.0_ireals) THEN
            ...
            w_snow_mult(i,j,1,nx) = max(w_snow_mult(i,j,1,nx) + zdwsndt(i,j)*zdtdrhw,0.0_ireals)
            ...
            rho_snow_mult(i,j,1,nx) = max(w_snow_mult(i,j,1,nx)*rho_w/zdzh_snow(i,j,1),0.0_ireals)
          ELSE

            w_snow_mult(i,j,1,nx) = max(w_snow_mult(i,j,1,nx) + (zrs(i,j)+zrr(i,j))*zdtdrhw,0.0_ireals)
            ...
            IF(w_snow_mult(i,j,1,nx) .GT. 0._ireals) THEN
              rho_snow_mult(i,j,1,nx) = MAX(w_snow_mult(i,j,1,nx)*rho_w/zdzh_snow(i,j,1),0.0_ireals)
...
!EM                      (ztsnow_mult(i,j,ke_snow) - t_so(i,j,1,nx))/(-zhm_snow(i,j,ke_snow)+zdzms(1))
        ...
        zfor_s(i,j) = zrnet_s(i,j) + zshfl_s(i,j) + zlhfl_s(i,j) + zsprs(i,j)*(1._ireals - fr_snow(i,j))       &
!EM                         + fr_snow(i,j) * (1._ireals-ztsnow_pm(i,j)) * zgsb(i,j)
                         + (1._ireals-ztsnow_pm(i,j)) * zgsb(i,j)
...
!em          zgsb(i,j) = zalas*(ztsnow(i,j) - zts(i,j))/zdz_snow_fl(i,j)/MAX(fr_snow(i,j),zepsi)
        END IF

        ! total forcing for uppermost soil layer
        zfor_s(i,j) = zrnet_s(i,j) + zshfl_s(i,j) + zlhfl_s(i,j) + zsprs(i,j)       &
                         + fr_snow(i,j) * (1._ireals-ztsnow_pm(i,j)) * zgsb(i,j)
      END IF          ! land-points only
    END DO
   END DO
...
!
!J.P. Schulz 2014-07-04 Begin
! Bug fix in heat conduction solver
...
!EM_bugfix          zrocs(i,j) = wliq_snow(i,j,1,nx)/zdzh_snow(i,j,1)*chc_w*rho_w + &
!EM_bugfix            (w_snow_mult(i,j,1,nx)-wliq_snow(i,j,1,nx))/zdzh_snow(i,j,1)*chc_i*rho_i
...
!EM_bugfix          zrocs(i,j) = wliq_snow(i,j,ke_snow,nx)/zdzh_snow(i,j,ke_snow)*chc_w*rho_w + &
!EM_bugfix            (w_snow_mult(i,j,ke_snow,nx) - &
!EM_bugfix            wliq_snow(i,j,ke_snow,nx))/zdzh_snow(i,j,ke_snow)*chc_i*rho_i
  ...
  DO ksn = 2, ke_snow-1
    DO   j = jstarts, jends
      DO i = istarts, iends
        IF (llandmask(i,j)) THEN          ! land-points only
          IF(zwsnew(i,j) .GE. zepsi) THEN
!EM_bugfix            zrocs(i,j) = wliq_snow(i,j,ksn,nx)/zdzh_snow(i,j,ksn)*chc_w*rho_w + &
!EM_bugfix              (w_snow_mult(i,j,ksn,nx) - wliq_snow(i,j,ksn,nx))/zdzh_snow(i,j,ksn)*chc_i*rho_i
  ...
  DO ksn = 1, ke_snow
    DO   j = jstarts, jends
      DO i = istarts, iends
        IF (llandmask(i,j)) THEN          ! land-points only
          IF(zwsnew(i,j) .GE. zepsi) THEN  !_br 27.11.14 to avoid division by 0
...
!EM            tmp_num = ztsnow(i,j) + zdt*2._ireals*(zfor_snow_mult(i,j) - zgsb(i,j)*fr_snow(i,j))  &
            tmp_num(i,j) = ztsnow(i,j) + zdt*2._ireals*(zfor_snow_mult(i,j) - zgsb(i,j))  &    !_br_20150123
...
!           tmp_num = ztsnow(i,j) + (tmp_num-ztsnow(i,j))/zfak
            tmp_num(i,j) = ztsnow(i,j) + (tmp_num(i,j)-ztsnow(i,j))/zfak  !_br_20150123
          END IF
        END IF          ! land-points only
      END DO
    END DO
...
!           ztsnown_mult(i,j,ksn) = tmp_num
            ztsnown_mult(i,j,ksn) = tmp_num(i,j) !_br_20150123
          END IF
        END IF          ! land-points only
      END DO
    END DO
...
DO   j = jstarts, jends
  DO i = istarts, iends
    IF (llandmask(i,j)) THEN          ! land-points only
      ...
      IF(zextinct(i,j,1).gt.0.0_ireals) ze_rad(i,j) = fr_snow(i,j) * sobs(i,j) 
      ...
      IF (llandmask(i,j)) THEN          ! land-points only
        IF (zwsnew(i,j) > zepsi) THEN        ! points with snow cover only
          ...
          IF(zdzh_snow(i,j,ksn) - wliq_snow(i,j,ksn,nx).GT.zepsi .OR. &
            w_snow_mult(i,j,ksn,nx) - wliq_snow(i,j,ksn,nx).GT.zepsi) THEN
            zrho_dry_old(i,j) = MAX(w_snow_mult(i,j,ksn,nx) - wliq_snow(i,j,ksn,nx), zepsi)*   &
          ...
          rho_snow_mult(i,j,ksn,nx) = MAX(w_snow_mult(i,j,ksn,nx)*rho_w/zdzh_snow(i,j,ksn),0.0_ireals)
...
!       T<0
          ...
          ELSE
            IF(zrefr(i,j) .GT. 0.0_ireals .OR. zmelt(i,j) .GT. 0.0_ireals) THEN
              ...
              rho_snow_mult(i,j,ksn,nx) = MAX(w_snow_mult(i,j,ksn,nx)*rho_w/zdzh_snow(i,j,ksn),0.0_ireals)
              IF(w_snow_mult(i,j,ksn,nx) .LE. 0.0_ireals) zdzh_snow(i,j,ksn) = 0.0_ireals
      ...
      DO i = istarts, iends
        IF (llandmask(i,j)) THEN  ! for landpoints only
          IF (w_snow(i,j,nnew) == 0.0_ireals) THEN
            ...
            w_snow_mult(i,j,ksn,nnew) = 0.0_ireals
            rho_snow_mult (i,j,ksn,nnew) = 0.0_ireals
            dzh_snow_mult (i,j,ksn,nnew) = 0.0_ireals
          ENDIF
        END IF          ! land-points only
      END DO
    END DO
    ...
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN  ! for landpoints only
        IF(w_snow(i,j,nnew) .GT. zepsi) THEN
          ...
          z_old(i,j,ksn) = -sum_weight(i,j) - dzh_snow_mult(i,j,ksn,nnew)/2._ireals
      ...
      DO i = istarts, iends
        IF (llandmask(i,j)) THEN  ! for landpoints only
          IF(w_snow(i,j,nnew) .GT. zepsi) THEN
            ...
            weight = MAX(MIN(z_old(i,j,k)+dz_old(i,j,k)/2._ireals,zhm_snow(i,j,ksn)+dzh_snow_mult(i,j,ksn,nnew)/2._ireals)-   &
                     MAX(z_old(i,j,k)-dz_old(i,j,k)/2._ireals, &
                     zhm_snow(i,j,ksn)-dzh_snow_mult(i,j,ksn,nnew)/2._ireals),0._ireals)/dzh_snow_mult(i,j,ksn,nnew)
