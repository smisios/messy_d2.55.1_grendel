...
!==============================================================================
  ...
  REAL (KIND=ireals), TARGET, ALLOCATABLE ::  &
      ...
      var_grid_bd (:,:),       & ! grid array of boundary variable
...
SUBROUTINE init_spectral_nudging
  ...
  IF (isc_sn < 1 .OR. jsc_sn < 1) THEN
    CALL init_grid_nudging
  ELSE
    CALL init_spectral_nudging2
  ENDIF
...
SUBROUTINE init_spectral_nudging2
...
!------------------------------------------------------------------------------
! Begin Subroutine init_spectral_nudging2
!------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------
  ...
  twopi = 8.0_ireals*ATAN(1.0_ireals) 
  ...
  DO ms = 1,isc_sn
    DO ir = 1,ie_tot
      arg =twopi*REAL(ms-1)*REAL(ir)/REAL(ie_tot)
      SinArg(ms,ir,1)=SIN(arg)
      CosArg(ms,ir,1)=COS(arg)
      SinArgDiv(ms,ir,1)=SinArg(ms,ir,1)/REAL(ie_tot)
      CosArgDiv(ms,ir,1)=CosArg(ms,ir,1)/REAL(ie_tot)
    ENDDO
  ENDDO

  DO ns = 1,jsc_sn
    DO jr = 1,je_tot
      arg =twopi*REAL(ns-1)*REAL(jr)/REAL(je_tot)
      SinArg(ns,jr,2)=SIN(arg)
      CosArg(ns,jr,2)=COS(arg)
      SinArgDiv(ns,jr,2)=SinArg(ns,jr,2)/REAL(je_tot)
      CosArgDiv(ns,jr,2)=CosArg(ns,jr,2)/REAL(je_tot)
    ENDDO
  ENDDO
  ...
  k_sn = ke
  DO k = 1 , ke
! for reference pressure   p0sl = 10E5_ireals
    IF (      (vcoord%sigm_coord(k)  <= pp_sn/1000._ireals)   &
        .AND. (vcoord%sigm_coord(k+1) > pp_sn/1000._ireals)) THEN
      k_sn = k
      EXIT
    ENDIF
  ENDDO
  ...
  DO k = 1, k_sn
    wgtvd_sn(k) = ( 1.0_ireals -                                               &
     MIN ( ((vcoord%sigm_coord(k)+vcoord%sigm_coord(k+1))*0.5E5_ireals),       &
                     pp_sn*100.0_ireals )/                                     &
                                           (pp_sn*100.0_ireals) )**2*alpha_sn
  ENDDO
...
!------------------------------------------------------------------------------
! End of module procedure init_spectral_nudging2
!------------------------------------------------------------------------------

END SUBROUTINE init_spectral_nudging2
...
! -------------
  ...
  CHARACTER (LEN=80)              :: &
    yerrmsg    ! string holding error message

!----------- End of header ----------------------------------------------------
 
!------------------------------------------------------------------------------
! Begin Subroutine spectral_nudging2
!------------------------------------------------------------------------------
  ...
  zdtibd = REAL (ntstep+1-nlastbound, ireals) / REAL (nincbound, ireals)
  zemzdr = 1. - zdtibd
...
!-------------------------------------------------------------------------------
! End of module procedure spectral_nudging2
!-------------------------------------------------------------------------------

END SUBROUTINE spectral_nudging2
...
!------------------------------------------------------------------------------
  ...
  DO ms = 1,isc_sn
    DO n  = 1,je_tot
      ...
      ENDDO
      asem(ms,n) = aafin(ms,1) + 2.*suma(ms,n)
      bsem(ms,n) = abfin(ms,1) + 2.*sumb(ms,n)
      asem_n(ms,n) = aafin_n(ms,1) + 2.*suma_n(ms,n)
      bsem_n(ms,n) = abfin_n(ms,1) + 2.*sumb_n(ms,n)
    ENDDO
  ENDDO
  ...
  DO n = 1,je_tot
    DO m = 1,ie_tot
     ! replace the original values in the grid point array by the nudged ones
      array(m,n) = array(m,n)  - asem(1,n)   - 2.*sum(m,n)    &
                               + asem_n(1,n) + 2.*sum_n(m,n)
    ENDDO
  ENDDO
...
!==============================================================================
!+ Module procedure for intializing grid nudging
!------------------------------------------------------------------------------

SUBROUTINE init_grid_nudging
...
!------------------------------------------------------------------------------
! Begin Subroutine init_grid_nudging
!------------------------------------------------------------------------------
  ...
  ! Check, whether all variables in yvar_sn are also contained in yvar_bd
    ...
    DO i = 1, nyvar_sn
      ...
      IF (.NOT. lzcontained) THEN
        ...
        ENDIF
        CALL model_abort(my_cart_id, 2029,                    &
               'Wrong variable in list for grid nudging', &
               'init_grid_nudging')
      ENDIF
    ENDDO
...
!----------------------------------------------------------------------------
  ...
  IF(istat /= 0) THEN
    CALL model_abort(my_cart_id, 2029,                                    &
                 'Error allocating fields for grid nudging', 'init_grid_nudging')
  ENDIF

  ! Determine the lowest vertical level for grid nudging
  k_sn = ke
  DO k = 1 , ke
! for reference pressure   p0sl = 10E5_ireals
    IF (      (vcoord%sigm_coord(k)  <= pp_sn/1000._ireals)   &
  ...
  ALLOCATE (var_grid    (ie, je, k_sn),   STAT=istat)
  ALLOCATE (var_grid_bd (ie, je),         STAT=istat)
  ALLOCATE (var_grid_bd1(ie, je, k_sn),   STAT=istat)
  ALLOCATE (var_grid_bd2(ie, je, k_sn),   STAT=istat)


!------------------------------------------------------------------------------
! End of module procedure init_grid_nudging
!------------------------------------------------------------------------------

END SUBROUTINE init_grid_nudging

!==============================================================================
!==============================================================================
!+ Module procedure for grid nudging
!------------------------------------------------------------------------------

SUBROUTINE grid_nudging 

!------------------------------------------------------------------------------
!
! Description:
!   This module procedure performs the grid nudging of the boundary data
!
! Method:
!
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
! Begin Subroutine grid_nudging
!------------------------------------------------------------------------------
  
  ! Preset values
  yroutine = 'grid_nudging'
  ierror = 0
  ...
  zemzdr = 1. - zdtibd
    ...
    ! Location of this variable in the variable table
     ...
     ! Loop over vertical layers
      DO  k = 1, k_sn
        var(i1,i2,i3)%p4(:,:,k,nnew) = var(i1,i2,i3)%p4(:,:,k,nnew) - &
          wgtvd_sn(k)*( var(i1,i2,i3)%p4(:,:,k,nnew) - &
          ( zemzdr*var(i1,i2,i3)%p4_bd(:,:,k,nbd1) + zdtibd*var(i1,i2,i3)%p4_bd(:,:,k,nbd2)  ) )
      ENDDO
...
!-------------------------------------------------------------------------------
! End of module procedure grid_nudging
!-------------------------------------------------------------------------------
